...
class MachoTestFixture : public ::testing::Test {
 protected:
  ...
  static constexpr char expected_strtab[] =
      "__text\0__const\0symbol1\0symbol2\0";
  ...
  static void SetUpTestSuite() {
    ...
    uint32_t header_sizeofcmds = sizeof(struct segment_command_64) +
    ...
    uint32_t text_section_offset =
        ...
        2 * sizeof(struct section_64) + sizeof(struct symtab_command);
    ...
    uint32_t symtab_command_stroff =
        symtab_command_symoff + NUM_SYMS * sizeof(struct nlist_64);
    ...
    if (fwrite(&test_text_segment, sizeof(struct segment_command_64), 1,
               file) != 1) {
      LOG_ERROR("Error occurred while writing to file %s", TEST_FILE);
      goto end;
    }
    ...
    if (fwrite(&test_symtab_command, sizeof(struct symtab_command), 1, file) !=
        1) {
      LOG_ERROR("Error occurred while writing to file %s", TEST_FILE);
      goto end;
    }
    ...
    // We use calloc for the below four calls to ensure that the untouched parts
    // are zeroized, as we will later memcmp the data to what we've read from
    // the file.
    ...
    expected_symtab =
        (struct nlist_64 *)malloc(NUM_SYMS * sizeof(struct nlist_64));
