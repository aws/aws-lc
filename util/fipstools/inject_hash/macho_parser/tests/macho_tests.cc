...
TEST_F(MachoTestFixture, TestReadMachoFile) {
  ...
  EXPECT_TRUE(memcmp(&test_macho_file.macho_header,
                     &expected_macho->macho_header,
                     sizeof(struct mach_header_64)) == 0);
  EXPECT_EQ(test_macho_file.num_sections, expected_macho->num_sections);
  EXPECT_TRUE(memcmp(test_macho_file.sections, expected_macho->sections,
                     test_macho_file.num_sections * sizeof(section_info)) == 0);
}
...
TEST_F(MachoTestFixture, TestGetMachoSectionData) {
  ...
  text_section.reset(get_macho_section_data(TEST_FILE, expected_macho, "__text",
                                            &text_section_size, NULL));
  const_section.reset(get_macho_section_data(
      TEST_FILE, expected_macho, "__const", &const_section_size, NULL));
  symbol_table.reset(get_macho_section_data(
      TEST_FILE, expected_macho, "__symbol_table", &symbol_table_size, NULL));
  string_table.reset(get_macho_section_data(
      TEST_FILE, expected_macho, "__string_table", &string_table_size, NULL));
  ...
  ASSERT_TRUE(memcmp(symbol_table.get(), expected_symtab, symbol_table_size) ==
              0);
  ASSERT_TRUE(memcmp(string_table.get(), expected_strtab, string_table_size) ==
              0);
}
...
TEST_F(MachoTestFixture, TestFindMachoSymbolIndex) {
  ...
  uint32_t symbol1_index = find_macho_symbol_index(
      symbol_table.get(), symbol_table_size, string_table.get(),
      string_table_size, "symbol1", NULL);
