...
// Documentation for the Mach-O structs can be found in macho-o/loader.h and
// mach-o/nlist.h
int read_macho_file(const char *filename, machofile *macho) {
  ...
  bytes_read =
      fread(&macho->macho_header, 1, sizeof(struct mach_header_64), file);
  ...
  // We're only looking for __text, __const in the __TEXT segment, and the
  ...
  // mach-o/loader.h explains that cmdsize (and by extension sizeofcmds) must be
  // a multiple of 8 on 64-bit systems. struct load_command will always be 8
  // bytes.
  for (size_t i = 0;
       i < macho->macho_header.sizeofcmds / sizeof(struct load_command);
       i += load_commands[i].cmdsize / sizeof(struct load_command)) {
    if (load_commands[i].cmd == LC_SEGMENT_64) {
      struct segment_command_64 *segment =
          (struct segment_command_64 *)&load_commands[i];
      ...
      struct symtab_command *symtab =
          (struct symtab_command *)&load_commands[i];
      macho->sections[SYMTABLE_INDEX].offset = symtab->symoff;
      macho->sections[SYMTABLE_INDEX].size =
...
uint8_t *get_macho_section_data(const char *filename, machofile *macho,
                                const char *section_name, size_t *size,
                                uint32_t *offset) {
  FILE *file = NULL;
  uint8_t *ret = NULL;
  uint32_t bytes_read;
  ...
  bytes_read =
      fread(section_data, 1, macho->sections[section_index].size, file);
...
uint32_t find_macho_symbol_index(uint8_t *symbol_table_data,
                                 size_t symbol_table_size,
                                 uint8_t *string_table_data,
                                 size_t string_table_size,
                                 const char *symbol_name, uint32_t *base) {
  char *string_table = NULL;
  uint32_t ret = 0;

  if (symbol_table_data == NULL || string_table_data == NULL) {
    LOG_ERROR(
        "Symbol and string table pointers cannot be null to find the symbol "
        "index");
    goto end;
  }
  ...
  for (size_t i = 0; i < symbol_table_size / sizeof(struct nlist_64); i++) {
    struct nlist_64 *symbol =
        (struct nlist_64 *)(symbol_table_data + i * sizeof(struct nlist_64));
