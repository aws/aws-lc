...
// Calculate HMAC hash
static std::vector<uint8_t> calculate_hash(
    const std::vector<uint8_t> &text_module,
  ...
  if (!HMAC_Final(ctx.get(), hash_result.data(), &hash_len)) {
  ...
  for (unsigned int i = 0; i < hash_len; i++) {
    char hex[3];
    snprintf(hex, sizeof(hex), "%02X", hash_result[i]);
    hash_str += hex;
  }
...
int main(int argc, char *argv[]) {
  ...
  if (!GetBoolArgument(&is_apple, "-apple", args_map)) {
    std::cerr << "Error getting apple flag" << std::endl;
    print_usage();
    return 1;
  }
