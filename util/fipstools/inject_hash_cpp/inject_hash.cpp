...
// the script for testing purpose calculates hash of the file path (as a string)
// using HMAC-SHA256. Later on if the file is .so or .dylib, it will use LIEF to
// parse the file
...
int main(int argc, char **argv) {
  ...
  if (input_file.substr(input_file.length() - 3) == ".so" ||
      input_file.substr(input_file.length() - 6) == ".dylib") {
    if (LIEF::Parser::parse(input_file)) {
      std::cout << "LIEF parser successfully loaded: " << input_file
                << std::endl;
    } else {
  ...
  if (!HMAC_Update(&ctx, reinterpret_cast<const uint8_t *>(input_str.data()),
                   input_str.length())) {
