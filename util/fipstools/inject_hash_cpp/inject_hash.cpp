...
static void print_usage() {
  LOG_INFO("Usage: inject_hash_cpp -o output_file -in-object input_file [-apple]");
...
static bool size_to_little_endian(size_t value_to_convert, uint8_t* result, size_t result_len) {
  ...
  static bool extract_linux_modules(const std::string& input_path,
    ...
    static bool calculate_hash(const std::vector<uint8_t>& text_module,
        ...
        if (!HMAC_Init_ex(ctx.get(), zero_key, sizeof(zero_key), EVP_sha256(), nullptr)) {
        ...
        if (!size_to_little_endian(text_module.size(), length_bytes, sizeof(length_bytes))) {
        ...
        if (!rodata_module.empty()) {
            if (!size_to_little_endian(rodata_module.size(), length_bytes, sizeof(length_bytes))) {
            LOG_ERROR("Buffer too small for little-endian conversion");
            // HMAC_CTX_free(ctx);
            return false;
            } 
    ...
    static bool process(const std::string& input_path,
        ...
        std::ifstream input_file(input_path, std::ios::binary | std::ios::ate); // Open the file in binary mode and move to the end to get size (the ate pointer gets to the end of the file)
        if (!input_file) {
