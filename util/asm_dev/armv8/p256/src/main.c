...
static const uint64_t order_words[4] = {0xf3b9cac2fc632551, 0xbce6faada7179e84,
...
static const uint64_t out_exp_2[4] = {0x79dce5617e3192a9, 0xde737d56d38bcf42,
                                      0x7fffffffffffffff, 0x7fffffff80000000};

static void beeu_print(const int res, const uint64_t out_exp[4],
                       const uint64_t out[4]) {
  uint32_t k;
  if (res == 1) {
    for (k = 0; k < 4; k++) {
      if (out_exp[k] != out[k]) {
...
int main() {
  ...
  for (index = 1; index <= NUM_ENTRIES; index++) {
    ...
    for (j = 0; j < ENTRY_LONG_SIZE; j++) {
      ...
      if (val[j] != table[idx_64]) {
        printf(
            "Error: difference at index: %d, j: %d, idx_64: %d, val[j]:%lX, "
            "table[idx_64]: %lX\n",
  ...
  for (index = 1; index <= NUM_ENTRIES_W7; index++) {
    ...
    for (j = 0; j < ENTRY_LONG_SIZE_W7; j++) {
      ...
      if (val_w7[j] != table_w7[idx_64]) {
        printf(
            "Error (w7): difference at index: %d, j: %d, idx_64: %d, "
  ...
  for (k = 1; k < K_MAX; k++) {
    ...
    if (beeu_res == 1) {
      ...
      if (beeu_res != 1 || a_out[0] != a[0] || a_out[1] != a[1] ||
