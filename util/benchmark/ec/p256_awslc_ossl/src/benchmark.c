...
uint64_t time_now(void) {
   ...
   * ISB: Instruction Synchronization Barrier; i.e. ensures the previous
   * instructions are done executing and changing the context, e.g. system
   * registers, cache, ... before the ones following it, where the context
   * changes would be visible. Read CNTVCT_EL0, the counter-timer virtual count
   * register similarly to
...
void report_results(uint64_t start, uint64_t end, uint64_t iterations,
  ...
  BIO_printf(bio_out, "%s: %lu operations in %luus (%.1f ops/sec)\n", benchmark,
             (unsigned long)iterations, (long unsigned)usec,
             ((double)iterations / usec) * 1000000);
}
