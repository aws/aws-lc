...
const char *bench_choices[NUM_BENCH] = {"ecdhp256", "ecdsap256"};
    ...
    while ((option = getopt(argc, argv, "t:m:")) != -1) {
      ...
      switch (option) {
        case 't':
          ...
          /* If the choice is not valid, print the valid choices and exit with
           * an error */
          if (!valid_choice) {
            BIO_printf(bio_err,
        ...
        case '?':
          BIO_printf(bio_err,
                     ...
                     " ec_benchmark_<awslc|ossl> [-t <benchmark_choice>] [-m "
