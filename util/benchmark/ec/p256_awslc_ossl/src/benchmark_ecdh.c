...
void benchmark_ecdh_p256(uint64_t msec) {
  ...
  if (1 == ecdh_checks) {
    if (EVP_PKEY_keygen(kctx, &key_A) <= 0 || /* generate secret key A */
        !(ctx =
              EVP_PKEY_CTX_new(key_A, NULL)) || /* derivation ctx from skeyA */
  ...
  if (1 == ecdh_checks) {
    ...
    if (!(test_ctx = EVP_PKEY_CTX_new(key_B, NULL)) || /* test ctx from skeyB */
        ...
        !EVP_PKEY_derive_set_peer(test_ctx,
                                  key_A) || /* set peer pubkey in test_ctx */
        !EVP_PKEY_derive(test_ctx, NULL,
                         &test_outlen) ||           /* determine max length */
        !EVP_PKEY_derive(ctx, secret_a, &outlen) || /* compute a*B */
        !EVP_PKEY_derive(test_ctx, secret_b, &test_outlen) || /* compute b*A */
        test_outlen != outlen /* compare output length */
        || CRYPTO_memcmp(secret_a, secret_b,
                         outlen)) /* compare A's and B's shared secret bytes */
    {
      ecdh_checks = 0;
  ...
  if (1 == ecdh_checks) {
    ...
    if (EVP_PKEY_keygen(kctx, &key_A) <= 0 || /* generate secret key A again */
        !EVP_PKEY_derive(ctx, secret_a, &outlen) || /* compute a*B */
        !CRYPTO_memcmp(secret_a, secret_b,
                       outlen) || /* ensure not same shared secret as before */
        !EVP_PKEY_derive(test_ctx, secret_b, &test_outlen) || /* compute b*A */
        CRYPTO_memcmp(secret_a, secret_b,
                      outlen)) /* compare A's and B's shared secret bytes */
    {
   ...
   */
  if (1 == ecdh_checks) {
    /* Warm up and instrument the function to calculate how many iterations it
     * should run for */
    start = time_now();
