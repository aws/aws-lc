...
class NormWhitespace(unittest.TestCase):
    # norm_ws canonicalizes whitespace before the pre-image compares a fix's
...
class IsNoiseLine(unittest.TestCase):
    # is_noise_line filters lines that carry no distinctive code, so introducer
    # tracing and the pre-image don't latch onto them. If real code slipped
    ...
    def test_blank_and_comments(self):
        ...
        for s in ("", "   ", "// comment", "/* block", "* doc", "*/"):
            self.assertTrue(engine.is_noise_line(s), repr(s))
    ...
    def test_hash_is_comment_in_non_c_but_code_in_c(self):
        ...
        # scripts/CMake/YAML ...
        self.assertTrue(engine.is_noise_line("# a cmake comment", "CMakeLists.txt"))
        # ... but a preprocessor directive (real code) in C/C++, so it must NOT be
        # dropped as noise there.
        self.assertFalse(engine.is_noise_line("#include <foo.h>", "a.c"))
        self.assertFalse(engine.is_noise_line("#if defined(X)", "b.h"))
    ...
    def test_real_code_is_not_noise(self):
        ...
        self.assertFalse(engine.is_noise_line("int rc = do_thing(ptr, len);", "a.c"))


class IsBoilerplateLine(unittest.TestCase):
    # is_boilerplate_line drops lines that are real code but too generic to
    # identify a specific fix (they recur across unrelated files/branches).
    # Keeping them would make the pre-image match by coincidence and over-flag.
    ...
    def test_bare_control_flow(self):
        ...
        for s in ("return;", "break;", "continue;", "goto err;", "return 0;"):
            self.assertTrue(engine.is_boilerplate_line(s), repr(s))

    def test_include(self):
        # Catches: an #include (present in countless files) counting as a
        # fingerprint of the fix.
        self.assertTrue(engine.is_boilerplate_line('#include "internal.h"'))

    def test_string_literal_only(self):
        # Catches: a bare string literal being treated as distinctive code.
        self.assertTrue(engine.is_boilerplate_line('"SHA2-512"'))

    def test_distinctive_code_is_kept(self):
        # Catches: the filter over-reaching and discarding a line that IS specific
        # to the fix -- that would weaken the pre-image and risk a miss.
        self.assertFalse(
            engine.is_boilerplate_line("if (EVP_MD_size(md) <= 0) return 0;")
        )


class ParseEosDate(unittest.TestCase):
    # parse_eos_date reads end-of-support dates from the FIPS manifest to apply
    ...
    def test_full_date(self):
        ...
        self.assertEqual(engine.parse_eos_date("2025-09-12"), date(2025, 9, 12))
    ...
    def test_year_month(self):
        ...
        self.assertEqual(engine.parse_eos_date("2025-09"), date(2025, 9, 1))
    ...
    def test_invalid(self):
        ...
        for bad in (None, "", "not-a-date", "2025/09/12"):
            self.assertIsNone(engine.parse_eos_date(bad), repr(bad))


class PatchIdPathspec(unittest.TestCase):
    # patch_id_pathspec builds the git pathspec that excludes generated files
    # from patch-id matching. The contract the callers rely on is only that it
    ...
    def test_returns_list(self):
        ...
        # already-patched / patch-id comparison).
        self.assertIsInstance(engine.patch_id_pathspec(), list)
