...
# Marks where trusted framing stops and repository content starts. The commit being
# analysed can be written by anyone, so its diff, message and comments are a place to
# put text aimed at the model. It cannot fabricate a verdict, since the only way to
# answer is the record_verdict tool, but it can still ask for one
...
# The shape of the answer lives in the tool schema, not here. This only has to say that
# the tool is the only way to answer, and which way to lean when the evidence runs out
ANSWER_FORMAT = (
