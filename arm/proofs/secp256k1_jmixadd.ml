...
let LOCAL_SQR_P256K1_TAC =
  ARM_MACRO_SIM_ABBREV_TAC secp256k1_jmixadd_mc 78 lvs
  `!(t:armstate) pcin pcout p3 n3 p1 n1.
    !n. read(memory :> bytes(word_add (read p1 t) (word n1),8 * 4)) t = n
    ==>
    aligned 16 (read SP t) /\
    nonoverlapping (word pc,0x121c) (word_add (read p3 t) (word n3),32)
    ==> ensures arm
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  ARM_ACCSTEPS_TAC SECP256K1_JMIXADD_EXEC (1--45) (1--45) THEN
  ...
  ARM_ACCSTEPS_TAC SECP256K1_JMIXADD_EXEC (46--64) (46--64) THEN
  ...
  ARM_STEPS_TAC SECP256K1_JMIXADD_EXEC [93] THEN
  ...
  ARM_ACCSTEPS_TAC SECP256K1_JMIXADD_EXEC (66--78) (66--78) THEN
...
let LOCAL_MUL_P256K1_TAC =
  ARM_MACRO_SIM_ABBREV_TAC secp256k1_jmixadd_mc 100 lvs
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  ARM_ACCSTEPS_TAC SECP256K1_JMIXADD_EXEC (1--67) (1--67) THEN
  MAP_EVERY ABBREV_TAC
  ...
  ARM_ACCSTEPS_TAC SECP256K1_JMIXADD_EXEC (68--86) (68--86) THEN
  RULE_ASSUM_TAC(REWRITE_RULE[COND_SWAP; GSYM WORD_BITVAL]) THEN
  ABBREV_TAC
   `ca = bignum_of_wordlist
  ...
  SUBGOAL_THEN `ca DIV 2 EXP 256 = val(sum_s86:int64)` SUBST_ALL_TAC THENL
   [EXPAND_TAC "ca" THEN CONV_TAC(LAND_CONV BIGNUM_OF_WORDLIST_DIV_CONV) THEN
    ...
    FIRST_ASSUM(ASSUME_TAC o MATCH_MP (ARITH_RULE
     `n + 1 < 2 EXP 64 ==> n < 2 EXP 64 - 1`))] THEN
  ARM_STEPS_TAC SECP256K1_JMIXADD_EXEC [87] THEN
  ...
  ARM_ACCSTEPS_TAC SECP256K1_JMIXADD_EXEC (88--100) (88--100) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  CONV_TAC(LAND_CONV BIGNUM_EXPAND_CONV) THEN ASM_REWRITE_TAC[] THEN
  CONV_TAC SYM_CONV THEN MATCH_MP_TAC MOD_UNIQ_BALANCED_REAL THEN
  MAP_EVERY EXISTS_TAC [`val(q:int64)`; `256`] THEN ASM_REWRITE_TAC[] THEN
...
let SECP256K1_JMIXADD_CORRECT = time prove
 (`!p3 p1 t1 p2 t2 pc stackpointer.
        ...
        ==> ensures arm
             ...
             (\s. read PC s = word (pc + 0x1214) /\
                  (!x1 y1 z1 x2 y2 z2.
                        ~(z1 = &0) /\ z2 = &1 /\
                        ~(jacobian_eq (integer_mod_ring p_256k1)
                                      (x1,y1,z1) (x2,y2,z2)) /\
                        ~(jacobian_eq (integer_mod_ring p_256k1)
                                      (jacobian_neg secp256k1 (x1,y1,z1))
                                      (x2,y2,z2)) /\
                        t1 = tripled secp256k1_encode (x1,y1,z1) /\
                        t2 = paired secp256k1_encode (x2,y2)
  ...
  (*** Remove some assumptions slowing down SUBSUMED_MAYCHANGE_TAC ***)

  FIRST_X_ASSUM(MP_TAC o
   check (can (term_match [] `(MAYCHANGE a ,, b) s s'` o concl))) THEN
  POP_ASSUM_LIST(MP_TAC o end_itlist CONJ o rev) THEN
  DISCH_THEN(fun th ->
    DISCH_TAC THEN ENSURES_FINAL_STATE_TAC THEN MP_TAC th) THEN
  ...
  REWRITE_TAC[tripled; paired; secp256k1_encode; PAIR_EQ; modular_encode] THEN
  ...
  ASM_REWRITE_TAC[tripled; paired; jacobian_add; secp256k1] THEN
  ...
  REWRITE_TAC[tripled; paired; modular_encode] THEN
