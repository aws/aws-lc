...
(* bignum_montmul_p521_mc without callee-save register spills + ret. *)
...
let BIGNUM_MONTMUL_P521_CORE_CORRECT = prove
 (`!z x y a b pc stackpointer.
        ...
        ALL (nonoverlapping (stackpointer,80))
            [(word pc,LENGTH bignum_montmul_p521_core_mc); (z,8 * 9);
             (x,8 * 9); (y,8 * 9)] /\
        nonoverlapping (z,8 * 9) (word pc,LENGTH bignum_montmul_p521_core_mc)
        ==> ensures arm
             (\s. aligned_bytes_loaded s (word pc) bignum_montmul_p521_core_mc /\
                  read PC s = word(pc) /\
                  read SP s = stackpointer /\
                  C_ARGUMENTS [z; x; y] s /\
                  bignum_from_memory (x,9) s = a /\
                  bignum_from_memory (y,9) s = b)
             (\s. read PC s = word (pc + LENGTH bignum_montmul_p521_core_mc) /\
                  (a < p_521 /\ b < p_521
  ...
  ASM_CASES_TAC `a < p_521 /\ b < p_521` THENL
   [ASM_REWRITE_TAC[] THEN FIRST_X_ASSUM(CONJUNCTS_THEN ASSUME_TAC);
    ARM_SIM_TAC BIGNUM_MONTMUL_P521_CORE_EXEC (1--619)] THEN
  ...
  ARM_ACCSTEPS_TAC BIGNUM_MONTMUL_P521_CORE_EXEC
  ...
  STRIP_ASSUME_TAC THENL
   [REWRITE_TAC[MOD_LT_EQ] THEN UNDISCH_TAC `n < 2 EXP 576` THEN ARITH_TAC;
    ALL_TAC] THEN
  ARM_STEPS_TAC BIGNUM_MONTMUL_P521_CORE_EXEC (596--598) THEN
  ...
  ARM_ACCSTEPS_TAC BIGNUM_MONTMUL_P521_CORE_EXEC (599--601) (599--601) THEN
  SUBGOAL_THEN
   `carry_s601 <=>
    2 EXP 192 <=
  ...
  (ASSUME_TAC o SYM) THENL
   [MATCH_MP_TAC FLAG_FROM_CARRY_LE THEN EXISTS_TAC `192` THEN
    ...
    DISCH_THEN(fun th -> REWRITE_TAC[th]) THEN BOUNDER_TAC[];
    ACCUMULATOR_POP_ASSUM_LIST(K ALL_TAC)] THEN
  ARM_ACCSTEPS_TAC BIGNUM_MONTMUL_P521_CORE_EXEC (602--610) (602--610) THEN
  SUBGOAL_THEN
  ...
  ARM_STEPS_TAC BIGNUM_MONTMUL_P521_CORE_EXEC (611--619) THEN
...
let BIGNUM_MONTMUL_P521_CORRECT = time prove
   (`!z x y a b pc stackpointer.
        aligned 16 stackpointer /\
        ALL (nonoverlapping (stackpointer,80))
  ...
  ARM_SUB_LIST_OF_MC_TAC BIGNUM_MONTMUL_P521_CORE_CORRECT
    ...
    [fst BIGNUM_MONTMUL_P521_CORE_EXEC;fst BIGNUM_MONTMUL_P521_EXEC] THEN

  REPEAT (POP_ASSUM MP_TAC) THEN
  REWRITE_TAC([fst BIGNUM_MONTMUL_P521_CORE_EXEC;fst BIGNUM_MONTMUL_P521_EXEC;ALL;
...
let BIGNUM_MONTMUL_P521_SUBROUTINE_CORRECT = prove
 (`!z x y a b pc stackpointer returnaddress.
        aligned 16 stackpointer /\
        nonoverlapping (z,8 * 9) (word pc,0x9d8) /\
        ALL (nonoverlapping (word_sub stackpointer (word 144),144))
            [(word pc,0x9d8); (x,8 * 9); (y,8 * 9); (z,8 * 9)]
        ==> ensures arm
             (\s. aligned_bytes_loaded s (word pc) bignum_montmul_p521_mc /\
                  read PC s = word pc /\
                  read SP s = stackpointer /\
  ...
  let th = CONV_RULE (ONCE_DEPTH_CONV NUM_ADD_CONV)
    (REWRITE_RULE [fst BIGNUM_MONTMUL_P521_CORE_EXEC;fst BIGNUM_MONTMUL_P521_EXEC]
