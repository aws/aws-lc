 ...
 (`!z x n pc.
  ...
  CONJ_TAC THENL
   [AP_TERM_TAC THEN REWRITE_TAC[CONS_11] THEN REPEAT CONJ_TAC THEN
    ...
    REWRITE_TAC[VAL_MOD] THEN REWRITE_TAC[val_def] THEN
    REWRITE_TAC[DIMINDEX_8; BIT_WORD_ZX; BIT_WORD_USHR; DIMINDEX_32] THEN
    REWRITE_TAC[ARITH_RULE `i < 8 <=> 0 <= i /\ i <= 7`] THEN
    REWRITE_TAC[ARITH_RULE `i < 16 <=> 0 <= i /\ i <= 15`] THEN
    REWRITE_TAC[GSYM IN_NUMSEG; IN_GSPEC; DIMINDEX_32] THEN
    CONV_TAC NUM_REDUCE_CONV THEN REWRITE_TAC[IN_NUMSEG] THEN
