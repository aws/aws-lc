...
(**** print_literal_from_elf "arm/p256/p256_montjscalarmul.o";;
 ****)

let p256_montjscalarmul_mc = define_assert_from_elf
  "p256_montjscalarmul_mc" "arm/p256/p256_montjscalarmul.o"
[
  ...
  0x94000bd5;       (* arm_BL (word 12116) *)
  0x910083e0;       (* arm_ADD X0 SP (rvalue (word 32)) *)
  0x910083e1;       (* arm_ADD X1 SP (rvalue (word 32)) *)
  0x94000bd2;       (* arm_BL (word 12104) *)
  0x910083e0;       (* arm_ADD X0 SP (rvalue (word 32)) *)
  0x910083e1;       (* arm_ADD X1 SP (rvalue (word 32)) *)
  0x94000bcf;       (* arm_BL (word 12092) *)
  0x910083e0;       (* arm_ADD X0 SP (rvalue (word 32)) *)
  0x910083e1;       (* arm_ADD X1 SP (rvalue (word 32)) *)
  0x94000bcc;       (* arm_BL (word 12080) *)
...
let P256_MONTJSCALARMUL_EXEC = ARM_MK_EXEC_RULE p256_montjscalarmul_mc;;
...
let LOCAL_JDOUBLE_TAC =
  ...
  ARM_SUBROUTINE_SIM_TAC
   (p256_montjscalarmul_mc,P256_MONTJSCALARMUL_EXEC,
    0x33b4,p256_montjdouble_mc,th)
  [`read X0 s`; `read X1 s`;
   `read(memory :> bytes(read X1 s,8 * 4)) s,
    read(memory :> bytes(word_add (read X1 s) (word 32),8 * 4)) s,
    read(memory :> bytes(word_add (read X1 s) (word 64),8 * 4)) s`;
   `pc + 0x33b4`; `read SP s`; `read X30 s`];;
...
let P256_MONTJSCALARMUL_CORRECT = time prove
 (`!res scalar point n xyz pc stackpointer.
        ...
        ALL (nonoverlapping (stackpointer,1232))
            [(word pc,0x4974); (res,96); (scalar,32); (point,96)] /\
        nonoverlapping (res,96) (word pc,0x4974)
        ==> ensures arm
             (\s. aligned_bytes_loaded s (word pc) p256_montjscalarmul_mc /\
  ...
  REPEAT CONJ_TAC THENL
   [ARITH_TAC;
    ...
    ARM_ACCSTEPS_TAC P256_MONTJSCALARMUL_EXEC (15--18) (1--22) THEN
    ...
    ARM_ACCSTEPS_TAC P256_MONTJSCALARMUL_EXEC (23--26) (23--32) THEN
    MAP_EVERY REABBREV_TAC
     [`nn_0 = read X2 s32`;
      `nn_1 = read X3 s32`;
      `nn_2 = read X4 s32`;
    ...
    ARM_ACCSTEPS_TAC P256_MONTJSCALARMUL_EXEC [34;35;37;38] (33--40) THEN
    SUBGOAL_THEN
    ...
    ARM_ACCSTEPS_TAC P256_MONTJSCALARMUL_EXEC [48;50;52;53] (41--68) THEN
    SUBGOAL_THEN
     `read(memory :> bytes(word_add stackpointer (word 464),8 * 4)) s68 = x /\
      read(memory :> bytes(word_add stackpointer (word 528),8 * 4)) s68 = z`
    ...
    ARM_STEPS_TAC P256_MONTJSCALARMUL_EXEC (69--71) THEN
    LOCAL_JDOUBLE_TAC 72 THEN
    MAP_EVERY ABBREV_TAC
     [`x2 = read (memory :> bytes(word_add stackpointer (word 560),8 * 4)) s72`;
      `y2 = read (memory :> bytes(word_add stackpointer (word 592),8 * 4)) s72`;
      `z2 = read (memory :> bytes(word_add stackpointer (word 624),8 * 4)) s72`
     ] THEN

    (*** Computation of 3 * P ***)

    ARM_STEPS_TAC P256_MONTJSCALARMUL_EXEC (73--76) THEN
    LOCAL_JADD_TAC 77 THEN
    MAP_EVERY ABBREV_TAC
     [`x3 = read (memory :> bytes(word_add stackpointer (word 656),8 * 4)) s77`;
      `y3 = read (memory :> bytes(word_add stackpointer (word 688),8 * 4)) s77`;
      `z3 = read (memory :> bytes(word_add stackpointer (word 720),8 * 4)) s77`
     ] THEN

    (*** Computation of 4 * P ***)

    ARM_STEPS_TAC P256_MONTJSCALARMUL_EXEC (78--80) THEN
    LOCAL_JDOUBLE_TAC 81 THEN
    MAP_EVERY ABBREV_TAC
     [`x4 = read (memory :> bytes(word_add stackpointer (word 752),8 * 4)) s81`;
      `y4 = read (memory :> bytes(word_add stackpointer (word 784),8 * 4)) s81`;
      `z4 = read (memory :> bytes(word_add stackpointer (word 816),8 * 4)) s81`
     ] THEN

    (*** Computation of 5 * P ***)

    ARM_STEPS_TAC P256_MONTJSCALARMUL_EXEC (82--85) THEN
    LOCAL_JADD_TAC 86 THEN
    MAP_EVERY ABBREV_TAC
     [`x5 = read (memory :> bytes(word_add stackpointer (word 848),8 * 4)) s86`;
      `y5 = read (memory :> bytes(word_add stackpointer (word 880),8 * 4)) s86`;
      `z5 = read (memory :> bytes(word_add stackpointer (word 912),8 * 4)) s86`
     ] THEN

    (*** Computation of 6 * P ***)

    ARM_STEPS_TAC P256_MONTJSCALARMUL_EXEC (87--89) THEN
    LOCAL_JDOUBLE_TAC 90 THEN
    MAP_EVERY ABBREV_TAC
     [`x6 = read (memory :> bytes(word_add stackpointer (word 944),8 * 4)) s90`;
      `y6 = read (memory :> bytes(word_add stackpointer (word 976),8 * 4)) s90`;
      `z6 = read (memory :> bytes(word_add stackpointer (word 1008),8 * 4)) s90`
     ] THEN

    (*** Computation of 7 * P ***)

    ARM_STEPS_TAC P256_MONTJSCALARMUL_EXEC (91--94) THEN
    LOCAL_JADD_TAC 95 THEN
    MAP_EVERY ABBREV_TAC
     [`x7 = read (memory :> bytes(word_add stackpointer (word 1040),8 * 4)) s95`;
      `y7 = read (memory :> bytes(word_add stackpointer (word 1072),8 * 4)) s95`;
      `z7 = read (memory :> bytes(word_add stackpointer (word 1104),8 * 4)) s95`
     ] THEN

    (*** Computation of 8 * P ***)

    ARM_STEPS_TAC P256_MONTJSCALARMUL_EXEC (96--98) THEN
    LOCAL_JDOUBLE_TAC 99 THEN
    MAP_EVERY ABBREV_TAC
     [`x8 = read (memory :> bytes(word_add stackpointer (word 1136),8 * 4)) s99`;
      `y8 = read (memory :> bytes(word_add stackpointer (word 1168),8 * 4)) s99`;
      `z8 = read (memory :> bytes(word_add stackpointer (word 1200),8 * 4)) s99`
     ] THEN

    (*** Selection of the top bitfield ***)

    ARM_STEPS_TAC P256_MONTJSCALARMUL_EXEC (100--101) THEN
    ...
    BIGNUM_LDIGITIZE_TAC "fab_"
     `read(memory :> bytes(word_add stackpointer (word 464),8 * 96)) s101` THEN
    ARM_STEPS_TAC P256_MONTJSCALARMUL_EXEC (102--281) THEN
    ...
    ARM_SIM_TAC P256_MONTJSCALARMUL_EXEC [1] THEN
    ...
    ARM_STEPS_TAC P256_MONTJSCALARMUL_EXEC (1--13) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
    CONV_TAC(ONCE_DEPTH_CONV BIGNUM_LEXPAND_CONV) THEN ASM_REWRITE_TAC[] THEN
  ...
  REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN ENSURES_INIT_TAC "s0" THEN
  ARM_STEPS_TAC P256_MONTJSCALARMUL_EXEC (1--4) THEN
  ...
  ARM_STEPS_TAC P256_MONTJSCALARMUL_EXEC (6--8) THEN LOCAL_JDOUBLE_TAC 9 THEN
  MAP_EVERY ABBREV_TAC
  ...
  ARM_STEPS_TAC P256_MONTJSCALARMUL_EXEC (10--12) THEN
  ...
  ARM_STEPS_TAC P256_MONTJSCALARMUL_EXEC (14--16) THEN
  ...
  ARM_STEPS_TAC P256_MONTJSCALARMUL_EXEC (18--24) THEN
  ...
  ARM_STEPS_TAC P256_MONTJSCALARMUL_EXEC (25--197) THEN
  MAP_EVERY REABBREV_TAC
   [`tab0 = read X0 s197`;
    `tab1 = read X1 s197`;
    `tab2 = read X2 s197`;
  ...
  ARM_ACCSTEPS_TAC P256_MONTJSCALARMUL_EXEC [201;203;205;206] (198--219) THEN
  MAP_EVERY ABBREV_TAC
   [`Xt = read (memory :> bytes(word_add stackpointer (word 368),8 * 4)) s219`;
    `Yt = read (memory :> bytes(word_add stackpointer (word 400),8 * 4)) s219`;
    `Zt = read (memory :> bytes(word_add stackpointer (word 432),8 * 4)) s219`
   ] THEN
  SUBGOAL_THEN
   `!P. P IN group_carrier p256_group /\ y < p_256 /\ represents_p256 P (x,y',z)
  ...
  ARM_STEPS_TAC P256_MONTJSCALARMUL_EXEC (220--223) THEN
  LOCAL_JADD_TAC 224 THEN
  MAP_EVERY ABBREV_TAC
   [`X' = read (memory :> bytes(word_add stackpointer (word 272),8 * 4)) s224`;
    `Y' = read (memory :> bytes(word_add stackpointer (word 304),8 * 4)) s224`;
    `Z' = read (memory :> bytes(word_add stackpointer (word 336),8 * 4)) s224`
   ] THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
...
let P256_MONTJSCALARMUL_SUBROUTINE_CORRECT = time prove
 (`!res scalar point n xyz pc stackpointer returnaddress.
   ...
   P256_MONTJSCALARMUL_CORRECT `[X19; X20; X21; X30]` 1264);;
