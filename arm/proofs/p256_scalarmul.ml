...
[
  ...
  0x940011dd;       (* arm_BL (word 18292) *)
  0x910083e0;       (* arm_ADD X0 SP (rvalue (word 32)) *)
  0x910083e1;       (* arm_ADD X1 SP (rvalue (word 32)) *)
  0x940011da;       (* arm_BL (word 18280) *)
  0x910083e0;       (* arm_ADD X0 SP (rvalue (word 32)) *)
  0x910083e1;       (* arm_ADD X1 SP (rvalue (word 32)) *)
  0x940011d7;       (* arm_BL (word 18268) *)
  0x910083e0;       (* arm_ADD X0 SP (rvalue (word 32)) *)
  0x910083e1;       (* arm_ADD X1 SP (rvalue (word 32)) *)
  0x940011d4;       (* arm_BL (word 18256) *)
...
let LOCAL_SQR_TAC =
  ARM_SUBROUTINE_SIM_TAC
   (p256_scalarmul_mc,P256_SCALARMUL_EXEC,
    0x1c40,bignum_montsqr_p256_mc,
    BIGNUM_MONTSQR_P256_SUBROUTINE_CORRECT)
  [`read X0 s`; `read X1 s`;
   `read(memory :> bytes(read X1 s,8 * 4)) s`;
   `pc + 0x1c40`; `read X30 s`];;

let LOCAL_TOMONT_TAC =
  ARM_SUBROUTINE_SIM_TAC
   (p256_scalarmul_mc,P256_SCALARMUL_EXEC,
    0x1e34,bignum_tomont_p256_mc,BIGNUM_TOMONT_P256_SUBROUTINE_CORRECT)
  [`read X0 s`; `read X1 s`;
   `read(memory :> bytes(read X1 s,8 * 4)) s`;
   `pc + 0x1e34`; `read X30 s`];;
...
let LOCAL_JADD_TAC =
  ...
  [`read X0 s`; `read X1 s`;
   ...
   `pc + 0x2024`; `read SP s`; `read X30 s`];;
...
let LOCAL_JDOUBLE_TAC =
  ...
  ARM_SUBROUTINE_SIM_TAC
   (p256_scalarmul_mc,P256_SCALARMUL_EXEC,
    0x4bd4,p256_montjdouble_mc,th)
  [`read X0 s`; `read X1 s`;
   `read(memory :> bytes(read X1 s,8 * 4)) s,
    read(memory :> bytes(word_add (read X1 s) (word 32),8 * 4)) s,
    read(memory :> bytes(word_add (read X1 s) (word 64),8 * 4)) s`;
   `pc + 0x4bd4`; `read SP s`; `read X30 s`];;
...
let LOCAL_JMIXADD_TAC =
  ...
  [`read X0 s`; `read X1 s`;
   ...
   `read(memory :> bytes(read X2 s,8 * 4)) s,
    read(memory :> bytes(word_add (read X2 s) (word 32),8 * 4)) s`;
   `pc + 0x6194`; `read SP s`; `read X30 s`];;
...
let P256_SCALARMUL_CORRECT = time prove
 (`!res scalar point n xy pc stackpointer.
        ...
        ALL (nonoverlapping (stackpointer,1232))
            [(word pc,0x8010); (res,64); (scalar,32); (point,64)] /\
        nonoverlapping (res,64) (word pc,0x8010)
        ==> ensures arm
  ...
  REPEAT CONJ_TAC THENL
   [ARITH_TAC;
    ...
    SUBGOAL_THEN
     `read(memory :> bytes(word_add stackpointer (word 528),8 * 4)) s54 = z'`
    ASSUME_TAC THENL
    ...
    BIGNUM_LDIGITIZE_TAC "ym_"
     `read (memory :> bytes(word_add stackpointer (word 496),8 * 4)) s54` THEN
    ARM_ACCSTEPS_TAC P256_SCALARMUL_EXEC [58;60;62;63] (55--70) THEN
    SUBGOAL_THEN
     `read(memory :> bytes(word_add stackpointer (word 496),8 * 4)) s70 = y'`
    ASSUME_TAC THENL
     [CONV_TAC(LAND_CONV BIGNUM_LEXPAND_CONV) THEN ASM_REWRITE_TAC[] THEN
    ...
    MAP_EVERY ABBREV_TAC
     [`x2 = read (memory :> bytes(word_add stackpointer (word 560),8 * 4)) s74`;
      `y2 = read (memory :> bytes(word_add stackpointer (word 592),8 * 4)) s74`;
      `z2 = read (memory :> bytes(word_add stackpointer (word 624),8 * 4)) s74`
     ] THEN
    ...
    MAP_EVERY ABBREV_TAC
     [`x3 = read (memory :> bytes(word_add stackpointer (word 656),8 * 4)) s79`;
      `y3 = read (memory :> bytes(word_add stackpointer (word 688),8 * 4)) s79`;
      `z3 = read (memory :> bytes(word_add stackpointer (word 720),8 * 4)) s79`
     ] THEN
    ...
    MAP_EVERY ABBREV_TAC
     [`x4 = read (memory :> bytes(word_add stackpointer (word 752),8 * 4)) s83`;
      `y4 = read (memory :> bytes(word_add stackpointer (word 784),8 * 4)) s83`;
      `z4 = read (memory :> bytes(word_add stackpointer (word 816),8 * 4)) s83`
     ] THEN
    ...
    MAP_EVERY ABBREV_TAC
     [`x5 = read (memory :> bytes(word_add stackpointer (word 848),8 * 4)) s88`;
      `y5 = read (memory :> bytes(word_add stackpointer (word 880),8 * 4)) s88`;
      `z5 = read (memory :> bytes(word_add stackpointer (word 912),8 * 4)) s88`
     ] THEN
    ...
    MAP_EVERY ABBREV_TAC
     [`x6 = read (memory :> bytes(word_add stackpointer (word 944),8 * 4)) s92`;
      `y6 = read (memory :> bytes(word_add stackpointer (word 976),8 * 4)) s92`;
      `z6 = read (memory :> bytes(word_add stackpointer (word 1008),8 * 4)) s92`
     ] THEN
    ...
    MAP_EVERY ABBREV_TAC
     [`x7 = read (memory :> bytes(word_add stackpointer (word 1040),8 * 4)) s97`;
      `y7 = read (memory :> bytes(word_add stackpointer (word 1072),8 * 4)) s97`;
      `z7 = read (memory :> bytes(word_add stackpointer (word 1104),8 * 4)) s97`
     ] THEN
    ...
    MAP_EVERY ABBREV_TAC
     [`x8 = read (memory :> bytes(word_add stackpointer (word 1136),8 * 4)) s101`;
      `y8 = read (memory :> bytes(word_add stackpointer (word 1168),8 * 4)) s101`;
      `z8 = read (memory :> bytes(word_add stackpointer (word 1200),8 * 4)) s101`
     ] THEN
    ...
    BIGNUM_LDIGITIZE_TAC "fab_"
     `read(memory :> bytes(word_add stackpointer (word 464),8 * 96)) s103` THEN
    ...
    MAP_EVERY ABBREV_TAC
     [`Xt = read (memory :> bytes(word_add stackpointer (word 368),8 * 4)) s215`;
      `Yt = read (memory :> bytes(word_add stackpointer (word 400),8 * 4)) s215`;
      `Zt = read (memory :> bytes(word_add stackpointer (word 432),8 * 4)) s215`
     ] THEN
    SUBGOAL_THEN
     `!P. P IN group_carrier p256_group /\ represents2_p256 P (x',y')
    ...
    MAP_EVERY ABBREV_TAC
     [`X' = read (memory :> bytes(word_add stackpointer (word 272),8 * 4)) s220`;
      `Y' = read (memory :> bytes(word_add stackpointer (word 304),8 * 4)) s220`;
      `Z' = read (memory :> bytes(word_add stackpointer (word 336),8 * 4)) s220`
     ] THEN
  ...
  ABBREV_TAC
   `Z2 = read(memory :> bytes(word_add stackpointer (word 464),8 * 4)) s5` THEN
  ARM_STEPS_TAC P256_SCALARMUL_EXEC (6--9) THEN LOCAL_MUL_TAC 10 THEN
  ABBREV_TAC
   `Z3 =
    read(memory :> bytes(word_add stackpointer (word 496),8 * 4)) s10` THEN
  ARM_STEPS_TAC P256_SCALARMUL_EXEC (11--13) THEN LOCAL_DEMONT_TAC 14 THEN
  ABBREV_TAC
   `Z3' =
    read(memory :> bytes(word_add stackpointer (word 464),8 * 4)) s14` THEN
  ARM_STEPS_TAC P256_SCALARMUL_EXEC (15--17) THEN LOCAL_INV_TAC 18 THEN
  ABBREV_TAC
   `I3 =
    read(memory :> bytes(word_add stackpointer (word 496),8 * 4)) s18` THEN
  ARM_STEPS_TAC P256_SCALARMUL_EXEC (19--22) THEN LOCAL_MUL_TAC 23 THEN
  ABBREV_TAC
   `I2 =
    read (memory :> bytes(word_add stackpointer (word 464),8 * 4)) s23` THEN
...
let P256_SCALARMUL_SUBROUTINE_CORRECT = time prove
 (`!res scalar point n xy pc stackpointer returnaddress.
   ...
   P256_SCALARMUL_CORRECT `[X19; X20; X21; X30]` 1264);;
