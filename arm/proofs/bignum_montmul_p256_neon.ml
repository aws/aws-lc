...
let BIGNUM_MONTMUL_P256_CORE_EQUIV2 = prove(
  ...
  REWRITE_TAC[MODIFIABLE_SIMD_REGS;SOME_FLAGS;
    ...
    fst BIGNUM_MONTMUL_P256_NEON_CORE_EXEC] THEN
  REPEAT STRIP_TAC THEN
  (** Initialize **)
  EQUIV_INITIATE_TAC montmul_p256_eqin THEN
  REPEAT (FIRST_X_ASSUM BIGNUM_EXPAND_AND_DIGITIZE_TAC) THEN
...
let BIGNUM_MONTMUL_P256_CORE_EQUIV = prove(equiv_goal,
  ...
  SUBGOAL_THEN
    ...
    ASM_REWRITE_TAC
      [ALL;NONOVERLAPPING_CLAUSES;
       ...
       fst BIGNUM_MONTMUL_P256_CORE_EXEC;GSYM CONJ_ASSOC] THEN
    STRIP_TAC THEN
    ASM_REWRITE_TAC[] THEN
    FIND_HOLE_TAC;
  ...
  REPEAT CONJ_TAC THENL [
    ...
    PROVE_CONJ_OF_EQ_READS_TAC BIGNUM_MONTMUL_P256_INTERM1_CORE_EXEC THENL [
      UNDISCH_TAC `montmul_p256_eqin (s,s') x y z` THEN
      REWRITE_TAC[montmul_p256_eqin;C_ARGUMENTS;BIGNUM_FROM_MEMORY_BYTES;
                  fst BIGNUM_MONTMUL_P256_INTERM1_CORE_EXEC] THEN
    ...
    ASM_REWRITE_TAC[] THEN ASM_MESON_TAC[montmul_p256_eqout_TRANS];
...
let event_n_at_pc_goal = mk_eventually_n_at_pc_statement_simple
    `nonoverlapping
      (word pc:int64,
        LENGTH (APPEND bignum_montmul_p256_core_mc barrier_inst_bytes))
      (z:int64,8 * 4)`
    [`z:int64`;`x:int64`;`y:int64`] bignum_montmul_p256_core_mc
    `\s0. C_ARGUMENTS [z;x;y] s0`;;

let BIGNUM_MONTMUL_P256_EVENTUALLY_N_AT_PC = prove(event_n_at_pc_goal,

  REWRITE_TAC[LENGTH_APPEND;fst BIGNUM_MONTMUL_P256_CORE_EXEC;
  ...
  SUBGOAL_THEN `4 divides (LENGTH bignum_montmul_p256_core_mc)`
        (fun th -> REWRITE_TAC[MATCH_MP aligned_bytes_loaded_append th;
                               fst BIGNUM_MONTMUL_P256_CORE_EXEC]) THENL [
    REWRITE_TAC[fst BIGNUM_MONTMUL_P256_CORE_EXEC] THEN CONV_TAC NUM_DIVIDES_CONV;
    ALL_TAC] THEN
...
let BIGNUM_MONTMUL_P256_NEON_CORE_CORRECT = prove(
  `!z x y a b pc2.
    ...
    ==> ensures arm
          ...
          (\s. read PC s = word (pc2 + LENGTH bignum_montmul_p256_neon_core_mc) /\
               (a * b <= 2 EXP 256 * p_256
  ...
  SUBGOAL_THEN
    `?pc.
      nonoverlapping (word pc,
        LENGTH (APPEND bignum_montmul_p256_core_mc barrier_inst_bytes)) (z:int64,8 * 4) /\
      nonoverlapping (word pc,
        LENGTH (APPEND bignum_montmul_p256_core_mc barrier_inst_bytes)) (x:int64,8 * 4) /\
      nonoverlapping (word pc,
        LENGTH (APPEND bignum_montmul_p256_core_mc barrier_inst_bytes)) (y:int64,8 * 4) /\
  ...
  VCGEN_EQUIV_TAC BIGNUM_MONTMUL_P256_CORE_EQUIV BIGNUM_MONTMUL_P256_CORE_CORRECT_N
    [fst BIGNUM_MONTMUL_P256_CORE_EXEC;fst BIGNUM_MONTMUL_P256_NEON_CORE_EXEC] THEN
  ...
  REPEAT CONJ_TAC THENL [
    ...
    ASM_REWRITE_TAC[montmul_p256_eqin;C_ARGUMENTS] THEN
    EXISTS_TAC
      `write (memory :> bytelist
    ...
    (** SUBGOAL 2. Postcond **)
    MESON_TAC[montmul_p256_eqout;BIGNUM_FROM_MEMORY_BYTES;
        fst BIGNUM_MONTMUL_P256_CORE_EXEC;
        fst BIGNUM_MONTMUL_P256_NEON_CORE_EXEC];
...
let BIGNUM_MONTMUL_P256_NEON_CORRECT = time prove
 (`!z x y a b pc.
  ...
  ARM_SUB_LIST_OF_MC_TAC BIGNUM_MONTMUL_P256_NEON_CORE_CORRECT
      bignum_montmul_p256_neon_core_mc_def
      [fst BIGNUM_MONTMUL_P256_NEON_EXEC;fst BIGNUM_MONTMUL_P256_NEON_CORE_EXEC]);;
 ...
 (`!z x y a b pc returnaddress.
        ...
        ==> ensures arm
             ...
             (MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * 4)])`,
  REWRITE_TAC[fst BIGNUM_MONTMUL_P256_NEON_EXEC] THEN
  ARM_ADD_RETURN_NOSTACK_TAC BIGNUM_MONTMUL_P256_NEON_EXEC
    (REWRITE_RULE[fst BIGNUM_MONTMUL_P256_NEON_EXEC;
                  fst BIGNUM_MONTMUL_P256_NEON_CORE_EXEC]
      BIGNUM_MONTMUL_P256_NEON_CORRECT));;
...
let BIGNUM_AMONTMUL_P256_NEON_CORE_CORRECT = prove(
  ...
  SUBGOAL_THEN
    `?pc.
      nonoverlapping (word pc,LENGTH
          (APPEND bignum_montmul_p256_core_mc barrier_inst_bytes)) (z:int64,8 * 4) /\
      nonoverlapping (word pc,LENGTH
          (APPEND bignum_montmul_p256_core_mc barrier_inst_bytes)) (x:int64,8 * 4) /\
      nonoverlapping (word pc,LENGTH
          (APPEND bignum_montmul_p256_core_mc barrier_inst_bytes)) (y:int64,8 * 4) /\
      4 divides val (word pc:int64)` MP_TAC THENL [
    REWRITE_TAC[LENGTH_APPEND;BARRIER_INST_BYTES_LENGTH;
        fst BIGNUM_MONTMUL_P256_CORE_EXEC;NONOVERLAPPING_CLAUSES;ALL] THEN
    FIND_HOLE_TAC;
  ...
  VCGEN_EQUIV_TAC BIGNUM_MONTMUL_P256_CORE_EQUIV BIGNUM_AMONTMUL_P256_CORE_CORRECT_N
    [fst BIGNUM_MONTMUL_P256_CORE_EXEC;fst BIGNUM_MONTMUL_P256_NEON_CORE_EXEC] THEN

  (* unfold definitions that may block tactics *)
  RULE_ASSUM_TAC (REWRITE_RULE[NONOVERLAPPING_CLAUSES;fst BIGNUM_MONTMUL_P256_EXEC;
             fst BIGNUM_MONTMUL_P256_NEON_EXEC]) THEN
  REWRITE_TAC[C_ARGUMENTS;BIGNUM_FROM_MEMORY_BYTES] THEN
  REPEAT CONJ_TAC THENL [
    (** SUBGOAL 1. Precond **)
    X_GEN_TAC `s2:armstate` THEN REPEAT STRIP_TAC THEN
...
let BIGNUM_AMONTMUL_P256_NEON_CORRECT = time prove
 (`!z x y a b pc.
  ...
  ARM_SUB_LIST_OF_MC_TAC BIGNUM_AMONTMUL_P256_NEON_CORE_CORRECT
      bignum_montmul_p256_neon_core_mc_def
      [fst BIGNUM_MONTMUL_P256_NEON_EXEC;fst BIGNUM_MONTMUL_P256_NEON_CORE_EXEC]);;
...
let BIGNUM_AMONTMUL_P256_NEON_SUBROUTINE_CORRECT = time prove
 (`!z x y a b pc returnaddress.
  ...
  ARM_ADD_RETURN_NOSTACK_TAC BIGNUM_MONTMUL_P256_NEON_EXEC
    (REWRITE_RULE[fst BIGNUM_MONTMUL_P256_NEON_EXEC;
                  fst BIGNUM_MONTMUL_P256_NEON_CORE_EXEC]
    BIGNUM_AMONTMUL_P256_NEON_CORRECT));;
