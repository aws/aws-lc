...
let bignum_kmul_32_64_neon_mc = define_assert_from_elf "bignum_kmul_32_64_neon_mc" "arm/fastmul/bignum_kmul_32_64_neon.o"
[
...
let BIGNUM_KMUL_32_64_NEON_EXEC = ARM_MK_EXEC_RULE bignum_kmul_32_64_neon_mc;;
...
let LOCAL_MUL_8_16_NEON_CORRECT = prove
 (`!z x y a b pc returnaddress.
      ...
      ==> ensures arm
            (\s. aligned_bytes_loaded s (word(pc + 0)) bignum_kmul_32_64_neon_mc /\
...
let LOCAL_MUL_8_16_NEON_TAC =
  ARM_SUBROUTINE_SIM_TAC
    (bignum_kmul_32_64_neon_mc,BIGNUM_KMUL_32_64_NEON_EXEC,
     0x0,bignum_kmul_32_64_neon_mc,LOCAL_MUL_8_16_NEON_CORRECT)
    [`read X0 s`; `read X1 s`; `read X2 s`;
     `bignum_from_memory (read X1 s,8) s`;
     `bignum_from_memory (read X2 s,8) s`;
     `pc:num`; `read X30 s`];;

let LOCAL_KMUL_16_32_NEON_CORRECT = prove
 (`!z x y a b t pc.
  ...
  ARM_STEPS_TAC BIGNUM_KMUL_32_64_NEON_EXEC (1--5) THEN
  LOCAL_MUL_8_16_NEON_TAC 6 THEN
  ...
  ARM_STEPS_TAC BIGNUM_KMUL_32_64_NEON_EXEC (45--48) THEN
  LOCAL_MUL_8_16_NEON_TAC 49 THEN
  ...
  ARM_STEPS_TAC BIGNUM_KMUL_32_64_NEON_EXEC (125--128) THEN
  LOCAL_MUL_8_16_NEON_TAC 129 THEN
...
let LOCAL_KMUL_16_32_NEON_SUBR_CORRECT = prove
 (`!z x y a b t pc stackpointer returnaddress.
  ...
  ARM_ADD_RETURN_STACK_TAC
   BIGNUM_KMUL_32_64_NEON_EXEC LOCAL_KMUL_16_32_NEON_CORRECT
    `[X19;X20;X21;X22;X23;X30]` 48);;

let LOCAL_KMUL_16_32_NEON_TAC =
  ARM_SUBROUTINE_SIM_TAC
    (bignum_kmul_32_64_neon_mc,BIGNUM_KMUL_32_64_NEON_EXEC,
     0x0,bignum_kmul_32_64_neon_mc,LOCAL_KMUL_16_32_NEON_SUBR_CORRECT)
    [`read X0 s`; `read X1 s`; `read X2 s`;
...
let BIGNUM_KMUL_32_64_NEON_SUBROUTINE_CORRECT = prove(
  `!z x y a b t pc stackpointer returnaddress.
        aligned 16 stackpointer /\
        PAIRWISE nonoverlapping
  ...
  REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THENL
   [ENSURES_EXISTING_PRESERVED_TAC `SP`;
    DISCH_THEN(fun th ->
      ...
      ENSURES_INIT_TAC "s0" THEN
      ARM_STEPS_TAC BIGNUM_KMUL_32_64_NEON_EXEC (1--6) THEN
      MP_TAC th) THEN
    ARM_BIGSTEP_TAC BIGNUM_KMUL_32_64_NEON_EXEC "s7" THEN
    ARM_STEPS_TAC BIGNUM_KMUL_32_64_NEON_EXEC (8--14) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[]] THEN
  ...
  ARM_STEPS_TAC BIGNUM_KMUL_32_64_NEON_EXEC (1--5) THEN
  LOCAL_KMUL_16_32_NEON_TAC 6 THEN

  (*** Second nested multiply: high part ***)

  ARM_STEPS_TAC BIGNUM_KMUL_32_64_NEON_EXEC (7--11) THEN
  LOCAL_KMUL_16_32_NEON_TAC 12 THEN
  ...
  ARM_STEPS_TAC BIGNUM_KMUL_32_64_NEON_EXEC [161] THEN
  RULE_ASSUM_TAC(REWRITE_RULE[WORD_XOR_MASKS]) THEN
 ...
 (*** Split L into L_top and L_bot and form H' = H + L_top ***)
  ...
  ARM_STEPS_TAC BIGNUM_KMUL_32_64_NEON_EXEC (234--238) THEN
  LOCAL_KMUL_16_32_NEON_TAC 239 THEN
