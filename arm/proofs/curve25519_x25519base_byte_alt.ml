...
(**** print_coda_from_elf (-1) "arm/curve25519/curve25519_x25519base_byte_alt.o";;
 ****)

let curve25519_x25519base_byte_alt_mc,curve25519_x25519base_byte_alt_data =
  define_coda_literal_from_elf
  "curve25519_x25519base_byte_alt_mc" "curve25519_x25519base_byte_alt_data"
...
[
  ...
  0x1000c0aa;       (* arm_ADR X10 (word 6164) *)
  0x1000c38b;       (* arm_ADR X11 (word 6256) *)
  ...
  0x1000c233;       (* arm_ADR X19 (word 6212) *)
  ...
  0x10003bc3;       (* arm_ADR X3 (word 1912) *)
...
let BYTES_LOADED_DATA = prove
 (`bytes_loaded s (word (pc + 0x190c)) curve25519_x25519base_byte_alt_data <=>
   read (memory :> bytes(word (pc + 0x190c),48608)) s =
   num_of_bytelist curve25519_x25519base_byte_alt_data`,
  REWRITE_TAC[bytes_loaded; READ_BYTELIST_EQ_BYTES;
    CONV_RULE (RAND_CONV LENGTH_CONV)
     (AP_TERM `LENGTH:byte list->num` curve25519_x25519base_byte_alt_data)]);;

let X25519BASE_TABLE_LEMMA = prove
 (`read (memory :> bytes(word (pc + 0x190c),48608)) s =
   num_of_bytelist curve25519_x25519base_byte_alt_data
   ==> bignum_from_memory(word(pc + 0x190c),4) s = p_25519 /\
       edwards25519_exprojective
        (group_pow edwards25519_group E_25519 (2 EXP 254))
        (bignum_from_memory(word(pc + 0x192c),4) s,
         bignum_from_memory(word(pc + 0x194c),4) s,
         1,
         bignum_from_memory(word(pc + 0x196c),4) s) /\
       edwards25519_exprojective
        (group_pow edwards25519_group E_25519 (2 EXP 254 + 8))
        (bignum_from_memory(word(pc + 0x198c),4) s,
         bignum_from_memory(word(pc + 0x19ac),4) s,
         1,
         bignum_from_memory(word(pc + 0x19cc),4) s) /\
       !i. i < 63
           ==> !j. j < 8
                   ==> edwards25519_epprojective
                        (group_pow edwards25519_group E_25519
                           (2 EXP (4 * (i + 1)) * (j + 1)))
         (bignum_from_memory(word(pc + 0x19ec + 768 * i + 96 * j),4) s,
          bignum_from_memory(word(pc + 0x19ec + 768 * i + 96 * j + 32),4) s,
          bignum_from_memory(word(pc + 0x19ec + 768 * i + 96 * j + 64),4) s) /\
         ~(bignum_from_memory(word(pc + 0x19ec + 768 * i + 96 * j + 64),4) s =
           0)`,
  let GE25519_POWERS =
    end_itlist CONJ
  ...
  REWRITE_TAC[GSYM BYTES_LOADED_DATA; curve25519_x25519base_byte_alt_data] THEN
...
let LOCAL_MUL_P25519_TAC =
  ARM_MACRO_SIM_ABBREV_TAC curve25519_x25519base_byte_alt_mc 100 lvs
   `!(t:armstate) pcin pcout p3 n3 p1 n1 p2 n2.
      ...
      nonoverlapping (word pc,0xd6ec) (word_add (read p3 t) (word n3),8 * 4)
      ==> ensures arm
           (\s. aligned_bytes_loaded s (word pc) curve25519_x25519base_byte_alt_mc /\
                read PC s = pcin /\
                read SP s = read SP t /\
...
let LOCAL_MODINV_TAC =
  ...
  ARM_SUBROUTINE_SIM_TAC
   (curve25519_x25519base_byte_alt_mc,CURVE25519_X25519BASE_BYTE_ALT_EXEC,0x119c,cth,th)
   [`read X1 s`; `read X2 s`;
...
let CURVE25519_X25519BASE_BYTE_ALT_CORRECT = time prove
 (`!res scalar n pc stackpointer.
    ...
    ALL (nonoverlapping (stackpointer,448))
        [(word pc,0xd6ec); (res,32); (scalar,32)] /\
    nonoverlapping (res,32) (word pc,0xd6ec)
    ==> ensures arm
         (\s. aligned_bytes_loaded s (word pc)
               (APPEND curve25519_x25519base_byte_alt_mc
                       curve25519_x25519base_byte_alt_data) /\
              read PC s = word(pc + 0x10) /\
              read SP s = stackpointer /\
              C_ARGUMENTS [res; scalar] s /\
              read (memory :> bytes(scalar,32)) s = n)
         (\s. read PC s = word (pc + 0x18f8) /\
  ...
  REWRITE_TAC[CONJUNCT1 CURVE25519_X25519BASE_BYTE_ALT_EXEC] THEN
  ...
  ENSURES_WHILE_AUP_TAC `1` `64` `pc + 0x1b0` `pc + 0x1100`
   `\i s.
      ...
      read SP s = stackpointer /\
      read X23 s = res /\
      read X19 s = word(pc + 0x19ec + 768 * (i - 1)) /\
      read X20 s = word (4 * i) /\
      val(read X21 s) <= 1 /\
   ...
   [ARITH_TAC;
    ...
    SUBGOAL_THEN
     `nonoverlapping_modulo (2 EXP 64) (val(stackpointer:int64),448)
                                       (val(wpc:int64),0xd6ec)`
    ASSUME_TAC THENL
     [EXPAND_TAC "wpc" THEN NONOVERLAPPING_TAC; ALL_TAC] THEN
    REPEAT(DISCH_THEN(CONJUNCTS_THEN2 ASSUME_TAC MP_TAC)) THEN
    DISCH_THEN(K ALL_TAC) THEN
    BIGNUM_LDIGITIZE_TAC "x0_"
      `bignum_from_memory(word_add wpc (word 0x192c),4) s0` THEN
    BIGNUM_LDIGITIZE_TAC "y0_"
      `bignum_from_memory(word_add wpc (word 0x194c),4) s0` THEN
    BIGNUM_LDIGITIZE_TAC "t0_"
      `bignum_from_memory(word_add wpc (word 0x196c),4) s0` THEN
    BIGNUM_LDIGITIZE_TAC "x1_"
      `bignum_from_memory(word_add wpc (word 0x198c),4) s0` THEN
    BIGNUM_LDIGITIZE_TAC "y1_"
      `bignum_from_memory(word_add wpc (word 0x19ac),4) s0` THEN
    BIGNUM_LDIGITIZE_TAC "t1_"
      `bignum_from_memory(word_add wpc (word 0x19cc),4) s0` THEN
    ...
    ABBREV_TAC `tab:int64 = word(pc + 0x19ec + 768 * (i - 1))` THEN
  ...
  ARM_STEPS_TAC CURVE25519_X25519BASE_BYTE_ALT_EXEC (5--9) THEN
  ...
  ARM_STEPS_TAC CURVE25519_X25519BASE_BYTE_ALT_EXEC (12--73) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
...
let CURVE25519_X25519BASE_BYTE_ALT_SUBROUTINE_CORRECT = time prove
 (`!res scalar n pc stackpointer returnaddress.
  ...
  REWRITE_TAC[ALIGNED_BYTES_LOADED_APPEND_CLAUSE; BYTES_LOADED_DATA;
                 CONJUNCT1 CURVE25519_X25519BASE_BYTE_ALT_EXEC] THEN
  ARM_ADD_RETURN_STACK_TAC CURVE25519_X25519BASE_BYTE_ALT_EXEC
    (REWRITE_RULE[ALIGNED_BYTES_LOADED_APPEND_CLAUSE; BYTES_LOADED_DATA;
                 CONJUNCT1 CURVE25519_X25519BASE_BYTE_ALT_EXEC]
