...
[
  ...
  0x100118ea;       (* arm_ADR X10 (word 8988) *)
  0x10011bcb;       (* arm_ADR X11 (word 9080) *)
  ...
  0x10011a73;       (* arm_ADR X19 (word 9036) *)
...
let BYTES_LOADED_DATA = prove
 (`bytes_loaded s (word (pc + 0x2434)) curve25519_x25519base_byte_alt_data <=>
   read (memory :> bytes(word (pc + 0x2434),48576)) s =
...
let X25519BASE_TABLE_LEMMA = prove
 (`read (memory :> bytes(word (pc + 0x2434),48576)) s =
   ...
   ==> edwards25519_exprojective
        ...
        (bignum_from_memory(word(pc + 0x2434),4) s,
         bignum_from_memory(word(pc + 0x2454),4) s,
         1,
         bignum_from_memory(word(pc + 0x2474),4) s) /\
       edwards25519_exprojective
        (group_pow edwards25519_group E_25519 (2 EXP 254 + 8))
        (bignum_from_memory(word(pc + 0x2494),4) s,
         bignum_from_memory(word(pc + 0x24b4),4) s,
         1,
         bignum_from_memory(word(pc + 0x24d4),4) s) /\
       !i. i < 63
           ==> !j. j < 8
                   ==> edwards25519_epprojective
                        (group_pow edwards25519_group E_25519
                           (2 EXP (4 * (i + 1)) * (j + 1)))
         (bignum_from_memory(word(pc + 0x24f4 + 768 * i + 96 * j),4) s,
          bignum_from_memory(word(pc + 0x24f4 + 768 * i + 96 * j + 32),4) s,
          bignum_from_memory(word(pc + 0x24f4 + 768 * i + 96 * j + 64),4) s) /\
         ~(bignum_from_memory(word(pc + 0x24f4 + 768 * i + 96 * j + 64),4) s =
           0)`,
  let GE25519_POWERS =
    end_itlist CONJ
...
let LOCAL_MUL_P25519_TAC =
  ARM_MACRO_SIM_ABBREV_TAC curve25519_x25519base_byte_alt_mc 100 lvs
   `!(t:armstate) pcin pcout p3 n3 p1 n1 p2 n2.
      ...
      nonoverlapping (word pc,0xe1f4) (word_add (read p3 t) (word n3),8 * 4)
      ==> ensures arm
...
let CURVE25519_X25519BASE_BYTE_ALT_CORRECT = time prove
 (`!res scalar n pc stackpointer.
    ...
    ALL (nonoverlapping (stackpointer,448))
        [(word pc,0xe1f4); (res,32); (scalar,32)] /\
    nonoverlapping (res,32) (word pc,0xe1f4)
    ==> ensures arm
  ...
  ENSURES_WHILE_AUP_TAC `1` `64` `pc + 0x1b0` `pc + 0x1100`
   `\i s.
      ...
      read SP s = stackpointer /\
      read X23 s = res /\
      read X19 s = word(pc + 0x24f4 + 768 * (i - 1)) /\
      read X20 s = word (4 * i) /\
      val(read X21 s) <= 1 /\
   ...
   [ARITH_TAC;
    ...
    SUBGOAL_THEN
     `nonoverlapping_modulo (2 EXP 64) (val(stackpointer:int64),448)
                                       (val(wpc:int64),0xe1f4)`
    ASSUME_TAC THENL
     [EXPAND_TAC "wpc" THEN NONOVERLAPPING_TAC; ALL_TAC] THEN
    REPEAT(DISCH_THEN(CONJUNCTS_THEN2 ASSUME_TAC MP_TAC)) THEN
    DISCH_THEN(K ALL_TAC) THEN
    BIGNUM_LDIGITIZE_TAC "x0_"
      `bignum_from_memory(word_add wpc (word 0x2434),4) s0` THEN
    BIGNUM_LDIGITIZE_TAC "y0_"
      `bignum_from_memory(word_add wpc (word 0x2454),4) s0` THEN
    BIGNUM_LDIGITIZE_TAC "t0_"
      `bignum_from_memory(word_add wpc (word 0x2474),4) s0` THEN
    BIGNUM_LDIGITIZE_TAC "x1_"
      `bignum_from_memory(word_add wpc (word 0x2494),4) s0` THEN
    BIGNUM_LDIGITIZE_TAC "y1_"
      `bignum_from_memory(word_add wpc (word 0x24b4),4) s0` THEN
    BIGNUM_LDIGITIZE_TAC "t1_"
      `bignum_from_memory(word_add wpc (word 0x24d4),4) s0` THEN
    ...
    ABBREV_TAC `tab:int64 = word(pc + 0x24f4 + 768 * (i - 1))` THEN
