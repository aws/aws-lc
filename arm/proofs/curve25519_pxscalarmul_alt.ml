...
[
  ...
  0x54ff8782;       (* arm_BCS (word 2093296) *)
  0xa9400660;       (* arm_LDP X0 X1 X19 (Immediate_Offset (iword (&0))) *)
...
let LOCAL_MUL_P25519_TAC =
  ARM_MACRO_SIM_ABBREV_TAC curve25519_pxscalarmul_alt_mc 102 lvs
   `!(t:armstate) pcin pcout p3 n3 p1 n1 p2 n2.
      ...
      nonoverlapping (word pc,0xffc) (word_add (read p3 t) (word n3),8 * 4)
      ==> ensures arm
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  ARM_ACCSTEPS_TAC CURVE25519_PXSCALARMUL_ALT_EXEC (68--85) (68--85) THEN
  ...
  ARM_ACCSTEPS_TAC CURVE25519_PXSCALARMUL_ALT_EXEC (86--88) (86--88) THEN
  ...
  SUBGOAL_THEN `ca DIV 2 EXP 255 = val(sum_s88:int64)` SUBST_ALL_TAC THENL
   [UNDISCH_TAC `ca DIV 2 EXP 255 + 1 <= 78` THEN REWRITE_TAC[ARITH_RULE
    ...
    ACCUMULATOR_POP_ASSUM_LIST(K ALL_TAC)] THEN
  ARM_STEPS_TAC CURVE25519_PXSCALARMUL_ALT_EXEC (89--90) THEN
  ...
  ARM_ACCSTEPS_TAC CURVE25519_PXSCALARMUL_ALT_EXEC (91--102) (91--102) THEN
...
let LOCAL_SQR_P25519_TAC =
  ARM_MACRO_SIM_ABBREV_TAC curve25519_pxscalarmul_alt_mc 80 lvs
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  ARM_ACCSTEPS_TAC CURVE25519_PXSCALARMUL_ALT_EXEC (46--63) (46--63) THEN
  RULE_ASSUM_TAC(REWRITE_RULE[COND_SWAP; GSYM WORD_BITVAL]) THEN
  ABBREV_TAC
   `ca = bignum_of_wordlist
  ...
  ARM_ACCSTEPS_TAC CURVE25519_PXSCALARMUL_ALT_EXEC (64--66) (64--66) THEN
  ABBREV_TAC `t = bignum_of_wordlist
  ...
  SUBGOAL_THEN `ca DIV 2 EXP 255 = val(sum_s66:int64)` SUBST_ALL_TAC THENL
   [UNDISCH_TAC `ca DIV 2 EXP 255 + 1 <= 78` THEN REWRITE_TAC[ARITH_RULE
    ...
    DISCH_THEN(fun th -> REWRITE_TAC[th]) THEN REAL_INTEGER_TAC;
    ACCUMULATOR_POP_ASSUM_LIST(K ALL_TAC)] THEN
  ARM_STEPS_TAC CURVE25519_PXSCALARMUL_ALT_EXEC (67--68) THEN
  ...
  ARM_ACCSTEPS_TAC CURVE25519_PXSCALARMUL_ALT_EXEC (69--80) (69--80) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  CONV_TAC(LAND_CONV BIGNUM_EXPAND_CONV) THEN ASM_REWRITE_TAC[] THEN
  CONV_TAC SYM_CONV THEN MATCH_MP_TAC MOD_UNIQ_BALANCED_REAL THEN
...
let LOCAL_MUL_5_TAC =
  ARM_MACRO_SIM_ABBREV_TAC curve25519_pxscalarmul_alt_mc 88 lvs
   `!(t:armstate) pcin pcout p3 n3 p1 n1 p2 n2.
      !m. read(memory :> bytes(word_add (read p1 t) (word n1),8 * 4)) t = m
      ==>
      !n. read(memory :> bytes(word_add (read p2 t) (word n2),8 * 4)) t = n
      ==>
      aligned 16 (read SP t) /\
      nonoverlapping (word pc,0xffc) (word_add (read p3 t) (word n3),8 * 5)
      ==> ensures arm
           (\s. aligned_bytes_loaded s (word pc) curve25519_pxscalarmul_alt_mc /\
                read PC s = pcin /\
                read SP s = read SP t /\
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  SUBST1_TAC THENL
   [ONCE_REWRITE_TAC[GSYM MOD_ADD_MOD] THEN
    ...
    REWRITE_TAC[p_25519] THEN CONV_TAC NUM_REDUCE_CONV;
    ALL_TAC] THEN
  ARM_ACCSTEPS_TAC CURVE25519_PXSCALARMUL_ALT_EXEC (68--85) (68--88) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
...
let LOCAL_SQR_4_TAC =
  ARM_MACRO_SIM_ABBREV_TAC curve25519_pxscalarmul_alt_mc 74 lvs
   `!(t:armstate) pcin pcout p3 n3 p1 n1.
      !n.
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  ARM_ACCSTEPS_TAC CURVE25519_PXSCALARMUL_ALT_EXEC (69--72) (69--74) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
...
let LOCAL_ADD5_4_TAC =
  ARM_MACRO_SIM_ABBREV_TAC curve25519_pxscalarmul_alt_mc 22 lvs
   `!(t:armstate) pcin pcout p3 n3 p1 n1 p2 n2.
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  ASM_CASES_TAC `m + n < 2 EXP 58 * 2 EXP 256` THENL
   [ASM_REWRITE_TAC[]; ARM_SIM_TAC CURVE25519_PXSCALARMUL_ALT_EXEC (1--22)] THEN
  REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN ENSURES_INIT_TAC "s0" THEN
  FIRST_ASSUM(BIGNUM_LDIGITIZE_TAC "n_" o lhand o concl) THEN
  FIRST_ASSUM(BIGNUM_LDIGITIZE_TAC "m_" o lhand o concl) THEN

  ARM_ACCSTEPS_TAC CURVE25519_PXSCALARMUL_ALT_EXEC [3;4;7;8;11] (1--11) THEN
  ABBREV_TAC
  ...
  ARM_ACCSTEPS_TAC CURVE25519_PXSCALARMUL_ALT_EXEC (12--14) (12--14) THEN
  ...
  THENL
   [FIRST_ASSUM(MP_TAC o MATCH_MP (ARITH_RULE
    ...
    DISCH_THEN(fun th -> REWRITE_TAC[th]) THEN REAL_INTEGER_TAC;
    ACCUMULATOR_POP_ASSUM_LIST(K ALL_TAC)] THEN
  ARM_STEPS_TAC CURVE25519_PXSCALARMUL_ALT_EXEC (15--16) THEN
  ...
  ARM_ACCSTEPS_TAC CURVE25519_PXSCALARMUL_ALT_EXEC (17--20) (17--22) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  REWRITE_TAC[GSYM CONG; num_congruent] THEN
  REWRITE_TAC[GSYM INT_OF_NUM_CLAUSES] THEN
  MATCH_MP_TAC(MESON[]
...
let LOCAL_SUB5_4_TAC =
  ARM_MACRO_SIM_ABBREV_TAC curve25519_pxscalarmul_alt_mc 29 lvs
   `!(t:armstate) pcin pcout p3 n3 p1 n1 p2 n2.
      !m. read(memory :> bytes(word_add (read p1 t) (word n1),8 * 5)) t = m
      ==>
      !n. read(memory :> bytes(word_add (read p2 t) (word n2),8 * 5)) t = n
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  ASM_CASES_TAC `m < 39 * 2 EXP 256 /\ n < 39 * 2 EXP 256` THENL
   [ASM_REWRITE_TAC[]; ARM_SIM_TAC CURVE25519_PXSCALARMUL_ALT_EXEC (1--29)] THEN
  REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN ENSURES_INIT_TAC "s0" THEN
  FIRST_ASSUM(BIGNUM_LDIGITIZE_TAC "n_" o lhand o concl) THEN
  FIRST_ASSUM(BIGNUM_LDIGITIZE_TAC "m_" o lhand o concl) THEN
  ARM_ACCSTEPS_TAC CURVE25519_PXSCALARMUL_ALT_EXEC
  ...
  ARM_ACCSTEPS_TAC CURVE25519_PXSCALARMUL_ALT_EXEC (19--21) (19--21) THEN
  ...
  THENL
   [FIRST_ASSUM(MP_TAC o MATCH_MP (ARITH_RULE
    ...
    DISCH_THEN(fun th -> REWRITE_TAC[th]) THEN REAL_INTEGER_TAC;
    ACCUMULATOR_POP_ASSUM_LIST(K ALL_TAC)] THEN
  ARM_STEPS_TAC CURVE25519_PXSCALARMUL_ALT_EXEC (22--23) THEN
  ...
  ARM_ACCSTEPS_TAC CURVE25519_PXSCALARMUL_ALT_EXEC (24--27) (24--29) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  REWRITE_TAC[GSYM CONG; num_congruent] THEN
  REWRITE_TAC[GSYM INT_OF_NUM_CLAUSES] THEN
  MATCH_MP_TAC(MESON[]
...
let CURVE25519_PXSCALARMUL_ALT_CORRECT = time prove
 (`!res point X scalar n pc stackpointer.
  ...
  ENSURES_WHILE_PDOWN_TAC `256` `pc + 0x50` `pc + 0xf60`
   `\i s.
     (read SP s = stackpointer /\
      read X17 s = res /\
      read X19 s = point /\
  ...
  REPEAT CONJ_TAC THENL
   [ARITH_TAC;
    ...
    SUBGOAL_THEN
     `(if ~(val
            (word_sub (word_or (word_or x_0 x_1) (word_or x_2 x_3):int64) (word 0)) =
            0)
       then word 0
       else word 1):int64 =
      word(bitval(X = 0))`
  ...
  RULE_ASSUM_TAC(ONCE_REWRITE_RULE[TAUT `(ODD n <=> b) <=> (b <=> ODD n)`]) THEN
