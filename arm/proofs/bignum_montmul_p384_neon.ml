...
let bignum_montmul_p384_interm1_core_mc_def,
  ...
  mk_sublist_of_mc "bignum_montmul_p384_interm1_core_mc"
    ...
    BIGNUM_MONTMUL_P384_INTERM1_EXEC;;
...
let BIGNUM_MONTMUL_P384_CORE_EQUIV2 = time prove(
  ...
  REWRITE_TAC[MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI;SOME_FLAGS;
    ALLPAIRS;ALL;NONOVERLAPPING_CLAUSES;
    BIGNUM_MONTMUL_P384_INTERM1_CORE_EXEC;
    BIGNUM_MONTMUL_P384_NEON_CORE_EXEC] THEN
  REPEAT STRIP_TAC THEN
  (** Initialize **)
  EQUIV_INITIATE_TAC equiv_input_states THEN
  REPEAT (FIRST_X_ASSUM BIGNUM_EXPAND_AND_DIGITIZE_TAC) THEN
  ...
  SUBGOAL_THEN
    ...
    ASM_REWRITE_TAC
      [ALL;NONOVERLAPPING_CLAUSES;
       BIGNUM_MONTMUL_P384_INTERM1_CORE_EXEC;
       BIGNUM_MONTMUL_P384_NEON_CORE_EXEC;
       BIGNUM_MONTMUL_P384_CORE_EXEC;GSYM CONJ_ASSOC] THEN
    STRIP_TAC THEN ASM_REWRITE_TAC[] THEN POP_ASSUM_LIST (K ALL_TAC) THEN
    FIND_HOLE_TAC;

    ALL_TAC
  ] THEN
  DISCH_THEN (CHOOSE_THEN (DESTRUCT_TAC "h1 h2 h3 h4")) THEN


  FIRST_X_ASSUM (fun th -> ASSUME_TAC (SPEC_ALL (MATCH_MP BIGNUM_MONTMUL_P384_CORE_EQUIV1 th))) THEN
  FIRST_X_ASSUM (fun th -> ASSUME_TAC (SPEC_ALL (MATCH_MP BIGNUM_MONTMUL_P384_CORE_EQUIV2 th))) THEN
  ...
  REPEAT CONJ_TAC THENL [
    ...
    EXISTS_TAC
      `write (memory :> bytelist
          (word pc3,LENGTH bignum_montmul_p384_interm1_core_mc))
          bignum_montmul_p384_interm1_core_mc
          (write PC (word pc3) s')` THEN
    PROVE_CONJ_OF_EQ_READS_TAC BIGNUM_MONTMUL_P384_INTERM1_CORE_EXEC THENL [
      UNDISCH_TAC `equiv_input_states (s,s') x y z` THEN
      REWRITE_TAC[equiv_input_states;C_ARGUMENTS;BIGNUM_FROM_MEMORY_BYTES;BIGNUM_MONTMUL_P384_INTERM1_CORE_EXEC] THEN
      STRIP_TAC THEN ASM_REWRITE_TAC[] THEN
      MAP_EVERY EXISTS_TAC [`a:num`;`b:num`] THEN
      REWRITE_TAC[] THEN
      PROVE_CONJ_OF_EQ_READS_TAC BIGNUM_MONTMUL_P384_INTERM1_CORE_EXEC;
...
let event_n_at_pc_goal = mk_eventually_n_at_pc_statement
    ...
    bignum_montmul_p384_core_mc (*pc_ofs*)0
    `\s0. C_ARGUMENTS [z;x;y] s0`;;

let BIGNUM_MONTMUL_P384_EVENTUALLY_N_AT_PC = prove(event_n_at_pc_goal,

  REWRITE_TAC[LENGTH_APPEND;BIGNUM_MONTMUL_P384_CORE_EXEC;BARRIER_INST_BYTES_LENGTH] THEN
  ...
  SUBGOAL_THEN `4 divides (LENGTH bignum_montmul_p384_core_mc)`
        (fun th -> REWRITE_TAC[MATCH_MP aligned_bytes_loaded_append th;
                              BIGNUM_MONTMUL_P384_CORE_EXEC]) THENL [
    REWRITE_TAC[BIGNUM_MONTMUL_P384_CORE_EXEC]
...
let BIGNUM_MONTMUL_P384_NEON_CORE_CORRECT = time prove(
  ...
  SUBGOAL_THEN
    ...
    REWRITE_TAC[BIGNUM_MONTMUL_P384_CORE_EXEC;NONOVERLAPPING_CLAUSES;ALL;
  ...
  VCGEN_EQUIV_TAC BIGNUM_MONTMUL_P384_CORE_EQUIV BIGNUM_MONTMUL_P384_CORE_CORRECT_N
    [BIGNUM_MONTMUL_P384_CORE_EXEC] THEN

  (* unfold definitions that may block tactics *)
  RULE_ASSUM_TAC (REWRITE_RULE[NONOVERLAPPING_CLAUSES;
      BIGNUM_MONTMUL_P384_EXEC;BIGNUM_MONTMUL_P384_NEON_EXEC]) THEN
  ...
  REPEAT CONJ_TAC THENL [
    ...
    EXISTS_TAC
      `write (memory :> bytelist
          (word pc,LENGTH (APPEND bignum_montmul_p384_core_mc barrier_inst_bytes)))
          (APPEND bignum_montmul_p384_core_mc barrier_inst_bytes)
          (write PC (word pc) s2)` THEN
    (* Expand variables appearing in the equiv relation *)
    REPEAT CONJ_TAC THEN
    TRY (PROVE_CONJ_OF_EQ_READS_TAC BIGNUM_MONTMUL_P384_CORE_EXEC) THEN
...
let BIGNUM_MONTMUL_P384_NEON_CORRECT = time prove(
  ...
  ARM_SUB_LIST_OF_MC_TAC BIGNUM_MONTMUL_P384_NEON_CORE_CORRECT
      ...
      [BIGNUM_MONTMUL_P384_NEON_EXEC;BIGNUM_MONTMUL_P384_NEON_CORE_EXEC]);;
...
let BIGNUM_MONTMUL_P384_NEON_SUBROUTINE_CORRECT = time prove
 (`!z x y a b pc stackpointer returnaddress.
  ...
  REWRITE_TAC[BIGNUM_MONTMUL_P384_NEON_EXEC] THEN
  ARM_ADD_RETURN_STACK_TAC
   BIGNUM_MONTMUL_P384_NEON_EXEC
   (let th = REWRITE_RULE [BIGNUM_MONTMUL_P384_NEON_CORE_EXEC;
        BIGNUM_MONTMUL_P384_NEON_EXEC;GSYM ADD_ASSOC]
...
let BIGNUM_AMONTMUL_P384_NEON_CORE_CORRECT = time prove(
  ...
  VCGEN_EQUIV_TAC BIGNUM_MONTMUL_P384_CORE_EQUIV BIGNUM_AMONTMUL_P384_CORE_CORRECT_N
    [BIGNUM_MONTMUL_P384_CORE_EXEC] THEN
