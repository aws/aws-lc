...
(**** print_literal_from_elf "arm/p256/p256_montjadd.o";;
 ****)

let p256_montjadd_mc = define_assert_from_elf
  "p256_montjadd_mc" "arm/p256/p256_montjadd.o"
[
...
let LOCAL_MONTSQR_P256_TAC =
  ARM_MACRO_SIM_ABBREV_TAC p256_montjadd_mc 124 lvs
  `!(t:armstate) pcin pcout p3 n3 p1 n1.
    !a. read(memory :> bytes(word_add (read p1 t) (word n1),8 * 4)) t = a
    ==>
    aligned 16 (read SP t) /\
    nonoverlapping (word pc,0x2bb0) (word_add (read p3 t) (word n3),32)
    ==> ensures arm
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  ARM_ACCSTEPS_TAC P256_MONTJADD_EXEC (66--111) (66--111) THEN
  ABBREV_TAC
...
let LOCAL_MONTMUL_P256_TAC =
  ARM_MACRO_SIM_ABBREV_TAC p256_montjadd_mc 175 lvs
...
(REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
 ...
 (*** Globalize the a * b <= 2 EXP 256 * p_256  assumption ***)
  ...
  ASM_CASES_TAC `a * b <= 2 EXP 256 * p_256` THENL
   [ASM_REWRITE_TAC[]; ARM_SIM_TAC P256_MONTJADD_EXEC (1--175)] THEN
  ENSURES_INIT_TAC "s0" THEN
  ...
  ***)
  ...
  ARM_ACCSTEPS_TAC P256_MONTJADD_EXEC (29--46) (29--46) THEN
  SUBGOAL_THEN
  ...
  ARM_ACCSTEPS_TAC P256_MONTJADD_EXEC [71;72;76;78] (71--78) THEN
  ...
  ARM_ACCSTEPS_TAC P256_MONTJADD_EXEC [79;80;84;86] (79--86) THEN
  RULE_ASSUM_TAC(REWRITE_RULE[ADD_CLAUSES; WORD_RULE
   `word_sub (word 0) x = word_neg x`]) THEN
  SUBGOAL_THEN
  ...
  ARM_STEPS_TAC P256_MONTJADD_EXEC [87] THEN
  ...
  ARM_ACCSTEPS_TAC P256_MONTJADD_EXEC (88--94) (88--94) THEN
  RULE_ASSUM_TAC(REWRITE_RULE[ADD_CLAUSES]) THEN
  SUBGOAL_THEN
   `bignum_of_wordlist
  ...
  ARM_ACCSTEPS_TAC P256_MONTJADD_EXEC (136--154) (136--154) THEN
  ABBREV_TAC
   `t = bignum_of_wordlist
  ...
  ARM_ACCSTEPS_TAC P256_MONTJADD_EXEC [166;169;170;173] (166--175) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  CONV_TAC(LAND_CONV BIGNUM_EXPAND_CONV) THEN ASM_REWRITE_TAC[] THEN
  TRANS_TAC EQ_TRANS `t MOD p_256` THEN CONJ_TAC THENL
   [ALL_TAC;
...
let LOCAL_SUB_P256_TAC =
  ARM_MACRO_SIM_ABBREV_TAC p256_montjadd_mc 19 lvs
  `!(t:armstate) pcin pcout p3 n3 p1 n1 p2 n2.
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  ARM_ACCSTEPS_TAC P256_MONTJADD_EXEC (1--8) (1--8) THEN
  ...
  ARM_STEPS_TAC P256_MONTJADD_EXEC [9] THEN
  RULE_ASSUM_TAC(REWRITE_RULE[WORD_UNMASK_64; NOT_LE]) THEN
  ARM_ACCSTEPS_TAC P256_MONTJADD_EXEC (10--19) (10--19) THEN
...
let P256_MONTJADD_CORRECT = time prove
 (`!p3 p1 t1 p2 t2 pc stackpointer.
        ...
        ALL (nonoverlapping (stackpointer,224))
            [(word pc,0x2bb0); (p1,96); (p2,96); (p3,96)] /\
        nonoverlapping (p3,96) (word pc,0x2bb0)
        ==> ensures arm
             (\s. aligned_bytes_loaded s (word pc) p256_montjadd_mc /\
  ...
  ARM_STEPS_TAC P256_MONTJADD_EXEC (27--85) THEN
...
let P256_MONTJADD_SUBROUTINE_CORRECT = time prove
 (`!p3 p1 t1 p2 t2 pc stackpointer returnaddress.
  ...
  ARM_ADD_RETURN_STACK_TAC P256_MONTJADD_EXEC
   P256_MONTJADD_CORRECT `[X19; X20]` 240);;
