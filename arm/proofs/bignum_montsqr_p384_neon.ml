...
let bignum_montsqr_p384_interm1_core_mc_def,
  ...
  mk_sublist_of_mc "bignum_montsqr_p384_interm1_core_mc"
    ...
    BIGNUM_MONTSQR_P384_INTERM1_EXEC;;
...
let BIGNUM_MONTSQR_P384_CORE_EQUIV2 = time prove(
  ...
  REWRITE_TAC[MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI;SOME_FLAGS;
    ALLPAIRS;ALL;NONOVERLAPPING_CLAUSES;
    BIGNUM_MONTSQR_P384_INTERM1_CORE_EXEC;
    BIGNUM_MONTSQR_P384_NEON_CORE_EXEC] THEN
  REPEAT STRIP_TAC THEN
  (** Initialize **)
  EQUIV_INITIATE_TAC equiv_input_states THEN
  REPEAT (FIRST_X_ASSUM BIGNUM_EXPAND_AND_DIGITIZE_TAC) THEN
...
let BIGNUM_MONTSQR_P384_CORE_EQUIV = time prove(equiv_goal,
  ...
  FIRST_X_ASSUM (fun th -> ASSUME_TAC (SPEC_ALL (MATCH_MP BIGNUM_MONTSQR_P384_CORE_EQUIV1 th))) THEN
  FIRST_X_ASSUM (fun th -> ASSUME_TAC (SPEC_ALL (MATCH_MP BIGNUM_MONTSQR_P384_CORE_EQUIV2 th))) THEN
  ...
  REPEAT CONJ_TAC THENL [
    ...
    PROVE_CONJ_OF_EQ_READS_TAC BIGNUM_MONTSQR_P384_INTERM1_CORE_EXEC THENL [
      ...
      REWRITE_TAC[equiv_input_states;C_ARGUMENTS;BIGNUM_FROM_MEMORY_BYTES;BIGNUM_MONTSQR_P384_INTERM1_CORE_EXEC] THEN
...
let event_n_at_pc_goal = mk_eventually_n_at_pc_statement
    ...
    bignum_montsqr_p384_core_mc (*pc_ofs*)0
    `\s0. C_ARGUMENTS [z;x] s0`;;

let BIGNUM_MONTSQR_P384_EVENTUALLY_N_AT_PC = prove(event_n_at_pc_goal,

  REWRITE_TAC[LENGTH_APPEND;BIGNUM_MONTSQR_P384_CORE_EXEC;BARRIER_INST_BYTES_LENGTH] THEN
  ...
  SUBGOAL_THEN `4 divides (LENGTH bignum_montsqr_p384_core_mc)`
        (fun th -> REWRITE_TAC[MATCH_MP aligned_bytes_loaded_append th;
                              BIGNUM_MONTSQR_P384_CORE_EXEC]) THENL [
    REWRITE_TAC[BIGNUM_MONTSQR_P384_CORE_EXEC] THEN CONV_TAC NUM_DIVIDES_CONV;
    ALL_TAC] THEN
  REPEAT GEN_TAC THEN STRIP_TAC THEN
  (* now start..! *)
...
let BIGNUM_MONTSQR_P384_NEON_CORE_CORRECT = prove(
  ...
  SUBGOAL_THEN
    ...
    REWRITE_TAC[BIGNUM_MONTSQR_P384_CORE_EXEC;NONOVERLAPPING_CLAUSES;ALL;
  ...
  VCGEN_EQUIV_TAC BIGNUM_MONTSQR_P384_CORE_EQUIV BIGNUM_MONTSQR_P384_CORE_CORRECT_N
    [BIGNUM_MONTSQR_P384_CORE_EXEC;BIGNUM_MONTSQR_P384_NEON_CORE_EXEC] THEN

  (* unfold definitions that may block tactics *)
  RULE_ASSUM_TAC (REWRITE_RULE[NONOVERLAPPING_CLAUSES;BIGNUM_MONTSQR_P384_EXEC;
             BIGNUM_MONTSQR_P384_NEON_EXEC]) THEN
  ...
  REPEAT CONJ_TAC THENL [
    ...
    MAP_EVERY EXISTS_TAC [`a:num`] THEN
    REPEAT CONJ_TAC THEN
    TRY (PROVE_CONJ_OF_EQ_READS_TAC BIGNUM_MONTSQR_P384_CORE_EXEC);
...
let BIGNUM_MONTSQR_P384_NEON_CORRECT = time prove(
  ...
  ARM_SUB_LIST_OF_MC_TAC BIGNUM_MONTSQR_P384_NEON_CORE_CORRECT
    ...
    [BIGNUM_MONTSQR_P384_NEON_CORE_EXEC;BIGNUM_MONTSQR_P384_NEON_EXEC]);;
...
let BIGNUM_MONTSQR_P384_NEON_SUBROUTINE_CORRECT = time prove
 (`!z x a pc returnaddress.
        ...
        ==> ensures arm
             ...
             (MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * 6)])`,
  REWRITE_TAC[BIGNUM_MONTSQR_P384_NEON_EXEC] THEN
  ARM_ADD_RETURN_NOSTACK_TAC BIGNUM_MONTSQR_P384_NEON_EXEC
    (REWRITE_RULE [BIGNUM_MONTSQR_P384_NEON_EXEC;
        BIGNUM_MONTSQR_P384_NEON_CORE_EXEC]
