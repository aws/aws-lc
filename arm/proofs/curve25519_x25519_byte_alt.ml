...
let LOCAL_MUL_P25519_TAC =
  ARM_MACRO_SIM_ABBREV_TAC curve25519_x25519_byte_alt_mc 100 lvs
   `!(t:armstate) pcin pcout p3 n3 p1 n1 p2 n2.
      ...
      nonoverlapping (word pc,0x32d0) (word_add (read p3 t) (word n3),8 * 4)
      ==> ensures arm
...
let LOCAL_MODINV_TAC =
  ...
  ARM_SUBROUTINE_SIM_TAC
   (curve25519_x25519_byte_alt_mc,CURVE25519_X25519_BYTE_ALT_EXEC,0x2b28,cth,th)
   [`read X1 s`; `read X2 s`;
...
let CURVE25519_X25519_BYTE_ALT_CORRECT = time prove
 (`!res scalar n point X pc stackpointer.
    ...
    ALL (nonoverlapping (stackpointer,320))
        [(word pc,0x32d0); (res,32); (scalar,32); (point,32)] /\
    nonoverlapping (res,32) (word pc,0x32d0)
    ==> ensures arm
         (\s. aligned_bytes_loaded s (word pc) curve25519_x25519_byte_alt_mc /\
  ...
  ARM_STEPS_TAC CURVE25519_X25519_BYTE_ALT_EXEC (36--45) THEN
  SUBGOAL_THEN
  ...
  ARM_STEPS_TAC CURVE25519_X25519_BYTE_ALT_EXEC (47--60) THEN
  ...
  LOCAL_MUL_P25519_TAC 0 ["zn"; "xn"; "zm"] THEN

  (*** Bytewise store and finish of simulation ***)

  BIGNUM_LDIGITIZE_TAC "res_"
   `read(memory :> bytes(word_add stackpointer (word 160),8 * 4)) s61` THEN
  ARM_STEPS_TAC CURVE25519_X25519_BYTE_ALT_EXEC (62--123) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
