...
[
  ...
  0x1001672a;       (* arm_ADR X10 (word 11492) *)
  0x10016a0b;       (* arm_ADR X11 (word 11584) *)
  ...
  0x100168b3;       (* arm_ADR X19 (word 11540) *)
...
let CURVE25519_X25519BASE_BYTE_CORRECT = time prove
 (`!res scalar n pc stackpointer.
  ...
  ENSURES_WHILE_AUP_TAC `1` `64` `pc + 0x1b0` `pc + 0x1988`
   `\i s.
      ...
      read SP s = stackpointer /\
      read X23 s = res /\
      read X19 s = word(pc + 0x2ebc + 768 * (i - 1)) /\
      read X20 s = word (4 * i) /\
      val(read X21 s) <= 1 /\
  ...
  REPEAT CONJ_TAC THENL
   [ARITH_TAC;
    ...
    BIGNUM_LDIGITIZE_TAC "x0_"
      `bignum_from_memory(word_add wpc (word 0x2dfc),4) s0` THEN
    BIGNUM_LDIGITIZE_TAC "y0_"
      `bignum_from_memory(word_add wpc (word 0x2e1c),4) s0` THEN
    BIGNUM_LDIGITIZE_TAC "t0_"
      `bignum_from_memory(word_add wpc (word 0x2e3c),4) s0` THEN
    BIGNUM_LDIGITIZE_TAC "x1_"
      `bignum_from_memory(word_add wpc (word 0x2e5c),4) s0` THEN
    BIGNUM_LDIGITIZE_TAC "y1_"
      `bignum_from_memory(word_add wpc (word 0x2e7c),4) s0` THEN
    BIGNUM_LDIGITIZE_TAC "t1_"
      `bignum_from_memory(word_add wpc (word 0x2e9c),4) s0` THEN
    ...
    ABBREV_TAC `tab:int64 = word(pc + 0x2ebc + 768 * (i - 1))` THEN
  ...
  ARM_STEPS_TAC CURVE25519_X25519BASE_BYTE_EXEC (5--6) THEN
 ...
 (*** Bytewise store and finish of simulation ***)
  ...
  ARM_STEPS_TAC CURVE25519_X25519BASE_BYTE_EXEC (9--70) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
