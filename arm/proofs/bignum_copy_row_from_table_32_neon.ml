...
let BIGNUM_COPY_ROW_FROM_TABLE_32_NEON_EXEC =
  ARM_MK_EXEC_RULE bignum_copy_row_from_table_32_neon_mc;;
...
let BIGNUM_COPY_ROW_FROM_TABLE_32_NEON_SUBROUTINE_CORRECT = prove(
  `!z table height idx pc n m returnaddress.
    ...
    val idx < val height
    ==> ensures arm
      (\s. aligned_bytes_loaded s (word pc) bignum_copy_row_from_table_32_neon_mc /\
           read PC s = word pc /\
  ...
  ENSURES_WHILE_PUP_TAC `val (height:64 word):num` `pc + 0x44` `pc + 0xdc`
    ...
    (* 2. to loop start *)
    ARM_SIM_TAC BIGNUM_COPY_ROW_FROM_TABLE_32_NEON_EXEC (1--17) THEN
    ...
    (* 4. loop backedge *)
    REPEAT STRIP_TAC THEN
    ARM_SIM_TAC BIGNUM_COPY_ROW_FROM_TABLE_32_NEON_EXEC (1--1);
    ...
    ARM_STEPS_TAC BIGNUM_COPY_ROW_FROM_TABLE_32_NEON_EXEC (1--18) THEN
  ...
  ASM_CASES_TAC `i <= val (idx:int64)` THENL [
    ...
    ARM_STEPS_TAC BIGNUM_COPY_ROW_FROM_TABLE_32_NEON_EXEC (1--38) THEN
    ASM_CASES_TAC `i = val (idx:int64)` THENL [
