...
(**** print_literal_from_elf "arm/p256/p256_montjadd_alt.o";;
 ****)

let p256_montjadd_alt_mc = define_assert_from_elf
  "p256_montjadd_alt_mc" "arm/p256/p256_montjadd_alt.o"
[
...
let P256_MONTJADD_ALT_EXEC = ARM_MK_EXEC_RULE p256_montjadd_alt_mc;;
...
let LOCAL_MONTSQR_P256_TAC =
  ARM_MACRO_SIM_ABBREV_TAC p256_montjadd_alt_mc 95 lvs
  `!(t:armstate) pcin pcout p3 n3 p1 n1.
    !a. read(memory :> bytes(word_add (read p1 t) (word n1),8 * 4)) t = a
    ==>
    aligned 16 (read SP t) /\
    nonoverlapping (word pc,0x1ee0) (word_add (read p3 t) (word n3),32)
    ==> ensures arm
         (\s. aligned_bytes_loaded s (word pc) p256_montjadd_alt_mc /\
              read PC s = pcin /\
              read SP s = read SP t /\
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  ASM_CASES_TAC `a EXP 2 <= 2 EXP 256 * p_256` THENL
   [ASM_REWRITE_TAC[]; ARM_SIM_TAC P256_MONTJADD_ALT_EXEC (1--95)] THEN
  ENSURES_INIT_TAC "s0" THEN
  FIRST_ASSUM(BIGNUM_DIGITIZE_TAC "x_" o lhand o concl) THEN

  (*** Simulate the core pre-reduced result accumulation ***)

  ARM_ACCSTEPS_TAC P256_MONTJADD_ALT_EXEC (1--82) (1--82) THEN
  ...
  ARM_ACCSTEPS_TAC P256_MONTJADD_ALT_EXEC (85--89) (83--95) THEN
...
let LOCAL_MONTMUL_P256_TAC =
  ARM_MACRO_SIM_ABBREV_TAC p256_montjadd_alt_mc 117 lvs
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  ASM_CASES_TAC `a * b <= 2 EXP 256 * p_256` THENL
   [ASM_REWRITE_TAC[]; ARM_SIM_TAC P256_MONTJADD_ALT_EXEC (1--117)] THEN
  ENSURES_INIT_TAC "s0" THEN
  ...
  ARM_ACCSTEPS_TAC P256_MONTJADD_ALT_EXEC (1--104) (1--104) THEN
  ...
  ARM_ACCSTEPS_TAC P256_MONTJADD_ALT_EXEC (107--111) (105--117) THEN
  RULE_ASSUM_TAC(REWRITE_RULE
   [GSYM WORD_BITVAL; COND_SWAP; REAL_BITVAL_NOT]) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  CONV_TAC(LAND_CONV BIGNUM_EXPAND_CONV) THEN ASM_REWRITE_TAC[] THEN
...
let LOCAL_SUB_P256_TAC =
  ARM_MACRO_SIM_ABBREV_TAC p256_montjadd_alt_mc 19 lvs
  `!(t:armstate) pcin pcout p3 n3 p1 n1 p2 n2.
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  ARM_ACCSTEPS_TAC P256_MONTJADD_ALT_EXEC (1--8) (1--8) THEN
  ...
  ARM_STEPS_TAC P256_MONTJADD_ALT_EXEC [9] THEN
  RULE_ASSUM_TAC(REWRITE_RULE[WORD_UNMASK_64; NOT_LE]) THEN
  ARM_ACCSTEPS_TAC P256_MONTJADD_ALT_EXEC (10--19) (10--19) THEN
...
let LOCAL_AMONTSQR_P256_TAC =
  ARM_MACRO_SIM_ABBREV_TAC p256_montjadd_alt_mc 93 lvs
  `!(t:armstate) pcin pcout p3 n3 p1 n1.
    !a. read(memory :> bytes(word_add (read p1 t) (word n1),8 * 4)) t = a
    ==>
    aligned 16 (read SP t) /\
 ...
 (*** Simulate the core pre-reduced result accumulation ***)
  ...
  ARM_ACCSTEPS_TAC P256_MONTJADD_ALT_EXEC (1--81) (1--81) THEN
  RULE_ASSUM_TAC(REWRITE_RULE[COND_SWAP; GSYM WORD_BITVAL]) THEN
  ABBREV_TAC
   `t = bignum_of_wordlist
  ...
  ARM_ACCSTEPS_TAC P256_MONTJADD_ALT_EXEC (88--91) (82--93) THEN
  RULE_ASSUM_TAC(REWRITE_RULE
   [GSYM WORD_BITVAL; COND_SWAP; REAL_BITVAL_NOT]) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  CONJ_TAC THENL
...
let P256_MONTJADD_ALT_CORRECT = time prove
 (`!p3 p1 t1 p2 t2 pc stackpointer.
        ...
        ALL (nonoverlapping (stackpointer,224))
            [(word pc,0x1ee0); (p1,96); (p2,96); (p3,96)] /\
        nonoverlapping (p3,96) (word pc,0x1ee0)
        ==> ensures arm
             (\s. aligned_bytes_loaded s (word pc) p256_montjadd_alt_mc /\
  ...
  LOCAL_AMONTSQR_P256_TAC 3 ["z1sq";"z_1"] THEN
  ...
  ARM_STEPS_TAC P256_MONTJADD_ALT_EXEC (27--85) THEN
