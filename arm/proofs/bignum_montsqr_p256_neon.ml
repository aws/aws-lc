...
let BIGNUM_MONTSQR_P256_CORE_EQUIV2 = prove(
  ...
  REWRITE_TAC[MODIFIABLE_SIMD_REGS;SOME_FLAGS;
    ...
    fst BIGNUM_MONTSQR_P256_NEON_CORE_EXEC] THEN
  REPEAT STRIP_TAC THEN
  (** Initialize **)
  EQUIV_INITIATE_TAC montsqr_p256_eqin THEN
  REPEAT (FIRST_X_ASSUM BIGNUM_EXPAND_AND_DIGITIZE_TAC) THEN
...
let BIGNUM_MONTSQR_P256_CORE_EQUIV = prove(equiv_goal,
  ...
  SUBGOAL_THEN
    ...
    ASM_REWRITE_TAC
      [ALL;NONOVERLAPPING_CLAUSES;
       ...
       fst BIGNUM_MONTSQR_P256_CORE_EXEC;GSYM CONJ_ASSOC] THEN
    STRIP_TAC THEN
    ASM_REWRITE_TAC[] THEN
    FIND_HOLE_TAC;
  ...
  ENSURES2_TRANS_TAC BIGNUM_MONTSQR_P256_CORE_EQUIV1 BIGNUM_MONTSQR_P256_CORE_EQUIV2 THEN
  ...
  REPEAT CONJ_TAC THENL [
    ...
    PROVE_CONJ_OF_EQ_READS_TAC BIGNUM_MONTSQR_P256_INTERM1_CORE_EXEC THENL [
      UNDISCH_TAC `montsqr_p256_eqin (s,s') x z` THEN
      REWRITE_TAC[montsqr_p256_eqin;C_ARGUMENTS;BIGNUM_FROM_MEMORY_BYTES;
                  fst BIGNUM_MONTSQR_P256_INTERM1_CORE_EXEC] THEN
      STRIP_TAC THEN ASM_REWRITE_TAC[] THEN
      REPEAT (TRY HINT_EXISTS_REFL_TAC THEN
        PROVE_CONJ_OF_EQ_READS_TAC BIGNUM_MONTSQR_P256_INTERM1_CORE_EXEC);
    ...
    ASM_REWRITE_TAC[] THEN ASM_MESON_TAC[montsqr_p256_eqout_TRANS];
...
let event_n_at_pc_goal = mk_eventually_n_at_pc_statement_simple
    `nonoverlapping (word pc:int64,LENGTH
        (APPEND bignum_montsqr_p256_core_mc barrier_inst_bytes)) (z:int64,8 * 4)`
    [`z:int64`;`x:int64`] bignum_montsqr_p256_core_mc
    `\s0. C_ARGUMENTS [z;x] s0`;;

let BIGNUM_MONTSQR_P256_EVENTUALLY_N_AT_PC = prove(event_n_at_pc_goal,
  REWRITE_TAC[LENGTH_APPEND;fst BIGNUM_MONTSQR_P256_CORE_EXEC;BARRIER_INST_BYTES_LENGTH] THEN
  ...
  SUBGOAL_THEN `4 divides (LENGTH bignum_montsqr_p256_core_mc)`
        (fun th -> REWRITE_TAC[MATCH_MP aligned_bytes_loaded_append th;
                               fst BIGNUM_MONTSQR_P256_CORE_EXEC]) THENL [
    REWRITE_TAC[fst BIGNUM_MONTSQR_P256_CORE_EXEC] THEN CONV_TAC NUM_DIVIDES_CONV;
    ALL_TAC] THEN
...
let BIGNUM_MONTSQR_P256_NEON_CORE_CORRECT = prove(
  `!z x a pc2.
    nonoverlapping (word pc2,LENGTH bignum_montsqr_p256_neon_core_mc) (z,8 * 4)
    ==> ensures arm
          (\s. aligned_bytes_loaded s (word pc2) bignum_montsqr_p256_neon_core_mc /\
              read PC s = word pc2 /\
              C_ARGUMENTS [z; x] s /\
              bignum_from_memory (x,4) s = a)
          (\s. read PC s = word (pc2 + LENGTH bignum_montsqr_p256_neon_core_mc) /\
  ...
  SUBGOAL_THEN
    `?pc.
      nonoverlapping (word pc,LENGTH
        (APPEND bignum_montsqr_p256_core_mc barrier_inst_bytes)) (z:int64,8 * 4) /\
      nonoverlapping (word pc,LENGTH
        (APPEND bignum_montsqr_p256_core_mc barrier_inst_bytes)) (x:int64,8 * 4) /\
      4 divides val (word pc:int64)` MP_TAC THENL [
    REWRITE_TAC[LENGTH_APPEND;BARRIER_INST_BYTES_LENGTH;
      fst BIGNUM_MONTSQR_P256_CORE_EXEC;NONOVERLAPPING_CLAUSES;ALL] THEN
    FIND_HOLE_TAC;
  ...
  VCGEN_EQUIV_TAC BIGNUM_MONTSQR_P256_CORE_EQUIV BIGNUM_MONTSQR_P256_CORE_CORRECT_N
    [fst BIGNUM_MONTSQR_P256_CORE_EXEC;fst BIGNUM_MONTSQR_P256_NEON_CORE_EXEC] THEN
  ...
  REPEAT CONJ_TAC THENL [
    ...
    ASM_REWRITE_TAC[montsqr_p256_eqin;C_ARGUMENTS] THEN
    EXISTS_TAC
      `write (memory :> bytelist
    ...
    REPEAT (TRY HINT_EXISTS_REFL_TAC THEN
        PROVE_CONJ_OF_EQ_READS_TAC BIGNUM_MONTSQR_P256_CORE_EXEC);

    (** SUBGOAL 2. Postcond **)
    MESON_TAC[montsqr_p256_eqout;BIGNUM_FROM_MEMORY_BYTES;
      fst BIGNUM_MONTSQR_P256_CORE_EXEC;
      fst BIGNUM_MONTSQR_P256_NEON_CORE_EXEC];
 ...
 (`!z x a pc.
      ...
      ==> ensures arm
          (\s. aligned_bytes_loaded s (word pc) bignum_montsqr_p256_neon_mc /\
              ...
              C_ARGUMENTS [z; x] s /\
              bignum_from_memory (x,4) s = a)
          (\s. read PC s = word (pc + LENGTH bignum_montsqr_p256_neon_core_mc) /\
              (a EXP 2 <= 2 EXP 256 * p_256
                ==> bignum_from_memory (z,4) s =
                    (inverse_mod p_256 (2 EXP 256) * a EXP 2) MOD p_256))
          (MAYCHANGE [PC; X1; X2; X3; X4; X5; X6; X7; X8; X9; X10; X11; X12;
  ...
  ARM_SUB_LIST_OF_MC_TAC BIGNUM_MONTSQR_P256_NEON_CORE_CORRECT
      bignum_montsqr_p256_neon_core_mc_def
      [fst BIGNUM_MONTSQR_P256_NEON_EXEC;
       fst BIGNUM_MONTSQR_P256_NEON_CORE_EXEC]);;
 ...
 (`!z x a pc returnaddress.
        ...
        ==> ensures arm
             ...
             (MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * 4)])`,
  REWRITE_TAC[fst BIGNUM_MONTSQR_P256_NEON_EXEC] THEN
  ARM_ADD_RETURN_NOSTACK_TAC BIGNUM_MONTSQR_P256_NEON_EXEC
    (REWRITE_RULE[fst BIGNUM_MONTSQR_P256_NEON_EXEC;fst BIGNUM_MONTSQR_P256_NEON_CORE_EXEC] BIGNUM_MONTSQR_P256_NEON_CORRECT));;
...
let BIGNUM_AMONTSQR_P256_NEON_CORE_CORRECT = prove(
  ...
  VCGEN_EQUIV_TAC BIGNUM_MONTSQR_P256_CORE_EQUIV BIGNUM_AMONTSQR_P256_CORE_CORRECT_N
    [fst BIGNUM_MONTSQR_P256_CORE_EXEC;fst BIGNUM_MONTSQR_P256_NEON_CORE_EXEC] THEN
 ...
 (`!z x a pc.
        ...
        ==> ensures arm
             ...
             (MAYCHANGE [PC; X1; X2; X3; X4; X5; X6; X7; X8; X9; X10; X11; X12;
              ...
              MAYCHANGE [memory :> bytes(z,8 * 4)] ,,
              MAYCHANGE SOME_FLAGS)`,
  ARM_SUB_LIST_OF_MC_TAC BIGNUM_AMONTSQR_P256_NEON_CORE_CORRECT
    bignum_montsqr_p256_neon_core_mc_def
    [fst BIGNUM_MONTSQR_P256_NEON_EXEC;
     fst BIGNUM_MONTSQR_P256_NEON_CORE_EXEC]);;
...
let BIGNUM_AMONTSQR_P256_NEON_SUBROUTINE_CORRECT = time prove
 (`!z x a pc returnaddress.
  ...
  REWRITE_TAC[fst BIGNUM_MONTSQR_P256_NEON_EXEC] THEN
  ARM_ADD_RETURN_NOSTACK_TAC BIGNUM_MONTSQR_P256_NEON_EXEC
    (REWRITE_RULE[fst BIGNUM_MONTSQR_P256_NEON_EXEC;
                  fst BIGNUM_MONTSQR_P256_NEON_CORE_EXEC]
