...
let BIGNUM_MUL_8_16_EQUIV = prove(
  `!pc pc2 z x y.
    ALL (nonoverlapping (z,8 * 16)) [
      (word pc,LENGTH bignum_mul_8_16_mc); (word pc2,LENGTH bignum_mul_8_16_neon_mc);
      (x,8 * 8); (y,8 * 8)]
    ==> ensures2 arm
      (\(s,s2). aligned_bytes_loaded s (word (pc + 0x0c)) bignum_mul_8_16_core_mc /\
                read PC s = word (pc + 0x0c) /\
                aligned_bytes_loaded s2 (word pc2) bignum_mul_8_16_neon_mc /\
                read PC s2 = word (pc2 + 0x0c) /\
                bignum_mul_8_16_equiv_input_states (s,s2) x y z)
      (\(s,s2). aligned_bytes_loaded s (word (pc + 0x0c)) bignum_mul_8_16_core_mc /\
                read PC s = word (pc + 4*(461-1)) /\
                aligned_bytes_loaded s2 (word pc2) bignum_mul_8_16_neon_mc /\
                read PC s2 = word (pc2 + 4*(497-1)) /\
                bignum_mul_8_16_equiv_output_states (s,s2) x y z)
      (\(s,s2) (s',s2').
      ...
      (\s. (4*(461-1) - 0x0c) DIV 4) (\s2. (4*(497-1) - 0x0c) DIV 4)`,
  ...
  (* necessary to run ldr qs. TODO: automatize this process *)
  ...
  BYTES128_EQ_JOIN64_TAC `read (memory :> bytes128 y) s0'` `b'_1:int64` `b'_0:int64` THEN
  BYTES128_EQ_JOIN64_TAC `read (memory :> bytes128 (word_add y (word 16))) s0'` `b'_3:int64` `b'_2:int64` THEN
  BYTES128_EQ_JOIN64_TAC `read (memory :> bytes128 (word_add y (word 32))) s0'` `b'_5:int64` `b'_4:int64` THEN
  BYTES128_EQ_JOIN64_TAC `read (memory :> bytes128 (word_add y (word 48))) s0'` `b'_7:int64` `b'_6:int64` THEN
  REPEAT_N 2 must_be_smarter_tactic THEN
  (* Abbreviate pc to help tactics. *)
  ABBREV_TAC `pc' = pc + 12` THEN
  SUBGOAL_THEN `nonoverlapping_modulo (2 EXP 64)
                (val (z:int64),8 * 16) (pc',LENGTH bignum_mul_8_16_core_mc)`
  (fun th -> ASSUME_TAC (REWRITE_RULE[BIGNUM_MUL_8_16_CORE_EXEC] th)) THENL [
  REWRITE_TAC[BIGNUM_MUL_8_16_CORE_EXEC] THEN EXPAND_TAC "pc'" THEN
  NONOVERLAPPING_TAC;
  ALL_TAC] THEN
  ...
  MAP_EVERY
    (fun action ->
      EQUIV_STEP_TAC action BIGNUM_MUL_8_16_CORE_EXEC BIGNUM_MUL_8_16_NEON_EXEC)
    actions THEN
...
(** Now we will prove that bignum_mul_8_16_neon is correct using
    ...
    (1) An updated 'ensures' that has a 'barrier' attached at the end of
        the core
**)

let BIGNUM_MUL_8_16_CORE_WITH_BARRIER_CORRECT =
  prove_correct_barrier_appended BIGNUM_MUL_8_16_CORE_CORRECT
    `bignum_mul_8_16_core_mc` BIGNUM_MUL_8_16_CORE_EXEC;;

(**
    (2) A proof stating that, after a specified amount of steps,
        any postcondition will be satisfied at the end of the program.
**)

let BIGNUM_MUL_8_16_EVENTUALLY_N_AT_PC = prove(
  `!pc z x y.
    ALL (nonoverlapping (z,8 * 16)) [
      (word pc,LENGTH (bignum_mul_8_16_mc)); (x,8 * 8); (y,8 * 8)]
    ==> eventually_n_at_pc
          (pc+0x0c) (APPEND bignum_mul_8_16_core_mc barrier_inst_bytes)
          (pc+0x0c) (pc+0x0c+LENGTH bignum_mul_8_16_core_mc)
          ((LENGTH bignum_mul_8_16_core_mc) DIV 4)
  ...
  SUBGOAL_THEN
      `nonoverlapping_modulo (2 EXP 64) (val (z:int64), 128)
                             (pc', LENGTH bignum_mul_8_16_mc - 12)`
      MP_TAC THENL [
    REWRITE_TAC[BIGNUM_MUL_8_16_EXEC] THEN
    CONV_TAC (ONCE_DEPTH_CONV NUM_SUB_CONV) THEN
    EXPAND_TAC "pc'" THEN NONOVERLAPPING_TAC; ALL_TAC] THEN
  DISCH_THEN (fun th ->
    let th = REWRITE_RULE[BIGNUM_MUL_8_16_EXEC] th in
  ...
  REPEAT_I_N 0 457
    (fun i -> EVENTUALLY_TAKE_STEP_RIGHT_FORALL_TAC
        BIGNUM_MUL_8_16_CORE_EXEC `s0:armstate` 0 i 457 THEN
      DISCARD_OLDSTATE_TAC ("s" ^ (if i = 456 then "_final" else string_of_int (i+1)))) THEN
  ...
  REPEAT_I_N 0 457 (fun i ->
    ...
    DISJ_CASES_THEN2
      ...
      SOLVE_EXISTS_ARM_TAC (snd (CONJ_PAIR BIGNUM_MUL_8_16_CORE_EXEC));

      (** SUBGOAL 2 **)
      EVENTUALLY_STEPS_EXISTS_STEP_TAC BIGNUM_MUL_8_16_CORE_EXEC i (4*(i+1)) THEN
...
(** By combining BIGNUM_MUL_8_16_CORE_WITH_BARRIER_CORRECT and
    BIGNUM_MUL_8_16_EVENTUALLY_N_AT_PC, now we can prove
    ensures_n version of BIGNUM_MUL_8_16_CORE_CORRECT
**)
...
let BIGNUM_MUL_8_16_NEON_CORRECT = prove(
  ...
  SUBGOAL_THEN
    ...
    (** SUBGOAL 1 **)
    REWRITE_TAC[LENGTH_APPEND;BIGNUM_MUL_8_16_EXEC;BARRIER_INST_BYTES_LENGTH;
  ...
  EXISTS_TAC `\(s:armstate). (4*(497-1) - 0x0c) DIV 4` THEN
  (* Prepare a 'conjunction' of SPEC and EQUIV *)
  MP_TAC (
      let ensures_n_part =
          SPECL [`z:int64`;`x:int64`;`y:int64`;`a:num`;`b:num`;`pc:num`]
                BIGNUM_MUL_8_16_CORE_CORRECT_N in
      let equiv_part =
          SPECL [`pc:num`;`pc2:num`;`z:int64`;`x:int64`;`y:int64`] BIGNUM_MUL_8_16_EQUIV in
  ...
  REPEAT CONJ_TAC THENL [
    ...
    SUBGOAL_THEN `4 divides val (word (pc+12):int64)` (fun th -> REWRITE_TAC[th]) THENL [
      (** SUBGOAL 1 **)
    ...
    REPEAT CONJ_TAC THEN
      ...
      (TRY (EXPAND_RHS_TAC THEN
            REWRITE_TAC[LENGTH_APPEND;BIGNUM_MUL_8_16_CORE_EXEC;BARRIER_INST_BYTES_LENGTH] THEN
