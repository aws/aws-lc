...
let LOCAL_MUL_P25519_TAC =
  ARM_MACRO_SIM_ABBREV_TAC curve25519_x25519_byte_mc 180 lvs
   `!(t:armstate) pcin pcout p3 n3 p1 n1 p2 n2.
      ...
      nonoverlapping (word pc,0x4ed0) (word_add (read p3 t) (word n3),8 * 4)
      ==> ensures arm
...
let LOCAL_MODINV_TAC =
  ...
  ARM_SUBROUTINE_SIM_TAC
   (curve25519_x25519_byte_mc,CURVE25519_X25519_BYTE_EXEC,0x45e8,cth,th)
   [`read X1 s`; `read X2 s`;
...
let CURVE25519_X25519_BYTE_CORRECT = time prove
 (`!res scalar n point X pc stackpointer.
    ...
    ALL (nonoverlapping (stackpointer,320))
        [(word pc,0x4ed0); (res,32); (scalar,32); (point,32)] /\
    nonoverlapping (res,32) (word pc,0x4ed0)
    ==> ensures arm
         (\s. aligned_bytes_loaded s (word pc) curve25519_x25519_byte_mc /\
  ...
  ARM_STEPS_TAC CURVE25519_X25519_BYTE_EXEC (36--45) THEN
  SUBGOAL_THEN
  ...
  ARM_STEPS_TAC CURVE25519_X25519_BYTE_EXEC (47--60) THEN
  ...
  ARM_STEPS_TAC CURVE25519_X25519_BYTE_EXEC (62--123) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
