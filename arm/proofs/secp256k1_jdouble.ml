...
let secp256k1_encode = new_definition
  `secp256k1_encode = modular_encode(256,p_256k1)`;;

let nintlemma = prove
 (`&(num_of_int(x rem &p_256k1)) = x rem &p_256k1`,
  MATCH_MP_TAC INT_OF_NUM_OF_INT THEN MATCH_MP_TAC INT_REM_POS THEN
  REWRITE_TAC[INT_OF_NUM_EQ; p_256k1] THEN CONV_TAC NUM_REDUCE_CONV);;
...
let unilemma2 = prove
 (`X = num_of_int(x rem &p_256k1) ==> X < p_256k1 /\ &X = x rem &p_256k1`,
  DISCH_THEN SUBST1_TAC THEN
  REWRITE_TAC[GSYM INT_OF_NUM_LT; nintlemma; INT_LT_REM_EQ] THEN
  REWRITE_TAC[INT_OF_NUM_LT; p_256k1] THEN CONV_TAC NUM_REDUCE_CONV);;
...
let LOCAL_SQR_P256K1_TAC =
  ARM_MACRO_SIM_ABBREV_TAC secp256k1_jdouble_mc 76 lvs
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  ARM_ACCSTEPS_TAC SECP256K1_JDOUBLE_EXEC (1--45) (1--45) THEN
  ...
  ARM_ACCSTEPS_TAC SECP256K1_JDOUBLE_EXEC (46--62) (46--62) THEN
  ...
  ARM_STEPS_TAC SECP256K1_JDOUBLE_EXEC [63] THEN
  ...
  ARM_ACCSTEPS_TAC SECP256K1_JDOUBLE_EXEC (64--76) (64--76) THEN
...
let LOCAL_MUL_P256K1_TAC =
  ARM_MACRO_SIM_ABBREV_TAC secp256k1_jdouble_mc 98 lvs
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  ARM_ACCSTEPS_TAC SECP256K1_JDOUBLE_EXEC (1--67) (1--67) THEN
  MAP_EVERY ABBREV_TAC
  ...
  ARM_ACCSTEPS_TAC SECP256K1_JDOUBLE_EXEC (68--84) (68--84) THEN
  RULE_ASSUM_TAC(REWRITE_RULE[COND_SWAP; GSYM WORD_BITVAL]) THEN
  ABBREV_TAC
   `ca = bignum_of_wordlist
  ...
  SUBGOAL_THEN `ca DIV 2 EXP 256 = val(sum_s84:int64)` SUBST_ALL_TAC THENL
   [EXPAND_TAC "ca" THEN CONV_TAC(LAND_CONV BIGNUM_OF_WORDLIST_DIV_CONV) THEN
    ...
    FIRST_ASSUM(ASSUME_TAC o MATCH_MP (ARITH_RULE
     `n + 1 < 2 EXP 64 ==> n < 2 EXP 64 - 1`))] THEN
  ARM_STEPS_TAC SECP256K1_JDOUBLE_EXEC [85] THEN
  ...
  ARM_ACCSTEPS_TAC SECP256K1_JDOUBLE_EXEC (86--98) (86--98) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  CONV_TAC(LAND_CONV BIGNUM_EXPAND_CONV) THEN ASM_REWRITE_TAC[] THEN
  CONV_TAC SYM_CONV THEN MATCH_MP_TAC MOD_UNIQ_BALANCED_REAL THEN
  MAP_EVERY EXISTS_TAC [`val(q:int64)`; `256`] THEN ASM_REWRITE_TAC[] THEN
...
let LOCAL_ROUGHSQR_P256K1_TAC =
  ARM_MACRO_SIM_ABBREV_TAC secp256k1_jdouble_mc 65 lvs
  `!(t:armstate) pcin pcout p3 n3 p1 n1.
    !n. read(memory :> bytes(word_add (read p1 t) (word n1),8 * 4)) t = n
    ==>
    aligned 16 (read SP t) /\
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  SUBST1_TAC THENL
   [ONCE_REWRITE_TAC[GSYM MOD_ADD_MOD] THEN
    ...
    REWRITE_TAC[p_256k1] THEN CONV_TAC NUM_REDUCE_CONV;
    ALL_TAC] THEN
  ARM_ACCSTEPS_TAC SECP256K1_JDOUBLE_EXEC (46--62) (46--65) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
...
let LOCAL_ROUGHMUL_P256K1_TAC =
  ARM_MACRO_SIM_ABBREV_TAC secp256k1_jdouble_mc 87 lvs
  `!(t:armstate) pcin pcout p3 n3 p1 n1 p2 n2.
    !m. read(memory :> bytes(word_add (read p1 t) (word n1),8 * 4)) t = m
    ==>
    !n. read(memory :> bytes(word_add (read p2 t) (word n2),8 * 4)) t = n
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  SUBST1_TAC THENL
   [ONCE_REWRITE_TAC[GSYM MOD_ADD_MOD] THEN
    ...
    REWRITE_TAC[p_256k1] THEN CONV_TAC NUM_REDUCE_CONV;
    ALL_TAC] THEN
  ARM_ACCSTEPS_TAC SECP256K1_JDOUBLE_EXEC (68--84) (68--87) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
 ...
 (`!p3 p1 t1 pc stackpointer.
        ...
        ==> ensures arm
             ...
             (\s. read PC s = word (pc + 0xb1c) /\
                  (!x y z.
                        ~(z = &0) /\
                        t1 = tripled secp256k1_encode (x,y,z)
                        ==> bignum_triple_from_memory(p3,4) s =
                            tripled secp256k1_encode
                             (jacobian_add secp256k1 (x,y,z) (x,y,z))))
  ...
  REWRITE_TAC[tripled; secp256k1_encode; PAIR_EQ; modular_encode] THEN
  ...
  (*** Manually push around a few moduli to avoid fuss later ***)

  SUBGOAL_THEN
   `!a b x y p. (a * x - b * y) rem p = (a * x rem p - b * y rem p) rem p`
   (fun th -> RULE_ASSUM_TAC(ONCE_REWRITE_RULE[th]))
  THENL
   [CONV_TAC INT_REM_DOWN_CONV THEN REPEAT GEN_TAC THEN REFL_TAC;
    ALL_TAC] THEN
  SUBGOAL_THEN
   `!a x y p. (a * x - y) rem p = (a * x rem p - y) rem p`
   (fun th -> RULE_ASSUM_TAC(ONCE_REWRITE_RULE[th]))
  THENL
   [CONV_TAC INT_REM_DOWN_CONV THEN REPEAT GEN_TAC THEN REFL_TAC;
    ALL_TAC] THEN
  SUBGOAL_THEN `(z * tmp) MOD p_256k1 = (z * tmp MOD p_256k1) MOD p_256k1`
  SUBST_ALL_TAC THENL [CONV_TAC MOD_DOWN_CONV THEN REFL_TAC; ALL_TAC] THEN
  ...
  ASM_REWRITE_TAC[tripled; jacobian_add; jacobian_eq; secp256k1] THEN
  ASM_REWRITE_TAC[GSYM secp256k1] THEN
