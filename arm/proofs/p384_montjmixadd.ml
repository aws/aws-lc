...
let LOCAL_MONTSQR_P384_TAC =
  ARM_MACRO_SIM_ABBREV_TAC p384_montjmixadd_mc 215 lvs
  `!(t:armstate) pcin pcout p3 n3 p1 n1.
    !a. read(memory :> bytes(word_add (read p1 t) (word n1),8 * 6)) t = a
    ==>
    aligned 16 (read SP t) /\
    nonoverlapping (word pc,0x2ef0) (word_add (read p3 t) (word n3),48)
    ==> ensures arm
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  ASM_CASES_TAC `a EXP 2 <= 2 EXP 384 * p_384` THENL
   [ASM_REWRITE_TAC[]; ARM_SIM_TAC P384_MONTJMIXADD_EXEC (1--215)] THEN
  ENSURES_INIT_TAC "s0" THEN
  FIRST_ASSUM(BIGNUM_DIGITIZE_TAC "x_" o lhand o concl) THEN

  (*** Main squaring block ***)

  ARM_ACCSTEPS_TAC P384_MONTJMIXADD_EXEC (1--93) (1--93) THEN
...
let LOCAL_MONTMUL_P384_TAC =
  ARM_MACRO_SIM_ABBREV_TAC p384_montjmixadd_mc 271 lvs
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  ASM_CASES_TAC `a * b <= 2 EXP 384 * p_384` THENL
   [ASM_REWRITE_TAC[]; ARM_SIM_TAC P384_MONTJMIXADD_EXEC (1--271)] THEN
  ENSURES_INIT_TAC "s0" THEN
  ...
  ARM_ACCSTEPS_TAC P384_MONTJMIXADD_EXEC (1--149) (1--149) THEN
...
let LOCAL_AMONTSQR_P384_TAC =
  ARM_MACRO_SIM_ABBREV_TAC p384_montjmixadd_mc 209 lvs
  `!(t:armstate) pcin pcout p3 n3 p1 n1.
    !a. read(memory :> bytes(word_add (read p1 t) (word n1),8 * 6)) t = a
    ==>
    aligned 16 (read SP t) /\
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  ARM_ACCSTEPS_TAC P384_MONTJMIXADD_EXEC (201--206) (197--209) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  CONJ_TAC THENL
...
let P384_MONTJMIXADD_CORRECT = time prove
 (`!p3 p1 t1 p2 t2 pc stackpointer.
        ...
        ALLPAIRS nonoverlapping
         ...
         [(word pc,0x2ef0); (p1,144); (p2,96)] /\
        nonoverlapping (p3,144) (stackpointer,288)
        ==> ensures arm
             (\s. aligned_bytes_loaded s (word pc) p384_montjmixadd_mc /\
  ...
  LOCAL_MONTMUL_P384_TAC 0 ["z_3";"xd";"z_1"] THEN
...
let P384_MONTJMIXADD_SUBROUTINE_CORRECT = time prove
 (`!p3 p1 t1 p2 t2 pc stackpointer returnaddress.
        ...
        ALLPAIRS nonoverlapping
         ...
         [(word pc,0x2ef0); (p1,144); (p2,96)] /\
        nonoverlapping (p3,144) (word_sub stackpointer (word 352),352)
        ==> ensures arm
             (\s. aligned_bytes_loaded s (word pc) p384_montjmixadd_mc /\
                  read PC s = word pc /\
                  read SP s = stackpointer /\
