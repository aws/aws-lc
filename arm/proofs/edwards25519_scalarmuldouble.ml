...
[
  ...
  0x10033d0e;       (* arm_ADR X14 (word 26528) *)
  0xd2800020;       (* arm_MOV X0 (rvalue (word 1)) *)
  0xaa1f03e1;       (* arm_MOV X1 XZR *)
  0xaa1f03e2;       (* arm_MOV X2 XZR *)
  0xaa1f03e3;       (* arm_MOV X3 XZR *)
  ...
  0x910103f6;       (* arm_ADD X22 SP (rvalue (word 64)) *)
  0x910103f7;       (* arm_ADD X23 SP (rvalue (word 64)) *)
  0x940007d7;       (* arm_BL (word 8028) *)
  0x910303f6;       (* arm_ADD X22 SP (rvalue (word 192)) *)
  0x910303f7;       (* arm_ADD X23 SP (rvalue (word 192)) *)
  0x910503f8;       (* arm_ADD X24 SP (rvalue (word 320)) *)
  0x940012ef;       (* arm_BL (word 19388) *)
  0x910103f6;       (* arm_ADD X22 SP (rvalue (word 64)) *)
  0x910103f7;       (* arm_ADD X23 SP (rvalue (word 64)) *)
  0x940007d0;       (* arm_BL (word 8000) *)
  0x910103f6;       (* arm_ADD X22 SP (rvalue (word 64)) *)
  0x910103f7;       (* arm_ADD X23 SP (rvalue (word 64)) *)
  ...
  0x100300a3;       (* arm_ADR X3 (word 24596) *)
...
let BYTES_LOADED_DATA = prove
 (`bytes_loaded s (word (pc + 0x7278)) edwards25519_scalarmuldouble_data <=>
   read (memory :> bytes(word (pc + 0x7278),800)) s =
...
let EDWARDS25519DOUBLEBASE_TABLE_LEMMA = prove
 (`read (memory :> bytes(word (pc + 0x7278),800)) s =
   num_of_bytelist edwards25519_scalarmuldouble_data
   ==> bignum_from_memory(word(pc + 0x7278),4) s = p_25519 /\
       !i. i < 8
           ==> edwards25519_epprojective
                (group_pow edwards25519_group E_25519 (i + 1))
         (bignum_from_memory(word(pc + 0x7298 + 96 * i),4) s,
          bignum_from_memory(word(pc + 0x7298 + 96 * i + 32),4) s,
          bignum_from_memory(word(pc + 0x7298 + 96 * i + 64),4) s) /\
         ~(bignum_from_memory(word(pc + 0x7298 + 96 * i + 64),4) s =
           0)`,
  let GE25519_POWERS =
    end_itlist CONJ
...
let LOCAL_MUL_P25519_TAC =
  ARM_MACRO_SIM_ABBREV_TAC edwards25519_scalarmuldouble_mc 180 lvs
   `!(t:armstate) pcin pcout p3 n3 p1 n1 p2 n2.
      ...
      nonoverlapping (word pc,0x7598) (word_add (read p3 t) (word n3),8 * 4)
      ==> ensures arm
...
let LOCAL_EPDOUBLE_CORRECT = time prove
 (`!p3 p1 T1 pc stackpointer.
    ...
    ALL (nonoverlapping (stackpointer,160))
        [(word pc,0x7598); (p3,128); (p1,96)] /\
    nonoverlapping (p3,128) (word pc,0x7598)
    ==> ensures arm
         (\s. aligned_bytes_loaded s (word pc)
                 edwards25519_scalarmuldouble_mc /\
              read PC s = word(pc + 0x1d14) /\
              read SP s = stackpointer /\
              read X22 s = p3 /\
              read X23 s = p1 /\
              bignum_triple_from_memory(p1,4) s = T1)
         (\s. read PC s = word (pc + 0x316c) /\
...
let LOCAL_PDOUBLE_CORRECT = time prove
 (`!p3 p1 T1 pc stackpointer.
    aligned 16 stackpointer /\
    ALL (nonoverlapping (stackpointer,160))
        [(word pc,0x7598); (p3,96); (p1,96)] /\
    nonoverlapping (p3,96) (word pc,0x7598)
    ==> ensures arm
         (\s. aligned_bytes_loaded s (word pc)
                 edwards25519_scalarmuldouble_mc /\
              read PC s = word(pc + 0x3178) /\
              read SP s = stackpointer /\
              read X22 s = p3 /\
              read X23 s = p1 /\
              bignum_triple_from_memory(p1,4) s = T1)
         (\s. read PC s = word (pc + 0x4320) /\
              !P1. P1 IN group_carrier edwards25519_group /\
                   edwards25519_projective2 P1 T1
...
let LOCAL_EPADD_CORRECT = time prove
 (`!p3 p1 Q1 p2 Q2 pc stackpointer.
    ...
    ALL (nonoverlapping (stackpointer,192))
        [(word pc,0x7598); (p3,128); (p1,128); (p2,128)] /\
    nonoverlapping (p3,128) (word pc,0x7598)
    ==> ensures arm
         (\s. aligned_bytes_loaded s (word pc)
                 edwards25519_scalarmuldouble_mc /\
              read PC s = word(pc + 0x432c) /\
              read SP s = stackpointer /\
              read X22 s = p3 /\
              read X23 s = p1 /\
              read X24 s = p2 /\
...
let LOCAL_PEPADD_CORRECT = time prove
 (`!p3 p1 Q1 p2 T2 pc stackpointer.
    aligned 16 stackpointer /\
    ALL (nonoverlapping (stackpointer,192))
        [(word pc,0x7598); (p3,128); (p1,128); (p2,96)] /\
    nonoverlapping (p3,128) (word pc,0x7598)
    ==> ensures arm
         (\s. aligned_bytes_loaded s (word pc)
                 edwards25519_scalarmuldouble_mc /\
              read PC s = word(pc + 0x5de8) /\
              read SP s = stackpointer /\
              read X22 s = p3 /\
              read X23 s = p1 /\
              read X24 s = p2 /\
...
let EDWARDS25519_SCALARMULDOUBLE_CORRECT = time prove
 (`!res scalar point bscalar n xy m pc stackpointer.
    ...
    ALL (nonoverlapping (stackpointer,1632))
        [(word pc,0x7598); (res,64); (scalar,32); (point,64); (bscalar,32)] /\
    nonoverlapping (res,64) (word pc,0x7598)
    ==> ensures arm
         (\s. aligned_bytes_loaded s (word pc)
  ...
  REPEAT CONJ_TAC THENL
   [ARITH_TAC;
    ...
    REWRITE_TAC[ARITH_RULE `pc + 0x7298 + x = (pc + 0x7298) + x`] THEN
    GEN_REWRITE_TAC (LAND_CONV o ONCE_DEPTH_CONV) [WORD_ADD] THEN
    ABBREV_TAC `wpc:int64 = word(pc + 0x7298)` THEN
    ...
    ARM_STEPS_TAC EDWARDS25519_SCALARMULDOUBLE_EXEC (1--6) THEN
