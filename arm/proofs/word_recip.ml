 ...
 (`!a pc.
  ...
  STRIP_ASSUME_TAC THENL
   [REWRITE_TAC[GSYM REAL_NOT_LT] THEN
    SIMP_TAC[REAL_LT_INTEGERS; INTEGER_CLOSED] THEN
    REWRITE_TAC[REAL_NOT_LE] THEN
    POP_ASSUM(SUBST1_TAC o SYM) THEN CONJ_TAC THEN ASM BOUNDER_TAC[];
    ALL_TAC] THEN
  ...
  STRIP_ASSUME_TAC THENL
   [REWRITE_TAC[GSYM REAL_NOT_LT] THEN CONV_TAC REAL_RAT_REDUCE_CONV THEN
    SIMP_TAC[REAL_LT_INTEGERS; INTEGER_CLOSED] THEN
    REWRITE_TAC[REAL_NOT_LE] THEN
    POP_ASSUM(SUBST1_TAC o SYM) THEN CONJ_TAC THEN ASM BOUNDER_TAC[];
    ALL_TAC] THEN
  ...
  STRIP_ASSUME_TAC THENL
   [REWRITE_TAC[GSYM REAL_NOT_LT] THEN CONV_TAC REAL_RAT_REDUCE_CONV THEN
    SIMP_TAC[REAL_LT_INTEGERS; INTEGER_CLOSED] THEN
    REWRITE_TAC[REAL_NOT_LE] THEN FIRST_X_ASSUM(SUBST1_TAC o SYM) THEN
    CONJ_TAC THENL [ALL_TAC; ASM BOUNDER_TAC[]] THEN
  ...
  STRIP_ASSUME_TAC THENL
   [REWRITE_TAC[GSYM REAL_NOT_LT] THEN CONV_TAC REAL_RAT_REDUCE_CONV THEN
    SIMP_TAC[REAL_LT_INTEGERS; INTEGER_CLOSED] THEN
    REWRITE_TAC[REAL_NOT_LE] THEN CONJ_TAC THENL
     [FIRST_X_ASSUM(SUBST1_TAC o SYM) THEN
      REWRITE_TAC[REAL_ARITH `(e * z + z * d):real = z * (e + d)`] THEN
      REPEAT(FIRST_X_ASSUM(SUBST_ALL_TAC o MATCH_MP (REAL_ARITH
       `&(val x) = a ==> a = &(val x)`))) THEN
  ...
  RULE_ASSUM_TAC(REWRITE_RULE[DIMINDEX_64; NUM_MOD_CONV `60 MOD 64`]) THEN
