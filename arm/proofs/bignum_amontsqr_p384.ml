...
(* ========================================================================= *)
(* Almost-Montgomery squaring modulo p_384.                                  *)
(* ========================================================================= *)

(**** print_literal_from_elf "arm/p384/bignum_amontsqr_p384.o";;
 ****)

let bignum_amontsqr_p384_mc =
  define_assert_from_elf "bignum_amontsqr_p384_mc" "arm/p384/bignum_amontsqr_p384.o"
[
...
let BIGNUM_AMONTSQR_P384_EXEC = ARM_MK_EXEC_RULE bignum_amontsqr_p384_mc;;
...
let BIGNUM_AMONTSQR_P384_CORRECT = time prove
 (`!z x a pc.
        nonoverlapping (word pc,0x3f8) (z,8 * 6)
        ==> ensures arm
             (\s. aligned_bytes_loaded s (word pc) bignum_amontsqr_p384_mc /\
                  read PC s = word pc /\
                  C_ARGUMENTS [z; x] s /\
                  bignum_from_memory (x,6) s = a)
             (\s. read PC s = word (pc + 0x3f4) /\
  ...
  ARM_ACCSTEPS_TAC BIGNUM_AMONTSQR_P384_EXEC (1--28) (1--28) THEN
  SUBGOAL_THEN
  ...
  montred_tac BIGNUM_AMONTSQR_P384_EXEC
  ...
  montred_subst_tac BIGNUM_AMONTSQR_P384_EXEC
  ...
  ARM_STEPS_TAC BIGNUM_AMONTSQR_P384_EXEC (74--76) THEN

  (*** ADK cross-product ***)

  ARM_ACCSTEPS_TAC BIGNUM_AMONTSQR_P384_EXEC
 ...
 (*** Double the cross-product and add the Montgomerified lower square ***)
  ...
  ARM_ACCSTEPS_TAC BIGNUM_AMONTSQR_P384_EXEC (139--155) (139--155) THEN
  ...
  ARM_ACCSTEPS_TAC BIGNUM_AMONTSQR_P384_EXEC (201--237) (201--237) THEN
  RULE_ASSUM_TAC(REWRITE_RULE[ADD_CLAUSES; VAL_WORD_BITVAL]) THEN
  ...
  SUBGOAL_THEN `sum_s237:int64 = word(bitval(2 EXP 384 <= t))`
  SUBST_ALL_TAC THENL
  ...
  ARM_ACCSTEPS_TAC BIGNUM_AMONTSQR_P384_EXEC
   [241;244;247;248;249;250] (238--253) THEN
 ...
 (`!z x a pc returnaddress.
        ...
        ==> ensures arm
             ...
             (MAYCHANGE [PC; X1; X2; X3; X4; X5; X6; X7; X8; X9; X10; X11; X12;
              ...
              MAYCHANGE [memory :> bytes(z,8 * 6)] ,,
              MAYCHANGE SOME_FLAGS)`,
  ARM_ADD_RETURN_NOSTACK_TAC BIGNUM_AMONTSQR_P384_EXEC
