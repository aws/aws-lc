...
let BIGNUM_TRIPLE_P521_CORRECT = time prove
 (`!z x n pc.
  ...
  RULE_ASSUM_TAC(REWRITE_RULE[DIMINDEX_64; NUM_REDUCE_CONV `63 MOD 64`]) THEN
  ...
  SUBST_ALL_TAC THENL
   [TRANS_TAC EQ_TRANS `512 <= (m + n + 1) DIV 2 EXP (8 * 64)` THEN
    ...
    CONV_TAC(funpow 4 RAND_CONV (LAND_CONV (TOP_DEPTH_CONV num_CONV))) THEN
