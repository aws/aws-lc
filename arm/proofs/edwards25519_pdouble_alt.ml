...
(**** print_literal_from_elf "arm/curve25519/edwards25519_pdouble_alt.o";;
 ****)

let edwards25519_pdouble_alt_mc = define_assert_from_elf
  "edwards25519_pdouble_alt_mc" "arm/curve25519/edwards25519_pdouble_alt.o"
[
...
let EDWARDS25519_PDOUBLE_ALT_EXEC = ARM_MK_EXEC_RULE edwards25519_pdouble_alt_mc;;
...
let LOCAL_MUL_P25519_TAC =
  ARM_MACRO_SIM_ABBREV_TAC edwards25519_pdouble_alt_mc 102 lvs
   `!(t:armstate) pcin pcout p3 n3 p1 n1 p2 n2.
      ...
      nonoverlapping (word pc,0xae4) (word_add (read p3 t) (word n3),8 * 4)
      ==> ensures arm
           (\s. aligned_bytes_loaded s (word pc) edwards25519_pdouble_alt_mc /\
                read PC s = pcin /\
                read SP s = read SP t /\
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  ARM_ACCSTEPS_TAC EDWARDS25519_PDOUBLE_ALT_EXEC (1--67) (1--67) THEN
  ...
  ARM_ACCSTEPS_TAC EDWARDS25519_PDOUBLE_ALT_EXEC (68--85) (68--85) THEN
  ...
  ARM_ACCSTEPS_TAC EDWARDS25519_PDOUBLE_ALT_EXEC (86--88) (86--88) THEN
  ...
  SUBGOAL_THEN `ca DIV 2 EXP 255 = val(sum_s88:int64)` SUBST_ALL_TAC THENL
   [UNDISCH_TAC `ca DIV 2 EXP 255 + 1 <= 78` THEN REWRITE_TAC[ARITH_RULE
    ...
    ACCUMULATOR_POP_ASSUM_LIST(K ALL_TAC)] THEN
  ARM_STEPS_TAC EDWARDS25519_PDOUBLE_ALT_EXEC (89--90) THEN
  ...
  ARM_ACCSTEPS_TAC EDWARDS25519_PDOUBLE_ALT_EXEC (91--102) (91--102) THEN
...
let LOCAL_SQR_4_TAC =
  ARM_MACRO_SIM_ABBREV_TAC edwards25519_pdouble_alt_mc 74 lvs
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  ARM_ACCSTEPS_TAC EDWARDS25519_PDOUBLE_ALT_EXEC (1--45) (1--45) THEN
  ...
  ARM_ACCSTEPS_TAC EDWARDS25519_PDOUBLE_ALT_EXEC (46--63) (46--63) THEN
  RULE_ASSUM_TAC(REWRITE_RULE[COND_SWAP; GSYM WORD_BITVAL]) THEN
  ABBREV_TAC
   `ca = bignum_of_wordlist
  ...
  ARM_ACCSTEPS_TAC EDWARDS25519_PDOUBLE_ALT_EXEC (64--66) (64--66) THEN
  ...
  THENL
   [UNDISCH_TAC `ca DIV 2 EXP 255 + 1 <= 78` THEN REWRITE_TAC[ARITH_RULE
    ...
    DISCH_THEN(fun th -> REWRITE_TAC[th]) THEN REAL_INTEGER_TAC;
    ACCUMULATOR_POP_ASSUM_LIST(K ALL_TAC)] THEN
  ARM_STEPS_TAC EDWARDS25519_PDOUBLE_ALT_EXEC (67--68) THEN
  ...
  ARM_ACCSTEPS_TAC EDWARDS25519_PDOUBLE_ALT_EXEC (69--72) (69--74) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
...
let LOCAL_ADD_4_TAC =
  ARM_MACRO_SIM_ABBREV_TAC edwards25519_pdouble_alt_mc 10 lvs
   `!(t:armstate) pcin pcout p3 n3 p1 n1 p2 n2.
      !m. read(memory :> bytes(word_add (read p1 t) (word n1),8 * 4)) t = m
      ==>
      !n. read(memory :> bytes(word_add (read p2 t) (word n2),8 * 4)) t = n
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  FIRST_ASSUM(BIGNUM_LDIGITIZE_TAC "n_" o lhand o concl) THEN
  FIRST_ASSUM(BIGNUM_LDIGITIZE_TAC "m_" o lhand o concl) THEN
  ARM_ACCSTEPS_TAC EDWARDS25519_PDOUBLE_ALT_EXEC [3;4;7;8] (1--10) THEN
...
let LOCAL_ADD_TWICE4_TAC =
  ARM_MACRO_SIM_ABBREV_TAC edwards25519_pdouble_alt_mc 16 lvs
   `!(t:armstate) pcin pcout p3 n3 p1 n1 p2 n2.
      !m. read(memory :> bytes(word_add (read p1 t) (word n1),8 * 4)) t = m
      ==>
      !n. read(memory :> bytes(word_add (read p2 t) (word n2),8 * 4)) t = n
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  FIRST_ASSUM(BIGNUM_LDIGITIZE_TAC "n_" o lhand o concl) THEN
  FIRST_ASSUM(BIGNUM_LDIGITIZE_TAC "m_" o lhand o concl) THEN
  ARM_ACCSTEPS_TAC EDWARDS25519_PDOUBLE_ALT_EXEC (1--8) (1--8) THEN
  SUBGOAL_THEN `carry_s8 <=> 2 EXP 256 <= m + n` SUBST_ALL_TAC THENL
   [MATCH_MP_TAC FLAG_FROM_CARRY_LE THEN EXISTS_TAC `256` THEN
    ...
    ALL_TAC] THEN
  ARM_ACCSTEPS_TAC EDWARDS25519_PDOUBLE_ALT_EXEC (11--14) (9--16) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN DISCH_TAC THEN
...
let LOCAL_DOUBLE_TWICE4_TAC =
  ARM_MACRO_SIM_ABBREV_TAC edwards25519_pdouble_alt_mc 14 lvs
   `!(t:armstate) pcin pcout p3 n3 p1 n1.
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN ENSURES_INIT_TAC "s0" THEN
  FIRST_ASSUM(BIGNUM_LDIGITIZE_TAC "n_" o lhand o concl) THEN
  ARM_ACCSTEPS_TAC EDWARDS25519_PDOUBLE_ALT_EXEC (1--6) (1--6) THEN
  SUBGOAL_THEN `carry_s6 <=> 2 EXP 256 <= 2 * n` SUBST_ALL_TAC THENL
   [MATCH_MP_TAC FLAG_FROM_CARRY_LE THEN EXISTS_TAC `256` THEN
    ...
    DISCH_THEN(fun th -> REWRITE_TAC[th]) THEN BOUNDER_TAC[];
    ALL_TAC] THEN
  ARM_ACCSTEPS_TAC EDWARDS25519_PDOUBLE_ALT_EXEC (9--12) (7--14) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN DISCH_TAC THEN
  REWRITE_TAC[num_congruent; GSYM INT_OF_NUM_ADD] THEN
  MATCH_MP_TAC(MESON[INT_OF_NUM_LT]
   `!x':int. (x' == a) (mod p) /\ x = x'
...
let EDWARDS25519_PDOUBLE_ALT_CORRECT = time prove
 (`!p3 p1 T1 pc stackpointer.
    ...
    ALL (nonoverlapping (stackpointer,160))
        [(word pc,0xae4); (p3,96); (p1,96)] /\
    nonoverlapping (p3,96) (word pc,0xae4)
    ==> ensures arm
         (\s. aligned_bytes_loaded s (word pc) edwards25519_pdouble_alt_mc /\
...
let EDWARDS25519_PDOUBLE_ALT_SUBROUTINE_CORRECT = time prove
 (`!p3 p1 T1 pc stackpointer returnaddress.
  ...
  ARM_ADD_RETURN_STACK_TAC EDWARDS25519_PDOUBLE_ALT_EXEC
    EDWARDS25519_PDOUBLE_ALT_CORRECT `[X19; X20]` 176);;
