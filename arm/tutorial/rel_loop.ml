...
let LOOP_EQUIV = prove(
  ...
  REPEAT CONJ_TAC THENL [
    ...
    CONJ_TAC THENL [
      ...
      ASSUME_TAC (ISPECL [`p2:armstate`] MAYCHANGE_STARTER) THEN
      MONOTONE_MAYCHANGE_TAC;
    ];
    ...
    ARM_STUTTER_LEFT_TAC LOOP_EXEC (1--3) None THEN
    (* Symbolically execute the right program only. *)
    ARM_STUTTER_RIGHT_TAC LOOP2_EXEC (1--4) "'" None THEN
    ...
    ARM_STUTTER_LEFT_TAC LOOP_EXEC (1--1) None THEN
    ARM_STUTTER_RIGHT_TAC LOOP2_EXEC (1--1) "'" None THEN
    REPEAT_N 2 ENSURES_FINAL_STATE'_TAC THEN
    ASM_REWRITE_TAC[WORD_ADD] THEN
    ...
    REPEAT STRIP_TAC THENL [
      ASSUME_TAC(ISPEC (mk_var("p1",`:armstate`)) MAYCHANGE_STARTER) THEN
      MONOTONE_MAYCHANGE_TAC;

      ASSUME_TAC(ISPEC (mk_var("p2",`:armstate`)) MAYCHANGE_STARTER) THEN
      MONOTONE_MAYCHANGE_TAC;
    ];
