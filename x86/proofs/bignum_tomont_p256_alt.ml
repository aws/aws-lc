...
let windows_bignum_tomont_p256_alt_mc = define_from_elf
   "windows_bignum_tomont_p256_alt_mc" "x86/p256/bignum_tomont_p256_alt.obj";;

let WINDOWS_BIGNUM_TOMONT_P256_ALT_SUBROUTINE_CORRECT = time prove
 (`!z x a pc stackpointer returnaddress.
        nonoverlapping (z,8 * 4) (word_sub stackpointer (word 48),56) /\
        ALL (nonoverlapping (word_sub stackpointer (word 48),48))
            [(word pc,0x24a); (x,8 * 4)] /\
        nonoverlapping (word pc,0x24a) (z,8 * 4)
        ==> ensures x86
             (\s. bytes_loaded s (word pc) windows_bignum_tomont_p256_alt_mc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
  ...
  WINDOWS_X86_WRAP_STACK_TAC
    windows_bignum_tomont_p256_alt_mc bignum_tomont_p256_alt_mc
