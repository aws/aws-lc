...
let LOCAL_MONTSQR_P256_TAC =
  ...
  `!(t:x86state) pcin pcout p3 n3 p1 n1.
    ...
    ==>
    nonoverlapping (word pc,0x286e) (word_add (read p3 t) (word n3),32)
    ==> ensures x86
...
let P256_MONTJADD_CORRECT = time prove
 (`!p3 p1 t1 p2 t2 pc stackpointer.
        ALL (nonoverlapping (stackpointer,232))
            [(word pc,0x286e); (p1,96); (p2,96); (p3,96)] /\
        nonoverlapping (p3,96) (word pc,0x286e)
        ==> ensures x86
             (\s. bytes_loaded s (word pc) (BUTLAST p256_montjadd_mc) /\
  ...
  LOCAL_MONTMUL_P256_TAC 1 ["resz";"xd";"z_2"] THEN
  ...
  X86_STEPS_TAC P256_MONTJADD_EXEC (27--50) THEN
  ...
  ASM_CASES_TAC `&z1d rem &p_256 = &0` THENL
   [ASM_REWRITE_TAC[weierstrass_of_jacobian; INTEGER_MOD_RING_CLAUSES];
    ALL_TAC] THEN
  ASM_CASES_TAC `&z2d rem &p_256 = &0` THENL
   [ASM_REWRITE_TAC[weierstrass_of_jacobian; INTEGER_MOD_RING_CLAUSES];
    ALL_TAC] THEN
...
let P256_MONTJADD_SUBROUTINE_CORRECT = time prove
 (`!p3 p1 t1 p2 t2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 280),280))
            [(word pc,0x286e); (p1,96); (p2,96)] /\
        ALL (nonoverlapping (p3,96))
            [(word pc,0x286e); (word_sub stackpointer (word 280),288)]
        ==> ensures x86
...
let WINDOWS_P256_MONTJADD_SUBROUTINE_CORRECT = time prove
 (`!p3 p1 t1 p2 t2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 296),296))
            [(word pc,0x287b); (p1,96); (p2,96)] /\
        ALL (nonoverlapping (p3,96))
            [(word pc,0x287b); (word_sub stackpointer (word 296),304)]
        ==> ensures x86
