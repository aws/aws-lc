...
let windows_bignum_montmul_p256_mc = define_from_elf
   "windows_bignum_montmul_p256_mc" "x86/p256/bignum_montmul_p256.obj";;

let WINDOWS_BIGNUM_MONTMUL_P256_SUBROUTINE_CORRECT = time prove
 (`!z x y a b pc stackpointer returnaddress.
        nonoverlapping (z,8 * 4) (word_sub stackpointer (word 56),64) /\
        ALL (nonoverlapping (word_sub stackpointer (word 56),56))
            [(word pc,0x24f); (x,8 * 4); (y,8 * 4)] /\
        nonoverlapping (word pc,0x24f) (z,8 * 4)
        ==> ensures x86
             (\s. bytes_loaded s (word pc) windows_bignum_montmul_p256_mc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
  ...
  WINDOWS_X86_WRAP_STACK_TAC
   windows_bignum_montmul_p256_mc bignum_montmul_p256_mc
   BIGNUM_MONTMUL_P256_CORRECT `[RBX; R12; R13; R14; R15]` 40);;

let WINDOWS_BIGNUM_AMONTMUL_P256_SUBROUTINE_CORRECT = time prove
 (`!z x y a b pc stackpointer returnaddress.
        nonoverlapping (z,8 * 4) (word_sub stackpointer (word 56),64) /\
        ALL (nonoverlapping (word_sub stackpointer (word 56),56))
            [(word pc,0x24f); (x,8 * 4); (y,8 * 4)] /\
