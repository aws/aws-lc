...
let BIGNUM_MOD_SM2_EXEC = X86_MK_EXEC_RULE bignum_mod_sm2_mc;;
...
let windows_bignum_mod_sm2_mc = define_from_elf
   "windows_bignum_mod_sm2_mc" "x86/sm2/bignum_mod_sm2.obj";;

let WINDOWS_BIGNUM_MOD_SM2_CORRECT = time prove
 (`!z k x n pc.
      nonoverlapping (word pc,0x13b) (z,32)
      ==> ensures x86
           (\s. bytes_loaded s (word pc) windows_bignum_mod_sm2_mc /\
          ...
          (MAYCHANGE [RIP; RSI; RAX; RDX; RCX; RBX; R8; R9; R10; R11; R12] ,,
           MAYCHANGE SOME_FLAGS ,,
           MAYCHANGE [memory :> bignum(z,4)])`,
  tac (X86_MK_EXEC_RULE windows_bignum_mod_sm2_mc)
      (curry mk_comb `(+) (pc:num)` o mk_small_numeral o (fun n -> n+11)));;

let WINDOWS_BIGNUM_MOD_SM2_SUBROUTINE_CORRECT = time prove
 (`!z k x n pc stackpointer returnaddress.
  ...
  GEN_X86_ADD_RETURN_STACK_TAC (X86_MK_EXEC_RULE windows_bignum_mod_sm2_mc)
    WINDOWS_BIGNUM_MOD_SM2_CORRECT
