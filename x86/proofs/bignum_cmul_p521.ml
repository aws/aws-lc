...
let BIGNUM_CMUL_P521_EXEC = X86_MK_EXEC_RULE bignum_cmul_p521_mc;;
 ...
 (`!z c x a pc.
   ...
   [REWRITE_TAC[REAL_OF_NUM_CLAUSES; VAL_WORD_ADD_CASES] THEN
    ...
    SIMP_TAC[MOD_LT] THEN FIRST_X_ASSUM(MATCH_MP_TAC o MATCH_MP (ARITH_RULE
     `n < (2 EXP 64 - 1) * 2 EXP 521
      ==> n DIV 2 EXP (8 * 64) = x ==> x DIV 2 EXP 9 < 2 EXP 64 - 1`)) THEN
    EXPAND_TAC "n" THEN
  ...
  SUBGOAL_THEN `val(h:int64) = n DIV 2 EXP 521` SUBST_ALL_TAC THENL
   [EXPAND_TAC "h" THEN
    ...
    MATCH_MP_TAC(ARITH_RULE
     `n DIV 2 EXP (8 * 64) = x ==> x DIV 2 EXP 9 = n DIV 2 EXP 521`) THEN
    EXPAND_TAC "n" THEN
    REWRITE_TAC[TOP_DEPTH_CONV num_CONV `8`; MULT_CLAUSES; ADD_CLAUSES] THEN
    REWRITE_TAC[EXP_ADD; GSYM DIV_DIV; BIGNUM_OF_WORDLIST_DIV] THEN
    REWRITE_TAC[bignum_of_wordlist] THEN ARITH_TAC;
...
let BIGNUM_CMUL_P521_SUBROUTINE_CORRECT = time prove
 (`!z c x a pc stackpointer returnaddress.
       ...
       ALL (nonoverlapping (word pc,0xdf))
       ...
       ==> ensures x86
            ...
            (MAYCHANGE [RIP; RSP; RSI; RAX; RCX; RDX; R8; R9; R10; R11] ,,
             ...
             MAYCHANGE SOME_FLAGS)`,
  X86_ADD_RETURN_STACK_TAC BIGNUM_CMUL_P521_EXEC BIGNUM_CMUL_P521_CORRECT
   `[RBX; RBP; R12; R13]` 32);;
