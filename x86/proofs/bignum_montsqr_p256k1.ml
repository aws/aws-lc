...
let BIGNUM_MONTSQR_P256K1_EXEC = X86_MK_CORE_EXEC_RULE bignum_montsqr_p256k1_mc;;
...
let BIGNUM_MONTSQR_P256K1_CORRECT = time prove
 (`!z x a pc.
        ...
        ==> ensures x86
             (\s. bytes_loaded s (word pc) (BUTLAST bignum_montsqr_p256k1_mc) /\
...
let BIGNUM_MONTSQR_P256K1_SUBROUTINE_CORRECT = time prove
 (`!z x a pc stackpointer returnaddress.
  ...
  X86_PROMOTE_RETURN_STACK_TAC
   bignum_montsqr_p256k1_mc BIGNUM_MONTSQR_P256K1_CORRECT
...
let windows_bignum_montsqr_p256k1_mc = define_from_elf
   "windows_bignum_montsqr_p256k1_mc" "x86/secp256k1/bignum_montsqr_p256k1.obj";;

let WINDOWS_BIGNUM_MONTSQR_P256K1_SUBROUTINE_CORRECT = time prove
 (`!z x a pc stackpointer returnaddress.
        nonoverlapping (z,8 * 4) (word_sub stackpointer (word 64),72) /\
        ALL (nonoverlapping (word_sub stackpointer (word 64),64))
            [(word pc,0x1ab); (x,8 * 4)] /\
        nonoverlapping (word pc,0x1ab) (z,8 * 4)
        ==> ensures x86
             (\s. bytes_loaded s (word pc) windows_bignum_montsqr_p256k1_mc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
  ...
  WINDOWS_X86_WRAP_STACK_TAC
   windows_bignum_montsqr_p256k1_mc bignum_montsqr_p256k1_mc
