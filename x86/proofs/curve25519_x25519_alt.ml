...
[
  ...
  0x48; 0x0b; 0x84; 0x24; 0xe8; 0x00; 0x00; 0x00;
                           (* OR (% rax) (Memop Quadword (%% (rsp,232))) *)
  0x48; 0x0b; 0x84; 0x24; 0xf0; 0x00; 0x00; 0x00;
                           (* OR (% rax) (Memop Quadword (%% (rsp,240))) *)
  0x48; 0x0b; 0x84; 0x24; 0xf8; 0x00; 0x00; 0x00;
                           (* OR (% rax) (Memop Quadword (%% (rsp,248))) *)
  0x48; 0x8b; 0x8c; 0x24; 0x40; 0x01; 0x00; 0x00;
                           (* MOV (% rcx) (Memop Quadword (%% (rsp,320))) *)
...
let LOCAL_MUL_P25519_TAC =
  X86_MACRO_SIM_ABBREV_TAC (X86_TRIM_EXEC_RULE curve25519_x25519_alt_mc) 129 lvs
   `!(t:x86state) pcin pcout p3 n3 p1 n1 p2 n2.
      ...
      ==>
      nonoverlapping (word pc,0x55c2) (word_add (read p3 t) (word n3),8 * 4)
      ==> ensures x86
           (\s. bytes_loaded s (word pc) (BUTLAST curve25519_x25519_alt_mc) /\
                read RIP s = pcin /\
...
let LOCAL_MUL_4_TAC =
  X86_MACRO_SIM_ABBREV_TAC (X86_TRIM_EXEC_RULE curve25519_x25519_alt_mc) 120 lvs
   `!(t:x86state) pcin pcout q3 n3 q1 n1 q2 n2.
      ...
      ==>
      nonoverlapping (word pc,0x55c2) (word_add (read q3 t) (word n3),8 * 4)
      ==> ensures x86
           (\s. bytes_loaded s (word pc) (BUTLAST curve25519_x25519_alt_mc) /\
                read RIP s = pcin /\
                read RSP s = read RSP t /\
...
let LOCAL_MUL_5_TAC =
  X86_MACRO_SIM_ABBREV_TAC (X86_TRIM_EXEC_RULE curve25519_x25519_alt_mc) 113 lvs
   `!(t:x86state) pcin pcout p3 n3 p1 n1 p2 n2.
      !m. read(memory :> bytes(word_add (read p1 t) (word n1),8 * 4)) t = m
      ==>
      !n. read(memory :> bytes(word_add (read p2 t) (word n2),8 * 4)) t = n
      ==>
      nonoverlapping (word pc,0x55c2) (word_add (read p3 t) (word n3),8 * 5)
      ==> ensures x86
           (\s. bytes_loaded s (word pc) (BUTLAST curve25519_x25519_alt_mc) /\
                read RIP s = pcin /\
                read RSP s = read RSP t /\
...
let LOCAL_SQR_4_TAC =
  X86_MACRO_SIM_ABBREV_TAC (X86_TRIM_EXEC_RULE curve25519_x25519_alt_mc) 109 lvs
...
let LOCAL_ADD5_4_TAC =
  X86_MACRO_SIM_ABBREV_TAC (X86_TRIM_EXEC_RULE curve25519_x25519_alt_mc) 23 lvs
   `!(t:x86state) pcin pcout p3 n3 p1 n1 p2 n2.
...
let LOCAL_ADD_TWICE4_TAC =
  X86_MACRO_SIM_ABBREV_TAC (X86_TRIM_EXEC_RULE curve25519_x25519_alt_mc) 19 lvs
   `!(t:x86state) pcin pcout q3 n3 q1 n1 q2 n2.
      !m. read(memory :> bytes(word_add (read q1 t) (word n1),8 * 4)) t = m
      ==>
      !n. read(memory :> bytes(word_add (read q2 t) (word n2),8 * 4)) t = n
...
let LOCAL_SUB5_4_TAC =
  X86_MACRO_SIM_ABBREV_TAC (X86_TRIM_EXEC_RULE curve25519_x25519_alt_mc) 29 lvs
   `!(t:x86state) pcin pcout p3 n3 p1 n1 p2 n2.
      !m. read(memory :> bytes(word_add (read p1 t) (word n1),8 * 5)) t = m
      ==>
      !n. read(memory :> bytes(word_add (read p2 t) (word n2),8 * 5)) t = n
...
let LOCAL_CMADD_4_TAC =
  X86_MACRO_SIM_ABBREV_TAC (X86_TRIM_EXEC_RULE curve25519_x25519_alt_mc) 37 lvs
   `!(t:x86state) pcin pcout p3 n3 p1 n1 p2 n2.
     !m. read(memory :> bytes(word_add (read p1 t) (word n1),8 * 4)) t = m
     ==>
     !n. read(memory :> bytes(word_add (read p2 t) (word n2),8 * 4)) t = n
...
let LOCAL_MUX_4_TAC =
  X86_MACRO_SIM_ABBREV_TAC (X86_TRIM_EXEC_RULE curve25519_x25519_alt_mc) 16 lvs
   `!(t:x86state) pcin pcout p3 n3 p1 n1 p2 n2.
     !b. read ZF t = b
     ==>
     !m. read(memory :> bytes(word_add (read p1 t) (word n1),8 * 4)) t = m
...
let LOCAL_MODINV_TAC =
  X86_SUBROUTINE_SIM_TAC
   ((GEN_REWRITE_CONV RAND_CONV [curve25519_x25519_alt_mc] THENC
     REWRITE_CONV[BUTLAST_CLAUSES])
    `BUTLAST curve25519_x25519_alt_mc`,
...
let CURVE25519_X25519_ALT_CORRECT = time prove
 (`!res scalar n point X pc stackpointer.
    ALL (nonoverlapping (stackpointer,416))
        [(word pc,0x55c2); (res,32); (scalar,32); (point,32)] /\
    nonoverlapping (res,32) (word pc,0x55c2)
    ==> ensures x86
         (\s. bytes_loaded s (word pc) (BUTLAST curve25519_x25519_alt_mc) /\
  ...
  ENSURES_WHILE_ADOWN_TAC `254` `3` `pc + 0xa96` `pc + 0x205d`
   `\i s.
      ...
      curve25519x_halfcanonically_represents f
       ...
       (bignum_from_memory(word_add stackpointer (word 256),4) s,
        bignum_from_memory(word_add stackpointer (word 96),4) s)` THEN
  REPEAT CONJ_TAC THENL
   [ARITH_TAC;
 ...
 GHOST_INTRO_TAC `xn:num`
  ...
  MAP_EVERY (fun t -> REABBREV_TAC t THEN POP_ASSUM SUBST_ALL_TAC)
   [`xn8 = read(memory :> bytes(word_add stackpointer (word 320),8 * 4)) s6`;
    `zn8 =
     read(memory :> bytes(word_add stackpointer (word 224),8 * 4)) s6`] THEN
  SUBGOAL_THEN
  ...
  ABBREV_TAC
   `zn' =
    read(memory :> bytes(word_add stackpointer (word 96),8 * 4)) s33` THEN

  (*** The tweaking to force xn = 0 whenever zn = 0 ***)

  BIGNUM_LDIGITIZE_TAC "xn_"
   `read(memory :> bytes(word_add stackpointer (word 320),8 * 4)) s33` THEN
  BIGNUM_LDIGITIZE_TAC "zn_"
   `read(memory :> bytes(word_add stackpointer (word 224),8 * 4)) s33` THEN
  X86_STEPS_TAC CURVE25519_X25519_ALT_EXEC (34--49) THEN

  SUBGOAL_THEN
   `read(memory :> bytes(word_add stackpointer (word 320),8 * 4)) s49 =
  ...
  LOCAL_MUL_P25519_TAC 1 ["resx"; "xn"; "xm"] THEN
...
let CURVE25519_X25519_ALT_SUBROUTINE_CORRECT = time prove
 (`!res scalar n point X pc stackpointer returnaddress.
    ALL (nonoverlapping (word_sub stackpointer (word 464),464))
        [(word pc,0x55c2); (point,32); (scalar,32)] /\
    nonoverlapping (res,32) (word pc,0x55c2) /\
...
let WINDOWS_CURVE25519_X25519_ALT_SUBROUTINE_CORRECT = time prove
 (`!res scalar n point X pc stackpointer returnaddress.
    ALL (nonoverlapping (word_sub stackpointer (word 480),480))
        [(word pc,0x55cf); (point,32); (scalar,32)] /\
    nonoverlapping (res,32) (word pc,0x55cf) /\
    nonoverlapping (res,32) (word_sub stackpointer (word 480),488)
    ==> ensures x86
         (\s. bytes_loaded s (word pc) windows_curve25519_x25519_alt_mc /\
              read RIP s = word pc /\
              read RSP s = stackpointer /\
              read (memory :> bytes64 stackpointer) s = returnaddress /\
  ...
  WINDOWS_X86_WRAP_STACK_TAC
   windows_curve25519_x25519_alt_mc curve25519_x25519_alt_mc
