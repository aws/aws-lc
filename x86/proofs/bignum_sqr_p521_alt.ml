...
[
  ...
  0x4c; 0x8b; 0x47; 0x40;  (* MOV (% r8) (Memop Quadword (%% (rdi,64))) *)
...
let BIGNUM_SQR_P521_ALT_EXEC = X86_MK_EXEC_RULE bignum_sqr_p521_alt_mc;;
...
let BIGNUM_SQR_P521_ALT_CORRECT = time prove
 (`!z x n pc.
        nonoverlapping (word pc,0x4e6) (z,8 * 9) /\
  ...
  MAP_EVERY (fun s -> REABBREV_TAC s THEN POP_ASSUM SUBST_ALL_TAC)
   [`h0 = read (memory :> bytes64 (word_add z (word 64))) s310`;
    `h1 = read R9 s310`;
    `h2 = read R10 s310`;
    `h3 = read R11 s310`;
    `h4 = read R12 s310`;
