...
let BIGNUM_MONTSQR_P384_CORRECT = time prove
 (`!z x a pc.
        ...
        ==> ensures x86
             (\s. bytes_loaded s (word pc) (BUTLAST bignum_montsqr_p384_mc) /\
...
let BIGNUM_MONTSQR_P384_SUBROUTINE_CORRECT = time prove
 (`!z x a pc stackpointer returnaddress.
  ...
  X86_PROMOTE_RETURN_STACK_TAC
   bignum_montsqr_p384_mc BIGNUM_MONTSQR_P384_CORRECT
        ...
        ==> ensures x86
             ...
             (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * 6);
                     memory :> bytes(word_sub stackpointer (word 48),48)])`,
  X86_PROMOTE_RETURN_STACK_TAC
   bignum_montsqr_p384_mc BIGNUM_AMONTSQR_P384_CORRECT
   `[RBX; RBP; R12; R13; R14; R15]` 48);;
...
let WINDOWS_BIGNUM_MONTSQR_P384_SUBROUTINE_CORRECT = time prove
 (`!z x a pc stackpointer returnaddress.
        ...
        ALL (nonoverlapping (word_sub stackpointer (word 64),64))
            [(word pc,0x42d); (x,8 * 6)] /\
        nonoverlapping (word pc,0x42d) (z,8 * 6)
        ==> ensures x86
             (\s. bytes_loaded s (word pc) windows_bignum_montsqr_p384_mc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
  ...
  WINDOWS_X86_WRAP_STACK_TAC
   windows_bignum_montsqr_p384_mc bignum_montsqr_p384_mc
   BIGNUM_MONTSQR_P384_CORRECT `[RBX; RBP; R12; R13; R14; R15]` 48);;

let WINDOWS_BIGNUM_AMONTSQR_P384_SUBROUTINE_CORRECT = time prove
 (`!z x a pc stackpointer returnaddress.
        nonoverlapping (z,8 * 6) (word_sub stackpointer (word 64),72) /\
        ALL (nonoverlapping (word_sub stackpointer (word 64),64))
            [(word pc,0x42d); (x,8 * 6)] /\
