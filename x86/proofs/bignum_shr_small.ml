...
let BIGNUM_SHR_SMALL_CORRECT = prove
 (`!p z n x a c pc.
        ...
        ==> ensures x86
             (\s. bytes_loaded s (word pc) (BUTLAST bignum_shr_small_mc) /\
                  read RIP s = word pc /\
 ...
 (`!p z n x a c pc stackpointer returnaddress.
        ...
        ==> ensures x86
             ...
             (\s. read RIP s = returnaddress /\
                  ...
                  lowdigits (a DIV 2 EXP (val c MOD 64)) (val p) /\
                  C_RETURN s = word(a MOD 2 EXP (val c MOD 64)))
             (MAYCHANGE [RIP; RSP; RDI; RAX; RCX; R8; R9; R10] ,,
              MAYCHANGE SOME_FLAGS ,,
              MAYCHANGE [memory :> bignum(z,val p)])`,
  X86_PROMOTE_RETURN_NOSTACK_TAC bignum_shr_small_mc BIGNUM_SHR_SMALL_CORRECT);;
...
let WINDOWS_BIGNUM_SHR_SMALL_SUBROUTINE_CORRECT = prove
 (`!p z n x a c pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 16),16))
            [(word pc,0x63); (x,8 * val n)] /\
        nonoverlapping (word pc,0x63) (z,8 * val p) /\
        nonoverlapping (word_sub stackpointer (word 16),24) (z,8 * val p) /\
        (x = z \/ nonoverlapping(x,8 * val n) (z,8 * val p))
        ==> ensures x86
             (\s. bytes_loaded s (word pc) windows_bignum_shr_small_mc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
  ...
  WINDOWS_X86_WRAP_NOSTACK_TAC windows_bignum_shr_small_mc bignum_shr_small_mc
