...
let BIGNUM_TOMONT_SM2_EXEC = X86_MK_CORE_EXEC_RULE bignum_tomont_sm2_mc;;
...
let BIGNUM_TOMONT_SM2_CORRECT = time prove
 (`!z x a pc.
        ...
        ==> ensures x86
             (\s. bytes_loaded s (word pc) (BUTLAST bignum_tomont_sm2_mc) /\
                  read RIP s = word pc /\
                  C_ARGUMENTS [z; x] s /\
...
let BIGNUM_TOMONT_SM2_SUBROUTINE_CORRECT = time prove
 (`!z x a pc stackpointer returnaddress.
  ...
  X86_PROMOTE_RETURN_NOSTACK_TAC
    bignum_tomont_sm2_mc BIGNUM_TOMONT_SM2_CORRECT);;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let windows_bignum_tomont_sm2_mc = define_from_elf
   "windows_bignum_tomont_sm2_mc" "x86/sm2/bignum_tomont_sm2.obj";;

let WINDOWS_BIGNUM_TOMONT_SM2_SUBROUTINE_CORRECT = time prove
 (`!z x a pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 16),16))
            [(word pc,0x231); (x,8 * 4)] /\
        nonoverlapping (word pc,0x231) (z,8 * 4) /\
        nonoverlapping (word_sub stackpointer (word 16),24) (z,8 * 4)
        ==> ensures x86
             (\s. bytes_loaded s (word pc) windows_bignum_tomont_sm2_mc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
  ...
  WINDOWS_X86_WRAP_NOSTACK_TAC windows_bignum_tomont_sm2_mc
    bignum_tomont_sm2_mc BIGNUM_TOMONT_SM2_CORRECT);;
