...
let BIGNUM_MADD_N25519_CORRECT = time prove
 (`!z x m y n c r pc.
      ...
      ==> ensures x86
           (\s. bytes_loaded s (word pc) (BUTLAST bignum_madd_n25519_mc) /\
...
let BIGNUM_MADD_N25519_SUBROUTINE_CORRECT = time prove
 (`!z x m y n c r pc stackpointer returnaddress.
      ...
      ALL (nonoverlapping (word_sub stackpointer (word 48),48))
          [(word pc,0x407); (x,8 * 4); (y,8 * 4); (c,8 * 4)] /\
      nonoverlapping (word pc,0x407) (z,8 * 4)
      ==> ensures x86
  ...
  X86_PROMOTE_RETURN_STACK_TAC
    bignum_madd_n25519_mc BIGNUM_MADD_N25519_CORRECT
...
let WINDOWS_BIGNUM_MADD_N25519_SUBROUTINE_CORRECT = time prove
 (`!z x m y n c r pc stackpointer returnaddress.
      ...
      ALL (nonoverlapping (word_sub stackpointer (word 64),64))
          [(word pc,0x417); (x,8 * 4); (y,8 * 4); (c,8 * 4)] /\
      nonoverlapping (word pc,0x417) (z,8 * 4)
      ==> ensures x86
           (\s. bytes_loaded s (word pc) windows_bignum_madd_n25519_mc /\
                read RIP s = word pc /\
                read RSP s = stackpointer /\
                read (memory :> bytes64 stackpointer) s = returnaddress /\
  ...
  WINDOWS_X86_WRAP_STACK_TAC
    windows_bignum_madd_n25519_mc bignum_madd_n25519_mc
