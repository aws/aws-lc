...
let BIGNUM_INV_P384_EXEC = X86_MK_CORE_EXEC_RULE bignum_inv_p384_mc;;
...
let CORE_INV_P384_EXEC =
    X86_MK_EXEC_RULE
     ((GEN_REWRITE_CONV RAND_CONV [bignum_inv_p384_mc] THENC TRIM_LIST_CONV)
      `TRIM_LIST (17,18) bignum_inv_p384_mc`);;
...
let BIGNUM_INV_P384_CORRECT = time prove
 (`!z x n pc stackpointer.
        ...
        ==> ensures x86
             (\s. bytes_loaded s (word pc) (BUTLAST bignum_inv_p384_mc) /\
  ...
  X86_SUBROUTINE_SIM_TAC
      ((GEN_REWRITE_CONV RAND_CONV [bignum_inv_p384_mc] THENC
        REWRITE_CONV[BUTLAST_CLAUSES])
       `BUTLAST bignum_inv_p384_mc`,
...
let BIGNUM_INV_P384_SUBROUTINE_CORRECT = time prove
 (`!z x n pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 384),384))
            [(word pc,0x19b4); (x,8 * 6)] /\
        ALL (nonoverlapping (z,8 * 6))
            [(word_sub stackpointer (word 384),392); (word pc,0x19b4)]
        ==> ensures x86
  ...
  X86_PROMOTE_RETURN_STACK_TAC bignum_inv_p384_mc
...
let windows_bignum_inv_p384_mc = define_from_elf "windows_bignum_inv_p384_mc"
      "x86/p384/bignum_inv_p384.obj";;

let WINDOWS_BIGNUM_INV_P384_SUBROUTINE_CORRECT = time prove
 (`!z x n pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 400),400))
            [(word pc,0x19be); (x,8 * 6)] /\
        ALL (nonoverlapping (z,8 * 6))
            [(word_sub stackpointer (word 400),408); (word pc,0x19be)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) windows_bignum_inv_p384_mc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
  ...
  WINDOWS_X86_WRAP_STACK_TAC windows_bignum_inv_p384_mc bignum_inv_p384_mc
     BIGNUM_INV_P384_CORRECT
      `[RBX; RBP; R12; R13; R14; R15]` 384);;
