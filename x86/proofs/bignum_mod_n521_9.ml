...
let BIGNUM_MOD_N521_9_EXEC = X86_MK_EXEC_RULE bignum_mod_n521_9_mc;;
...
let BIGNUM_MOD_N521_9_CORRECT = time prove
 (`!z x n pc.
  ...
  SUBGOAL_THEN `n DIV 2 EXP 521 = val(n_8:int64) DIV 2 EXP 9` ASSUME_TAC THENL
   [ASM_REWRITE_TAC[ARITH_RULE `521 = 8 * 64 + 9`] THEN
    REWRITE_TAC[TOP_DEPTH_CONV num_CONV `8`; MULT_CLAUSES; ADD_CLAUSES] THEN
    REWRITE_TAC[EXP_ADD; GSYM DIV_DIV] THEN EXPAND_TAC "n" THEN
