...
let windows_bignum_montmul_p384_alt_mc = define_from_elf
   "windows_bignum_montmul_p384_alt_mc" "x86/p384/bignum_montmul_p384_alt.obj";;

let WINDOWS_BIGNUM_MONTMUL_P384_ALT_SUBROUTINE_CORRECT = time prove
 (`!z x y a b pc stackpointer returnaddress.
        nonoverlapping (z,8 * 6) (word_sub stackpointer (word 64),72) /\
        ALL (nonoverlapping (word_sub stackpointer (word 64),64))
            [(word pc,0x4fe); (x,8 * 6); (y,8 * 6)] /\
        nonoverlapping (word pc,0x4fe) (z,8 * 6)
        ==> ensures x86
             (\s. bytes_loaded s (word pc) windows_bignum_montmul_p384_alt_mc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
  ...
  WINDOWS_X86_WRAP_STACK_TAC
   windows_bignum_montmul_p384_alt_mc bignum_montmul_p384_alt_mc
   BIGNUM_MONTMUL_P384_ALT_CORRECT `[RBX; RBP; R12; R13; R14; R15]` 48);;

let WINDOWS_BIGNUM_AMONTMUL_P384_ALT_SUBROUTINE_CORRECT = time prove
 (`!z x y a b pc stackpointer returnaddress.
        nonoverlapping (z,8 * 6) (word_sub stackpointer (word 64),72) /\
        ALL (nonoverlapping (word_sub stackpointer (word 64),64))
            [(word pc,0x4fe); (x,8 * 6); (y,8 * 6)] /\
