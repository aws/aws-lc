...
let BIGNUM_OPTNEG_P256_EXEC = X86_MK_CORE_EXEC_RULE bignum_optneg_p256_mc;;
...
let BIGNUM_OPTNEG_P256_CORRECT = time prove
 (`!z q x n pc.
        ...
        ==> ensures x86
             (\s. bytes_loaded s (word pc) (BUTLAST bignum_optneg_p256_mc) /\
                  read RIP s = word pc /\
                  C_ARGUMENTS [z; q; x] s /\
...
let BIGNUM_OPTNEG_P256_SUBROUTINE_CORRECT = time prove
 (`!z q x n pc stackpointer returnaddress.
        ALL (nonoverlapping (z,8 * 4)) [(word pc,0x6e); (stackpointer,8)]
        ==> ensures x86
...
let windows_bignum_optneg_p256_mc = define_from_elf
   "windows_bignum_optneg_p256_mc" "x86/p256/bignum_optneg_p256.obj";;

let WINDOWS_BIGNUM_OPTNEG_P256_SUBROUTINE_CORRECT = time prove
 (`!z q x n pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 16),16))
            [(word pc,0x7b); (x,8 * 4)] /\
        ALL (nonoverlapping (z,8 * 4))
            [(word pc,0x7b); (word_sub stackpointer (word 16),24)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) windows_bignum_optneg_p256_mc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
  ...
  WINDOWS_X86_WRAP_NOSTACK_TAC
    windows_bignum_optneg_p256_mc bignum_optneg_p256_mc
    BIGNUM_OPTNEG_P256_CORRECT);;
