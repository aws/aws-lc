...
let BIGNUM_SQR_8_16_ALT_EXEC = X86_MK_CORE_EXEC_RULE bignum_sqr_8_16_alt_mc;;
...
let BIGNUM_SQR_8_16_ALT_CORRECT = time prove
 (`!z x a pc.
     ...
     ==> ensures x86
          (\s. bytes_loaded s (word pc) (BUTLAST bignum_sqr_8_16_alt_mc) /\
               read RIP s = word pc /\
               C_ARGUMENTS [z; x] s /\
...
let BIGNUM_SQR_8_16_ALT_SUBROUTINE_CORRECT = time prove
 (`!z x a pc stackpointer returnaddress.
     ALL (nonoverlapping (z,8 * 16))
         [(word pc,0x386); (x,8 * 8); (stackpointer,8)]
     ==> ensures x86
  ...
  X86_PROMOTE_RETURN_NOSTACK_TAC
   bignum_sqr_8_16_alt_mc BIGNUM_SQR_8_16_ALT_CORRECT);;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let windows_bignum_sqr_8_16_alt_mc = define_from_elf
   "windows_bignum_sqr_8_16_alt_mc" "x86/fastmul/bignum_sqr_8_16_alt.obj";;

let WINDOWS_BIGNUM_SQR_8_16_ALT_SUBROUTINE_CORRECT = time prove
 (`!z x a pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 16),16))
            [(word pc,0x390); (x,8 * 8)] /\
     ALL (nonoverlapping (z,8 * 16))
         [(word pc,0x390); (x,8 * 8); (word_sub stackpointer (word 16),24)]
     ==> ensures x86
          (\s. bytes_loaded s (word pc) windows_bignum_sqr_8_16_alt_mc /\
               read RIP s = word pc /\
               read RSP s = stackpointer /\
               read (memory :> bytes64 stackpointer) s = returnaddress /\
  ...
  WINDOWS_X86_WRAP_NOSTACK_TAC
   windows_bignum_sqr_8_16_alt_mc bignum_sqr_8_16_alt_mc
