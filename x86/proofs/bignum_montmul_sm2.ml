...
let BIGNUM_MONTMUL_SM2_EXEC = X86_MK_CORE_EXEC_RULE bignum_montmul_sm2_mc;;
...
let BIGNUM_MONTMUL_SM2_CORRECT = time prove
 (`!z x y a b pc.
        ...
        ==> ensures x86
             (\s. bytes_loaded s (word pc) (BUTLAST bignum_montmul_sm2_mc) /\
...
let BIGNUM_MONTMUL_SM2_SUBROUTINE_CORRECT = time prove
 (`!z x y a b pc stackpointer returnaddress.
  ...
  X86_PROMOTE_RETURN_STACK_TAC
   bignum_montmul_sm2_mc BIGNUM_MONTMUL_SM2_CORRECT
        ...
        ==> ensures x86
             ...
             (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * 4);
                         memory :> bytes(word_sub stackpointer (word 48),48)])`,
  X86_PROMOTE_RETURN_STACK_TAC
   bignum_montmul_sm2_mc BIGNUM_AMONTMUL_SM2_CORRECT
   `[RBX; RBP; R12; R13; R14; R15]` 48);;
...
let windows_bignum_montmul_sm2_mc = define_from_elf
   "windows_bignum_montmul_sm2_mc" "x86/sm2/bignum_montmul_sm2.obj";;

let WINDOWS_BIGNUM_MONTMUL_SM2_SUBROUTINE_CORRECT = time prove
 (`!z x y a b pc stackpointer returnaddress.
        nonoverlapping (z,8 * 4) (word_sub stackpointer (word 64),72) /\
        ALL (nonoverlapping (word_sub stackpointer (word 64),64))
            [(word pc,0x235); (x,8 * 4); (y,8 * 4)] /\
        nonoverlapping (word pc,0x235) (z,8 * 4)
        ==> ensures x86
             (\s. bytes_loaded s (word pc) windows_bignum_montmul_sm2_mc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
  ...
  WINDOWS_X86_WRAP_STACK_TAC
   windows_bignum_montmul_sm2_mc bignum_montmul_sm2_mc
   BIGNUM_MONTMUL_SM2_CORRECT `[RBX; RBP; R12; R13; R14; R15]` 48);;

let WINDOWS_BIGNUM_AMONTMUL_SM2_SUBROUTINE_CORRECT = time prove
 (`!z x y a b pc stackpointer returnaddress.
        nonoverlapping (z,8 * 4) (word_sub stackpointer (word 64),72) /\
        ALL (nonoverlapping (word_sub stackpointer (word 64),64))
            [(word pc,0x235); (x,8 * 4); (y,8 * 4)] /\
