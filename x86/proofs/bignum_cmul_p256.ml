...
let BIGNUM_CMUL_P256_EXEC = X86_MK_CORE_EXEC_RULE bignum_cmul_p256_mc;;
...
let BIGNUM_CMUL_P256_CORRECT = time prove
 (`!z c x a pc.
        ...
        ==> ensures x86
             (\s. bytes_loaded s (word pc) (BUTLAST bignum_cmul_p256_mc) /\
                  read RIP s = word pc /\
                  C_ARGUMENTS [z; c; x] s /\
 ...
 (`!z c x a pc stackpointer returnaddress.
        ...
        ==> ensures x86
             ...
             (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * 4)])`,
  X86_PROMOTE_RETURN_NOSTACK_TAC bignum_cmul_p256_mc BIGNUM_CMUL_P256_CORRECT);;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let windows_bignum_cmul_p256_mc = define_from_elf
   "windows_bignum_cmul_p256_mc" "x86/p256/bignum_cmul_p256.obj";;

let WINDOWS_BIGNUM_CMUL_P256_SUBROUTINE_CORRECT = time prove
 (`!z c x a pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 16),16))
            [(word pc,0xb3); (x,8 * 4)] /\
        nonoverlapping (word pc,0xb3) (z,8 * 4) /\
        nonoverlapping (word_sub stackpointer (word 16),24) (z,8 * 4)
        ==> ensures x86
             (\s. bytes_loaded s (word pc) windows_bignum_cmul_p256_mc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
  ...
  WINDOWS_X86_WRAP_NOSTACK_TAC windows_bignum_cmul_p256_mc bignum_cmul_p256_mc
