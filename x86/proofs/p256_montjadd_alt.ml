...
let LOCAL_MONTSQR_P256_TAC =
  X86_MACRO_SIM_ABBREV_TAC (X86_TRIM_EXEC_RULE p256_montjadd_alt_mc) 137 lvs
  `!(t:x86state) pcin pcout p3 n3 p1 n1.
    ...
    ==> ensures x86
         (\s. bytes_loaded s (word pc) (BUTLAST p256_montjadd_alt_mc) /\
              read RIP s = pcin /\
...
let LOCAL_MONTMUL_P256_TAC =
  X86_MACRO_SIM_ABBREV_TAC (X86_TRIM_EXEC_RULE p256_montjadd_alt_mc) 166 lvs
...
let LOCAL_SUB_P256_TAC =
  X86_MACRO_SIM_ABBREV_TAC (X86_TRIM_EXEC_RULE p256_montjadd_alt_mc) 21 lvs
  `!(t:x86state) pcin pcout p3 n3 p1 n1 p2 n2.
...
let P256_MONTJADD_ALT_SUBROUTINE_CORRECT = time prove
 (`!p3 p1 t1 p2 t2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 272),272))
            [(word pc,0x2768); (p1,96); (p2,96)] /\
        ALL (nonoverlapping (p3,96))
            [(word pc,0x2768); (word_sub stackpointer (word 272),280)]
        ==> ensures x86
...
let WINDOWS_P256_MONTJADD_ALT_SUBROUTINE_CORRECT = time prove
 (`!p3 p1 t1 p2 t2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 288),288))
            [(word pc,0x2775); (p1,96); (p2,96)] /\
        ALL (nonoverlapping (p3,96))
            [(word pc,0x2775); (word_sub stackpointer (word 288),296)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) windows_p256_montjadd_alt_mc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
  ...
  WINDOWS_X86_WRAP_STACK_TAC
   windows_p256_montjadd_alt_mc p256_montjadd_alt_mc
