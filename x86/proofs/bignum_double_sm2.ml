...
let BIGNUM_DOUBLE_SM2_EXEC = X86_MK_CORE_EXEC_RULE bignum_double_sm2_mc;;
...
let BIGNUM_DOUBLE_SM2_CORRECT = time prove
 (`!z x n pc.
        ...
        ==> ensures x86
             (\s. bytes_loaded s (word pc) (BUTLAST bignum_double_sm2_mc) /\
                  read RIP s = word pc /\
                  C_ARGUMENTS [z; x] s /\
...
let BIGNUM_DOUBLE_SM2_SUBROUTINE_CORRECT = time prove
 (`!z x n pc stackpointer returnaddress.
  ...
  X86_PROMOTE_RETURN_NOSTACK_TAC
    bignum_double_sm2_mc BIGNUM_DOUBLE_SM2_CORRECT);;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let windows_bignum_double_sm2_mc = define_from_elf
   "windows_bignum_double_sm2_mc" "x86/sm2/bignum_double_sm2.obj";;

let WINDOWS_BIGNUM_DOUBLE_SM2_SUBROUTINE_CORRECT = time prove
 (`!z x n pc stackpointer returnaddress.
        ...
        ==> ensures x86
             (\s. bytes_loaded s (word pc) windows_bignum_double_sm2_mc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
  ...
  WINDOWS_X86_WRAP_NOSTACK_TAC
    windows_bignum_double_sm2_mc bignum_double_sm2_mc
