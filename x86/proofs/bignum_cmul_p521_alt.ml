...
let BIGNUM_CMUL_P521_ALT_SUBROUTINE_CORRECT = time prove
 (`!z c x a pc stackpointer returnaddress.
       ...
       ALL (nonoverlapping (word pc,0x11b))
...
let WINDOWS_BIGNUM_CMUL_P521_ALT_SUBROUTINE_CORRECT = time prove
 (`!z c x a pc stackpointer returnaddress.
       ...
       ALL (nonoverlapping (word pc,0x128))
           [(z,8 * 9); (word_sub stackpointer (word 48),48)]
       ==> ensures x86
            (\s. bytes_loaded s (word pc) windows_bignum_cmul_p521_alt_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
  ...
  WINDOWS_X86_WRAP_STACK_TAC windows_bignum_cmul_p521_alt_mc
   bignum_cmul_p521_alt_mc BIGNUM_CMUL_P521_ALT_CORRECT
   `[RBX; RBP; R12; R13]` 32);;
