...
let BIGNUM_DOUBLE_P256K1_EXEC = X86_MK_CORE_EXEC_RULE bignum_double_p256k1_mc;;
...
let BIGNUM_DOUBLE_P256K1_CORRECT = time prove
 (`!z x n pc.
        ...
        ==> ensures x86
             (\s. bytes_loaded s (word pc) (BUTLAST bignum_double_p256k1_mc) /\
                  read RIP s = word pc /\
                  C_ARGUMENTS [z; x] s /\
  ...
  SUBGOAL_THEN `2 EXP 256 * bitval topbit + l = 2 * n` ASSUME_TAC THENL
   [MAP_EVERY EXPAND_TAC ["l"; "n"; "topbit"] THEN
    ...
    REWRITE_TAC[BIT_WORD_JOIN; BIT_WORD_SUBWORD; DIMINDEX_64;
                BIT_WORD_SHL; DIMINDEX_128] THEN
    CONV_TAC NUM_REDUCE_CONV THEN
    CONV_TAC(ONCE_DEPTH_CONV EXPAND_NSUM_CONV) THEN
    CONV_TAC NUM_REDUCE_CONV THEN REWRITE_TAC[BITVAL_CLAUSES] THEN ARITH_TAC;
    ALL_TAC] THEN
...
let BIGNUM_DOUBLE_P256K1_SUBROUTINE_CORRECT = time prove
 (`!z x n pc stackpointer returnaddress.
  ...
  X86_PROMOTE_RETURN_NOSTACK_TAC
    bignum_double_p256k1_mc BIGNUM_DOUBLE_P256K1_CORRECT);;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let windows_bignum_double_p256k1_mc = define_from_elf
   "windows_bignum_double_p256k1_mc" "x86/secp256k1/bignum_double_p256k1.obj";;

let WINDOWS_BIGNUM_DOUBLE_P256K1_SUBROUTINE_CORRECT = time prove
 (`!z x n pc stackpointer returnaddress.
        ...
        ==> ensures x86
             (\s. bytes_loaded s (word pc) windows_bignum_double_p256k1_mc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
  ...
  WINDOWS_X86_WRAP_NOSTACK_TAC
    windows_bignum_double_p256k1_mc bignum_double_p256k1_mc
