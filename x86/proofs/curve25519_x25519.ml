...
[
  ...
  0x0f; 0x83; 0x2f; 0xeb; 0xff; 0xff;
                           (* JAE (Imm32 (word 4294961967)) *)
...
let LOCAL_MUL_P25519_TAC =
  X86_MACRO_SIM_ABBREV_TAC (X86_TRIM_EXEC_RULE curve25519_x25519_mc) 91 lvs
   `!(t:x86state) pcin pcout p3 n3 p1 n1 p2 n2.
      ...
      ==>
      nonoverlapping (word pc,0x5260) (word_add (read p3 t) (word n3),8 * 4)
      ==> ensures x86
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  X86_STEPS_TAC CURVE25519_X25519_EXEC (72--75) THEN
...
let LOCAL_MUL_4_TAC =
  X86_MACRO_SIM_ABBREV_TAC (X86_TRIM_EXEC_RULE curve25519_x25519_mc) 82 lvs
   `!(t:x86state) pcin pcout q3 n3 q1 n1 q2 n2.
      ...
      ==>
      nonoverlapping (word pc,0x5260) (word_add (read q3 t) (word n3),8 * 4)
      ==> ensures x86
           (\s. bytes_loaded s (word pc) (BUTLAST curve25519_x25519_mc) /\
                read RIP s = pcin /\
                read RSP s = read RSP t /\
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  X86_ACCSTEPS_TAC CURVE25519_X25519_EXEC (76--79) (76--83) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
...
let LOCAL_MUL_5_TAC =
  X86_MACRO_SIM_ABBREV_TAC (X86_TRIM_EXEC_RULE curve25519_x25519_mc) 75 lvs
   `!(t:x86state) pcin pcout p3 n3 p1 n1 p2 n2.
      ...
      !n. read(memory :> bytes(word_add (read p2 t) (word n2),8 * 4)) t = n
      ==>
      nonoverlapping (word pc,0x5260) (word_add (read p3 t) (word n3),8 * 5)
      ==> ensures x86
           (\s. bytes_loaded s (word pc) (BUTLAST curve25519_x25519_mc) /\
                read RIP s = pcin /\
                read RSP s = read RSP t /\
...
let LOCAL_ADD_TWICE4_TAC =
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  FIRST_ASSUM(BIGNUM_LDIGITIZE_TAC "n_" o lhand o concl) THEN
  FIRST_ASSUM(BIGNUM_LDIGITIZE_TAC "m_" o lhand o concl) THEN
  X86_ACCSTEPS_TAC CURVE25519_X25519_EXEC [3;5;7;9] (1--11) THEN
  SUBGOAL_THEN `carry_s9 <=> 2 EXP 256 <= m + n` SUBST_ALL_TAC THENL
   [MATCH_MP_TAC FLAG_FROM_CARRY_LE THEN EXISTS_TAC `256` THEN
    ...
    ALL_TAC] THEN
  X86_ACCSTEPS_TAC CURVE25519_X25519_EXEC (12--15) (12--19) THEN
...
let CURVE25519_X25519_CORRECT = time prove
 (`!res scalar n point X pc stackpointer.
    ALL (nonoverlapping (stackpointer,416))
        [(word pc,0x5260); (res,32); (scalar,32); (point,32)] /\
    nonoverlapping (res,32) (word pc,0x5260)
    ==> ensures x86
         (\s. bytes_loaded s (word pc) (BUTLAST curve25519_x25519_mc) /\
  ...
  ENSURES_WHILE_ADOWN_TAC `254` `3` `pc + 0xa08` `pc + 0x1ecf`
   `\i s.
      read RSP s = stackpointer /\
  ...
  REPEAT CONJ_TAC THENL
   [ARITH_TAC;
    ...
    X86_STEPS_TAC CURVE25519_X25519_EXEC (1--29) THEN
    SUBGOAL_THEN
    ...
    X86_STEPS_TAC CURVE25519_X25519_EXEC (38--39) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
    ...
    ASSUME_TAC THENL
     [EXPAND_TAC "n_input" THEN REWRITE_TAC[GSYM BIGNUM_FROM_MEMORY_BYTES] THEN
      ...
      ALL_TAC] THEN
    X86_STEPS_TAC CURVE25519_X25519_EXEC (5--12) THEN
    SUBGOAL_THEN
     `word_and
    ...
    REPEAT STRIP_TAC THEN X86_SIM_TAC CURVE25519_X25519_EXEC (1--2) THEN
 ...
 GHOST_INTRO_TAC `xn:num`
  ...
  X86_STEPS_TAC CURVE25519_X25519_EXEC (31--32) THEN
  ...
  X86_STEPS_TAC CURVE25519_X25519_EXEC (34--49) THEN
...
let CURVE25519_X25519_SUBROUTINE_CORRECT = time prove
 (`!res scalar n point X pc stackpointer returnaddress.
    ALL (nonoverlapping (word_sub stackpointer (word 464),464))
        [(word pc,0x5260); (point,32); (scalar,32)] /\
    nonoverlapping (res,32) (word pc,0x5260) /\
...
let WINDOWS_CURVE25519_X25519_SUBROUTINE_CORRECT = time prove
 (`!res scalar n point X pc stackpointer returnaddress.
    ALL (nonoverlapping (word_sub stackpointer (word 480),480))
        [(word pc,0x526d); (point,32); (scalar,32)] /\
    nonoverlapping (res,32) (word pc,0x526d) /\
