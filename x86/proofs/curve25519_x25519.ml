...
let LOCAL_MUL_P25519_TAC =
  X86_MACRO_SIM_ABBREV_TAC (X86_TRIM_EXEC_RULE curve25519_x25519_mc) 91 lvs
   `!(t:x86state) pcin pcout p3 n3 p1 n1 p2 n2.
      ...
      ==>
      nonoverlapping (word pc,0x4435) (word_add (read p3 t) (word n3),8 * 4)
      ==> ensures x86
...
let LOCAL_MUL_4_TAC =
  X86_MACRO_SIM_ABBREV_TAC (X86_TRIM_EXEC_RULE curve25519_x25519_mc) 82 lvs
   `!(t:x86state) pcin pcout q3 n3 q1 n1 q2 n2.
      ...
      ==>
      nonoverlapping (word pc,0x4435) (word_add (read q3 t) (word n3),8 * 4)
      ==> ensures x86
           (\s. bytes_loaded s (word pc) (BUTLAST curve25519_x25519_mc) /\
                read RIP s = pcin /\
                read RSP s = read RSP t /\
...
let LOCAL_MUL_5_TAC =
  X86_MACRO_SIM_ABBREV_TAC (X86_TRIM_EXEC_RULE curve25519_x25519_mc) 75 lvs
   `!(t:x86state) pcin pcout p3 n3 p1 n1 p2 n2.
      ...
      !n. read(memory :> bytes(word_add (read p2 t) (word n2),8 * 4)) t = n
      ==>
      nonoverlapping (word pc,0x4435) (word_add (read p3 t) (word n3),8 * 5)
      ==> ensures x86
           (\s. bytes_loaded s (word pc) (BUTLAST curve25519_x25519_mc) /\
                read RIP s = pcin /\
                read RSP s = read RSP t /\
...
let LOCAL_MODINV_TAC =
  ...
  X86_SUBROUTINE_SIM_TAC(bth,CURVE25519_X25519_EXEC,0x3c79,cth,th)
...
let CURVE25519_X25519_CORRECT = time prove
 (`!res scalar n point X pc stackpointer.
    ALL (nonoverlapping (stackpointer,416))
        [(word pc,0x4435); (res,32); (scalar,32); (point,32)] /\
    nonoverlapping (res,32) (word pc,0x4435)
    ==> ensures x86
         (\s. bytes_loaded s (word pc) (BUTLAST curve25519_x25519_mc) /\
 ...
 GHOST_INTRO_TAC `xn:num`
  ...
  X86_STEPS_TAC CURVE25519_X25519_EXEC (31--42) THEN
  SUBGOAL_THEN
  ...
  X86_STEPS_TAC CURVE25519_X25519_EXEC (44--59) THEN
...
let CURVE25519_X25519_SUBROUTINE_CORRECT = time prove
 (`!res scalar n point X pc stackpointer returnaddress.
    ALL (nonoverlapping (word_sub stackpointer (word 464),464))
        [(word pc,0x4435); (point,32); (scalar,32)] /\
    nonoverlapping (res,32) (word pc,0x4435) /\
...
let WINDOWS_CURVE25519_X25519_SUBROUTINE_CORRECT = time prove
 (`!res scalar n point X pc stackpointer returnaddress.
    ALL (nonoverlapping (word_sub stackpointer (word 480),480))
        [(word pc,0x4442); (point,32); (scalar,32)] /\
    nonoverlapping (res,32) (word pc,0x4442) /\
