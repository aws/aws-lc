...
let BIGNUM_CMUL_P384_ALT_SUBROUTINE_CORRECT = time prove
 (`!z c x a pc stackpointer returnaddress.
        ...
        ALL (nonoverlapping (word pc,0xe3))
...
let windows_bignum_cmul_p384_alt_mc = define_from_elf
   "windows_bignum_cmul_p384_alt_mc" "x86/p384/bignum_cmul_p384_alt.obj";;

let WINDOWS_BIGNUM_CMUL_P384_ALT_SUBROUTINE_CORRECT = time prove
 (`!z c x a pc stackpointer returnaddress.
        nonoverlapping (word_sub stackpointer (word 24),24) (x,8 * 6) /\
        nonoverlapping (z,8 * 6) (word_sub stackpointer (word 24),32) /\
        ALL (nonoverlapping (word pc,0xf0))
            [(z,8 * 6); (word_sub stackpointer (word 24),24)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) windows_bignum_cmul_p384_alt_mc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
  ...
  WINDOWS_X86_WRAP_STACK_TAC
    windows_bignum_cmul_p384_alt_mc bignum_cmul_p384_alt_mc
