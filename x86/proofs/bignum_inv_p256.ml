...
let BIGNUM_INV_P256_EXEC = X86_MK_CORE_EXEC_RULE bignum_inv_p256_mc;;
...
let CORE_INV_P256_EXEC =
    X86_MK_EXEC_RULE
     ((GEN_REWRITE_CONV RAND_CONV [bignum_inv_p256_mc] THENC TRIM_LIST_CONV)
      `TRIM_LIST (17,18) bignum_inv_p256_mc`);;
...
let BIGNUM_INV_P256_CORRECT = time prove
 (`!z x n pc stackpointer.
        ...
        ==> ensures x86
             (\s. bytes_loaded s (word pc) (BUTLAST bignum_inv_p256_mc) /\
  ...
  X86_SUBROUTINE_SIM_TAC
      ((GEN_REWRITE_CONV RAND_CONV [bignum_inv_p256_mc] THENC
        REWRITE_CONV[BUTLAST_CLAUSES])
       `BUTLAST bignum_inv_p256_mc`,
...
let BIGNUM_INV_P256_SUBROUTINE_CORRECT = time prove
 (`!z x n pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 288),288))
            [(word pc,0x156f); (x,8 * 4)] /\
        ALL (nonoverlapping (z,8 * 4))
            [(word_sub stackpointer (word 288),296); (word pc,0x156f)]
        ==> ensures x86
...
let windows_bignum_inv_p256_mc = define_from_elf "windows_bignum_inv_p256_mc"
      "x86/p256/bignum_inv_p256.obj";;

let WINDOWS_BIGNUM_INV_P256_SUBROUTINE_CORRECT = time prove
 (`!z x n pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 304),304))
            [(word pc,0x1579); (x,8 * 4)] /\
        ALL (nonoverlapping (z,8 * 4))
            [(word_sub stackpointer (word 304),312); (word pc,0x1579)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) windows_bignum_inv_p256_mc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
  ...
  WINDOWS_X86_WRAP_STACK_TAC windows_bignum_inv_p256_mc bignum_inv_p256_mc
     BIGNUM_INV_P256_CORRECT
      `[RBX; RBP; R12; R13; R14; R15]` 288);;
