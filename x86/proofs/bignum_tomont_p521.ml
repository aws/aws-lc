...
let BIGNUM_TOMONT_P521_EXEC = X86_MK_EXEC_RULE bignum_tomont_p521_mc;;
 ...
 (`!z x n pc.
  ...
  (CONJUNCTS_THEN SUBST_ALL_TAC) THENL
   [MAP_EVERY EXPAND_TAC ["n"; "h"; "l"] THEN CONJ_TAC THENL
     [REWRITE_TAC[EXP_ADD; GSYM DIV_DIV; BIGNUM_OF_WORDLIST_DIV; ARITH_RULE
       `521 = 64 + 64 + 64 + 64 + 64 + 64 + 64 + 64 + 9`] THEN
      REWRITE_TAC[SYM(NUM_REDUCE_CONV `2 EXP 9 - 1`)] THEN
      REWRITE_TAC[VAL_WORD_USHR; VAL_WORD_AND_MASK_WORD;
                  BIGNUM_OF_WORDLIST_SING];
  ...
  SUBST_ALL_TAC THENL
   [TRANS_TAC EQ_TRANS
     `(val(h:int64) + l + 1) DIV 2 EXP (8 * 64) < 2 EXP 9` THEN CONJ_TAC THENL
...
let BIGNUM_TOMONT_P521_SUBROUTINE_CORRECT = prove
 (`!z x n pc stackpointer returnaddress.
  ...
  X86_ADD_RETURN_STACK_TAC BIGNUM_TOMONT_P521_EXEC BIGNUM_TOMONT_P521_CORRECT
   `[RBX]` 8);;
