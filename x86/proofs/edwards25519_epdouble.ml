...
let EDWARDS25519_EPDOUBLE_EXEC = X86_MK_CORE_EXEC_RULE edwards25519_epdouble_mc;;
...
let LOCAL_MUL_P25519_TAC =
  X86_MACRO_SIM_ABBREV_TAC (X86_TRIM_EXEC_RULE edwards25519_epdouble_mc) 91 lvs
   `!(t:x86state) pcin pcout q3 n3 q1 n1 q2 n2.
      ...
      ==> ensures x86
           (\s. bytes_loaded s (word pc) (BUTLAST edwards25519_epdouble_mc) /\
                read RIP s = pcin /\
...
let LOCAL_SQR_4_TAC =
  X86_MACRO_SIM_ABBREV_TAC (X86_TRIM_EXEC_RULE edwards25519_epdouble_mc) 68 lvs
...
let LOCAL_ADD_4_TAC =
  X86_MACRO_SIM_ABBREV_TAC (X86_TRIM_EXEC_RULE edwards25519_epdouble_mc) 12 lvs
   `!(t:x86state) pcin pcout q3 n3 q1 n1 q2 n2.
      !m. read(memory :> bytes(word_add (read q1 t) (word n1),8 * 4)) t = m
      ==>
      !n. read(memory :> bytes(word_add (read q2 t) (word n2),8 * 4)) t = n
...
let LOCAL_ADD_TWICE4_TAC =
  X86_MACRO_SIM_ABBREV_TAC (X86_TRIM_EXEC_RULE edwards25519_epdouble_mc) 19 lvs
   `!(t:x86state) pcin pcout q3 n3 q1 n1 q2 n2.
      !m. read(memory :> bytes(word_add (read q1 t) (word n1),8 * 4)) t = m
      ==>
      !n. read(memory :> bytes(word_add (read q2 t) (word n2),8 * 4)) t = n
...
let EDWARDS25519_EPDOUBLE_SUBROUTINE_CORRECT = time prove
 (`!p3 p1 T1 pc stackpointer returnaddress.
  ...
  X86_PROMOTE_RETURN_STACK_TAC
    edwards25519_epdouble_mc EDWARDS25519_EPDOUBLE_CORRECT
...
let windows_edwards25519_epdouble_mc = define_from_elf
  "windows_edwards25519_epdouble_mc"
  "x86/curve25519/edwards25519_epdouble.obj";;

let WINDOWS_EDWARDS25519_EPDOUBLE_SUBROUTINE_CORRECT = prove
 (`!p3 p1 T1 pc stackpointer returnaddress.
    ALL (nonoverlapping (word_sub stackpointer (word 216),216))
        [(word pc,0xf05); (p1,96)] /\
    nonoverlapping (p3,128) (word pc,0xf05) /\
    nonoverlapping (p3,128) (word_sub stackpointer (word 216),224)
    ==> ensures x86
         (\s. bytes_loaded s (word pc) windows_edwards25519_epdouble_mc /\
              read RIP s = word pc /\
              read RSP s = stackpointer /\
              read (memory :> bytes64 stackpointer) s = returnaddress /\
  ...
  WINDOWS_X86_WRAP_STACK_TAC
   windows_edwards25519_epdouble_mc edwards25519_epdouble_mc
