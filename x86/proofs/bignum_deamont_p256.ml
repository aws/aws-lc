...
let windows_bignum_deamont_p256_mc = define_from_elf
   "windows_bignum_deamont_p256_mc" "x86/p256/bignum_deamont_p256.obj";;

let WINDOWS_BIGNUM_DEAMONT_P256_SUBROUTINE_CORRECT = time prove
 (`!z x a pc stackpointer returnaddress.
        nonoverlapping (z,8 * 4) (word_sub stackpointer (word 24),32) /\
        ALL (nonoverlapping (word_sub stackpointer (word 24),24))
            [(word pc,0x140); (x,8 * 4)] /\
        nonoverlapping (word pc,0x140) (z,8 * 4)
        ==> ensures x86
             (\s. bytes_loaded s (word pc) windows_bignum_deamont_p256_mc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
  ...
  WINDOWS_X86_WRAP_STACK_TAC
    windows_bignum_deamont_p256_mc bignum_deamont_p256_mc
