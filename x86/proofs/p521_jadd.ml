...
let LOCAL_SQR_P521_TAC =
  ...
  `!(t:x86state) pcin pcout p3 n3 p1 n1.
    ...
    ==>
    nonoverlapping (word pc,0x7e6e) (stackpointer,568) /\
    ...
    nonoverlapping (word pc,0x7e6e) (read RDI t,216)
    ==> ensures x86
...
let LOCAL_SUB_P521_TAC =
  ...
  `!(t:x86state) pcin pcout p3 n3 p1 n1 p2 n2.
    ...
    ==>
    nonoverlapping (word pc,0x7e6e) (word_add (read p3 t) (word n3),72)
    ==> ensures x86
         (\s. bytes_loaded s (word pc) (BUTLAST p521_jadd_mc) /\
              read RIP s = pcin /\
              read RSP s = read RSP t /\
...
let P521_JADD_CORRECT = time prove
 (`!p3 p1 t1 p2 t2 pc stackpointer.
        ALL (nonoverlapping (stackpointer,568))
            [(word pc,0x7e6e); (p1,216); (p2,216); (p3,216)] /\
        nonoverlapping (p3,216) (word pc,0x7e6e)
        ==> ensures x86
             (\s. bytes_loaded s (word pc) (BUTLAST p521_jadd_mc) /\
  ...
  X86_STEPS_TAC P521_JADD_EXEC (25--78) THEN
  CONV_TAC(ONCE_DEPTH_CONV BIGNUM_LEXPAND_CONV) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  DISCARD_STATE_TAC "s78" THEN
...
let P521_JADD_SUBROUTINE_CORRECT = time prove
 (`!p3 p1 t1 p2 t2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 616),616))
            [(word pc,0x7e6e); (p1,216); (p2,216)] /\
        ALL (nonoverlapping (p3,216))
            [(word pc,0x7e6e); (word_sub stackpointer (word 616),624)]
        ==> ensures x86
...
let WINDOWS_P521_JADD_SUBROUTINE_CORRECT = time prove
 (`!p3 p1 t1 p2 t2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 632),632))
            [(word pc,0x7e7b); (p1,216); (p2,216)] /\
        ALL (nonoverlapping (p3,216))
            [(word pc,0x7e7b); (word_sub stackpointer (word 632),640)]
        ==> ensures x86
