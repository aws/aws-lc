...
let BIGNUM_DEMONT_SM2_EXEC = X86_MK_CORE_EXEC_RULE bignum_demont_sm2_mc;;
...
let BIGNUM_DEMONT_SM2_CORRECT = time prove
 (`!z x a pc.
        ...
        ==> ensures x86
             (\s. bytes_loaded s (word pc) (BUTLAST bignum_demont_sm2_mc) /\
                  read RIP s = word pc /\
                  C_ARGUMENTS [z; x] s /\
...
let windows_bignum_demont_sm2_mc = define_from_elf
   "windows_bignum_demont_sm2_mc" "x86/sm2/bignum_demont_sm2.obj";;

let WINDOWS_BIGNUM_DEMONT_SM2_SUBROUTINE_CORRECT = time prove
 (`!z x a pc stackpointer returnaddress.
        nonoverlapping (z,8 * 4) (word_sub stackpointer (word 16),24) /\
        ALL (nonoverlapping (word_sub stackpointer (word 16),16))
            [(word pc,0xc5); (x,8 * 4)] /\
        nonoverlapping (word pc,0xc5) (z,8 * 4)
        ==> ensures x86
             (\s. bytes_loaded s (word pc) windows_bignum_demont_sm2_mc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
