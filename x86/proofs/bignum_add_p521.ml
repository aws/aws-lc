...
let BIGNUM_ADD_P521_EXEC = X86_MK_EXEC_RULE bignum_add_p521_mc;;
 ...
 (`!z x y m n pc.
  ...
  SUBGOAL_THEN `bit 9 (sum_s19:int64) <=> p_521 <= m + n` SUBST_ALL_TAC THENL
   [REWRITE_TAC[BIT_VAL_MOD] THEN
    SUBGOAL_THEN `val(sum_s19:int64) = (m + n + 1) DIV 2 EXP (8 * 64)`
    SUBST1_TAC THENL
     [FIRST_X_ASSUM(fun th ->
        GEN_REWRITE_TAC (RAND_CONV o LAND_CONV) [SYM th]) THEN
      CONV_TAC(funpow 3 RAND_CONV (LAND_CONV (TOP_DEPTH_CONV num_CONV))) THEN
...
let BIGNUM_ADD_P521_SUBROUTINE_CORRECT = time prove
 (`!z x y m n pc stackpointer returnaddress.
  ...
  X86_ADD_RETURN_STACK_TAC BIGNUM_ADD_P521_EXEC BIGNUM_ADD_P521_CORRECT
    `[RBX; R12]` 16);;
