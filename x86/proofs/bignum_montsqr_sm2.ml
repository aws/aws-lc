...
let BIGNUM_MONTSQR_SM2_EXEC = X86_MK_CORE_EXEC_RULE bignum_montsqr_sm2_mc;;
...
let BIGNUM_MONTSQR_SM2_CORRECT = time prove
 (`!z x a pc.
        ...
        ==> ensures x86
             (\s. bytes_loaded s (word pc) (BUTLAST bignum_montsqr_sm2_mc) /\
...
let BIGNUM_MONTSQR_SM2_SUBROUTINE_CORRECT = time prove
 (`!z x a pc stackpointer returnaddress.
  ...
  X86_PROMOTE_RETURN_STACK_TAC
   bignum_montsqr_sm2_mc BIGNUM_MONTSQR_SM2_CORRECT
        ...
        ==> ensures x86
             ...
             (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * 4);
                       memory :> bytes(word_sub stackpointer (word 48),48)])`,
  X86_PROMOTE_RETURN_STACK_TAC
   bignum_montsqr_sm2_mc BIGNUM_AMONTSQR_SM2_CORRECT
   `[RBX; RBP; R12; R13; R14; R15]` 48);;
...
let windows_bignum_montsqr_sm2_mc = define_from_elf
   "windows_bignum_montsqr_sm2_mc" "x86/sm2/bignum_montsqr_sm2.obj";;

let WINDOWS_BIGNUM_MONTSQR_SM2_SUBROUTINE_CORRECT = time prove
 (`!z x a pc stackpointer returnaddress.
  ...
  WINDOWS_X86_WRAP_STACK_TAC
   windows_bignum_montsqr_sm2_mc bignum_montsqr_sm2_mc
   BIGNUM_MONTSQR_SM2_CORRECT `[RBX; RBP; R12; R13; R14; R15]` 48);;

let WINDOWS_BIGNUM_AMONTSQR_SM2_SUBROUTINE_CORRECT = time prove
 (`!z x a pc stackpointer returnaddress.
        nonoverlapping (z,8 * 4) (word_sub stackpointer (word 64),72) /\
        ALL (nonoverlapping (word_sub stackpointer (word 64),64))
            [(word pc,0x1e7); (x,8 * 4)] /\
