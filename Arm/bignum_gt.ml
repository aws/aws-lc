...
(**** print_literal_from_elf "Arm/bignum_gt.o";;
 ****)

let bignum_gt_mc = define_assert_from_elf "bignum_gt_mc" "Arm/bignum_gt.o"
[
  0xaa1f03e4;       (* arm_MOV X4 XZR *)
  0xeb000042;       (* arm_SUBS X2 X2 X0 *)
  0x54000203;       (* arm_BCC (word 64) *)
  0xb40000e0;       (* arm_CBZ X0 (word 28) *)
  0xf8647865;       (* arm_LDR X5 X3 (Shiftreg_Offset X4 3) *)
  0xf8647826;       (* arm_LDR X6 X1 (Shiftreg_Offset X4 3) *)
...
let BIGNUM_GT_EXEC = ARM_MK_EXEC_RULE bignum_gt_mc;;
...
let BIGNUM_GT_CORRECT = prove
 (`!m a x n b y pc.
        ensures arm
          (\s. aligned_bytes_loaded s (word pc) bignum_gt_mc /\
               read PC s = word pc /\
  ...
  REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; BIGNUM_GT_EXEC] THEN
    ...
    CONJ_TAC THENL
     [ASM_CASES_TAC `m = 0` THENL
       [UNDISCH_THEN `m = 0` SUBST_ALL_TAC THEN
        ...
        ENSURES_INIT_TAC "s0" THEN ARM_STEPS_TAC BIGNUM_GT_EXEC (1--4) THEN
        ENSURES_FINAL_STATE_TAC THEN
        ASM_REWRITE_TAC[LOWDIGITS_0; LE_REFL] THEN CONV_TAC WORD_RULE;
        ALL_TAC] THEN
      ENSURES_WHILE_UP_TAC `m:num` `pc + 0x10` `pc + 0x24`
       `\i s. read X2 s = word (n - m) /\
              ...
              bignum_from_memory (b,n) s = y /\
              bignum_from_memory (a,m) s = x /\
              (read CF s <=> lowdigits x i <= lowdigits y i)` THEN
      ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
       [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES; SUB_0] THEN
        ...
        ENSURES_INIT_TAC "s0" THEN ARM_STEPS_TAC BIGNUM_GT_EXEC [1] THEN
        ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
        VAL_INT64_TAC `m - i:num` THEN ASM_REWRITE_TAC[SUB_EQ_0; NOT_LE];
        ARM_SIM_TAC BIGNUM_GT_EXEC [1]] THEN
      ...
      ASSUME_TAC THENL
       [MAP_EVERY EXPAND_TAC ["y"; "x"] THEN
        ...
        ALL_TAC] THEN
      ARM_STEPS_TAC BIGNUM_GT_EXEC (1--5) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
      ...
      ASM_CASES_TAC `n:num = m` THENL
       [UNDISCH_THEN `n:num = m` SUBST_ALL_TAC THEN
        ...
        ENSURES_INIT_TAC "s0" THEN ARM_STEPS_TAC BIGNUM_GT_EXEC (1--2) THEN
      ...
      ENSURES_WHILE_UP_TAC `n - m:num` `pc + 0x2c` `pc + 0x3c`
       `\i s. read X2 s = word (n - m - i) /\
              read X3 s = b /\
              read X4 s = word(m + i) /\
              bignum_from_memory (b,n) s = y /\
              (read CF s <=> lowdigits x (m + i) <= lowdigits y (m + i))` THEN
      ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
       [SIMPLE_ARITH_TAC;
        ASM_REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
        ENSURES_INIT_TAC "s0" THEN ARM_STEPS_TAC BIGNUM_GT_EXEC [1] THEN
        ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[ADD_CLAUSES; SUB_0] THEN
        VAL_INT64_TAC `n - m:num` THEN ASM_REWRITE_TAC[SUB_EQ_0] THEN
      ...
      ASSUME_TAC THENL
       [EXPAND_TAC "y" THEN
        ...
        ASM_REWRITE_TAC[WORD_VAL];
        ALL_TAC] THEN
      ARM_STEPS_TAC BIGNUM_GT_EXEC (1--4) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
    ...
    CONJ_TAC THENL
     [ASM_CASES_TAC `n = 0` THENL
       [UNDISCH_THEN `n = 0` SUBST_ALL_TAC THEN
        ASM_REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES; SUB_0] THEN
        ENSURES_INIT_TAC "s0" THEN ARM_STEPS_TAC BIGNUM_GT_EXEC (1--5) THEN
        ENSURES_FINAL_STATE_TAC THEN
        ...
        ALL_TAC] THEN
      ENSURES_WHILE_UP_TAC `n:num` `pc + 0x54` `pc + 0x68`
      ...
      ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
       [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES; SUB_0] THEN
        ASSUME_TAC(WORD_RULE
         `word_add (word_sub (word n) (word m)) (word m):int64 = word n`) THEN
        ENSURES_INIT_TAC "s0" THEN ARM_STEPS_TAC BIGNUM_GT_EXEC (1--6) THEN
      ...
      STRIP_ASSUME_TAC THENL [SIMPLE_ARITH_TAC; ALL_TAC] THEN
      ENSURES_WHILE_UP_TAC `m - n:num` `pc + 0x6c` `pc + 0x7c`
       `\i s. read X0 s = word (m - n - i) /\
              read X1 s = a /\
              read X4 s = word(n + i) /\
              bignum_from_memory (a,m) s = x /\
              (read CF s <=> lowdigits x (n + i) <= lowdigits y (n + i))` THEN
      ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
       [SIMPLE_ARITH_TAC;
        ASM_REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
