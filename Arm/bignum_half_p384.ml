...
(**** print_literal_from_elf "Arm/bignum_half_p384.o";;
 ****)

let bignum_half_p384_mc = define_assert_from_elf "bignum_half_p384_mc" "Arm/bignum_half_p384.o"
[
  ...
  0x8a0a016b;       (* arm_AND X11 X11 X10 *)
  ...
  0x93c504c5;       (* arm_EXTR X5 X6 X5 (rvalue (word 1)) *)
  0x93c604e6;       (* arm_EXTR X6 X7 X6 (rvalue (word 1)) *)
  0x93c70507;       (* arm_EXTR X7 X8 X7 (rvalue (word 1)) *)
...
let BIGNUM_HALF_P384_CORRECT = time prove
 (`!z x n pc.
  ...
  ARM_STEPS_TAC BIGNUM_HALF_P384_EXEC (1--7) THEN
  ...
  SUBGOAL_THEN `bit 0 (n_0:int64) <=> ODD n` SUBST_ALL_TAC THENL
   [EXPAND_TAC "n" THEN REWRITE_TAC[BIT_LSB; ODD_ADD; ODD_MULT; ODD_EXP] THEN
    ...
    ALL_TAC] THEN
  ARM_ACCSTEPS_TAC BIGNUM_HALF_P384_EXEC (8--17) (8--27) THEN
  ...
  SUBST1_TAC THENL
   [REWRITE_TAC[MOD_UNIQUE] THEN CONJ_TAC THENL
    ...
    ALL_TAC] THEN
  SUBGOAL_THEN
   `2 EXP 384 * bitval(carry_s17) +
    bignum_of_wordlist [sum_s8; sum_s11; sum_s14; sum_s15; sum_s16; sum_s17] =
