...
(**** print_literal_from_elf "Arm/bignum_montsqr.o";;
 ****)

let bignum_montsqr_mc =
  define_assert_from_elf "bignum_montsqr_mc" "Arm/bignum_montsqr.o"
[
  ...
  0xfa0b01bf;       (* arm_SBCS XZR X13 X11 *)
  0x91000529;       (* arm_ADD X9 X9 (rvalue (word 1)) *)
  0xcb00012d;       (* arm_SUB X13 X9 X0 *)
  0xb5ffff6d;       (* arm_CBNZ X13 (word 2097132) *)
...
let BIGNUM_MONTSQR_EXEC = ARM_MK_EXEC_RULE bignum_montsqr_mc;;
...
let BIGNUM_MONTSQR_CORRECT = time prove
 (`!k z x m a n pc.
      ALL (nonoverlapping (z,8 * val k))
          [(word pc,0x150); (x,8 * val k); (m,8 * val k)]
      ==> ensures arm
           (\s. aligned_bytes_loaded s (word pc) bignum_montsqr_mc /\
                read PC s = word pc /\
  ...
  ASM_CASES_TAC `k = 0` THENL
   [UNDISCH_THEN `k = 0` SUBST_ALL_TAC THEN
    ...
    ENSURES_INIT_TAC "s0" THEN ARM_STEPS_TAC BIGNUM_MONTSQR_EXEC [1] THEN
  ...
  CONJ_TAC THENL
   [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN ENSURES_INIT_TAC "s0" THEN
    ...
    ARM_STEPS_TAC BIGNUM_MONTSQR_EXEC (1--5) THEN
    ...
    ARM_STEPS_TAC BIGNUM_MONTSQR_EXEC (6--14) THEN
   ...
   [ENSURES_WHILE_UP_TAC `k:num` `pc + 0x3c` `pc + 0x44`
    ...
    ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
     [REWRITE_TAC[BIGNUM_FROM_MEMORY_TRIVIAL] THEN
      ...
      REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
      ENSURES_INIT_TAC "s0" THEN ARM_STEPS_TAC BIGNUM_MONTSQR_EXEC (1--2) THEN
      ...
      ENSURES_INIT_TAC "s0" THEN ARM_STEPS_TAC BIGNUM_MONTSQR_EXEC (1--2) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[];
      ARM_SIM_TAC BIGNUM_MONTSQR_EXEC (1--4) THEN
    ...
    ENSURES_WHILE_UP_TAC `k:num` `pc + 0x108` `pc + 0x118`
     ...
     ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
      [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES; SUB_0] THEN
       ENSURES_INIT_TAC "s0" THEN ARM_STEPS_TAC BIGNUM_MONTSQR_EXEC (1--3) THEN
       ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
       ...
       ARM_STEPS_TAC BIGNUM_MONTSQR_EXEC (1--4) THEN
       ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
    ...
    ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
     [GHOST_INTRO_TAC `cin:bool` `read CF` THEN
      ...
      ARM_VSTEPS_TAC BIGNUM_MONTSQR_EXEC (3--5) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
      ...
      REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
      ENSURES_INIT_TAC "s0" THEN
      ARM_ACCSTEPS_TAC BIGNUM_MONTSQR_EXEC [4] (1--6) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[GSYM WORD_ADD] THEN
   ...
   [ENSURES_WHILE_UP_TAC `k:num` `pc + 0x60` `pc + 0x84`
    ...
    ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
     [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
      ...
      REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
      ENSURES_INIT_TAC "s0" THEN
      ARM_ACCSTEPS_TAC BIGNUM_MONTSQR_EXEC [3;4;6;7] (1--9) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[GSYM WORD_ADD] THEN
      ...
      ARM_ACCSTEPS_TAC BIGNUM_MONTSQR_EXEC [3] (3--4) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
   ...
   [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN ENSURES_INIT_TAC "s0" THEN
    ...
    MP_TAC THENL
     [ASM_REWRITE_TAC[HIGHDIGITS_0; BIGNUM_FROM_MEMORY_BYTES];
      ...
      ASM_REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES; ADD_CLAUSES] THEN
      STRIP_TAC] THEN
    ARM_ACCSTEPS_TAC BIGNUM_MONTSQR_EXEC [4;6] (1--8) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
   ...
   [ASM_CASES_TAC `k = 1` THENL
    ...
    REPEAT CONJ_TAC THENL
     [ASM_REWRITE_TAC[ARITH_RULE `1 < k <=> ~(k = 0 \/ k = 1)`];
      ...
      ARM_ACCSTEPS_TAC BIGNUM_MONTSQR_EXEC [3;4;6;7] (1--10) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  ...
  ASSUME_TAC THENL
   [ASM_REWRITE_TAC[WORD_SUB; ARITH_RULE `1 <= k <=> ~(k = 0)`];
    ALL_TAC] THEN
  ARM_ACCSTEPS_TAC BIGNUM_MONTSQR_EXEC [1;2] (1--5) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  CONJ_TAC THENL [CONV_TAC WORD_RULE; ALL_TAC] THEN
