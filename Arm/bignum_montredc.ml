...
(**** print_literal_from_elf "Arm/bignum_montredc.o";;
 ****)

let bignum_montredc_mc =
  define_assert_from_elf "bignum_montredc_mc" "Arm/bignum_montredc.o"
[
  0xb4000a80;       (* arm_CBZ X0 (word 336) *)
  ...
  0xb4000485;       (* arm_CBZ X5 (word 144) *)
  0xaa1f03e7;       (* arm_MOV X7 XZR *)
...
let BIGNUM_MONTREDC_EXEC = ARM_MK_EXEC_RULE bignum_montredc_mc;;
 ...
 (`!k z r x m p a n pc.
  ...
  ASM_CASES_TAC `k = 0` THENL
   [UNDISCH_THEN `k = 0` SUBST_ALL_TAC THEN
    ...
    ENSURES_INIT_TAC "s0" THEN ARM_STEPS_TAC BIGNUM_MONTREDC_EXEC [1] THEN
  ...
  CONJ_TAC THENL
   [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN ENSURES_INIT_TAC "s0" THEN
    ...
    ARM_STEPS_TAC BIGNUM_MONTREDC_EXEC (1--5) THEN
    ...
    ARM_STEPS_TAC BIGNUM_MONTREDC_EXEC (6--14) THEN
   ...
   [ASM_CASES_TAC `nx = 0` THENL
    ...
    ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
     [ASM_REWRITE_TAC[ARITH_RULE `MIN a b = 0 <=> a = 0 \/ b = 0`];
      ...
      X_GEN_TAC `i:num` THEN STRIP_TAC THEN VAL_INT64_TAC `i:num` THEN
      ARM_SIM_TAC BIGNUM_MONTREDC_EXEC (1--2) THEN
    ...
    ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
     [ARM_SIM_TAC BIGNUM_MONTREDC_EXEC (1--4) THEN
      ...
      X_GEN_TAC `i:num` THEN STRIP_TAC THEN VAL_INT64_TAC `i:num` THEN
      ARM_SIM_TAC BIGNUM_MONTREDC_EXEC (1--2);
      ARM_SIM_TAC BIGNUM_MONTREDC_EXEC (1--3) THEN
  ...
  ENSURES_SEQUENCE_TAC `pc + 0x108`
   `\s. read X0 s = word k /\
        read X1 s = z /\
        read X4 s = m /\
        bignum_from_memory (m,k) s = n /\
        ?q r. q < 2 EXP (64 * p) /\ r < 2 EXP (64 * p) /\
              2 EXP (64 * p) *
  ...
  CONJ_TAC THENL
   [ALL_TAC;
    ...
    ENSURES_WHILE_UP_TAC `k:num` `pc + 0x10c` `pc + 0x11c`
      `\i s. read X0 s = word k /\
             read X1 s = z /\
             read X4 s = m /\
     ...
     ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
      [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES; SUB_0] THEN
       ENSURES_INIT_TAC "s0" THEN
       ARM_STEPS_TAC BIGNUM_MONTREDC_EXEC [1] THEN
       ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
       ...
       REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
       ENSURES_INIT_TAC "s0" THEN
       ARM_STEPS_TAC BIGNUM_MONTREDC_EXEC (1--4) THEN
       ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
    ...
    ENSURES_WHILE_UP_TAC `k:num` `pc + 0x130` `pc + 0x148`
     `\i s. read X0 s = word k /\
            read X1 s = z /\
            read X4 s = m /\
    ...
    ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
     [GHOST_INTRO_TAC `cin:bool` `read CF` THEN
      REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
      ENSURES_INIT_TAC "s0" THEN ARM_STEPS_TAC BIGNUM_MONTREDC_EXEC (1--2) THEN
      RULE_ASSUM_TAC(REWRITE_RULE[WORD_SUB_REFL; VAL_WORD_0]) THEN
      ARM_VSTEPS_TAC BIGNUM_MONTREDC_EXEC (3--5) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
      ...
      REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
      ENSURES_INIT_TAC "s0" THEN
      ARM_ACCSTEPS_TAC BIGNUM_MONTREDC_EXEC [4] (1--6) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[GSYM WORD_ADD] THEN
  ...
  ASM_CASES_TAC `p = 0` THENL
   [UNDISCH_THEN `p = 0` SUBST_ALL_TAC THEN
    REPEAT(FIRST_X_ASSUM(SUBST_ALL_TAC o MATCH_MP (ARITH_RULE
     `a < 2 EXP (64 * 0) ==> a = 0`))) THEN
    ARM_SIM_TAC BIGNUM_MONTREDC_EXEC [1] THEN
   ...
   [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN ENSURES_INIT_TAC "s0" THEN
    ...
    MP_TAC THENL
     [ASM_REWRITE_TAC[HIGHDIGITS_0; BIGNUM_FROM_MEMORY_BYTES];
      ...
      ASM_REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES; ADD_CLAUSES] THEN
      STRIP_TAC] THEN
    ARM_ACCSTEPS_TAC BIGNUM_MONTREDC_EXEC [4;6] (1--8) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
   ...
   [ASM_CASES_TAC `k = 1` THENL
    ...
    REPEAT CONJ_TAC THENL
     [ASM_REWRITE_TAC[ARITH_RULE `1 < k <=> ~(k = 0 \/ k = 1)`];
      ...
      ARM_ACCSTEPS_TAC BIGNUM_MONTREDC_EXEC [3;4;6;7] (1--10) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  ...
  ASM_CASES_TAC `nx:num <= k + i` THENL
   [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN ENSURES_INIT_TAC "s0" THEN
    ARM_ACCSTEPS_TAC BIGNUM_MONTREDC_EXEC [1] (1--8) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
    ...
    REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN ENSURES_INIT_TAC "s0" THEN
    ARM_ACCSTEPS_TAC BIGNUM_MONTREDC_EXEC [1;7;8] (1--11) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
    CONJ_TAC THENL [CONV_TAC WORD_RULE; ALL_TAC]] THEN
 ...
 (`!k z r x m p a n pc returnaddress.
        ...
        ==> ensures arm
             ...
             (MAYCHANGE [PC; X6; X7; X8; X9; X10; X11; X12; X13; X14] ,,
              MAYCHANGE [memory :> bytes(z,8 * val k)] ,,
              MAYCHANGE SOME_FLAGS)`,
  ARM_ADD_RETURN_NOSTACK_TAC BIGNUM_MONTREDC_EXEC BIGNUM_MONTREDC_CORRECT);;
