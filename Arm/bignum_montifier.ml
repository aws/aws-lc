...
(**** print_literal_from_elf "Arm/bignum_montifier.o";;
 ****)

let bignum_montifier_mc =
  define_assert_from_elf "bignum_montifier_mc" "Arm/bignum_montifier.o"
[
  0xb4001e40;       (* arm_CBZ X0 (word 968) *)
  ...
  0xd37ef4e4;       (* arm_LSL X4 X7 (rvalue (word 2)) *)
  0xcb0400e4;       (* arm_SUB X4 X7 X4 *)
  0xd27f0084;       (* arm_EOR X4 X4 (rvalue (word 2)) *)
  0xd2800028;       (* arm_MOV X8 (rvalue (word 1)) *)
  0x9b0420e9;       (* arm_MADD X9 X7 X4 X8 *)
  0x9b097d2a;       (* arm_MUL X10 X9 X9 *)
  0x9b041124;       (* arm_MADD X4 X9 X4 X4 *)
  0x9b0a7d49;       (* arm_MUL X9 X10 X10 *)
  0x9b041144;       (* arm_MADD X4 X10 X4 X4 *)
  0x9b097d2a;       (* arm_MUL X10 X9 X9 *)
  0x9b041124;       (* arm_MADD X4 X9 X4 X4 *)
  0x9b041144;       (* arm_MADD X4 X10 X4 X4 *)
  0xf940006a;       (* arm_LDR X10 X3 (Immediate_Offset (word 0)) *)
  0x9b047d4b;       (* arm_MUL X11 X10 X4 *)
  0x9b077d68;       (* arm_MUL X8 X11 X7 *)
  0x9bc77d69;       (* arm_UMULH X9 X11 X7 *)
  ...
  0xf940002a;       (* arm_LDR X10 X1 (Immediate_Offset (word 0)) *)
  0x9b047d4b;       (* arm_MUL X11 X10 X4 *)
  0xf9400047;       (* arm_LDR X7 X2 (Immediate_Offset (word 0)) *)
  0x9b077d68;       (* arm_MUL X8 X11 X7 *)
  0x9bc77d69;       (* arm_UMULH X9 X11 X7 *)
  ...
  0xab08014a;       (* arm_ADDS X10 X10 X8 *)
  0xd10004a7;       (* arm_SUB X7 X5 (rvalue (word 1)) *)
  0xf827782a;       (* arm_STR X10 X1 (Shiftreg_Offset X7 3) *)
  0x910004a5;       (* arm_ADD X5 X5 (rvalue (word 1)) *)
  0xcb0000a7;       (* arm_SUB X7 X5 X0 *)
  0xb5fffea7;       (* arm_CBNZ X7 (word 2097108) *)
  0x9a1f0129;       (* arm_ADC X9 X9 XZR *)
  0xd1000407;       (* arm_SUB X7 X0 (rvalue (word 1)) *)
  0xf8277829;       (* arm_STR X9 X1 (Shiftreg_Offset X7 3) *)
  0xf10004c6;       (* arm_SUBS X6 X6 (rvalue (word 1)) *)
  0x54fffce1;       (* arm_BNE (word 2097052) *)
  0xeb1f03e5;       (* arm_NEGS X5 XZR *)
  0xf8657827;       (* arm_LDR X7 X1 (Shiftreg_Offset X5 3) *)
  0xf865784a;       (* arm_LDR X10 X2 (Shiftreg_Offset X5 3) *)
  0xfa0a00ff;       (* arm_SBCS XZR X7 X10 *)
  0x910004a5;       (* arm_ADD X5 X5 (rvalue (word 1)) *)
  0xcb0000a7;       (* arm_SUB X7 X5 X0 *)
  0xb5ffff67;       (* arm_CBNZ X7 (word 2097132) *)
...
let BIGNUM_MONTIFIER_EXEC = ARM_MK_EXEC_RULE bignum_montifier_mc;;
...
let BIGNUM_MONTIFIER_CORRECT = time prove
 (`!k z m t n pc.
        ...
        ALLPAIRS nonoverlapping [(z,8 * val k); (t,8 * val k)]
                                [(word pc,0x3cc); (m,8 * val k)]
        ==> ensures arm
             (\s. aligned_bytes_loaded s (word pc) bignum_montifier_mc /\
                  read PC s = word pc /\
                  C_ARGUMENTS [k; z; m; t] s /\
                  bignum_from_memory (m,val k) s = n)
             (\s. read PC s = word(pc + 0x3c8) /\
                  (ODD n
  ...
  ASM_CASES_TAC `k = 0` THENL
   [UNDISCH_THEN `k = 0` SUBST_ALL_TAC THEN
    ...
    ENSURES_INIT_TAC "s0" THEN ARM_STEPS_TAC BIGNUM_MONTIFIER_EXEC [1] THEN
  ...
  ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
   [ARM_SIM_TAC BIGNUM_MONTIFIER_EXEC (1--2) THEN
    ...
    ARM_STEPS_TAC BIGNUM_MONTIFIER_EXEC (1--3) THEN
    ...
    X_GEN_TAC `i:num` THEN STRIP_TAC THEN VAL_INT64_TAC `i:num` THEN
    ARM_SIM_TAC BIGNUM_MONTIFIER_EXEC (1--2);
  ...
  CONJ_TAC THENL
   [ASM_CASES_TAC `k = 1` THENL
     [UNDISCH_THEN `k = 1` SUBST_ALL_TAC THEN
      ARM_SIM_TAC BIGNUM_MONTIFIER_EXEC (1--4) THEN
    ...
    ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
     [MP_TAC(ISPECL [`word k:int64`; `word 1:int64`] VAL_WORD_SUB_CASES) THEN
      ...
      ARM_STEPS_TAC BIGNUM_MONTIFIER_EXEC (1--4) THEN
      ...
      X_GEN_TAC `i:num` THEN STRIP_TAC THEN VAL_INT64_TAC `i:num` THEN
      ARM_SIM_TAC BIGNUM_MONTIFIER_EXEC [1];
      ARM_SIM_TAC BIGNUM_MONTIFIER_EXEC [1]] THEN
    ...
    ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
     [ARM_SIM_TAC BIGNUM_MONTIFIER_EXEC (1--3) THEN
      ...
      REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN ENSURES_INIT_TAC "s0" THEN
      ARM_STEPS_TAC BIGNUM_MONTIFIER_EXEC (1--2) THEN
      RULE_ASSUM_TAC(REWRITE_RULE[VAL_WORD_SUB_EQ_0]) THEN
      ARM_STEPS_TAC BIGNUM_MONTIFIER_EXEC [3] THEN
    ...
    ARM_SIM_TAC BIGNUM_MONTIFIER_EXEC (1--5) THEN
  ...
  CONJ_TAC THENL
   [GHOST_INTRO_TAC `n:num` `bignum_from_memory (t,k)` THEN
    ...
    ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
     [ARM_SIM_TAC BIGNUM_MONTIFIER_EXEC (1--6) THEN
      ...
      REWRITE_TAC[BIGNUM_FROM_MEMORY_STEP] THEN
      REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN ENSURES_INIT_TAC "s0" THEN
      ARM_STEPS_TAC BIGNUM_MONTIFIER_EXEC (1--7) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[GSYM WORD_ADD] THEN
   ...
   [ENSURES_WHILE_PUP_TAC `62` `pc + 0xa4` `pc + 0xcc`
    ...
    REPEAT CONJ_TAC THENL
     [ARITH_TAC;
      ...
      ARM_STEPS_TAC BIGNUM_MONTIFIER_EXEC (1--5) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
      ...
      GLOBALIZE_PRECONDITION_TAC THEN
      REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN ENSURES_INIT_TAC "s0" THEN
      ARM_STEPS_TAC BIGNUM_MONTIFIER_EXEC (1--10) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
      SUBST1_TAC(SYM(WORD_REDUCE_CONV `word_not(word 0):int64`)) THEN
      ONCE_REWRITE_TAC[GSYM COND_SWAP] THEN REWRITE_TAC[GSYM WORD_MASK] THEN
   ...
   [ENSURES_WHILE_UP_TAC `k:num` `pc + 0xe0` `pc + 0xf8`
    ...
    ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
     [ARM_SIM_TAC BIGNUM_MONTIFIER_EXEC (1--2) THEN
      ...
      REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
      ENSURES_INIT_TAC "s0" THEN
      ARM_ACCSTEPS_TAC BIGNUM_MONTIFIER_EXEC [2;3] (1--6) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
      ...
      ARM_STEPS_TAC BIGNUM_MONTIFIER_EXEC (1--2) THEN
      RULE_ASSUM_TAC(REWRITE_RULE[VAL_WORD_SUB_EQ_0]) THEN
      ARM_ACCSTEPS_TAC BIGNUM_MONTIFIER_EXEC [3] [3] THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
      ACCUMULATOR_POP_ASSUM_LIST(MP_TAC o end_itlist CONJ) THEN
   ...
   [GHOST_INTRO_TAC `rhi:num` `\s. val(read X9 s)` THEN
    ...
    ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
     [ARM_SIM_TAC BIGNUM_MONTIFIER_EXEC (1--4) THEN
      ...
      REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
      ENSURES_INIT_TAC "s0" THEN
      ARM_ACCSTEPS_TAC BIGNUM_MONTIFIER_EXEC [4] (1--6) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
      REWRITE_TAC[LOWDIGITS_CLAUSES; GSYM WORD_ADD] THEN
   ...
   [GHOST_INTRO_TAC `p62:num` `bignum_from_memory (z,k)` THEN
    ...
    ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
     [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES; SUB_0] THEN
      ...
      REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
      ENSURES_INIT_TAC "s0" THEN
      ARM_ACCSTEPS_TAC BIGNUM_MONTIFIER_EXEC [4] (1--7) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
    ...
    ENSURES_WHILE_UP_TAC `k:num` `pc + 0x16c` `pc + 0x184`
     ...
     ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
      [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES; SUB_0] THEN
       ...
       ARM_STEPS_TAC BIGNUM_MONTIFIER_EXEC (1--2) THEN
       RULE_ASSUM_TAC(REWRITE_RULE[VAL_WORD_SUB_EQ_0]) THEN
       ARM_STEPS_TAC BIGNUM_MONTIFIER_EXEC (3--5) THEN
       ENSURES_FINAL_STATE_TAC THEN
  ...
  ENSURES_WHILE_PUP_TAC `2 * k` `pc + 0x1f0` `pc + 0x25c`
   `\i s. (read X0 s = word k /\
           read X1 s = z /\
           read X2 s = mm /\
           read X3 s = t /\
           read X4 s = word(2 * k - i) /\
           bignum_from_memory (mm,k) s = m /\
   ...
   [UNDISCH_TAC `~(k = 0)` THEN ARITH_TAC;
    ...
    ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
     [ARM_SIM_TAC BIGNUM_MONTIFIER_EXEC (1--3) THEN
      ...
      REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
      ENSURES_INIT_TAC "s0" THEN
      ARM_ACCSTEPS_TAC BIGNUM_MONTIFIER_EXEC [2;3;5;6] (1--9) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
      REPEAT CONJ_TAC THENL
       [CONV_TAC WORD_RULE;
    ...
    ARM_STEPS_TAC BIGNUM_MONTIFIER_EXEC (1--2) THEN
    RULE_ASSUM_TAC(REWRITE_RULE[VAL_WORD_SUB_EQ_0]) THEN
    ARM_ACCSTEPS_TAC BIGNUM_MONTIFIER_EXEC [3] (3--4) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
    ...
    X_GEN_TAC `i:num` THEN STRIP_TAC THEN VAL_INT64_TAC `i:num` THEN
    ASM_REWRITE_TAC[] THEN ARM_SIM_TAC BIGNUM_MONTIFIER_EXEC [1];
  ...
  ENSURES_SEQUENCE_TAC `pc + 0x294`
   `\s. read X0 s = word k /\
        ...
        (ODD m ==> (m * val(read X4 s) + 1 == 0) (mod (2 EXP 64)))` THEN
  CONJ_TAC THENL
   [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN ENSURES_INIT_TAC "s0" THEN
    ARM_STEPS_TAC BIGNUM_MONTIFIER_EXEC [1] THEN
    SUBGOAL_THEN `bignum_from_memory(mm,k) s1 = highdigits m 0` MP_TAC THENL
     [ASM_REWRITE_TAC[HIGHDIGITS_0; BIGNUM_FROM_MEMORY_BYTES];
      ...
      REWRITE_TAC[DIMINDEX_64] THEN STRIP_TAC] THEN
    ARM_STEPS_TAC BIGNUM_MONTIFIER_EXEC (2--5) THEN
    SUBGOAL_THEN `ODD m ==> (m * val (read X4 s5) + 1 == 0) (mod 16)`
    MP_TAC THENL [ASM_SIMP_TAC[WORD_NEGMODINV_SEED_LEMMA_16]; ALL_TAC] THEN
    REABBREV_TAC `x0 = read X4 s5` THEN DISCH_TAC THEN
    ARM_STEPS_TAC BIGNUM_MONTIFIER_EXEC (6--14) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
   ...
   [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN ENSURES_INIT_TAC "s0" THEN
    ...
    MP_TAC THENL
     [ASM_REWRITE_TAC[HIGHDIGITS_0; BIGNUM_FROM_MEMORY_BYTES];
      ...
      ASM_REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES; ADD_CLAUSES] THEN
      STRIP_TAC] THEN
    ARM_ACCSTEPS_TAC BIGNUM_MONTIFIER_EXEC [3; 7] (1--7) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
    MATCH_MP_TAC(TAUT `p /\ (p ==> q) ==> p /\ q`) THEN CONJ_TAC THENL
   ...
   [ASM_CASES_TAC `k = 1` THENL
    ...
    REPEAT CONJ_TAC THENL
     [ASM_REWRITE_TAC[ARITH_RULE `1 < k <=> ~(k = 0 \/ k = 1)`];
      ...
      ARM_ACCSTEPS_TAC BIGNUM_MONTIFIER_EXEC [3;4;6;7] (1--10) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
      CONJ_TAC THENL [CONV_TAC WORD_RULE; ALL_TAC] THEN
 ...
 (*** The final digit write ****)
   ...
   [GHOST_INTRO_TAC `cin:bool` `read CF` THEN
    ...
    ASSUME_TAC THENL
     [ASM_REWRITE_TAC[WORD_SUB; ARITH_RULE `1 <= k <=> ~(k = 0)`];
      ALL_TAC] THEN
    ARM_ACCSTEPS_TAC BIGNUM_MONTIFIER_EXEC [1] (1--4) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  ...
  ENSURES_WHILE_PUP_TAC `k:num` `pc + 0x31c` `pc + 0x380`
   `\i s. (read X0 s = word k /\
           read X1 s = z /\
           read X2 s = mm /\
           read X4 s = word w /\
  ...
  ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
   [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
    ...
    ENSURES_WHILE_UP_TAC `k:num` `pc + 0x388` `pc + 0x398`
      `\i s. read X0 s = word k /\
             read X1 s = z /\
             read X2 s = mm /\
             ...
             bignum_from_memory (word_add mm (word(8 * i)),k - i) s =
             highdigits m i /\
             (read CF s <=> lowdigits m i <= lowdigits z3 i)` THEN
     ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
      [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES; SUB_0] THEN
       ENSURES_INIT_TAC "s0" THEN
       ARM_STEPS_TAC BIGNUM_MONTIFIER_EXEC (1--2) THEN
    ...
    ENSURES_WHILE_UP_TAC `k:num` `pc + 0x3a8` `pc + 0x3c0`
     `\i s. read X0 s = word k /\
            read X1 s = z /\
            read X2 s = mm /\
            read X6 s = word_neg(word(bitval(m <= z3))) /\
            read X5 s = word i /\
            bignum_from_memory (word_add z (word(8 * i)),k - i) s =
            highdigits z3 i /\
            bignum_from_memory (word_add mm (word(8 * i)),k - i) s =
    ...
    ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
     [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
      ENSURES_INIT_TAC "s0" THEN ARM_STEPS_TAC BIGNUM_MONTIFIER_EXEC (1--2) THEN
      RULE_ASSUM_TAC(REWRITE_RULE[WORD_SUB_REFL; VAL_WORD_0]) THEN
      ARM_STEPS_TAC BIGNUM_MONTIFIER_EXEC (3--4) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
      REWRITE_TAC[WORD_SUB_LZERO; SUB_0; GSYM BIGNUM_FROM_MEMORY_BYTES] THEN
      REWRITE_TAC[WORD_ADD_0; MULT_CLAUSES; BITVAL_CLAUSES] THEN
      REWRITE_TAC[BIGNUM_FROM_MEMORY_TRIVIAL; LOWDIGITS_0] THEN
   ...
   [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN ENSURES_INIT_TAC "s0" THEN
    ...
    MP_TAC THENL
     [ASM_REWRITE_TAC[HIGHDIGITS_0; BIGNUM_FROM_MEMORY_BYTES];
      ...
      ASM_REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES; ADD_CLAUSES] THEN
      STRIP_TAC] THEN
    ARM_ACCSTEPS_TAC BIGNUM_MONTIFIER_EXEC [4; 8] (1--8) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
    MATCH_MP_TAC(TAUT `p /\ (p ==> q) ==> p /\ q`) THEN CONJ_TAC THENL
     [UNDISCH_THEN `(w * z1) MOD 2 EXP 64 = q0` (SUBST1_TAC o SYM) THEN
      ONCE_REWRITE_TAC[GSYM WORD_MOD_SIZE] THEN
      ...
      DISCH_THEN SUBST_ALL_TAC] THEN
    ASM_REWRITE_TAC[WORD_SUB; ARITH_RULE `1 <= k <=> ~(k = 0)`] THEN
    EXISTS_TAC `val(sum_s8:int64)` THEN REWRITE_TAC[VAL_BOUND_64] THEN
    REWRITE_TAC[GSYM REAL_OF_NUM_CLAUSES] THEN
    ACCUMULATOR_POP_ASSUM_LIST(MP_TAC o end_itlist CONJ o DECARRY_RULE) THEN
    DISCH_THEN(fun th -> REWRITE_TAC[th]) THEN
    ASM_SIMP_TAC[VAL_WORD_EQ; DIMINDEX_64; BIGDIGIT_BOUND] THEN
  ...
  CONJ_TAC THENL
   [ASM_CASES_TAC `k = 1` THENL
    ...
    ENSURES_WHILE_AUP_TAC `1` `k:num` `pc + 0x340` `pc + 0x368`
     `\j s. read X0 s = word k /\
            read X1 s = z /\
            read X2 s = mm /\
            bignum_from_memory (mm,k) s = m /\
            ...
            lowdigits z1 j + q0 * lowdigits m j` THEN
    REPEAT CONJ_TAC THENL
     [ASM_REWRITE_TAC[ARITH_RULE `1 < k <=> ~(k = 0 \/ k = 1)`];
