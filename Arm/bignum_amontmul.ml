...
(**** print_literal_from_elf "Arm/bignum_amontmul.o";;
 ****)

let bignum_amontmul_mc =
  define_assert_from_elf "bignum_amontmul_mc" "Arm/bignum_amontmul.o"
[
...
let BIGNUM_AMONTMUL_EXEC = ARM_MK_EXEC_RULE bignum_amontmul_mc;;
...
let BIGNUM_AMONTMUL_CORRECT = time prove
 (`!k z x y m a b n pc.
      ALL (nonoverlapping (z,8 * val k))
          [(word pc,0x130); (x,8 * val k); (y,8 * val k); (m,8 * val k)]
      ==> ensures arm
           (\s. aligned_bytes_loaded s (word pc) bignum_amontmul_mc /\
                read PC s = word pc /\
  ...
  ASM_CASES_TAC `k = 0` THENL
   [UNDISCH_THEN `k = 0` SUBST_ALL_TAC THEN
    ...
    ENSURES_INIT_TAC "s0" THEN ARM_STEPS_TAC BIGNUM_AMONTMUL_EXEC [1] THEN
  ...
  CONJ_TAC THENL
   [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN ENSURES_INIT_TAC "s0" THEN
    ...
    ARM_STEPS_TAC BIGNUM_AMONTMUL_EXEC (1--5) THEN
    ...
    ARM_STEPS_TAC BIGNUM_AMONTMUL_EXEC (6--14) THEN
  ...
  GLOBALIZE_PRECONDITION_TAC THEN VAL_INT64_TAC `w:num` THEN
   ...
   [ENSURES_WHILE_UP_TAC `k:num` `pc + 0x3c` `pc + 0x44`
    ...
    ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
     [REWRITE_TAC[BIGNUM_FROM_MEMORY_TRIVIAL] THEN
      ...
      REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
      ENSURES_INIT_TAC "s0" THEN ARM_STEPS_TAC BIGNUM_AMONTMUL_EXEC (1--2) THEN
      ...
      ENSURES_INIT_TAC "s0" THEN ARM_STEPS_TAC BIGNUM_AMONTMUL_EXEC (1--2) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[];
      ARM_SIM_TAC BIGNUM_AMONTMUL_EXEC (1--4) THEN
    ...
    ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
     [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
      ENSURES_INIT_TAC "s0" THEN ARM_STEPS_TAC BIGNUM_AMONTMUL_EXEC (1--4) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
      ...
      ARM_ACCSTEPS_TAC BIGNUM_AMONTMUL_EXEC [4] (1--6) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[GSYM WORD_ADD] THEN
  ...
  CONJ_TAC THENL
   [ENSURES_WHILE_UP_TAC `k:num` `pc + 0x60` `pc + 0x84`
    ...
    ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
     [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
      ENSURES_INIT_TAC "s0" THEN ARM_STEPS_TAC BIGNUM_AMONTMUL_EXEC (1--3) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
      ...
      REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
      ENSURES_INIT_TAC "s0" THEN
      ARM_ACCSTEPS_TAC BIGNUM_AMONTMUL_EXEC [3;4;6;7] (1--9) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[GSYM WORD_ADD] THEN
      ...
      ARM_ACCSTEPS_TAC BIGNUM_AMONTMUL_EXEC [3] (3--4) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
   ...
   [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN ENSURES_INIT_TAC "s0" THEN
    ...
    MP_TAC THENL
     [ASM_REWRITE_TAC[HIGHDIGITS_0; BIGNUM_FROM_MEMORY_BYTES];
      ...
      ASM_REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES; ADD_CLAUSES] THEN
      STRIP_TAC] THEN
    ARM_ACCSTEPS_TAC BIGNUM_AMONTMUL_EXEC [4;6] (1--8) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
   ...
   [ASM_CASES_TAC `k = 1` THENL
    ...
    REPEAT CONJ_TAC THENL
     [ASM_REWRITE_TAC[ARITH_RULE `1 < k <=> ~(k = 0 \/ k = 1)`];
      ...
      ARM_ACCSTEPS_TAC BIGNUM_AMONTMUL_EXEC [3;4;6;7] (1--10) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  ...
  ASSUME_TAC THENL
   [ASM_REWRITE_TAC[WORD_SUB; ARITH_RULE `1 <= k <=> ~(k = 0)`];
    ALL_TAC] THEN
  ARM_ACCSTEPS_TAC BIGNUM_AMONTMUL_EXEC [1;2] (1--5) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  CONJ_TAC THENL [CONV_TAC WORD_RULE; ALL_TAC] THEN
 ...
 (`!k z x y m a b n pc returnaddress.
      ...
      ==> ensures arm
           ...
           (MAYCHANGE [PC; X5; X6; X7; X8; X9; X10; X11; X12; X13; X14] ,,
            MAYCHANGE [memory :> bytes(z,8 * val k)] ,,
            MAYCHANGE SOME_FLAGS)`,
  ARM_ADD_RETURN_NOSTACK_TAC BIGNUM_AMONTMUL_EXEC BIGNUM_AMONTMUL_CORRECT);;
