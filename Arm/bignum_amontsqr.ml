...
(**** print_literal_from_elf "Arm/bignum_amontsqr.o";;
 ****)

let bignum_amontsqr_mc =
  define_assert_from_elf "bignum_amontsqr_mc" "Arm/bignum_amontsqr.o"
[
  0xb4000960;       (* arm_CBZ X0 (word 300) *)
  0xf940006d;       (* arm_LDR X13 X3 (Immediate_Offset (word 0)) *)
  0xd37ef5a4;       (* arm_LSL X4 X13 (rvalue (word 2)) *)
  0xcb0401a4;       (* arm_SUB X4 X13 X4 *)
  0xd27f0084;       (* arm_EOR X4 X4 (rvalue (word 2)) *)
  0xd2800025;       (* arm_MOV X5 (rvalue (word 1)) *)
  0x9b0415a5;       (* arm_MADD X5 X13 X4 X5 *)
  ...
  0xaa1f03e7;       (* arm_MOV X7 XZR *)
  0xf8677848;       (* arm_LDR X8 X2 (Shiftreg_Offset X7 3) *)
  0xaa1f03e9;       (* arm_MOV X9 XZR *)
  0xab1f03ea;       (* arm_ADDS X10 XZR XZR *)
  0xf869784d;       (* arm_LDR X13 X2 (Shiftreg_Offset X9 3) *)
  0xf869782b;       (* arm_LDR X11 X1 (Shiftreg_Offset X9 3) *)
  0x9b0d7d0c;       (* arm_MUL X12 X8 X13 *)
  0xba0a016b;       (* arm_ADCS X11 X11 X10 *)
  0x9bcd7d0a;       (* arm_UMULH X10 X8 X13 *)
  0x9a1f014a;       (* arm_ADC X10 X10 XZR *)
  0xab0c016b;       (* arm_ADDS X11 X11 X12 *)
  0xf829782b;       (* arm_STR X11 X1 (Shiftreg_Offset X9 3) *)
  0x91000529;       (* arm_ADD X9 X9 (rvalue (word 1)) *)
  0xcb00012d;       (* arm_SUB X13 X9 X0 *)
  0xb5fffecd;       (* arm_CBNZ X13 (word 2097112) *)
  0xba0a00a5;       (* arm_ADCS X5 X5 X10 *)
  0x9a1f03e6;       (* arm_ADC X6 XZR XZR *)
  0xf940002b;       (* arm_LDR X11 X1 (Immediate_Offset (word 0)) *)
  0x9b047d68;       (* arm_MUL X8 X11 X4 *)
  0xf940006d;       (* arm_LDR X13 X3 (Immediate_Offset (word 0)) *)
  0x9b0d7d0c;       (* arm_MUL X12 X8 X13 *)
  0x9bcd7d0a;       (* arm_UMULH X10 X8 X13 *)
  0xab0c016b;       (* arm_ADDS X11 X11 X12 *)
  ...
  0xf869786d;       (* arm_LDR X13 X3 (Shiftreg_Offset X9 3) *)
  0xf869782b;       (* arm_LDR X11 X1 (Shiftreg_Offset X9 3) *)
  0x9b0d7d0c;       (* arm_MUL X12 X8 X13 *)
  0xba0a016b;       (* arm_ADCS X11 X11 X10 *)
  0x9bcd7d0a;       (* arm_UMULH X10 X8 X13 *)
  0x9a1f014a;       (* arm_ADC X10 X10 XZR *)
  0xab0c016b;       (* arm_ADDS X11 X11 X12 *)
  0xd100052c;       (* arm_SUB X12 X9 (rvalue (word 1)) *)
  0xf82c782b;       (* arm_STR X11 X1 (Shiftreg_Offset X12 3) *)
  0x91000529;       (* arm_ADD X9 X9 (rvalue (word 1)) *)
  0xcb00012d;       (* arm_SUB X13 X9 X0 *)
  0xb5fffead;       (* arm_CBNZ X13 (word 2097108) *)
  0xba0a00aa;       (* arm_ADCS X10 X5 X10 *)
  0x9a1f00c5;       (* arm_ADC X5 X6 XZR *)
  0xd100052c;       (* arm_SUB X12 X9 (rvalue (word 1)) *)
  0xf82c782a;       (* arm_STR X10 X1 (Shiftreg_Offset X12 3) *)
  0x910004e7;       (* arm_ADD X7 X7 (rvalue (word 1)) *)
  0xeb0000ff;       (* arm_CMP X7 X0 *)
  0x54fffaa3;       (* arm_BCC (word 2096980) *)
  0xcb0503e5;       (* arm_NEG X5 X5 *)
  0xeb1f03e9;       (* arm_NEGS X9 XZR *)
  0xf869782d;       (* arm_LDR X13 X1 (Shiftreg_Offset X9 3) *)
  0xf869786b;       (* arm_LDR X11 X3 (Shiftreg_Offset X9 3) *)
  0x8a05016b;       (* arm_AND X11 X11 X5 *)
  0xfa0b01ad;       (* arm_SBCS X13 X13 X11 *)
  0xf829782d;       (* arm_STR X13 X1 (Shiftreg_Offset X9 3) *)
  0x91000529;       (* arm_ADD X9 X9 (rvalue (word 1)) *)
  0xcb00012d;       (* arm_SUB X13 X9 X0 *)
...
let BIGNUM_AMONTSQR_EXEC = ARM_MK_EXEC_RULE bignum_amontsqr_mc;;
...
let BIGNUM_AMONTSQR_CORRECT = time prove
 (`!k z x m a n pc.
      ALL (nonoverlapping (z,8 * val k))
          [(word pc,0x130); (x,8 * val k); (m,8 * val k)]
      ==> ensures arm
           (\s. aligned_bytes_loaded s (word pc) bignum_amontsqr_mc /\
                read PC s = word pc /\
                C_ARGUMENTS [k; z; x; m] s /\
                bignum_from_memory (x,val k) s = a /\
                bignum_from_memory (m,val k) s = n)
           (\s. read PC s = word(pc + 0x12c) /\
                (ODD n
  ...
  ASM_CASES_TAC `k = 0` THENL
   [UNDISCH_THEN `k = 0` SUBST_ALL_TAC THEN
    ...
    ENSURES_INIT_TAC "s0" THEN ARM_STEPS_TAC BIGNUM_AMONTSQR_EXEC [1] THEN
  ...
  CONJ_TAC THENL
   [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN ENSURES_INIT_TAC "s0" THEN
    ...
    ARM_STEPS_TAC BIGNUM_AMONTSQR_EXEC (1--5) THEN
    ...
    ARM_STEPS_TAC BIGNUM_AMONTSQR_EXEC (6--14) THEN
   ...
   [ENSURES_WHILE_UP_TAC `k:num` `pc + 0x3c` `pc + 0x44`
    ...
    ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
     [REWRITE_TAC[BIGNUM_FROM_MEMORY_TRIVIAL] THEN
      ...
      REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
      ENSURES_INIT_TAC "s0" THEN ARM_STEPS_TAC BIGNUM_AMONTSQR_EXEC (1--2) THEN
      ...
      ENSURES_INIT_TAC "s0" THEN ARM_STEPS_TAC BIGNUM_AMONTSQR_EXEC (1--2) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[];
      ARM_SIM_TAC BIGNUM_AMONTSQR_EXEC (1--4) THEN
    ...
    GHOST_INTRO_TAC `cout:num` `\s. val(read X5 s)` THEN
    ...
    ENSURES_WHILE_UP_TAC `k:num` `pc + 0x10c` `pc + 0x124`
     `\i s. read X0 s = word k /\
            read X1 s = z /\
            read X3 s = m /\
            read X5 s = word_neg(word(bitval c)) /\
            read X9 s = word i /\
    ...
    ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
     [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
      ENSURES_INIT_TAC "s0" THEN ARM_STEPS_TAC BIGNUM_AMONTSQR_EXEC (1--4) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
      ...
      ARM_ACCSTEPS_TAC BIGNUM_AMONTSQR_EXEC [4] (1--6) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[GSYM WORD_ADD] THEN
  ...
  CONJ_TAC THENL
   [ENSURES_WHILE_UP_TAC `k:num` `pc + 0x60` `pc + 0x84`
     `\j s. read X0 s = word k /\
            ...
            lowdigits z1 j + bigdigit a i * lowdigits a j` THEN
    ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
     [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
      ENSURES_INIT_TAC "s0" THEN ARM_STEPS_TAC BIGNUM_AMONTSQR_EXEC (1--3) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
      ...
      REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
      ENSURES_INIT_TAC "s0" THEN
      ARM_ACCSTEPS_TAC BIGNUM_AMONTSQR_EXEC [3;4;6;7] (1--9) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[GSYM WORD_ADD] THEN
      ...
      GHOST_INTRO_TAC `cin:bool` `read CF` THEN
      GHOST_INTRO_TAC `hout:int64` `read X10` THEN
      REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
      ENSURES_INIT_TAC "s0" THEN ARM_STEPS_TAC BIGNUM_AMONTSQR_EXEC (1--2) THEN
      RULE_ASSUM_TAC(REWRITE_RULE[VAL_WORD_SUB_EQ_0]) THEN
      ARM_ACCSTEPS_TAC BIGNUM_AMONTSQR_EXEC [3] (3--4) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  ...
  GHOST_INTRO_TAC `g6:int64` `read X5` THEN
  GHOST_INTRO_TAC `g7:int64` `read X6` THEN
  GLOBALIZE_PRECONDITION_TAC THEN ASM_REWRITE_TAC[] THEN
   ...
   [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN ENSURES_INIT_TAC "s0" THEN
    ...
    MP_TAC THENL
     [ASM_REWRITE_TAC[HIGHDIGITS_0; BIGNUM_FROM_MEMORY_BYTES];
      ...
      ASM_REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES; ADD_CLAUSES] THEN
      STRIP_TAC] THEN
    ARM_ACCSTEPS_TAC BIGNUM_AMONTSQR_EXEC [4;6] (1--8) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
   ...
   [ASM_CASES_TAC `k = 1` THENL
    ...
    REPEAT CONJ_TAC THENL
     [ASM_REWRITE_TAC[ARITH_RULE `1 < k <=> ~(k = 0 \/ k = 1)`];
      ...
      ARM_ACCSTEPS_TAC BIGNUM_AMONTSQR_EXEC [3;4;6;7] (1--10) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  ...
  ASSUME_TAC THENL
   [ASM_REWRITE_TAC[WORD_SUB; ARITH_RULE `1 <= k <=> ~(k = 0)`];
    ALL_TAC] THEN
  ARM_ACCSTEPS_TAC BIGNUM_AMONTSQR_EXEC [1;2] (1--5) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  CONJ_TAC THENL [CONV_TAC WORD_RULE; ALL_TAC] THEN
