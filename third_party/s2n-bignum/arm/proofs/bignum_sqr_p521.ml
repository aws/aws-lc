...
(* bignum_sqr_p521_mc without callee-save register spills + ret. *)
...
let BIGNUM_SQR_P521_CORE_CORRECT = time prove
 (`!z x n pc.
        ...
        ==> ensures arm
             (\s. aligned_bytes_loaded s (word pc) bignum_sqr_p521_core_mc /\
                  read PC s = word(pc) /\
                  C_ARGUMENTS [z; x] s /\
                  bignum_from_memory (x,9) s = n)
             (\s. read PC s = word (pc + LENGTH bignum_sqr_p521_core_mc) /\
                  (n < p_521
  ...
  ASM_CASES_TAC `n < p_521` THENL
   [ASM_REWRITE_TAC[]; ARM_SIM_TAC BIGNUM_SQR_P521_CORE_EXEC (1--412)] THEN
  ...
  ARM_ACCSTEPS_TAC BIGNUM_SQR_P521_CORE_EXEC
  ...
  ARM_STEPS_TAC BIGNUM_SQR_P521_CORE_EXEC (145--160) THEN
  ABBREV_TAC
   `htop:int64 =
  ...
  ARM_STEPS_TAC BIGNUM_SQR_P521_CORE_EXEC (392--394) THEN
  ...
  ARM_ACCSTEPS_TAC BIGNUM_SQR_P521_CORE_EXEC (395--397) (395--397) THEN
  SUBGOAL_THEN
   `carry_s397 <=>
    2 EXP 192 <=
  ...
  ARM_ACCSTEPS_TAC BIGNUM_SQR_P521_CORE_EXEC (398--406) (398--412) THEN
...
let BIGNUM_SQR_P521_CORRECT = time prove
 (`!z x n pc.
  ...
  ARM_SUB_LIST_OF_MC_TAC BIGNUM_SQR_P521_CORE_CORRECT
    ...
    [fst BIGNUM_SQR_P521_CORE_EXEC;fst BIGNUM_SQR_P521_EXEC]);;
...
let BIGNUM_SQR_P521_SUBROUTINE_CORRECT = time prove
 (`!z x n pc stackpointer returnaddress.
  ...
  let th = CONV_RULE (ONCE_DEPTH_CONV NUM_ADD_CONV)
    (REWRITE_RULE [fst BIGNUM_SQR_P521_CORE_EXEC;fst BIGNUM_SQR_P521_EXEC]
