...
let edwards25519_pdouble_tmc = define_trimmed "edwards25519_pdouble_tmc" edwards25519_pdouble_mc;;

let EDWARDS25519_PDOUBLE_EXEC = X86_MK_CORE_EXEC_RULE edwards25519_pdouble_tmc;;
...
let LOCAL_MUL_P25519_TAC =
  X86_MACRO_SIM_ABBREV_TAC (X86_TRIM_EXEC_RULE edwards25519_pdouble_tmc) 91 lvs
   `!(t:x86state) pcin pcout q3 n3 q1 n1 q2 n2.
      ...
      ==>
      nonoverlapping (word pc,0xd17) (word_add (read q3 t) (word n3),8 * 4)
      ==> ensures x86
           (\s. bytes_loaded s (word pc) (BUTLAST edwards25519_pdouble_tmc) /\
                read RIP s = pcin /\
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  X86_ACCSTEPS_TAC EDWARDS25519_PDOUBLE_EXEC (2--56) (2--56) THEN
  ...
  X86_ACCSTEPS_TAC EDWARDS25519_PDOUBLE_EXEC (57--71) (57--71) THEN
  ABBREV_TAC
  ...
  X86_STEPS_TAC EDWARDS25519_PDOUBLE_EXEC (72--75) THEN
  ...
  X86_ACCSTEPS_TAC EDWARDS25519_PDOUBLE_EXEC
...
let LOCAL_SQR_4_TAC =
  X86_MACRO_SIM_ABBREV_TAC (X86_TRIM_EXEC_RULE edwards25519_pdouble_tmc) 68 lvs
   `!(t:x86state) pcin pcout p3 n3 p1 n1.
      !n.
      read(memory :> bytes(word_add (read p1 t) (word n1),8 * 4)) t = n
      ==>
      nonoverlapping (word pc,0xd17) (word_add (read p3 t) (word n3),8 * 4)
      ==> ensures x86
           (\s. bytes_loaded s (word pc) (BUTLAST edwards25519_pdouble_tmc) /\
                read RIP s = pcin /\
                read RSP s = read RSP t /\
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  X86_ACCSTEPS_TAC EDWARDS25519_PDOUBLE_EXEC (1--41) (1--41) THEN
  MAP_EVERY ABBREV_TAC
  ...
  X86_ACCSTEPS_TAC EDWARDS25519_PDOUBLE_EXEC (42--56) (42--56) THEN
  ABBREV_TAC
  ...
  X86_STEPS_TAC EDWARDS25519_PDOUBLE_EXEC (57--60) THEN
  ABBREV_TAC `hw:int64 = word_subword
  ...
  X86_ACCSTEPS_TAC EDWARDS25519_PDOUBLE_EXEC (61--64) (61--68) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
...
let LOCAL_ADD_4_TAC =
  X86_MACRO_SIM_ABBREV_TAC (X86_TRIM_EXEC_RULE edwards25519_pdouble_tmc) 12 lvs
   `!(t:x86state) pcin pcout q3 n3 q1 n1 q2 n2.
      !m. read(memory :> bytes(word_add (read q1 t) (word n1),8 * 4)) t = m
      ==>
      !n. read(memory :> bytes(word_add (read q2 t) (word n2),8 * 4)) t = n
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  FIRST_ASSUM(BIGNUM_LDIGITIZE_TAC "n_" o lhand o concl) THEN
  FIRST_ASSUM(BIGNUM_LDIGITIZE_TAC "m_" o lhand o concl) THEN
  X86_ACCSTEPS_TAC EDWARDS25519_PDOUBLE_EXEC [2;5;8;11] (1--12) THEN
...
let LOCAL_ADD_TWICE4_TAC =
  X86_MACRO_SIM_ABBREV_TAC (X86_TRIM_EXEC_RULE edwards25519_pdouble_tmc) 19 lvs
   `!(t:x86state) pcin pcout q3 n3 q1 n1 q2 n2.
      !m. read(memory :> bytes(word_add (read q1 t) (word n1),8 * 4)) t = m
      ==>
      !n. read(memory :> bytes(word_add (read q2 t) (word n2),8 * 4)) t = n
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  FIRST_ASSUM(BIGNUM_LDIGITIZE_TAC "n_" o lhand o concl) THEN
  FIRST_ASSUM(BIGNUM_LDIGITIZE_TAC "m_" o lhand o concl) THEN
  X86_ACCSTEPS_TAC EDWARDS25519_PDOUBLE_EXEC [3;5;7;9] (1--11) THEN
  SUBGOAL_THEN `carry_s9 <=> 2 EXP 256 <= m + n` SUBST_ALL_TAC THENL
   [MATCH_MP_TAC FLAG_FROM_CARRY_LE THEN EXISTS_TAC `256` THEN
    ...
    ALL_TAC] THEN
  X86_ACCSTEPS_TAC EDWARDS25519_PDOUBLE_EXEC (12--15) (12--19) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN DISCH_TAC THEN
...
let LOCAL_SUB_TWICE4_TAC =
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  FIRST_ASSUM(BIGNUM_LDIGITIZE_TAC "n_" o lhand o concl) THEN
  FIRST_ASSUM(BIGNUM_LDIGITIZE_TAC "m_" o lhand o concl) THEN
  X86_ACCSTEPS_TAC EDWARDS25519_PDOUBLE_EXEC (1--10) (1--10) THEN
  SUBGOAL_THEN `carry_s10 <=> m < n` SUBST_ALL_TAC THENL
   [MATCH_MP_TAC FLAG_FROM_CARRY_LT THEN EXISTS_TAC `256` THEN
    ...
    DISCH_THEN(fun th -> REWRITE_TAC[th]) THEN BOUNDER_TAC[];
    ALL_TAC] THEN
  X86_ACCSTEPS_TAC EDWARDS25519_PDOUBLE_EXEC (12--15) (11--19) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  MATCH_MP_TAC(MESON[INT_OF_NUM_LT]
   `!x':int. (x' == &m - &n) (mod p) /\
             (m < p2 /\ n < p2 ==> x' < &p2) /\
...
let EDWARDS25519_PDOUBLE_NOIBT_SUBROUTINE_CORRECT = time prove
 (`!p3 p1 T1 pc stackpointer returnaddress.
    ALL (nonoverlapping (word_sub stackpointer (word 200),200))
        [(word pc,LENGTH edwards25519_pdouble_tmc); (p1,96)] /\
    nonoverlapping (p3,96) (word pc,LENGTH edwards25519_pdouble_tmc) /\
    nonoverlapping (p3,96) (word_sub stackpointer (word 200),208)
    ==> ensures x86
         (\s. bytes_loaded s (word pc) edwards25519_pdouble_tmc /\
              read RIP s = word pc /\
              read RSP s = stackpointer /\
  ...
  X86_PROMOTE_RETURN_STACK_TAC
    edwards25519_pdouble_tmc EDWARDS25519_PDOUBLE_CORRECT
 ...
 (`!p3 p1 T1 pc stackpointer returnaddress.
    ...
    ==> ensures x86
         ...
         (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bytes(p3,96);
                      memory :> bytes(word_sub stackpointer (word 200),200)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE EDWARDS25519_PDOUBLE_NOIBT_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let edwards25519_pdouble_windows_mc = define_from_elf
  "edwards25519_pdouble_windows_mc"
  "x86/curve25519/edwards25519_pdouble.obj";;

let edwards25519_pdouble_windows_tmc = define_trimmed "edwards25519_pdouble_windows_tmc" edwards25519_pdouble_windows_mc;;

let EDWARDS25519_PDOUBLE_NOIBT_WINDOWS_SUBROUTINE_CORRECT = prove
 (`!p3 p1 T1 pc stackpointer returnaddress.
    ALL (nonoverlapping (word_sub stackpointer (word 216),216))
        [(word pc,LENGTH edwards25519_pdouble_windows_tmc); (p1,96)] /\
    nonoverlapping (p3,96) (word pc,LENGTH edwards25519_pdouble_windows_tmc) /\
    nonoverlapping (p3,96) (word_sub stackpointer (word 216),224)
    ==> ensures x86
         (\s. bytes_loaded s (word pc) edwards25519_pdouble_windows_tmc /\
              read RIP s = word pc /\
              read RSP s = stackpointer /\
              read (memory :> bytes64 stackpointer) s = returnaddress /\
  ...
  WINDOWS_X86_WRAP_STACK_TAC
   edwards25519_pdouble_windows_tmc edwards25519_pdouble_tmc
   EDWARDS25519_PDOUBLE_CORRECT
    `[RBX; R12; R13; R14; R15]` 200);;

let EDWARDS25519_PDOUBLE_WINDOWS_SUBROUTINE_CORRECT = prove
 (`!p3 p1 T1 pc stackpointer returnaddress.
    ALL (nonoverlapping (word_sub stackpointer (word 216),216))
        [(word pc,LENGTH edwards25519_pdouble_windows_mc); (p1,96)] /\
    nonoverlapping (p3,96) (word pc,LENGTH edwards25519_pdouble_windows_mc) /\
    nonoverlapping (p3,96) (word_sub stackpointer (word 216),224)
    ==> ensures x86
         (\s. bytes_loaded s (word pc) edwards25519_pdouble_windows_mc /\
              read RIP s = word pc /\
              read RSP s = stackpointer /\
              read (memory :> bytes64 stackpointer) s = returnaddress /\
              WINDOWS_C_ARGUMENTS [p3; p1] s /\
         ...
         (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
          MAYCHANGE [memory :> bytes(p3,96);
                     memory :> bytes(word_sub stackpointer (word 216),216)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE EDWARDS25519_PDOUBLE_NOIBT_WINDOWS_SUBROUTINE_CORRECT));;
