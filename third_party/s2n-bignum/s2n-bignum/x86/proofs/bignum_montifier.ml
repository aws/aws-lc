...
[
  ...
  0x48; 0x13; 0x0c; 0xee;  (* ADC (% rcx) (Memop Quadword (%%% (rsi,3,rbp))) *)
  0x4d; 0x19; 0xd2;        (* SBB (% r10) (% r10) *)
  0x49; 0x8b; 0x04; 0xec;  (* MOV (% rax) (Memop Quadword (%%% (r12,3,rbp))) *)
  0x49; 0xf7; 0xe1;        (* MUL2 (% rdx,% rax) (% r9) *)
  0x4c; 0x29; 0xd2;        (* SUB (% rdx) (% r10) *)
  0x48; 0x01; 0xc8;        (* ADD (% rax) (% rcx) *)
  0x48; 0x89; 0x44; 0xee; 0xf8;
                           (* MOV (Memop Quadword (%%%% (rsi,3,rbp,-- &8))) (% rax) *)
  0x48; 0x89; 0xd1;        (* MOV (% rcx) (% rdx) *)
  0x48; 0xff; 0xc5;        (* INC (% rbp) *)
  0x49; 0xff; 0xc8;        (* DEC (% r8) *)
  0x75; 0xdc;              (* JNE (Imm8 (word 220)) *)
  0x48; 0x83; 0xd1; 0x00;  (* ADC (% rcx) (Imm8 (word 0)) *)
  0x48; 0x89; 0x4c; 0xfe; 0xf8;
                           (* MOV (Memop Quadword (%%%% (rsi,3,rdi,-- &8))) (% rcx) *)
  0x49; 0xff; 0xcb;        (* DEC (% r11) *)
  0x75; 0xaa;              (* JNE (Imm8 (word 170)) *)
  0x48; 0x31; 0xed;        (* XOR (% rbp) (% rbp) *)
  0x49; 0x89; 0xf8;        (* MOV (% r8) (% rdi) *)
  0x48; 0x8b; 0x04; 0xee;  (* MOV (% rax) (Memop Quadword (%%% (rsi,3,rbp))) *)
  0x49; 0x1b; 0x04; 0xec;  (* SBB (% rax) (Memop Quadword (%%% (r12,3,rbp))) *)
  0x48; 0xff; 0xc5;        (* INC (% rbp) *)
  0x49; 0xff; 0xc8;        (* DEC (% r8) *)
  ...
  0x48; 0x19; 0x04; 0xee;  (* SBB (Memop Quadword (%%% (rsi,3,rbp))) (% rax) *)
  0x4d; 0x19; 0xd2;        (* SBB (% r10) (% r10) *)
  0x48; 0xff; 0xc5;        (* INC (% rbp) *)
  0x48; 0x39; 0xfd;        (* CMP (% rbp) (% rdi) *)
...
let bignum_montifier_tmc = define_trimmed "bignum_montifier_tmc" bignum_montifier_mc;;

let BIGNUM_MONTIFIER_EXEC = X86_MK_CORE_EXEC_RULE bignum_montifier_tmc;;
 ...
 (`!k z m t n pc.
  ...
  ASM_CASES_TAC `k = 0` THENL
   [UNDISCH_THEN `k = 0` SUBST_ALL_TAC THEN
    ...
    ENSURES_INIT_TAC "s0" THEN X86_STEPS_TAC BIGNUM_MONTIFIER_EXEC (1--4) THEN
  ...
  ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
   [X86_SIM_TAC BIGNUM_MONTIFIER_EXEC (1--5) THEN
    ...
    X86_STEPS_TAC BIGNUM_MONTIFIER_EXEC (1--3) THEN
    ...
    X_GEN_TAC `i:num` THEN STRIP_TAC THEN VAL_INT64_TAC `i:num` THEN
    X86_SIM_TAC BIGNUM_MONTIFIER_EXEC (1--2);
   ...
   [ASM_CASES_TAC `k = 1` THENL
    ...
    ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
     [MP_TAC(ISPECL [`word k:int64`; `word 1:int64`] VAL_WORD_SUB_CASES) THEN
      ...
      X86_STEPS_TAC BIGNUM_MONTIFIER_EXEC (1--5) THEN
      ...
      X_GEN_TAC `i:num` THEN STRIP_TAC THEN VAL_INT64_TAC `i:num` THEN
      X86_SIM_TAC BIGNUM_MONTIFIER_EXEC [1];
      X86_SIM_TAC BIGNUM_MONTIFIER_EXEC [1]] THEN
    ...
    ENSURES_WHILE_PUP_TAC `k:num` `pc + 0x3f` `pc + 0x56`
     `\j s. (read RDI s = word k /\
            ...
            (read RCX s =
             ...
             (read ZF s <=> j = k))` THEN
    ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
     [X86_SIM_TAC BIGNUM_MONTIFIER_EXEC (1--4) THEN
      ...
      REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN ENSURES_INIT_TAC "s0" THEN
      X86_STEPS_TAC BIGNUM_MONTIFIER_EXEC (1--2) THEN
    ...
    REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN ENSURES_INIT_TAC "s0" THEN
    X86_STEPS_TAC BIGNUM_MONTIFIER_EXEC (1--6) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  ...
  CONJ_TAC THENL
   [GHOST_INTRO_TAC `n:num` `bignum_from_memory (t,k)` THEN
    ...
    CONJ_TAC THENL
     [X86_SIM_TAC BIGNUM_MONTIFIER_EXEC (1--2) THEN
    ...
    ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
     [X86_SIM_TAC BIGNUM_MONTIFIER_EXEC (1--2) THEN
      ...
      REWRITE_TAC[ARITH_RULE `k - i - 1 = k - (i + 1)`] THEN
      REWRITE_TAC[BIGNUM_FROM_MEMORY_STEP] THEN
      X86_SIM_TAC BIGNUM_MONTIFIER_EXEC (1--6) THEN
   ...
   [ENSURES_WHILE_PUP_TAC `62` `pc + 0x9d` `pc + 0xbe`
    ...
    REPEAT CONJ_TAC THENL
     [ARITH_TAC;
      ...
      GLOBALIZE_PRECONDITION_TAC THEN
      REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN ENSURES_INIT_TAC "s0" THEN
      X86_STEPS_TAC BIGNUM_MONTIFIER_EXEC (1--11) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
      MATCH_MP_TAC(TAUT `p /\ (p ==> q) ==> p /\ q`) THEN CONJ_TAC THENL
       [REWRITE_TAC[ARITH_RULE `a - (i + 1) = a - i - 1`] THEN
        GEN_REWRITE_TAC RAND_CONV [WORD_SUB] THEN
   ...
   [ENSURES_WHILE_UP_TAC `k:num` `pc + 0xd0` `pc + 0xe9`
    ...
    ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
     [X86_SIM_TAC BIGNUM_MONTIFIER_EXEC (1--2) THEN
      ...
      REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
      ENSURES_INIT_TAC "s0" THEN
      X86_ACCSTEPS_TAC BIGNUM_MONTIFIER_EXEC [2;3;4] (1--7) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
   ...
   [GHOST_INTRO_TAC `rhi:num` `\s. val(read RCX s)` THEN
    ...
    ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
     [X86_SIM_TAC BIGNUM_MONTIFIER_EXEC (1--6) THEN
      ...
      REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
      ENSURES_INIT_TAC "s0" THEN
      X86_ACCSTEPS_TAC BIGNUM_MONTIFIER_EXEC [4] (1--7) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
   ...
   [GHOST_INTRO_TAC `p62:num` `bignum_from_memory (z,k)` THEN
    ...
    ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
     [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES; SUB_0] THEN
      ...
      REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
      ENSURES_INIT_TAC "s0" THEN
      X86_ACCSTEPS_TAC BIGNUM_MONTIFIER_EXEC [4] (1--8) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
      ...
      X_GEN_TAC `i:num` THEN STRIP_TAC THEN VAL_INT64_TAC `i:num` THEN
      ASM_REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
      ENSURES_INIT_TAC "s0" THEN X86_STEPS_TAC BIGNUM_MONTIFIER_EXEC (1--2) THEN
      ENSURES_FINAL_STATE_TAC THEN
  ...
  ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
   [UNDISCH_TAC `~(k = 0)` THEN ARITH_TAC;
    ...
    ENSURES_WHILE_PUP_TAC `k:num` `pc + 0x1eb` `pc + 0x211`
     `\j s. (read RDI s = word k /\
             read RSI s = z /\
             read R12 s = mm /\
             read R13 s = t /\
    ...
    ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
     [X86_SIM_TAC BIGNUM_MONTIFIER_EXEC (1--4) THEN
      ...
      REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
      ENSURES_INIT_TAC "s0" THEN
      X86_ACCSTEPS_TAC BIGNUM_MONTIFIER_EXEC [1;4] (1--5) THEN
      RULE_ASSUM_TAC(REWRITE_RULE[WORD_RULE
       `word_sub x (word_neg y):int64 = word_add x y`]) THEN
      ACCUMULATE_ARITH_TAC "s5" THEN
      X86_ACCSTEPS_TAC BIGNUM_MONTIFIER_EXEC [6] (6--11) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
      CONJ_TAC THENL [CONV_TAC WORD_RULE; ALL_TAC] THEN
      MATCH_MP_TAC(TAUT `p /\ (p ==> q) ==> p /\ q`) THEN CONJ_TAC THENL
      ...
      X86_STEPS_TAC BIGNUM_MONTIFIER_EXEC [1] THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[VAL_WORD_SUB_EQ_0];
    ...
    CONJ_TAC THENL
     [GHOST_INTRO_TAC `hup:int64` `read RCX` THEN
      ...
      ASM_REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN ENSURES_INIT_TAC "s0" THEN
      X86_ACCSTEPS_TAC BIGNUM_MONTIFIER_EXEC [2] (1--3) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
      ACCUMULATOR_POP_ASSUM_LIST(MP_TAC o end_itlist CONJ) THEN
    ...
    ENSURES_WHILE_UP_TAC `k:num` `pc + 0x222` `pc + 0x23f`
     `\j s. read RDI s = word k /\
            read RSI s = z /\
            read R12 s = mm /\
            read R13 s = t /\
    ...
    ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
     [X86_SIM_TAC BIGNUM_MONTIFIER_EXEC (1--3) THEN
      REWRITE_TAC[GSYM BIGNUM_FROM_MEMORY_BYTES; LOWDIGITS_0] THEN
      REWRITE_TAC[MULT_CLAUSES; SUB_0; BITVAL_CLAUSES; VAL_WORD_0] THEN
      REWRITE_TAC[WORD_ADD_0; BIGNUM_FROM_MEMORY_TRIVIAL] THEN
      ASM_REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES; HIGHDIGITS_0] THEN
      ...
      REWRITE_TAC[REAL_OF_NUM_CLAUSES] THEN
      ENSURES_INIT_TAC "s0" THEN
      X86_ACCSTEPS_TAC BIGNUM_MONTIFIER_EXEC [5] (1--8) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[GSYM WORD_ADD] THEN
      REWRITE_TAC[WORD_NEG_NEG; VAL_WORD_BITVAL; BITVAL_BOUND] THEN
    ...
    X86_STEPS_TAC BIGNUM_MONTIFIER_EXEC [4] THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
    ...
    REPEAT CONJ_TAC THENL
     [REWRITE_TAC[ARITH_RULE `a - (i + 1) = a - i - 1`] THEN
      ...
      REWRITE_TAC[DIMINDEX_64] THEN ANTS_TAC THENL
       [ALL_TAC; DISCH_THEN SUBST1_TAC] THEN
      MAP_EVERY UNDISCH_TAC [`2 * 8 * k <= 2 EXP 64`; `i < 2 * k`] THEN
      ARITH_TAC;
      DISCH_THEN(fun th ->
        REPEAT(FIRST_X_ASSUM(STRIP_ASSUME_TAC o C MATCH_MP th)) THEN
        ASSUME_TAC th)] THEN
    ...
    X_GEN_TAC `i:num` THEN STRIP_TAC THEN VAL_INT64_TAC `i:num` THEN
    ASM_REWRITE_TAC[] THEN X86_SIM_TAC BIGNUM_MONTIFIER_EXEC [1];

    ALL_TAC] THEN

  (*** Word-level modular inverse computation ***)
   ...
   `\s. read RDI s = word k /\
        ...
        (ODD m
         ==> (2 EXP (64 * k) * val (read R11 s) +
              bignum_from_memory (t,k) s ==
              2 EXP (64 * (3 * k + 1))) (mod m)) /\
        (ODD m ==> (m * val(read RBX s) + 1 == 0) (mod (2 EXP 64)))` THEN
  CONJ_TAC THENL
   [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN ENSURES_INIT_TAC "s0" THEN
    X86_STEPS_TAC BIGNUM_MONTIFIER_EXEC [1] THEN
    SUBGOAL_THEN `bignum_from_memory(mm,k) s1 = highdigits m 0` MP_TAC THENL
     [ASM_REWRITE_TAC[HIGHDIGITS_0; BIGNUM_FROM_MEMORY_BYTES];
      ...
      REWRITE_TAC[DIMINDEX_64] THEN STRIP_TAC] THEN
    X86_STEPS_TAC BIGNUM_MONTIFIER_EXEC (2--7) THEN
    SUBGOAL_THEN `ODD m ==> (m * val (read RBX s7) + 1 == 0) (mod 16)`
    MP_TAC THENL [ASM_SIMP_TAC[WORD_NEGMODINV_SEED_LEMMA_16]; ALL_TAC] THEN
    REABBREV_TAC `x0 = read RBX s7` THEN DISCH_TAC THEN
    X86_STEPS_TAC BIGNUM_MONTIFIER_EXEC (8--25) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
    ASM_REWRITE_TAC[ARITH_RULE `3 * k + 1 = k + 2 * k + 1`] THEN
   ...
   [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN ENSURES_INIT_TAC "s0" THEN
    ...
    MP_TAC THENL
     [ASM_REWRITE_TAC[HIGHDIGITS_0; BIGNUM_FROM_MEMORY_BYTES];
      ...
      ASM_REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES; ADD_CLAUSES] THEN
      STRIP_TAC] THEN
    X86_ACCSTEPS_TAC BIGNUM_MONTIFIER_EXEC [6; 7] (1--10) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  ...
  (*** Break at "amontend" to share the end reasoning ***)
  ...
  CONJ_TAC THENL
   [ASM_CASES_TAC `k = 1` THENL
     [UNDISCH_THEN `k = 1` SUBST_ALL_TAC THEN
      REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
      ENSURES_INIT_TAC "s0" THEN X86_STEPS_TAC BIGNUM_MONTIFIER_EXEC [1] THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
    ...
    (*** The Montgomery reduction loop "amontloop" ***)

    VAL_INT64_TAC `k - 1` THEN

    ENSURES_WHILE_PAUP_TAC `1` `k:num` `pc + 0x2cd` `pc + 0x2f0`
     `\i s. (read RDI s = word k /\
             read RSI s = z /\
             read R12 s = mm /\
             read R13 s = t /\
    ...
    REPEAT CONJ_TAC THENL
     [ASM_REWRITE_TAC[ARITH_RULE `1 < k <=> ~(k = 0 \/ k = 1)`];
      ...
      RULE_ASSUM_TAC(REWRITE_RULE[WORD_RULE
       `word_sub x (word_neg y):int64 = word_add x y`]) THEN
      ACCUMULATE_ARITH_TAC "s5" THEN
      X86_ACCSTEPS_TAC BIGNUM_MONTIFIER_EXEC [6] (6--10) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
      CONJ_TAC THENL [CONV_TAC WORD_RULE; ALL_TAC] THEN
      MATCH_MP_TAC(TAUT `p /\ (p ==> q) ==> p /\ q`) THEN CONJ_TAC THENL
  ...
  CONJ_TAC THENL
   [GHOST_INTRO_TAC `cin:bool` `read CF` THEN
    ...
    MP_TAC(SPEC `k - 1` WORD_INDEX_WRAP) THEN
    ASM_SIMP_TAC[SUB_ADD; LE_1] THEN DISCH_TAC THEN
    X86_ACCSTEPS_TAC BIGNUM_MONTIFIER_EXEC [1] (1--3) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  ...
  (*** The final correction to keep inside k digits, "aosloop" ***)
  ...
  ENSURES_WHILE_UP_TAC `k:num` `pc + 0x303` `pc + 0x31f`
   `\i s. read RDI s = word k /\
          read RSI s = z /\
          read R12 s = mm /\
          read R13 s = t /\
  ...
  (*** The end of the first chapter of the code similar to amontifier ***)
  ...
  ENSURES_SEQUENCE_TAC `pc + 0x324`
   `\s. read RDI s = word k /\
        ...
        bignum_from_memory (mm,k) s = m /\
        (ODD m
         ==> (bignum_from_memory(z,k) s == 2 EXP (192 * k)) (mod m))` THEN
  CONJ_TAC THENL
   [X86_SIM_TAC BIGNUM_MONTIFIER_EXEC (1--2) THEN
    DISCH_THEN(fun th ->
      REPEAT(FIRST_X_ASSUM(STRIP_ASSUME_TAC o C MATCH_MP th)) THEN
    ...
    REWRITE_TAC[GSYM EXP_ADD; ARITH_RULE
     `64 + 192 * k = 64 * (3 * k + 1)`] THEN
    FIRST_ASSUM(MATCH_MP_TAC o MATCH_MP (NUMBER_RULE
    ...
    ASM_REWRITE_TAC[BITVAL_CLAUSES; MULT_CLAUSES;
                    ADD_CLAUSES; CONJUNCT1 LT] THEN
  ...
  ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
   [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
    ...
    GHOST_INTRO_TAC `z3:num` `bignum_from_memory(z,k)` THEN
    BIGNUM_TERMRANGE_TAC `k:num` `z3:num` THEN
    GLOBALIZE_PRECONDITION_TAC THEN ASM_REWRITE_TAC[] THEN
    ...
    ENSURES_WHILE_PUP_TAC `k:num` `pc + 0x383` `pc + 0x391`
    `\i s. (read RDI s = word k /\
            read RSI s = z /\
            read R12 s = mm /\
            bignum_from_memory (z,k) s = z3 /\
            bignum_from_memory (mm,k) s = m /\
            read RBP s = word i /\
            read R8 s = word(k - i) /\
            bignum_from_memory (word_add z (word(8 * i)),k - i) s =
            ...
            bignum_from_memory (word_add mm (word(8 * i)),k - i) s =
            highdigits m i /\
            (read CF s <=> lowdigits z3 i < lowdigits m i)) /\
           (read ZF s <=> i = k)` THEN
   ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
    [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES; SUB_0] THEN
     ENSURES_INIT_TAC "s0" THEN X86_STEPS_TAC BIGNUM_MONTIFIER_EXEC (1--3) THEN
     ENSURES_FINAL_STATE_TAC THEN
    ...
    ENSURES_WHILE_UP_TAC `k:num` `pc + 0x39f` `pc + 0x3b3`
     `\i s. read RDI s = word k /\
            read RSI s = z /\
            read R12 s = mm /\
            read RDX s = word_neg(word(bitval(m <= z3))) /\
            read RBP s = word i /\
            ...
            &(lowdigits z3 i) - &(bitval(m <= z3)) * &(lowdigits m i)` THEN
    ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
     [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
      ENSURES_INIT_TAC "s0" THEN X86_STEPS_TAC BIGNUM_MONTIFIER_EXEC (1--5) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
      REWRITE_TAC[WORD_SUB_LZERO; SUB_0; GSYM BIGNUM_FROM_MEMORY_BYTES] THEN
      ...
      REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
      ENSURES_INIT_TAC "s0" THEN
      X86_ACCSTEPS_TAC BIGNUM_MONTIFIER_EXEC [4] (1--6) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[GSYM WORD_ADD] THEN
      ...
      ENSURES_INIT_TAC "s0" THEN X86_STEPS_TAC BIGNUM_MONTIFIER_EXEC (1--2) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[VAL_WORD_SUB_EQ_0] THEN
      STRIP_TAC THEN UNDISCH_TAC `ODD m ==> r = 0` THEN
      ASM_REWRITE_TAC[] THEN DISCH_THEN SUBST_ALL_TAC THEN
      RULE_ASSUM_TAC(REWRITE_RULE[ADD_CLAUSES])] THEN
    FIRST_ASSUM(ASSUME_TAC o MATCH_MP (MESON[ODD] `ODD m ==> ~(m = 0)`)) THEN
    TRANS_TAC EQ_TRANS `z3 MOD m` THEN CONJ_TAC THENL
     [REWRITE_TAC[GSYM REAL_OF_NUM_CLAUSES] THEN
      MATCH_MP_TAC EQUAL_FROM_CONGRUENT_REAL THEN
      MAP_EVERY EXISTS_TAC [`64 * k`; `&0:real`] THEN CONJ_TAC THENL
       [REWRITE_TAC[REAL_OF_NUM_CLAUSES; GSYM BIGNUM_FROM_MEMORY_BYTES] THEN
      ...
      CONJ_TAC THENL
       [REWRITE_TAC[REAL_OF_NUM_CLAUSES] THEN TRANS_TAC LT_TRANS `m:num` THEN
        ASM_REWRITE_TAC[MOD_LT_EQ];
        ALL_TAC] THEN
      MP_TAC(SPECL [`z3:num`; `m:num`] MOD_CASES) THEN
      ANTS_TAC THENL
      ...
      FIRST_ASSUM(MATCH_MP_TAC o MATCH_MP (NUMBER_RULE
         `e * x:num = q * m + ab
          ==> (ab == e * ee) (mod m) /\ coprime(m,e)
              ==> (x == ee) (mod m)`)) THEN
      REWRITE_TAC[GSYM EXP_ADD; ARITH_RULE `64 * k + 128 * k = 192 * k`] THEN
      ASM_SIMP_TAC[COPRIME_REXP; COPRIME_2]]] THEN
  ...
  ENSURES_SEQUENCE_TAC `pc + 0x346`
   `\s. read RDI s = word k /\
        ...
        read RBP s = word 1 /\
        read R8 s = word k /\
        2 EXP 64 * (bitval(read CF s) + val(read RCX s)) + val(read RAX s) =
        q0 * bigdigit m 0 + bigdigit z1 0` THEN
  CONJ_TAC THENL
   [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN ENSURES_INIT_TAC "s0" THEN
    SUBGOAL_THEN
    ...
    MP_TAC THENL
     [ASM_REWRITE_TAC[HIGHDIGITS_0; BIGNUM_FROM_MEMORY_BYTES];
      ...
      ASM_REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES; ADD_CLAUSES] THEN
      STRIP_TAC] THEN
    X86_ACCSTEPS_TAC BIGNUM_MONTIFIER_EXEC [5; 6] (1--9) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
    MATCH_MP_TAC(TAUT `p /\ (p ==> q) ==> p /\ q`) THEN CONJ_TAC THENL
     [UNDISCH_THEN `(w * z1) MOD 2 EXP 64 = q0` (SUBST1_TAC o SYM) THEN
      REWRITE_TAC[GSYM WORD_MUL] THEN ONCE_REWRITE_TAC[GSYM WORD_MOD_SIZE] THEN
  ...
  CONJ_TAC THENL
   [ASM_CASES_TAC `k = 1` THENL
    ...
    ENSURES_WHILE_PAUP_TAC `1` `k:num` `pc + 0x34b` `pc + 0x36d`
     `\j s. (read RDI s = word k /\
             read RSI s = z /\
             read R12 s = mm /\
             bignum_from_memory (mm,k) s = m /\
             ...
             bignum_from_memory(word_add z (word (8 * j)),k - j) s =
             highdigits z1 j /\
             bignum_from_memory(word_add mm (word (8 * j)),k - j) s =
             highdigits m j /\
             2 EXP (64 * j) * (bitval(read CF s) + val(read RCX s)) +
             2 EXP 64 * bignum_from_memory(z,j-1) s + r0 =
             lowdigits z1 j + q0 * lowdigits m j) /\
            (read ZF s <=> j = k)` THEN
    REPEAT CONJ_TAC THENL
     [ASM_REWRITE_TAC[ARITH_RULE `1 < k <=> ~(k = 0 \/ k = 1)`];
  ...
  CONJ_TAC THENL
   [REWRITE_TAC[GSYM CONJ_ASSOC];
    VAL_INT64_TAC `k - (i + 1)` THEN ASM_REWRITE_TAC[] THEN
   ...
   [REWRITE_TAC[EXP_ADD; ARITH_RULE `64 * (i + 1) = 64 * i + 64`] THEN
    CONJ_TAC THEN MATCH_MP_TAC(ARITH_RULE
     `q1 < e /\ q0 < ee /\ (q1 < e ==> (q1 + 1) * ee <= e * ee)
      ==> ee * q1 + q0 < ee * e`) THEN
    ASM_REWRITE_TAC[LE_MULT_RCANCEL; EXP_EQ_0; ARITH_EQ] THEN
    ASM_REWRITE_TAC[ARITH_RULE `m + 1 <= mm <=> m < mm`];
    ALL_TAC] THEN
   ...
   [SUBGOAL_THEN `8 * k = 8 * ((k - 1) + 1)` SUBST1_TAC THENL
    ...
    ANTS_TAC THENL
     [SIMP_TAC[LT_MULT_RCANCEL; EXP_EQ_0; MULT_EQ_0; ARITH_EQ] THEN
      ...
      SUBST1_TAC(SYM(ASSUME `2 EXP (64 * i) * z1 + r = q * m + a`)) THEN
      CONV_TAC NUM_RING];
    DISCH_THEN(fun th ->
      REPEAT(FIRST_X_ASSUM(STRIP_ASSUME_TAC o C MATCH_MP th))) THEN
    ...
    REWRITE_TAC[GSYM CONG] THEN MATCH_MP_TAC(NUMBER_RULE
     `(m * w + 1 == 0) (mod e) ==> (z + (w * z) * m == 0) (mod e)`) THEN
    ASM_REWRITE_TAC[]]);;

let BIGNUM_MONTIFIER_NOIBT_SUBROUTINE_CORRECT = time prove
 (`!k z m t n pc stackpointer returnaddress.
        ...
        ALLPAIRS nonoverlapping [(z,8 * val k); (t,8 * val k);
                                ...
                                [(word pc,LENGTH bignum_montifier_tmc); (m,8 * val k)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_montifier_tmc /\
                  read RIP s = word pc /\
  ...
  X86_PROMOTE_RETURN_STACK_TAC bignum_montifier_tmc BIGNUM_MONTIFIER_CORRECT
   `[RBX; RBP; R12; R13]` 32);;

let BIGNUM_MONTIFIER_SUBROUTINE_CORRECT = time prove
 (`!k z m t n pc stackpointer returnaddress.
        nonoverlapping (z,8 * val k) (t,8 * val k) /\
        ALL (nonoverlapping (word_sub stackpointer (word 32),40))
            [(z,8 * val k); (t,8 * val k)] /\
        ALLPAIRS nonoverlapping [(z,8 * val k); (t,8 * val k);
                                 (word_sub stackpointer (word 32),32)]
                                [(word pc,LENGTH bignum_montifier_mc); (m,8 * val k)]
        ==> ensures x86
             ...
             (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * val k);
                         memory :> bytes(t,8 * val k);
                       memory :> bytes(word_sub stackpointer (word 32),32)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MONTIFIER_NOIBT_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let bignum_montifier_windows_mc = define_from_elf
   "bignum_montifier_windows_mc" "x86/generic/bignum_montifier.obj";;

let bignum_montifier_windows_tmc = define_trimmed "bignum_montifier_windows_tmc" bignum_montifier_windows_mc;;

let BIGNUM_MONTIFIER_NOIBT_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!k z m t n pc stackpointer returnaddress.
        nonoverlapping (z,8 * val k) (t,8 * val k) /\
        ...
        ALLPAIRS nonoverlapping [(z,8 * val k); (t,8 * val k);
                                ...
                                [(word pc,LENGTH bignum_montifier_windows_tmc); (m,8 * val k)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_montifier_windows_tmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
  ...
  WINDOWS_X86_WRAP_STACK_TAC bignum_montifier_windows_tmc bignum_montifier_tmc
   BIGNUM_MONTIFIER_CORRECT `[RBX; RBP; R12; R13]` 32);;

let BIGNUM_MONTIFIER_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!k z m t n pc stackpointer returnaddress.
        nonoverlapping (z,8 * val k) (t,8 * val k) /\
        ALL (nonoverlapping (word_sub stackpointer (word 48),56))
            [(z,8 * val k); (t,8 * val k)] /\
        ALLPAIRS nonoverlapping [(z,8 * val k); (t,8 * val k);
                                 (word_sub stackpointer (word 48),48)]
                                [(word pc,LENGTH bignum_montifier_windows_mc); (m,8 * val k)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_montifier_windows_mc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  WINDOWS_C_ARGUMENTS [k; z; m; t] s /\
             ...
             (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * val k);
                         memory :> bytes(t,8 * val k);
                       memory :> bytes(word_sub stackpointer (word 48),48)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MONTIFIER_NOIBT_WINDOWS_SUBROUTINE_CORRECT));;
