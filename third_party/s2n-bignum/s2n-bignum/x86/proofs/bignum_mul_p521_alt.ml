...
let bignum_mul_p521_alt_mc =
  define_assert_from_elf "bignum_mul_p521_alt_mc" "x86/p521/bignum_mul_p521_alt.o"
[
...
let bignum_mul_p521_alt_tmc = define_trimmed "bignum_mul_p521_alt_tmc" bignum_mul_p521_alt_mc;;

let BIGNUM_MUL_P521_ALT_EXEC = X86_MK_CORE_EXEC_RULE bignum_mul_p521_alt_tmc;;
...
let BIGNUM_MUL_P521_ALT_CORRECT = prove
 (`!z x y a b pc stackpointer.
        ...
        ==> ensures x86
             (\s. bytes_loaded s (word pc) (BUTLAST bignum_mul_p521_alt_tmc) /\
...
let BIGNUM_MUL_P521_ALT_NOIBT_SUBROUTINE_CORRECT = prove
 (`!z x y a b pc stackpointer returnaddress.
        ALL (nonoverlapping (z,8 * 9))
            [(word pc,LENGTH bignum_mul_p521_alt_tmc); (word_sub stackpointer (word 104),112)] /\
        ALL (nonoverlapping (word_sub stackpointer (word 104),104))
            [(word pc,LENGTH bignum_mul_p521_alt_tmc); (x,8 * 9); (y,8 * 9)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_mul_p521_alt_tmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
  ...
  X86_PROMOTE_RETURN_STACK_TAC
   bignum_mul_p521_alt_tmc BIGNUM_MUL_P521_ALT_CORRECT
   `[R12; R13; R14; R15]` 104);;

let BIGNUM_MUL_P521_ALT_SUBROUTINE_CORRECT = prove
 (`!z x y a b pc stackpointer returnaddress.
        ALL (nonoverlapping (z,8 * 9))
            [(word pc,LENGTH bignum_mul_p521_alt_mc); (word_sub stackpointer (word 104),112)] /\
        ALL (nonoverlapping (word_sub stackpointer (word 104),104))
            [(word pc,LENGTH bignum_mul_p521_alt_mc); (x,8 * 9); (y,8 * 9)]
        ==> ensures x86
             ...
             (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * 9);
                       memory :> bytes(word_sub stackpointer (word 104),104)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MUL_P521_ALT_NOIBT_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let bignum_mul_p521_alt_windows_mc = define_from_elf
   "bignum_mul_p521_alt_windows_mc" "x86/p521/bignum_mul_p521_alt.obj";;

let bignum_mul_p521_alt_windows_tmc = define_trimmed "bignum_mul_p521_alt_windows_tmc" bignum_mul_p521_alt_windows_mc;;

let BIGNUM_MUL_P521_ALT_NOIBT_WINDOWS_SUBROUTINE_CORRECT = prove
 (`!z x y a b pc stackpointer returnaddress.
        ALL (nonoverlapping (z,8 * 9))
            [(word pc,LENGTH bignum_mul_p521_alt_windows_tmc); (word_sub stackpointer (word 120),128)] /\
        ALL (nonoverlapping (word_sub stackpointer (word 120),120))
            [(word pc,LENGTH bignum_mul_p521_alt_windows_tmc); (x,8 * 9); (y,8 * 9)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_mul_p521_alt_windows_tmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
  ...
  WINDOWS_X86_WRAP_STACK_TAC
   bignum_mul_p521_alt_windows_tmc bignum_mul_p521_alt_tmc
   BIGNUM_MUL_P521_ALT_CORRECT `[R12; R13; R14; R15]` 104);;

let BIGNUM_MUL_P521_ALT_WINDOWS_SUBROUTINE_CORRECT = prove
 (`!z x y a b pc stackpointer returnaddress.
        ALL (nonoverlapping (z,8 * 9))
            [(word pc,LENGTH bignum_mul_p521_alt_windows_mc); (word_sub stackpointer (word 120),128)] /\
        ALL (nonoverlapping (word_sub stackpointer (word 120),120))
            [(word pc,LENGTH bignum_mul_p521_alt_windows_mc); (x,8 * 9); (y,8 * 9)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_mul_p521_alt_windows_mc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  WINDOWS_C_ARGUMENTS [z; x; y] s /\
             ...
             (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * 9);
                       memory :> bytes(word_sub stackpointer (word 120),120)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MUL_P521_ALT_NOIBT_WINDOWS_SUBROUTINE_CORRECT));;
