...
let bignum_modifier_tmc = define_trimmed "bignum_modifier_tmc" bignum_modifier_mc;;

let BIGNUM_MODIFIER_EXEC = X86_MK_CORE_EXEC_RULE bignum_modifier_tmc;;
...
let BIGNUM_MODIFIER_CORRECT = time prove
 (`!k z m t n pc.
        nonoverlapping (z,8 * val k) (t,8 * val k) /\
        ALLPAIRS nonoverlapping [(z,8 * val k); (t,8 * val k)]
                                [(word pc,0x3be); (m,8 * val k)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) (BUTLAST bignum_modifier_tmc) /\
                  read RIP s = word(pc + 0x6) /\
                  C_ARGUMENTS [k; z; m; t] s /\
                  bignum_from_memory (m,val k) s = n)
             (\s. read RIP s = word(pc + 0x3b7) /\
                  (ODD n
                   ==> bignum_from_memory (z,val k) s =
                        2 EXP (64 * val k) MOD n))
  ...
  ASM_CASES_TAC `k = 0` THENL
   [UNDISCH_THEN `k = 0` SUBST_ALL_TAC THEN
    ...
    ENSURES_INIT_TAC "s0" THEN X86_STEPS_TAC BIGNUM_MODIFIER_EXEC (1--4) THEN
  ...
  ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
   [X86_SIM_TAC BIGNUM_MODIFIER_EXEC (1--5) THEN
    ...
    X86_STEPS_TAC BIGNUM_MODIFIER_EXEC (1--3) THEN
    ...
    X_GEN_TAC `i:num` THEN STRIP_TAC THEN VAL_INT64_TAC `i:num` THEN
    X86_SIM_TAC BIGNUM_MODIFIER_EXEC (1--2);
   ...
   [ASM_CASES_TAC `k = 1` THENL
    ...
    ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
     [MP_TAC(ISPECL [`word k:int64`; `word 1:int64`] VAL_WORD_SUB_CASES) THEN
      ...
      X86_STEPS_TAC BIGNUM_MODIFIER_EXEC (1--5) THEN
      ...
      X_GEN_TAC `i:num` THEN STRIP_TAC THEN VAL_INT64_TAC `i:num` THEN
      X86_SIM_TAC BIGNUM_MODIFIER_EXEC [1];
      X86_SIM_TAC BIGNUM_MODIFIER_EXEC [1]] THEN
    ...
    ENSURES_WHILE_PUP_TAC `k:num` `pc + 0x3f` `pc + 0x56`
     `\j s. (read RDI s = word k /\
            ...
            (read RCX s =
             ...
             (read ZF s <=> j = k))` THEN
    ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
     [X86_SIM_TAC BIGNUM_MODIFIER_EXEC (1--4) THEN
      ...
      REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN ENSURES_INIT_TAC "s0" THEN
      X86_STEPS_TAC BIGNUM_MODIFIER_EXEC (1--2) THEN
    ...
    REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN ENSURES_INIT_TAC "s0" THEN
    X86_STEPS_TAC BIGNUM_MODIFIER_EXEC (1--6) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  ...
  CONJ_TAC THENL
   [GHOST_INTRO_TAC `n:num` `bignum_from_memory (t,k)` THEN
    ...
    CONJ_TAC THENL
     [X86_SIM_TAC BIGNUM_MODIFIER_EXEC (1--2) THEN
    ...
    ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
     [X86_SIM_TAC BIGNUM_MODIFIER_EXEC (1--2) THEN
      ...
      REWRITE_TAC[ARITH_RULE `k - i - 1 = k - (i + 1)`] THEN
      REWRITE_TAC[BIGNUM_FROM_MEMORY_STEP] THEN
      X86_SIM_TAC BIGNUM_MODIFIER_EXEC (1--6) THEN
   ...
   [ENSURES_WHILE_PUP_TAC `62` `pc + 0x9d` `pc + 0xbe`
    ...
    REPEAT CONJ_TAC THENL
     [ARITH_TAC;
      ...
      GLOBALIZE_PRECONDITION_TAC THEN
      REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN ENSURES_INIT_TAC "s0" THEN
      X86_STEPS_TAC BIGNUM_MODIFIER_EXEC (1--11) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
      MATCH_MP_TAC(TAUT `p /\ (p ==> q) ==> p /\ q`) THEN CONJ_TAC THENL
       [REWRITE_TAC[ARITH_RULE `a - (i + 1) = a - i - 1`] THEN
        GEN_REWRITE_TAC RAND_CONV [WORD_SUB] THEN
   ...
   [ENSURES_WHILE_UP_TAC `k:num` `pc + 0xd0` `pc + 0xe9`
    ...
    ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
     [X86_SIM_TAC BIGNUM_MODIFIER_EXEC (1--2) THEN
      ...
      REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
      ENSURES_INIT_TAC "s0" THEN
      X86_ACCSTEPS_TAC BIGNUM_MODIFIER_EXEC [2;3;4] (1--7) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
   ...
   [GHOST_INTRO_TAC `rhi:num` `\s. val(read RCX s)` THEN
    ...
    ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
     [X86_SIM_TAC BIGNUM_MODIFIER_EXEC (1--6) THEN
      ...
      REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
      ENSURES_INIT_TAC "s0" THEN
      X86_ACCSTEPS_TAC BIGNUM_MODIFIER_EXEC [4] (1--7) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
   ...
   [GHOST_INTRO_TAC `p62:num` `bignum_from_memory (z,k)` THEN
    ...
    ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
     [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES; SUB_0] THEN
      ...
      REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
      ENSURES_INIT_TAC "s0" THEN
      X86_ACCSTEPS_TAC BIGNUM_MODIFIER_EXEC [4] (1--8) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
      ...
      X_GEN_TAC `i:num` THEN STRIP_TAC THEN VAL_INT64_TAC `i:num` THEN
      ASM_REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
      ENSURES_INIT_TAC "s0" THEN X86_STEPS_TAC BIGNUM_MODIFIER_EXEC (1--2) THEN
      ENSURES_FINAL_STATE_TAC THEN
   ...
   [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES; ADD_CLAUSES; SUB_0] THEN
    ...
    ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
     [X86_SIM_TAC BIGNUM_MODIFIER_EXEC (1--4) THEN
      ...
      REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
      ENSURES_INIT_TAC "s0" THEN
      X86_ACCSTEPS_TAC BIGNUM_MODIFIER_EXEC [1;4] (1--5) THEN
      RULE_ASSUM_TAC(REWRITE_RULE[WORD_RULE
       `word_sub x (word_neg y):int64 = word_add x y`]) THEN
      ACCUMULATE_ARITH_TAC "s5" THEN
      X86_ACCSTEPS_TAC BIGNUM_MODIFIER_EXEC [6] (6--11) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
      ...
      X86_STEPS_TAC BIGNUM_MODIFIER_EXEC [1] THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[VAL_WORD_SUB_EQ_0];
    ...
    CONJ_TAC THENL
     [GHOST_INTRO_TAC `hup:int64` `read RCX` THEN
      ...
      ASM_REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN ENSURES_INIT_TAC "s0" THEN
      X86_ACCSTEPS_TAC BIGNUM_MODIFIER_EXEC [2] (1--3) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
      ACCUMULATOR_POP_ASSUM_LIST(MP_TAC o end_itlist CONJ) THEN
    ...
    ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
     [X86_SIM_TAC BIGNUM_MODIFIER_EXEC (1--3) THEN
      REWRITE_TAC[GSYM BIGNUM_FROM_MEMORY_BYTES; LOWDIGITS_0] THEN
      REWRITE_TAC[MULT_CLAUSES; SUB_0; BITVAL_CLAUSES; VAL_WORD_0] THEN
      REWRITE_TAC[WORD_ADD_0; BIGNUM_FROM_MEMORY_TRIVIAL] THEN
      ASM_REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES; HIGHDIGITS_0] THEN
      ...
      REWRITE_TAC[REAL_OF_NUM_CLAUSES] THEN
      ENSURES_INIT_TAC "s0" THEN
      X86_ACCSTEPS_TAC BIGNUM_MODIFIER_EXEC [5] (1--8) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[GSYM WORD_ADD] THEN
      REWRITE_TAC[WORD_NEG_NEG; VAL_WORD_BITVAL; BITVAL_BOUND] THEN
    ...
    X86_STEPS_TAC BIGNUM_MODIFIER_EXEC [4] THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
    ...
    X_GEN_TAC `i:num` THEN STRIP_TAC THEN VAL_INT64_TAC `i:num` THEN
    ASM_REWRITE_TAC[] THEN X86_SIM_TAC BIGNUM_MODIFIER_EXEC [1];
   ...
   [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN ENSURES_INIT_TAC "s0" THEN
    ...
    SUBGOAL_THEN `bignum_from_memory(mm,k) s1 = highdigits m 0` MP_TAC THENL
     [ASM_REWRITE_TAC[HIGHDIGITS_0; BIGNUM_FROM_MEMORY_BYTES];
      ...
      REWRITE_TAC[DIMINDEX_64] THEN STRIP_TAC] THEN
    X86_STEPS_TAC BIGNUM_MODIFIER_EXEC (2--7) THEN
    ...
    X86_STEPS_TAC BIGNUM_MODIFIER_EXEC (8--25) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
   ...
   [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN ENSURES_INIT_TAC "s0" THEN
    ...
    MP_TAC THENL
     [ASM_REWRITE_TAC[HIGHDIGITS_0; BIGNUM_FROM_MEMORY_BYTES];
      ...
      ASM_REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES; ADD_CLAUSES] THEN
      STRIP_TAC] THEN
    X86_ACCSTEPS_TAC BIGNUM_MODIFIER_EXEC [6; 7] (1--10) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  ...
  CONJ_TAC THENL
   [ASM_CASES_TAC `k = 1` THENL
     [UNDISCH_THEN `k = 1` SUBST_ALL_TAC THEN
      REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
      ENSURES_INIT_TAC "s0" THEN X86_STEPS_TAC BIGNUM_MODIFIER_EXEC [1] THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
    ...
    ENSURES_WHILE_PAUP_TAC `1` `k:num` `pc + 0x2cc` `pc + 0x2ef`
     `\i s. (read RDI s = word k /\
             read RSI s = z /\
             read R12 s = mm /\
             read R13 s = t /\
    ...
    REPEAT CONJ_TAC THENL
     [ASM_REWRITE_TAC[ARITH_RULE `1 < k <=> ~(k = 0 \/ k = 1)`];
      ...
      RULE_ASSUM_TAC(REWRITE_RULE[WORD_RULE
       `word_sub x (word_neg y):int64 = word_add x y`]) THEN
      ACCUMULATE_ARITH_TAC "s5" THEN
      X86_ACCSTEPS_TAC BIGNUM_MODIFIER_EXEC [6] (6--10) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
      CONJ_TAC THENL [CONV_TAC WORD_RULE; ALL_TAC] THEN
      MATCH_MP_TAC(TAUT `p /\ (p ==> q) ==> p /\ q`) THEN CONJ_TAC THENL
  ...
  CONJ_TAC THENL
   [GHOST_INTRO_TAC `cin:bool` `read CF` THEN
    ...
    MP_TAC(SPEC `k - 1` WORD_INDEX_WRAP) THEN
    ASM_SIMP_TAC[SUB_ADD; LE_1] THEN DISCH_TAC THEN
    X86_ACCSTEPS_TAC BIGNUM_MODIFIER_EXEC [1] (1--3) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  ...
  ENSURES_WHILE_UP_TAC `k:num` `pc + 0x302` `pc + 0x31e`
   `\i s. read RDI s = word k /\
          read RSI s = z /\
          read R12 s = mm /\
          read R13 s = t /\
  ...
  ENSURES_SEQUENCE_TAC `pc + 0x323`
   `\s. read RDI s = word k /\
        ...
        bignum_from_memory (mm,k) s = m /\
        (ODD m
         ==> (bignum_from_memory(z,k) s == 2 EXP (128 * k)) (mod m))` THEN
  CONJ_TAC THENL
   [X86_SIM_TAC BIGNUM_MODIFIER_EXEC (1--2) THEN
    DISCH_THEN(fun th ->
      REPEAT(FIRST_X_ASSUM(STRIP_ASSUME_TAC o C MATCH_MP th)) THEN
  ...
  ENSURES_WHILE_PUP_TAC `k:num` `pc + 0x326` `pc + 0x37a`
   `\i s. (read RDI s = word k /\
           read RSI s = z /\
           read R12 s = mm /\
           read RBX s = word w /\
  ...
  ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
   [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
    ...
    ENSURES_WHILE_PUP_TAC `k:num` `pc + 0x382` `pc + 0x390`
    `\i s. (read RDI s = word k /\
            read RSI s = z /\
            read R12 s = mm /\
   ...
   ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
    [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES; SUB_0] THEN
     ENSURES_INIT_TAC "s0" THEN X86_STEPS_TAC BIGNUM_MODIFIER_EXEC (1--3) THEN
     ENSURES_FINAL_STATE_TAC THEN
     ...
     REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
     ENSURES_INIT_TAC "s0" THEN
     X86_STEPS_TAC BIGNUM_MODIFIER_EXEC (1--4) THEN
     ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
    ...
    ENSURES_WHILE_UP_TAC `k:num` `pc + 0x39e` `pc + 0x3b2`
     `\i s. read RDI s = word k /\
            read RSI s = z /\
            read R12 s = mm /\
    ...
    ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
     [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
      ENSURES_INIT_TAC "s0" THEN X86_STEPS_TAC BIGNUM_MODIFIER_EXEC (1--5) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
      REWRITE_TAC[WORD_SUB_LZERO; SUB_0; GSYM BIGNUM_FROM_MEMORY_BYTES] THEN
      ...
      REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
      ENSURES_INIT_TAC "s0" THEN
      X86_ACCSTEPS_TAC BIGNUM_MODIFIER_EXEC [4] (1--6) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[GSYM WORD_ADD] THEN
    ...
    TRANS_TAC EQ_TRANS `z3 MOD m` THEN CONJ_TAC THENL
     [REWRITE_TAC[GSYM REAL_OF_NUM_CLAUSES] THEN
      ...
      FIRST_ASSUM(MATCH_MP_TAC o MATCH_MP (NUMBER_RULE
         `e * x:num = q * m + ab
          ==> (ab == e EXP 2) (mod m) /\ coprime(m,e)
              ==> (x == e) (mod m)`)) THEN
      REWRITE_TAC[EXP_EXP; ARITH_RULE `(64 * k) * 2 = 128 * k`] THEN
   ...
   [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN ENSURES_INIT_TAC "s0" THEN
    ...
    MP_TAC THENL
     [ASM_REWRITE_TAC[HIGHDIGITS_0; BIGNUM_FROM_MEMORY_BYTES];
      ...
      ASM_REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES; ADD_CLAUSES] THEN
      STRIP_TAC] THEN
    X86_ACCSTEPS_TAC BIGNUM_MODIFIER_EXEC [5; 6] (1--9) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
    MATCH_MP_TAC(TAUT `p /\ (p ==> q) ==> p /\ q`) THEN CONJ_TAC THENL
     [UNDISCH_THEN `(w * z1) MOD 2 EXP 64 = q0` (SUBST1_TAC o SYM) THEN
      REWRITE_TAC[GSYM WORD_MUL] THEN ONCE_REWRITE_TAC[GSYM WORD_MOD_SIZE] THEN
  ...
  CONJ_TAC THENL
   [ASM_CASES_TAC `k = 1` THENL
    ...
    ENSURES_WHILE_PAUP_TAC `1` `k:num` `pc + 0x34a` `pc + 0x36c`
     `\j s. (read RDI s = word k /\
             read RSI s = z /\
             read R12 s = mm /\
             bignum_from_memory (mm,k) s = m /\
...
let BIGNUM_MODIFIER_NOIBT_SUBROUTINE_CORRECT = time prove
 (`!k z m t n pc stackpointer returnaddress.
        ...
        ALLPAIRS nonoverlapping [(z,8 * val k); (t,8 * val k);
                                ...
                                [(word pc,LENGTH bignum_modifier_tmc); (m,8 * val k)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_modifier_tmc /\
                  read RIP s = word pc /\
  ...
  X86_PROMOTE_RETURN_STACK_TAC bignum_modifier_tmc BIGNUM_MODIFIER_CORRECT
   `[RBX; RBP; R12; R13]` 32);;

let BIGNUM_MODIFIER_SUBROUTINE_CORRECT = time prove
 (`!k z m t n pc stackpointer returnaddress.
        nonoverlapping (z,8 * val k) (t,8 * val k) /\
        ALL (nonoverlapping (word_sub stackpointer (word 32),40))
            [(z,8 * val k); (t,8 * val k)] /\
        ALLPAIRS nonoverlapping [(z,8 * val k); (t,8 * val k);
                                 (word_sub stackpointer (word 32),32)]
                                [(word pc,LENGTH bignum_modifier_mc); (m,8 * val k)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_modifier_mc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  C_ARGUMENTS [k; z; m; t] s /\
             ...
             (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * val k);
                         memory :> bytes(t,8 * val k);
                       memory :> bytes(word_sub stackpointer (word 32),32)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MODIFIER_NOIBT_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let bignum_modifier_windows_mc = define_from_elf
   "bignum_modifier_windows_mc" "x86/generic/bignum_modifier.obj";;

let bignum_modifier_windows_tmc = define_trimmed "bignum_modifier_windows_tmc" bignum_modifier_windows_mc;;

let BIGNUM_MODIFIER_NOIBT_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!k z m t n pc stackpointer returnaddress.
        nonoverlapping (z,8 * val k) (t,8 * val k) /\
        ...
        ALLPAIRS nonoverlapping [(z,8 * val k); (t,8 * val k);
                                ...
                                [(word pc,LENGTH bignum_modifier_windows_tmc); (m,8 * val k)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_modifier_windows_tmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
  ...
  WINDOWS_X86_WRAP_STACK_TAC bignum_modifier_windows_tmc bignum_modifier_tmc
   BIGNUM_MODIFIER_CORRECT `[RBX; RBP; R12; R13]` 32);;

let BIGNUM_MODIFIER_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!k z m t n pc stackpointer returnaddress.
        nonoverlapping (z,8 * val k) (t,8 * val k) /\
        ALL (nonoverlapping (word_sub stackpointer (word 48),56))
            [(z,8 * val k); (t,8 * val k)] /\
        ALLPAIRS nonoverlapping [(z,8 * val k); (t,8 * val k);
                                 (word_sub stackpointer (word 48),48)]
                                [(word pc,LENGTH bignum_modifier_windows_mc); (m,8 * val k)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_modifier_windows_mc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  WINDOWS_C_ARGUMENTS [k; z; m; t] s /\
             ...
             (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * val k);
                         memory :> bytes(t,8 * val k);
                       memory :> bytes(word_sub stackpointer (word 48),48)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MODIFIER_NOIBT_WINDOWS_SUBROUTINE_CORRECT));;
