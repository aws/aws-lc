...
[
  ...
  0x4c; 0x01; 0x2e;        (* ADD (Memop Quadword (%% (rsi,0))) (% r13) *)
...
let bignum_madd_tmc = define_trimmed "bignum_madd_tmc" bignum_madd_mc;;

let BIGNUM_MADD_EXEC = X86_MK_CORE_EXEC_RULE bignum_madd_tmc;;
...
let BIGNUM_MADD_CORRECT = prove
 (`!p m n z x y a b c pc.
     ...
     ==> ensures x86
          (\s. bytes_loaded s (word pc) (BUTLAST bignum_madd_tmc) /\
  ...
  ASM_CASES_TAC `p = 0` THENL
   [X86_SIM_TAC BIGNUM_MADD_EXEC (1--4) THEN
  ...
  ENSURES_WHILE_UP_TAC `p:num` `pc + 0x1e` `pc + 0x7c`
  ...
  ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
   [X86_SIM_TAC BIGNUM_MADD_EXEC (1--7) THEN
    REWRITE_TAC[GSYM BIGNUM_FROM_MEMORY_BYTES] THEN
    ...
    X86_SIM_TAC BIGNUM_MADD_EXEC (1--2);
    GHOST_INTRO_TAC `cout:int64` `read R13` THEN
    X86_SIM_TAC BIGNUM_MADD_EXEC (1--3) THEN CONJ_TAC THENL
  ...
  CONJ_TAC THENL
   [X86_SIM_TAC BIGNUM_MADD_EXEC (1--8) THEN
...
(*** Separate and handle the final add/writeback to z ***)
  ...
  CONJ_TAC THENL
   [ALL_TAC;
    ...
    X86_ACCSIM_TAC BIGNUM_MADD_EXEC (1--3) (1--6) THEN
    CONJ_TAC THENL [CONV_TAC WORD_RULE; ALL_TAC] THEN
    SUBGOAL_THEN `!i. i < MIN (k + 1) m ==> i + k - i = k` MP_TAC THENL
     [SIMPLE_ARITH_TAC; SIMP_TAC[IN_ELIM_THM] THEN DISCH_THEN(K ALL_TAC)] THEN
    SUBST1_TAC(SYM(ASSUME
     `2 EXP (64 * k) * (2 EXP 64 * h + l) + zsum = lowsum`)) THEN
    REWRITE_TAC[NSUM_LMUL; ARITH_RULE
     `a + z + b:num = (c + z) + d <=> a + b = c + d`] THEN
    REWRITE_TAC[ARITH_RULE `64 * (k + 1) = 64 * k + 64`; EXP_ADD] THEN
    REWRITE_TAC[GSYM MULT_ASSOC; GSYM LEFT_ADD_DISTRIB] THEN AP_TERM_TAC THEN
    GEN_REWRITE_TAC RAND_CONV [ADD_ASSOC] THEN FIRST_ASSUM(fun th ->
    ...
    CONJ_TAC THENL [BOUNDER_TAC[]; ALL_TAC] THEN CONJ_TAC THENL
     [REWRITE_TAC[REAL_OF_NUM_CLAUSES; LE_0] THEN ASM_REWRITE_TAC[] THEN
      MATCH_MP_TAC(ARITH_RULE
       `h < 2 EXP 64 /\ l < 2 EXP 64 /\ c < 2 EXP 64 /\
        s <= 2 EXP 64 * (2 EXP 64 - 1) EXP 2
        ==> ((2 EXP 64 * h + l) + s) + c < 2 EXP 192`) THEN
      ASM_REWRITE_TAC[BIGDIGIT_BOUND] THEN
      TRANS_TAC LE_TRANS
       `nsum {n | n < 2 EXP 64} (\i. bigdigit a i * bigdigit b (k - i))` THEN
      CONJ_TAC THENL
       [MATCH_MP_TAC NSUM_SUBSET_SIMPLE THEN REWRITE_TAC[FINITE_NUMSEG_LT] THEN
        MAP_EVERY UNDISCH_TAC [`k:num < p`; `p < 2 EXP 64`] THEN
 ...
 (*** Case split to trivialize the "no terms in sum" case ***)
  ...
  ENSURES_WHILE_PAUP_TAC `(k + 1) - n` `MIN (k + 1) m` `pc + 0x4c` `pc + 0x65`
  ...
  ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
   [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN ENSURES_INIT_TAC "s0" THEN
    X86_STEPS_TAC BIGNUM_MADD_EXEC (1--4) THEN
    FIND_ASM_THEN lhand `read RIP s4` MP_TAC THEN
    MAP_EVERY VAL_INT64_TAC [`MIN (k + 1) m`; `(k + 1) - n`] THEN
    ASM_REWRITE_TAC[VAL_WORD_SUB_EQ_0; GSYM LE_LT] THEN DISCH_TAC THEN
    X86_STEPS_TAC BIGNUM_MADD_EXEC (5--6) THEN ENSURES_FINAL_STATE_TAC THEN
    ...
    X86_SIM_TAC BIGNUM_MADD_EXEC [1];
    X86_SIM_TAC BIGNUM_MADD_EXEC [1]] THEN
  ...
  X86_ACCSTEPS_TAC BIGNUM_MADD_EXEC [2;4;5;6] (1--7) THEN
...
let BIGNUM_MADD_NOIBT_SUBROUTINE_CORRECT = prove
 (`!p m n z x y a b c pc stackpointer returnaddress.
     ...
     ALLPAIRS nonoverlapping
              ...
              [(word pc,LENGTH bignum_madd_tmc); (x,8 * val m); (y,8 * val n)]
     ==> ensures x86
          (\s. bytes_loaded s (word pc) bignum_madd_tmc /\
               read RIP s = word pc /\
  ...
  X86_PROMOTE_RETURN_STACK_TAC bignum_madd_tmc BIGNUM_MADD_CORRECT
 ...
 (`!p m n z x y a b c pc stackpointer returnaddress.
     ...
     ==> ensures x86
          ...
          (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bignum(z,val p);
                      memory :> bytes(word_sub stackpointer (word 48),56)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MADD_NOIBT_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let bignum_madd_windows_mc = define_from_elf
   "bignum_madd_windows_mc" "x86/generic/bignum_madd.obj";;

let bignum_madd_windows_tmc = define_trimmed "bignum_madd_windows_tmc" bignum_madd_windows_mc;;

let BIGNUM_MADD_NOIBT_WINDOWS_SUBROUTINE_CORRECT = prove
 (`!p m n z x y a b c pc stackpointer returnaddress.
     nonoverlapping (z,8 * val p) (word_sub stackpointer (word 64),72) /\
     ALLPAIRS nonoverlapping
              [(z,8 * val p); (word_sub stackpointer (word 64),72)]
              [(word pc,LENGTH bignum_madd_windows_tmc); (x,8 * val m); (y,8 * val n)]
     ==> ensures x86
          (\s. bytes_loaded s (word pc) bignum_madd_windows_tmc /\
               read RIP s = word pc /\
               read RSP s = stackpointer /\
               read (memory :> bytes64 stackpointer) s = returnaddress /\
  ...
  WINDOWS_X86_WRAP_STACK_TAC bignum_madd_windows_tmc bignum_madd_tmc
    BIGNUM_MADD_CORRECT `[RBX; RBP; R12; R13; R14; R15]` 48);;

let BIGNUM_MADD_WINDOWS_SUBROUTINE_CORRECT = prove
 (`!p m n z x y a b c pc stackpointer returnaddress.
     nonoverlapping (z,8 * val p) (word_sub stackpointer (word 64),72) /\
     ALLPAIRS nonoverlapping
              [(z,8 * val p); (word_sub stackpointer (word 64),72)]
              [(word pc,LENGTH bignum_madd_windows_mc); (x,8 * val m); (y,8 * val n)]
     ==> ensures x86
          (\s. bytes_loaded s (word pc) bignum_madd_windows_mc /\
               read RIP s = word pc /\
               read RSP s = stackpointer /\
               read (memory :> bytes64 stackpointer) s = returnaddress /\
               WINDOWS_C_ARGUMENTS [p; z; m; x; n; y] s /\
          ...
          (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bignum(z,val p);
                      memory :> bytes(word_sub stackpointer (word 64),72)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MADD_NOIBT_WINDOWS_SUBROUTINE_CORRECT));;
