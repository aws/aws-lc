...
let bignum_amontsqr_mc =
  define_assert_from_elf "bignum_amontsqr_mc" "x86/generic/bignum_amontsqr.o"
[
  ...
  0x49; 0x8b; 0x04; 0xd9;  (* MOV (% rax) (Memop Quadword (%%% (r9,3,rbx))) *)
  ...
  0x0f; 0x82; 0x65; 0xff; 0xff; 0xff;
                           (* JB (Imm32 (word 4294967141)) *)
...
let bignum_amontsqr_tmc = define_trimmed "bignum_amontsqr_tmc" bignum_amontsqr_mc;;

let BIGNUM_AMONTSQR_EXEC = X86_MK_CORE_EXEC_RULE bignum_amontsqr_tmc;;
...
let BIGNUM_AMONTSQR_CORRECT = time prove
 (`!k z x m a n pc.
       ALL (nonoverlapping (z,8 * val k))
           [(word pc,0x151); (x,8 * val k); (m,8 * val k)]
      ==> ensures x86
           (\s. bytes_loaded s (word pc) (BUTLAST bignum_amontsqr_tmc) /\
                read RIP s = word (pc + 0xa) /\
                C_ARGUMENTS [k; z; x; m] s /\
                bignum_from_memory (x,val k) s = a /\
                bignum_from_memory (m,val k) s = n)
           (\s. read RIP s = word(pc + 0x146) /\
                (ODD n
  ...
  ASM_CASES_TAC `k = 0` THENL
   [UNDISCH_THEN `k = 0` SUBST_ALL_TAC THEN
    ...
    ENSURES_INIT_TAC "s0" THEN X86_STEPS_TAC BIGNUM_AMONTSQR_EXEC (1--2) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[ODD];
    ALL_TAC] THEN

  (*** Initial word-level modular inverse ***)

  ENSURES_SEQUENCE_TAC `pc + 0x71`
   `\s. read RDI s = word k /\
        read RSI s = z /\
        read R9 s = x /\
        read RCX s = m /\
  ...
  CONJ_TAC THENL
   [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN ENSURES_INIT_TAC "s0" THEN
    ...
    X86_STEPS_TAC BIGNUM_AMONTSQR_EXEC (1--9) THEN
    SUBGOAL_THEN `ODD n ==> (n * val (read R8 s9) + 1 == 0) (mod 16)`
    MP_TAC THENL [ASM_SIMP_TAC[WORD_NEGMODINV_SEED_LEMMA_16]; ALL_TAC] THEN
    REABBREV_TAC `x0 = read R8 s9` THEN DISCH_TAC THEN
    X86_STEPS_TAC BIGNUM_AMONTSQR_EXEC (10--27) THEN
  ...
  ENSURES_WHILE_UP_TAC `k:num` `pc + 0x86` `pc + 0x118`
  ...
  ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
   [ENSURES_WHILE_UP_TAC `k:num` `pc + 0x77` `pc + 0x7e`
     `\i s. read RDI s = word k /\
            read RSI s = z /\
            read R9 s = x /\
            read RCX s = m /\
    ...
    ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
     [REWRITE_TAC[BIGNUM_FROM_MEMORY_TRIVIAL] THEN
      ...
      ENSURES_INIT_TAC "s0" THEN X86_STEPS_TAC BIGNUM_AMONTSQR_EXEC (1--2) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[];
      X86_SIM_TAC BIGNUM_AMONTSQR_EXEC (1--3) THEN
    ...
    SUBGOAL_THEN `cout < 2` MP_TAC THENL
     [SUBGOAL_THEN `q * n + a * a < 2 EXP (64 * k) EXP 2 * 2` MP_TAC THENL
    ...
    ENSURES_WHILE_UP_TAC `k:num` `pc + 0x12d` `pc + 0x141`
     `\i s. read RDI s = word k /\
            read RSI s = z /\
            read RCX s = m /\
    ...
    ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
     [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
      ENSURES_INIT_TAC "s0" THEN X86_STEPS_TAC BIGNUM_AMONTSQR_EXEC (1--6) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
      ...
      X86_ACCSTEPS_TAC BIGNUM_AMONTSQR_EXEC [4] (1--6) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[GSYM WORD_ADD] THEN
   ...
   CONJ_TAC THENL
    [MATCH_MP_TAC(INT_ARITH
     ...
     UNDISCH_TAC
      `2 EXP (64 * k) * (2 EXP (64 * k) * bitval c + mm) = q * n + a * a` THEN
  ...
  ENSURES_SEQUENCE_TAC `pc + 0xbf`
   `\s. read RDI s = word k /\
        ...
        ?c1 c0. c0 < 2 EXP 64 /\
                ...
                bignum_from_memory (z,k) s =
                (2 EXP (64 * k) * cout + z1) + bigdigit a i * a` THEN
  CONJ_TAC THENL
   [ENSURES_WHILE_PUP_TAC `k:num` `pc + 0x96` `pc + 0xb7`
     `\j s. (read RDI s = word k /\
             ...
             highdigits z1 j /\
             bignum_from_memory(word_add x (word (8 * j)),k - j) s =
             highdigits a j /\
             2 EXP (64 * j) * (bitval(read CF s) + val(read R10 s)) +
             bignum_from_memory(z,j) s =
             lowdigits z1 j + bigdigit a i * lowdigits a j) /\
            (read ZF s <=> j = k)` THEN

    ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
     [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
      ENSURES_INIT_TAC "s0" THEN X86_STEPS_TAC BIGNUM_AMONTSQR_EXEC (1--5) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
      ...
      REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
      ENSURES_INIT_TAC "s0" THEN
      X86_ACCSTEPS_TAC BIGNUM_AMONTSQR_EXEC [1;4] (1--5) THEN
      RULE_ASSUM_TAC(REWRITE_RULE[WORD_RULE
       `word_sub x (word_neg y):int64 = word_add x y`]) THEN
      ACCUMULATE_ARITH_TAC "s5" THEN
      X86_ACCSTEPS_TAC BIGNUM_AMONTSQR_EXEC [6] (6--10) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[GSYM WORD_ADD] THEN
      ...
      X_GEN_TAC `j:num` THEN STRIP_TAC THEN VAL_INT64_TAC `j:num` THEN
      REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
      ENSURES_INIT_TAC "s0" THEN X86_STEPS_TAC BIGNUM_AMONTSQR_EXEC [1] THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[];

      GHOST_INTRO_TAC `cin:bool` `read CF` THEN
      GHOST_INTRO_TAC `hout:int64` `read R10` THEN
      REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN ENSURES_INIT_TAC "s0" THEN
      X86_ACCSTEPS_TAC BIGNUM_AMONTSQR_EXEC [2] (1--3) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
   ...
   [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN ENSURES_INIT_TAC "s0" THEN
    ...
    MP_TAC THENL
     [ASM_REWRITE_TAC[HIGHDIGITS_0; BIGNUM_FROM_MEMORY_BYTES];
      ...
      ASM_REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES; ADD_CLAUSES] THEN
      STRIP_TAC] THEN
    X86_ACCSTEPS_TAC BIGNUM_AMONTSQR_EXEC [5; 6] (1--9) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
    MATCH_MP_TAC(TAUT `p /\ (p ==> q) ==> p /\ q`) THEN CONJ_TAC THENL
  ...
  CONJ_TAC THENL
   [ASM_CASES_TAC `k = 1` THENL
    ...
    ENSURES_WHILE_PAUP_TAC `1` `k:num` `pc + 0xe2` `pc + 0x104`
     `\j s. (read RDI s = word k /\
             read RSI s = z /\
             read R9 s = x /\
             read RCX s = m /\
  ...
  X86_ACCSTEPS_TAC BIGNUM_AMONTSQR_EXEC [1;2] (1--5) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  CONJ_TAC THENL [CONV_TAC WORD_RULE; ALL_TAC] THEN
...
let BIGNUM_AMONTSQR_NOIBT_SUBROUTINE_CORRECT = time prove
 (`!k z x m a n pc stackpointer returnaddress.
       ...
       ALLPAIRS nonoverlapping
           ...
           [(word pc,LENGTH bignum_amontsqr_tmc); (x,8 * val k); (m,8 * val k)]
      ==> ensures x86
           (\s. bytes_loaded s (word pc) bignum_amontsqr_tmc /\
                read RIP s = word pc /\
  ...
  X86_PROMOTE_RETURN_STACK_TAC bignum_amontsqr_tmc BIGNUM_AMONTSQR_CORRECT
   `[RBX; RBP; R12; R13; R14; R15]` 48);;

let BIGNUM_AMONTSQR_SUBROUTINE_CORRECT = time prove
 (`!k z x m a n pc stackpointer returnaddress.
       nonoverlapping (z,8 * val k) (word_sub stackpointer (word 48),56) /\
       ALLPAIRS nonoverlapping
           [(z,8 * val k); (word_sub stackpointer (word 48),48)]
           [(word pc,LENGTH bignum_amontsqr_mc); (x,8 * val k); (m,8 * val k)]
      ==> ensures x86
           (\s. bytes_loaded s (word pc) bignum_amontsqr_mc /\
                read RIP s = word pc /\
                read RSP s = stackpointer /\
                read (memory :> bytes64 stackpointer) s = returnaddress /\
                C_ARGUMENTS [k; z; x; m] s /\
           ...
           (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
            MAYCHANGE [memory :> bytes(z,8 * val k);
                       memory :> bytes(word_sub stackpointer (word 48),48)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_AMONTSQR_NOIBT_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let bignum_amontsqr_windows_mc = define_from_elf
   "bignum_amontsqr_windows_mc" "x86/generic/bignum_amontsqr.obj";;

let bignum_amontsqr_windows_tmc = define_trimmed "bignum_amontsqr_windows_tmc" bignum_amontsqr_windows_mc;;

let BIGNUM_AMONTSQR_NOIBT_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!k z x m a n pc stackpointer returnaddress.
       nonoverlapping (z,8 * val k) (word_sub stackpointer (word 64),72) /\
       ALLPAIRS nonoverlapping
           [(z,8 * val k); (word_sub stackpointer (word 64),64)]
           [(word pc,LENGTH bignum_amontsqr_windows_tmc); (x,8 * val k); (m,8 * val k)]
      ==> ensures x86
           (\s. bytes_loaded s (word pc) bignum_amontsqr_windows_tmc /\
                read RIP s = word pc /\
                read RSP s = stackpointer /\
                read (memory :> bytes64 stackpointer) s = returnaddress /\
  ...
  WINDOWS_X86_WRAP_STACK_TAC bignum_amontsqr_windows_tmc bignum_amontsqr_tmc
   BIGNUM_AMONTSQR_CORRECT `[RBX; RBP; R12; R13; R14; R15]` 48);;

let BIGNUM_AMONTSQR_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!k z x m a n pc stackpointer returnaddress.
       nonoverlapping (z,8 * val k) (word_sub stackpointer (word 64),72) /\
       ALLPAIRS nonoverlapping
           [(z,8 * val k); (word_sub stackpointer (word 64),64)]
           [(word pc,LENGTH bignum_amontsqr_windows_mc); (x,8 * val k); (m,8 * val k)]
      ==> ensures x86
           (\s. bytes_loaded s (word pc) bignum_amontsqr_windows_mc /\
                read RIP s = word pc /\
                read RSP s = stackpointer /\
                read (memory :> bytes64 stackpointer) s = returnaddress /\
                WINDOWS_C_ARGUMENTS [k; z; x; m] s /\
           ...
           (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
            MAYCHANGE [memory :> bytes(z,8 * val k);
                       memory :> bytes(word_sub stackpointer (word 64),64)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_AMONTSQR_NOIBT_WINDOWS_SUBROUTINE_CORRECT));;
