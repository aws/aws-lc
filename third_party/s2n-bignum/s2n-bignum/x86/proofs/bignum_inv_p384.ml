...
(* ========================================================================= *)
(* Inversion modulo p_384, the field characteristic for NIST P-384.          *)
(* ========================================================================= *)
...
(**** print_literal_from_elf "x86/p384/bignum_inv_p384.o";;
 ****)

let bignum_inv_p384_mc = define_assert_from_elf "bignum_inv_p384_mc" "x86/p384/bignum_inv_p384.o"
[
...
let bignum_inv_p384_tmc = define_trimmed "bignum_inv_p384_tmc" bignum_inv_p384_mc;;

let BIGNUM_INV_P384_EXEC = X86_MK_CORE_EXEC_RULE bignum_inv_p384_tmc;;
...
let CORE_INV_P384_EXEC =
    X86_MK_EXEC_RULE
     ((GEN_REWRITE_CONV RAND_CONV [bignum_inv_p384_tmc] THENC TRIM_LIST_CONV)
      `TRIM_LIST (17,18) bignum_inv_p384_tmc`);;
...
let LOCAL_WORD_DIVSTEP59_CORRECT = prove
 (`!d f g stackpointer pc.
   nonoverlapping (stackpointer,336) (word pc,0x1991)
   ==> ensures x86
        (\s. bytes_loaded s (word pc)
                  (TRIM_LIST (17,18) bignum_inv_p384_tmc) /\
             read RIP s = word(pc + 0x8db) /\
...
let CORE_INV_P384_CORRECT = time prove
 (`!z x n pc stackpointer.
        ALL (nonoverlapping (stackpointer,336))
            [(word pc,0x1991); (z,8 * 6); (x,8 * 6)] /\
        nonoverlapping (word pc,0x1991) (z,8 * 6)
        ==> ensures x86
             (\s. bytes_loaded s (word pc)
                    (TRIM_LIST (17,18) bignum_inv_p384_tmc) /\
                  read RIP s = word pc /\
             ...
             (\s. read RIP s = word (pc + 0x1991) /\
                  bignum_from_memory(z,6) s =
                  (if p_384 divides n then 0 else inverse_mod p_384 n))
          (MAYCHANGE [RIP; RDI; RSI; RAX; RBX; RCX; RDX; RBP;
                      R8; R9; R10; R11; R12; R13; R14; R15] ,,
           MAYCHANGE SOME_FLAGS ,,
  ...
  ENSURES_WHILE_UP_TAC `14` `pc + 0x8db` `pc + 0x8db`
   `\i s.
      read RSP s = stackpointer /\
      ...
      (&2 pow (5 * (15 - i)) * --(&1) pow i * divstep_f (59 * i) t ==
       &n * &(read (memory :> bytes(word_add stackpointer (word 128),8 * 6)) s))
      (mod (&p_384)) /\
      (&2 pow (5 * (15 - i)) * --(&1) pow i * divstep_g (59 * i) t ==
   ...
   [ARITH_TAC;
    ...
    THENL
     [REPEAT GEN_TAC THEN MATCH_MP_TAC(INTEGER_RULE
      ...
      ALL_TAC] THEN
    SUBGOAL_THEN
     `!x:int. &2 pow 64 * &2 pow (5 * (15 - (i + 1))) * x =
              &2 pow (5 * (15 - i)) * &2 pow 59 * x`
    (fun th -> REWRITE_TAC[th]) THENL
      [ASM_SIMP_TAC[INT_POW_ADD; ARITH_RULE
        `i < 14 ==> 5 * (15 - i) = 5 * (15 - (i + 1)) + 5`] THEN
       INT_ARITH_TAC;
  ...
  REWRITE_TAC[p_384] THEN CONJ_TAC THENL [ALL_TAC; ARITH_TAC] THEN
  REWRITE_TAC[GSYM p_384; CONG] THEN CONV_TAC MOD_DOWN_CONV THEN
  ...
  SUBST1_TAC THENL
   [SUBST1_TAC(SYM(ASSUME `--((mm:int^2^2)$1$1) = m00`)) THEN
    ...
    ALL_TAC] THEN
  MATCH_MP_TAC(INTEGER_RULE
   `(e * -- &1 pow 14 * f:int == n * u) (mod p) /\
    (e * -- &1 pow 14 * g:int == n * v) (mod p)
    ==> (e * (a * f + b * g) == n * (a * u + b * v)) (mod p)`) THEN
  RULE_ASSUM_TAC(REWRITE_RULE[MULT_CLAUSES]) THEN ASM_REWRITE_TAC[]);;

let BIGNUM_INV_P384_CORRECT = time prove
 (`!z x n pc stackpointer.
        ALL (nonoverlapping (stackpointer,336))
            [(word pc,0x19b4); (z,8 * 6); (x,8 * 6)] /\
        nonoverlapping (word pc,0x19b4) (z,8 * 6)
        ==> ensures x86
             (\s. bytes_loaded s (word pc) (BUTLAST bignum_inv_p384_tmc) /\
                  read RIP s = word(pc + 0x11) /\
                  read RSP s = stackpointer /\
                  C_ARGUMENTS [z; x] s /\
                  bignum_from_memory (x,6) s = n)
             (\s. read RIP s = word (pc + 0x19a2) /\
                  bignum_from_memory(z,6) s =
                  (if p_384 divides n then 0 else inverse_mod p_384 n))
          (MAYCHANGE [RIP; RDI; RSI; RAX; RBX; RCX; RDX; RBP;
                      R8; R9; R10; R11; R12; R13; R14; R15] ,,
  ...
  X86_SUBROUTINE_SIM_TAC
      ((GEN_REWRITE_CONV RAND_CONV [bignum_inv_p384_tmc] THENC
        REWRITE_CONV[BUTLAST_CLAUSES])
       `BUTLAST bignum_inv_p384_tmc`,
       BIGNUM_INV_P384_EXEC,
       0x11,
       (GEN_REWRITE_CONV RAND_CONV [bignum_inv_p384_tmc] THENC TRIM_LIST_CONV)
       `TRIM_LIST (17,18) bignum_inv_p384_tmc`,
...
let BIGNUM_INV_P384_NOIBT_SUBROUTINE_CORRECT = time prove
 (`!z x n pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 384),384))
            [(word pc,LENGTH bignum_inv_p384_tmc); (x,8 * 6)] /\
        ALL (nonoverlapping (z,8 * 6))
            [(word_sub stackpointer (word 384),392); (word pc,LENGTH bignum_inv_p384_tmc)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_inv_p384_tmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
  ...
  X86_PROMOTE_RETURN_STACK_TAC bignum_inv_p384_tmc
     BIGNUM_INV_P384_CORRECT
      `[RBX; RBP; R12; R13; R14; R15]` 384);;

let BIGNUM_INV_P384_SUBROUTINE_CORRECT = time prove
 (`!z x n pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 384),384))
            [(word pc,LENGTH bignum_inv_p384_mc); (x,8 * 6)] /\
        ALL (nonoverlapping (z,8 * 6))
            [(word_sub stackpointer (word 384),392); (word pc,LENGTH bignum_inv_p384_mc)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_inv_p384_mc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  C_ARGUMENTS [z; x] s /\
             ...
             (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * 6);
                    memory :> bytes(word_sub stackpointer (word 384),384)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_INV_P384_NOIBT_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let bignum_inv_p384_windows_mc = define_from_elf "bignum_inv_p384_windows_mc"
      "x86/p384/bignum_inv_p384.obj";;

let bignum_inv_p384_windows_tmc = define_trimmed "bignum_inv_p384_windows_tmc" bignum_inv_p384_windows_mc;;

let BIGNUM_INV_P384_NOIBT_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!z x n pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 400),400))
            [(word pc,LENGTH bignum_inv_p384_windows_tmc); (x,8 * 6)] /\
        ALL (nonoverlapping (z,8 * 6))
            [(word_sub stackpointer (word 400),408); (word pc,LENGTH bignum_inv_p384_windows_tmc)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_inv_p384_windows_tmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
  ...
  WINDOWS_X86_WRAP_STACK_TAC bignum_inv_p384_windows_tmc bignum_inv_p384_tmc
     BIGNUM_INV_P384_CORRECT
      `[RBX; RBP; R12; R13; R14; R15]` 384);;

let BIGNUM_INV_P384_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!z x n pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 400),400))
            [(word pc,LENGTH bignum_inv_p384_windows_mc); (x,8 * 6)] /\
        ALL (nonoverlapping (z,8 * 6))
            [(word_sub stackpointer (word 400),408); (word pc,LENGTH bignum_inv_p384_windows_mc)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_inv_p384_windows_mc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  WINDOWS_C_ARGUMENTS [z; x] s /\
             ...
             (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * 6);
                    memory :> bytes(word_sub stackpointer (word 400),400)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_INV_P384_NOIBT_WINDOWS_SUBROUTINE_CORRECT));;
