...
let bignum_deamont_p384_mc =
  define_assert_from_elf "bignum_deamont_p384_mc" "x86/p384/bignum_deamont_p384.o"
[
...
let bignum_deamont_p384_tmc = define_trimmed "bignum_deamont_p384_tmc" bignum_deamont_p384_mc;;

let BIGNUM_DEAMONT_P384_EXEC = X86_MK_CORE_EXEC_RULE bignum_deamont_p384_tmc;;
...
let BIGNUM_DEAMONT_P384_CORRECT = time prove
 (`!z x a pc.
        ...
        ==> ensures x86
             (\s. bytes_loaded s (word pc) (BUTLAST bignum_deamont_p384_tmc) /\
                  read RIP s = word(pc + 0x04) /\
                  C_ARGUMENTS [z; x] s /\
                  bignum_from_memory (x,6) s = a)
             (\s. read RIP s = word (pc + 0x23b) /\
  ...
  MAP_EVERY (fun s ->
    X86_SINGLE_STEP_TAC BIGNUM_DEAMONT_P384_EXEC s THEN
 ...
 (*** Do the congruential reasoning on the chosen multipliers ***)
  ...
  ABBREV_TAC
   `dd = bignum_of_wordlist
          [sum_s108; sum_s109; sum_s110; sum_s111; sum_s112; sum_s114]` THEN
  ...
  X86_ACCSTEPS_TAC BIGNUM_DEAMONT_P384_EXEC
   [118;120;122;124;126;128; 133;134;135;136;137;138] (115--144) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN

  SUBGOAL_THEN `carry_s128 <=> p_384 <= dd` SUBST_ALL_TAC THENL
  ...
  ASM_REWRITE_TAC[] THEN DISCARD_STATE_TAC "s144" THEN
...
let BIGNUM_DEAMONT_P384_NOIBT_SUBROUTINE_CORRECT = time prove
 (`!z x a pc stackpointer returnaddress.
        nonoverlapping (z,8 * 6) (word_sub stackpointer (word 16),24) /\
        ALL (nonoverlapping (word_sub stackpointer (word 16),16))
            [(word pc,LENGTH bignum_deamont_p384_tmc); (x,8 * 6)] /\
        nonoverlapping (word pc,LENGTH bignum_deamont_p384_tmc) (z,8 * 6)
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_deamont_p384_tmc /\
                  read RIP s = word pc /\
  ...
  X86_PROMOTE_RETURN_STACK_TAC
   bignum_deamont_p384_tmc BIGNUM_DEAMONT_P384_CORRECT
   `[R12; R13]` 16);;
...
let BIGNUM_DEAMONT_P384_SUBROUTINE_CORRECT = time prove
 (`!z x a pc stackpointer returnaddress.
        nonoverlapping (z,8 * 6) (word_sub stackpointer (word 16),24) /\
        ALL (nonoverlapping (word_sub stackpointer (word 16),16))
            [(word pc,LENGTH bignum_deamont_p384_mc); (x,8 * 6)] /\
        nonoverlapping (word pc,LENGTH bignum_deamont_p384_mc) (z,8 * 6)
        ==> ensures x86
             ...
             (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * 6);
                     memory :> bytes(word_sub stackpointer (word 16),16)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_DEAMONT_P384_NOIBT_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let bignum_deamont_p384_windows_mc = define_from_elf
   "bignum_deamont_p384_windows_mc" "x86/p384/bignum_deamont_p384.obj";;

let bignum_deamont_p384_windows_tmc = define_trimmed "bignum_deamont_p384_windows_tmc" bignum_deamont_p384_windows_mc;;

let BIGNUM_DEAMONT_P384_NOIBT_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!z x a pc stackpointer returnaddress.
        nonoverlapping (z,8 * 6) (word_sub stackpointer (word 32),40) /\
        ALL (nonoverlapping (word_sub stackpointer (word 32),32))
            [(word pc,LENGTH bignum_deamont_p384_windows_tmc); (x,8 * 6)] /\
        nonoverlapping (word pc,LENGTH bignum_deamont_p384_windows_tmc) (z,8 * 6)
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_deamont_p384_windows_tmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
  ...
  WINDOWS_X86_WRAP_STACK_TAC
   bignum_deamont_p384_windows_tmc bignum_deamont_p384_tmc
   BIGNUM_DEAMONT_P384_CORRECT `[R12; R13]` 16);;

let BIGNUM_DEAMONT_P384_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!z x a pc stackpointer returnaddress.
        nonoverlapping (z,8 * 6) (word_sub stackpointer (word 32),40) /\
        ALL (nonoverlapping (word_sub stackpointer (word 32),32))
            [(word pc,LENGTH bignum_deamont_p384_windows_mc); (x,8 * 6)] /\
        nonoverlapping (word pc,LENGTH bignum_deamont_p384_windows_mc) (z,8 * 6)
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_deamont_p384_windows_mc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  WINDOWS_C_ARGUMENTS [z; x] s /\
             ...
             (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * 6);
                     memory :> bytes(word_sub stackpointer (word 32),32)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_DEAMONT_P384_NOIBT_WINDOWS_SUBROUTINE_CORRECT));;
