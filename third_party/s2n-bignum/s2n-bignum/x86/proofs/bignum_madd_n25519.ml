...
let bignum_madd_n25519_tmc = define_trimmed "bignum_madd_n25519_tmc" bignum_madd_n25519_mc;;

let BIGNUM_MADD_N25519_EXEC = X86_MK_CORE_EXEC_RULE bignum_madd_n25519_tmc;;
...
let BIGNUM_MADD_N25519_CORRECT = time prove
 (`!z x m y n c r pc.
      nonoverlapping (word pc,0x407) (z,8 * 4)
      ==> ensures x86
           (\s. bytes_loaded s (word pc) (BUTLAST bignum_madd_n25519_tmc) /\
                read RIP s = word(pc + 0xa) /\
                C_ARGUMENTS [z; x; y; c] s /\
                bignum_from_memory (x,4) s = m /\
                bignum_from_memory (y,4) s = n /\
                bignum_from_memory (c,4) s = r)
           (\s. read RIP s = word (pc + 0x3fc) /\
  ...
  CONJ_TAC THENL
   [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN ENSURES_INIT_TAC "s0" THEN
    ...
    X86_ACCSTEPS_TAC BIGNUM_MADD_N25519_EXEC (1--65) (1--65) THEN
  ...
  X86_ACCSTEPS_TAC BIGNUM_MADD_N25519_EXEC
...
let BIGNUM_MADD_N25519_NOIBT_SUBROUTINE_CORRECT = time prove
 (`!z x m y n c r pc stackpointer returnaddress.
      ...
      ==> ensures x86
           (\s. bytes_loaded s (word pc) bignum_madd_n25519_tmc /\
                read RIP s = word pc /\
  ...
  X86_PROMOTE_RETURN_STACK_TAC
    bignum_madd_n25519_tmc BIGNUM_MADD_N25519_CORRECT
...
let BIGNUM_MADD_N25519_SUBROUTINE_CORRECT = time prove
 (`!z x m y n c r pc stackpointer returnaddress.
      ...
      ==> ensures x86
           (\s. bytes_loaded s (word pc) bignum_madd_n25519_mc /\
                read RIP s = word pc /\
                read RSP s = stackpointer /\
                read (memory :> bytes64 stackpointer) s = returnaddress /\
                C_ARGUMENTS [z; x; y; c] s /\
          ...
          (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bignum(z,4);
                      memory :> bytes(word_sub stackpointer (word 48),48)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MADD_N25519_NOIBT_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let bignum_madd_n25519_windows_mc = define_from_elf
   "bignum_madd_n25519_windows_mc" "x86/curve25519/bignum_madd_n25519.obj";;

let bignum_madd_n25519_windows_tmc = define_trimmed "bignum_madd_n25519_windows_tmc" bignum_madd_n25519_windows_mc;;

let BIGNUM_MADD_N25519_NOIBT_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!z x m y n c r pc stackpointer returnaddress.
      nonoverlapping (word_sub stackpointer (word 64),72) (z,8 * 4) /\
      ALL (nonoverlapping (word_sub stackpointer (word 64),64))
          [(word pc,LENGTH bignum_madd_n25519_windows_tmc); (x,8 * 4); (y,8 * 4); (c,8 * 4)] /\
      nonoverlapping (word pc,LENGTH bignum_madd_n25519_windows_tmc) (z,8 * 4)
      ==> ensures x86
           (\s. bytes_loaded s (word pc) bignum_madd_n25519_windows_tmc /\
                read RIP s = word pc /\
                read RSP s = stackpointer /\
                read (memory :> bytes64 stackpointer) s = returnaddress /\
  ...
  WINDOWS_X86_WRAP_STACK_TAC
    ...
    BIGNUM_MADD_N25519_CORRECT `[RBX;RBP;R12;R13;R14;R15]` 48);;

let BIGNUM_MADD_N25519_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!z x m y n c r pc stackpointer returnaddress.
      nonoverlapping (word_sub stackpointer (word 64),72) (z,8 * 4) /\
      ALL (nonoverlapping (word_sub stackpointer (word 64),64))
          [(word pc,LENGTH bignum_madd_n25519_windows_mc); (x,8 * 4); (y,8 * 4); (c,8 * 4)] /\
      nonoverlapping (word pc,LENGTH bignum_madd_n25519_windows_mc) (z,8 * 4)
      ==> ensures x86
           (\s. bytes_loaded s (word pc) bignum_madd_n25519_windows_mc /\
                read RIP s = word pc /\
                read RSP s = stackpointer /\
                read (memory :> bytes64 stackpointer) s = returnaddress /\
                WINDOWS_C_ARGUMENTS [z; x; y; c] s /\
