...
let bignum_nonzero_6_mc =
  define_assert_from_elf "bignum_nonzero_6_mc" "arm/p384/bignum_nonzero_6.o"
[
  ...
  0xa9420803;       (* arm_LDP X3 X2 X0 (Immediate_Offset (iword (&32))) *)
  0xaa020063;       (* arm_ORR X3 X3 X2 *)
  0xaa030021;       (* arm_ORR X1 X1 X3 *)
...
let BIGNUM_NONZERO_6_EXEC = ARM_MK_EXEC_RULE bignum_nonzero_6_mc;;
...
let BIGNUM_NONZERO_6_CORRECT = prove
 (`!x n pc.
        ensures arm
          (\s. aligned_bytes_loaded s (word pc) bignum_nonzero_6_mc /\
               read PC s = word pc /\
               C_ARGUMENTS [x] s /\
  ...
  ARM_STEPS_TAC BIGNUM_NONZERO_6_EXEC (1--10) THEN
