...
(**** print_literal_from_elf "arm/p384/bignum_inv_p384.o";;
 ****)

let bignum_inv_p384_mc = define_assert_from_elf "bignum_inv_p384_mc" "arm/p384/bignum_inv_p384.o"
[
...
let BIGNUM_INV_P384_EXEC = ARM_MK_EXEC_RULE bignum_inv_p384_mc;;
...
let CORE_INV_P384_EXEC =
    ARM_MK_EXEC_RULE
     ((GEN_REWRITE_CONV RAND_CONV [bignum_inv_p384_mc] THENC TRIM_LIST_CONV)
      `TRIM_LIST (16,20) bignum_inv_p384_mc`);;
 ...
 (`!d f g pc.
       ensures arm
        (\s. aligned_bytes_loaded s (word pc)
                 (TRIM_LIST (16,20) bignum_inv_p384_mc) /\
             read PC s = word(pc + 0x784) /\
             read X1 s = d /\
             read X2 s = f /\
             read X3 s = g)
        (\s. read PC s = word(pc + 0x10f8) /\
             (ival d rem &2 = &1 /\
              ival f rem &2 = &1 /\
              abs(ival d) < &2 pow 62 ==>
...
let CORE_INV_P384_CORRECT = time prove
 (`!z x n pc stackpointer.
        ...
        ALL (nonoverlapping (stackpointer,256))
            [(word pc,0x13ac); (z,8 * 6); (x,8 * 6)] /\
        nonoverlapping (word pc,0x13ac) (z,8 * 6)
        ==> ensures arm
             (\s. aligned_bytes_loaded s (word pc)
                   (TRIM_LIST (16,20) bignum_inv_p384_mc) /\
                  read PC s = word pc /\
  ...
  ENSURES_WHILE_UP_TAC `14` `pc + 0x778` `pc + 0x778`
   `\i s.
      read SP s = stackpointer /\
      read X20 s = z /\
      ...
      (&2 pow (5 * (15 - i)) * divstep_f (59 * i) t ==
       &n * &(read (memory :> bytes(word_add stackpointer (word 128),8 * 6)) s))
      (mod (&p_384)) /\
      (&2 pow (5 * (15 - i)) * divstep_g (59 * i) t ==
...
let BIGNUM_INV_P384_CORRECT = time prove
 (`!z x n pc stackpointer.
        aligned 16 stackpointer /\
        ALL (nonoverlapping (stackpointer,256))
            [(word pc,0x13d0); (z,8 * 6); (x,8 * 6)] /\
        nonoverlapping (word pc,0x13d0) (z,8 * 6)
        ==> ensures arm
             (\s. aligned_bytes_loaded s (word pc) bignum_inv_p384_mc /\
  ...
  ARM_SUBROUTINE_SIM_TAC
      (bignum_inv_p384_mc,BIGNUM_INV_P384_EXEC,0x10,
       (GEN_REWRITE_CONV RAND_CONV [bignum_inv_p384_mc] THENC TRIM_LIST_CONV)
       `TRIM_LIST (16,20) bignum_inv_p384_mc`,
...
let BIGNUM_INV_P384_SUBROUTINE_CORRECT = time prove
 (`!z x n pc stackpointer returnaddress.
  ...
  ARM_ADD_RETURN_STACK_TAC
   BIGNUM_INV_P384_EXEC BIGNUM_INV_P384_CORRECT
