...
(**** print_literal_from_elf "x86/p256/bignum_inv_p256.o";;
 ****)

let bignum_inv_p256_mc = define_assert_from_elf "bignum_inv_p256_mc" "x86/p256/bignum_inv_p256.o"
[
...
let bignum_inv_p256_tmc = define_trimmed "bignum_inv_p256_tmc" bignum_inv_p256_mc;;

let BIGNUM_INV_P256_EXEC = X86_MK_CORE_EXEC_RULE bignum_inv_p256_tmc;;
...
let CORE_INV_P256_EXEC =
    X86_MK_EXEC_RULE
     ((GEN_REWRITE_CONV RAND_CONV [bignum_inv_p256_tmc] THENC TRIM_LIST_CONV)
      `TRIM_LIST (17,18) bignum_inv_p256_tmc`);;
...
let PACKED_DIVSTEP_TAC n shf s =
  ...
  X86_STEPS_TAC CORE_INV_P256_EXEC (s--s') THEN
  SUBGOAL_THEN
...
let LOCAL_WORD_DIVSTEP59_CORRECT = prove
 (`!d f g stackpointer pc.
   nonoverlapping (stackpointer,240) (word pc,0x154c)
   ==> ensures x86
        (\s. bytes_loaded s (word pc)
                  (TRIM_LIST (17,18) bignum_inv_p256_tmc) /\
             read RIP s = word(pc + 0x5ed) /\
  ...
  ASM_CASES_TAC `d rem &2 = &1 /\ f rem &2 = &1 /\ abs(d:int) < &2 pow 62` THENL
   [FIRST_X_ASSUM(CONJUNCTS_THEN STRIP_ASSUME_TAC) THEN ASM_REWRITE_TAC[];
    X86_QUICKSIM_TAC CORE_INV_P256_EXEC [`read RSP s = x`] (1--898)] THEN

  (*** The first packing ***)

  ENSURES_INIT_TAC "s0" THEN
  X86_STEPS_TAC CORE_INV_P256_EXEC (1--10) THEN
  ...
  X86_STEPS_TAC CORE_INV_P256_EXEC (11--20) THEN
  MAP_EVERY (fun n -> PACKED_DIVSTEP_TAC n [20] (13*n+8)) (1--20) THEN

  (*** The unpacking of the first block ***)

  X86_STEPS_TAC CORE_INV_P256_EXEC (276--291) THEN
  ...
  X86_STEPS_TAC CORE_INV_P256_EXEC (292--301) THEN
  ...
  X86_STEPS_TAC CORE_INV_P256_EXEC (302--309) THEN
  RULE_ASSUM_TAC(REWRITE_RULE[DIVSTEP_FGVEC_PACK]) THEN
  FIRST_X_ASSUM(MP_TAC o SPEC `iword(divstep_dvec 0 (d,f,g)):int64` o
    MATCH_MP(MESON[] `read RSI s = a ==> !x. a = x ==> read RSI s = x`)) THEN
  ANTS_TAC THENL [REWRITE_TAC[divstep_dvec; DIVSTEP_D]; DISCH_TAC] THEN
  ...
  X86_STEPS_TAC CORE_INV_P256_EXEC (586--595) THEN
  RULE_ASSUM_TAC(REWRITE_RULE[word_ishr]) THEN
  ...
  X86_STEPS_TAC CORE_INV_P256_EXEC (596--603) THEN
  RULE_ASSUM_TAC(REWRITE_RULE[DIVSTEP_FGVEC_PACK]) THEN
  FIRST_X_ASSUM(MP_TAC o SPEC `iword(divstep_dvec 0 (d,f,g)):int64` o
    MATCH_MP(MESON[] `read RSI s = a ==> !x. a = x ==> read RSI s = x`)) THEN
  ANTS_TAC THENL [REWRITE_TAC[divstep_dvec; DIVSTEP_D]; DISCH_TAC] THEN
  ...
  X86_STEPS_TAC CORE_INV_P256_EXEC (883--898) THEN
...
let CORE_INV_P256_CORRECT = time prove
 (`!z x n pc stackpointer.
        ALL (nonoverlapping (stackpointer,240))
            [(word pc,0x154c); (z,8 * 4); (x,8 * 4)] /\
        nonoverlapping (word pc,0x154c) (z,8 * 4)
        ==> ensures x86
             (\s. bytes_loaded s (word pc)
                    (TRIM_LIST (17,18) bignum_inv_p256_tmc) /\
                  read RIP s = word pc /\
  ...
  ENSURES_WHILE_UP_TAC `9` `pc + 0x5ed` `pc + 0x5ed`
   `\i s.
      read RSP s = stackpointer /\
      ...
      (&2 pow (5 * (10 - i)) * --(&1) pow i * divstep_f (59 * i) t ==
       &n * &(read (memory :> bytes(word_add stackpointer (word 80),8 * 4)) s))
      (mod (&p_256)) /\
      (&2 pow (5 * (10 - i)) * --(&1) pow i * divstep_g (59 * i) t ==
  ...
  THEN REPEAT CONJ_TAC THENL
   [ARITH_TAC;
    ...
    X86_ACCSTEPS_TAC CORE_INV_P256_EXEC
     [17;19;21;23] (1--46) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
    ...
    X86_BIGSTEP_TAC CORE_INV_P256_EXEC "s1" THEN
    FIRST_X_ASSUM(MP_TAC o check (is_imp o concl)) THEN
    ...
    X86_STEPS_TAC CORE_INV_P256_EXEC (2--4) THEN
    SUBGOAL_THEN
    ...
    X86_STEPS_TAC CORE_INV_P256_EXEC (5--20) THEN
    ...
    X86_ACCSTEPS_TAC CORE_INV_P256_EXEC [25;31] (21--32) THEN
    ...
    X86_ACCSTEPS_TAC CORE_INV_P256_EXEC [266;268;270;272;274] (265--274) THEN
    ABBREV_TAC
     `upos =
    ...
    X86_ACCSTEPS_TAC CORE_INV_P256_EXEC [298;300;302;304;306] (297--306) THEN
    ABBREV_TAC
     `vpos =
    ...
    X_GEN_TAC `i:num` THEN STRIP_TAC THEN
    X86_SIM_TAC CORE_INV_P256_EXEC [] THEN ASM_REWRITE_TAC[];
  ...
  SUBST1_TAC THENL
   [ASM_REWRITE_TAC[] THEN
    ...
    ALL_TAC] THEN
  MATCH_MP_TAC(INTEGER_RULE
   `(e * -- &1 pow 9 * f:int == n * u) (mod p) /\
    (e * -- &1 pow 9 * g:int == n * v) (mod p)
    ==> (--e * (a * f + b * g) == n * (a * u + b * v)) (mod p)`) THEN
  RULE_ASSUM_TAC(REWRITE_RULE[MULT_CLAUSES]) THEN ASM_REWRITE_TAC[]);;

let BIGNUM_INV_P256_CORRECT = time prove
 (`!z x n pc stackpointer.
        ALL (nonoverlapping (stackpointer,240))
            [(word pc,0x156f); (z,8 * 4); (x,8 * 4)] /\
        nonoverlapping (word pc,0x156f) (z,8 * 4)
        ==> ensures x86
             (\s. bytes_loaded s (word pc) (BUTLAST bignum_inv_p256_tmc) /\
                  read RIP s = word(pc + 0x11) /\
                  read RSP s = stackpointer /\
                  C_ARGUMENTS [z; x] s /\
                  bignum_from_memory (x,4) s = n)
             (\s. read RIP s = word (pc + 0x155d) /\
                  bignum_from_memory(z,4) s =
                  (if p_256 divides n then 0 else inverse_mod p_256 n))
          (MAYCHANGE [RIP; RDI; RSI; RAX; RBX; RCX; RDX; RBP;
                      R8; R9; R10; R11; R12; R13; R14; R15] ,,
  ...
  X86_SUBROUTINE_SIM_TAC
      ((GEN_REWRITE_CONV RAND_CONV [bignum_inv_p256_tmc] THENC
        REWRITE_CONV[BUTLAST_CLAUSES])
       `BUTLAST bignum_inv_p256_tmc`,
...
let BIGNUM_INV_P256_NOIBT_SUBROUTINE_CORRECT = time prove
 (`!z x n pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 288),288))
            [(word pc,LENGTH bignum_inv_p256_tmc); (x,8 * 4)] /\
        ALL (nonoverlapping (z,8 * 4))
            [(word_sub stackpointer (word 288),296); (word pc,LENGTH bignum_inv_p256_tmc)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_inv_p256_tmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
...
let BIGNUM_INV_P256_SUBROUTINE_CORRECT = time prove
 (`!z x n pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 288),288))
            [(word pc,LENGTH bignum_inv_p256_mc); (x,8 * 4)] /\
        ALL (nonoverlapping (z,8 * 4))
            [(word_sub stackpointer (word 288),296); (word pc,LENGTH bignum_inv_p256_mc)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_inv_p256_mc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  C_ARGUMENTS [z; x] s /\
             ...
             (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * 4);
                    memory :> bytes(word_sub stackpointer (word 288),288)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_INV_P256_NOIBT_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let bignum_inv_p256_windows_mc = define_from_elf "bignum_inv_p256_windows_mc"
      "x86/p256/bignum_inv_p256.obj";;

let bignum_inv_p256_windows_tmc = define_trimmed "bignum_inv_p256_windows_tmc" bignum_inv_p256_windows_mc;;

let BIGNUM_INV_P256_NOIBT_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!z x n pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 304),304))
            [(word pc,LENGTH bignum_inv_p256_windows_tmc); (x,8 * 4)] /\
        ALL (nonoverlapping (z,8 * 4))
            [(word_sub stackpointer (word 304),312); (word pc,LENGTH bignum_inv_p256_windows_tmc)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_inv_p256_windows_tmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
  ...
  WINDOWS_X86_WRAP_STACK_TAC bignum_inv_p256_windows_tmc bignum_inv_p256_tmc
     BIGNUM_INV_P256_CORRECT
      `[RBX; RBP; R12; R13; R14; R15]` 288);;

let BIGNUM_INV_P256_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!z x n pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 304),304))
            [(word pc,LENGTH bignum_inv_p256_windows_mc); (x,8 * 4)] /\
        ALL (nonoverlapping (z,8 * 4))
            [(word_sub stackpointer (word 304),312); (word pc,LENGTH bignum_inv_p256_windows_mc)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_inv_p256_windows_mc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  WINDOWS_C_ARGUMENTS [z; x] s /\
             ...
             (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * 4);
                    memory :> bytes(word_sub stackpointer (word 304),304)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_INV_P256_NOIBT_WINDOWS_SUBROUTINE_CORRECT));;
