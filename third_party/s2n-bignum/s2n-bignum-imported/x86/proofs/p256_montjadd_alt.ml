...
let p256_montjadd_alt_tmc = define_trimmed "p256_montjadd_alt_tmc" p256_montjadd_alt_mc;;

let P256_MONTJADD_ALT_EXEC = X86_MK_CORE_EXEC_RULE p256_montjadd_alt_tmc;;
...
let LOCAL_MONTSQR_P256_TAC =
  X86_MACRO_SIM_ABBREV_TAC (X86_TRIM_EXEC_RULE p256_montjadd_alt_tmc) 137 lvs
  `!(t:x86state) pcin pcout p3 n3 p1 n1.
    !a. read(memory :> bytes(word_add (read p1 t) (word n1),8 * 4)) t = a
    ==>
    nonoverlapping (word pc,0x2768) (word_add (read p3 t) (word n3),32)
    ==> ensures x86
         (\s. bytes_loaded s (word pc) (BUTLAST p256_montjadd_alt_tmc) /\
              read RIP s = pcin /\
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  ASM_CASES_TAC `a EXP 2 <= 2 EXP 256 * p_256` THENL
   [ASM_REWRITE_TAC[]; X86_SIM_TAC P256_MONTJADD_ALT_EXEC (1--137)] THEN
  ENSURES_INIT_TAC "s0" THEN
  FIRST_ASSUM(BIGNUM_DIGITIZE_TAC "x_" o lhand o concl) THEN

  MAP_EVERY (fun n ->
    X86_STEPS_TAC P256_MONTJADD_ALT_EXEC [n] THEN
  ...
  X86_ACCSTEPS_TAC P256_MONTJADD_ALT_EXEC
...
let LOCAL_MONTMUL_P256_TAC =
  X86_MACRO_SIM_ABBREV_TAC (X86_TRIM_EXEC_RULE p256_montjadd_alt_tmc) 166 lvs
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  ASM_CASES_TAC `a * b <= 2 EXP 256 * p_256` THENL
   [ASM_REWRITE_TAC[]; X86_SIM_TAC P256_MONTJADD_ALT_EXEC (1--166)] THEN
  ...
  X86_GEN_ACCSTEPS_TAC
    ...
    P256_MONTJADD_ALT_EXEC (2--149) (2--149) THEN
  ABBREV_TAC
   `t = bignum_of_wordlist
...
let LOCAL_SUB_P256_TAC =
  X86_MACRO_SIM_ABBREV_TAC (X86_TRIM_EXEC_RULE p256_montjadd_alt_tmc) 21 lvs
  `!(t:x86state) pcin pcout p3 n3 p1 n1 p2 n2.
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  X86_ACCSTEPS_TAC P256_MONTJADD_ALT_EXEC (1--8) (1--8) THEN
  ...
  X86_ACCSTEPS_TAC P256_MONTJADD_ALT_EXEC (14--21) (9--21) THEN
...
let P256_MONTJADD_ALT_CORRECT = time prove
 (`!p3 p1 t1 p2 t2 pc stackpointer.
        ALL (nonoverlapping (stackpointer,224))
            [(word pc,0x2768); (p1,96); (p2,96); (p3,96)] /\
        nonoverlapping (p3,96) (word pc,0x2768)
        ==> ensures x86
             (\s. bytes_loaded s (word pc) (BUTLAST p256_montjadd_alt_tmc) /\
  ...
  LOCAL_MONTSQR_P256_TAC 1 ["z1sq";"z_1"] THEN
  ...
  X86_STEPS_TAC P256_MONTJADD_ALT_EXEC (25--91) THEN
...
let P256_MONTJADD_ALT_NOIBT_SUBROUTINE_CORRECT = time prove
 (`!p3 p1 t1 p2 t2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 272),272))
            [(word pc,LENGTH p256_montjadd_alt_tmc); (p1,96); (p2,96)] /\
        ALL (nonoverlapping (p3,96))
            [(word pc,LENGTH p256_montjadd_alt_tmc); (word_sub stackpointer (word 272),280)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) p256_montjadd_alt_tmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
...
let P256_MONTJADD_ALT_SUBROUTINE_CORRECT = time prove
 (`!p3 p1 t1 p2 t2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 272),272))
            [(word pc,LENGTH p256_montjadd_alt_mc); (p1,96); (p2,96)] /\
        ALL (nonoverlapping (p3,96))
            [(word pc,LENGTH p256_montjadd_alt_mc); (word_sub stackpointer (word 272),280)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) p256_montjadd_alt_mc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  C_ARGUMENTS [p3; p1; p2] s /\
          ...
          (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bytes(p3,96);
                      memory :> bytes(word_sub stackpointer (word 272),272)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE P256_MONTJADD_ALT_NOIBT_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let p256_montjadd_alt_windows_mc = define_from_elf "p256_montjadd_alt_windows_mc"
      "x86/p256/p256_montjadd_alt.obj";;

let p256_montjadd_alt_windows_tmc = define_trimmed "p256_montjadd_alt_windows_tmc" p256_montjadd_alt_windows_mc;;

let P256_MONTJADD_ALT_NOIBT_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!p3 p1 t1 p2 t2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 288),288))
            [(word pc,LENGTH p256_montjadd_alt_windows_tmc); (p1,96); (p2,96)] /\
        ALL (nonoverlapping (p3,96))
            [(word pc,LENGTH p256_montjadd_alt_windows_tmc); (word_sub stackpointer (word 288),296)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) p256_montjadd_alt_windows_tmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
  ...
  WINDOWS_X86_WRAP_STACK_TAC
   p256_montjadd_alt_windows_tmc p256_montjadd_alt_tmc
   P256_MONTJADD_ALT_CORRECT
    `[RBX; RBP; R12; R13; R14; R15]` 272);;

let P256_MONTJADD_ALT_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!p3 p1 t1 p2 t2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 288),288))
            [(word pc,LENGTH p256_montjadd_alt_windows_mc); (p1,96); (p2,96)] /\
        ALL (nonoverlapping (p3,96))
            [(word pc,LENGTH p256_montjadd_alt_windows_mc); (word_sub stackpointer (word 288),296)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) p256_montjadd_alt_windows_mc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  WINDOWS_C_ARGUMENTS [p3; p1; p2] s /\
          ...
          (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bytes(p3,96);
                      memory :> bytes(word_sub stackpointer (word 288),288)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE P256_MONTJADD_ALT_NOIBT_WINDOWS_SUBROUTINE_CORRECT));;
