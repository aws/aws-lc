...
(* ========================================================================= *)
(* Montgomery inverse modulo p_256, the field characteristic for NIST P-256. *)
(* ========================================================================= *)
...
(**** print_literal_from_elf "x86/p256/bignum_montinv_p256.o";;
 ****)

let bignum_montinv_p256_mc = define_assert_from_elf "bignum_montinv_p256_mc" "x86/p256/bignum_montinv_p256.o"
[
  ...
  0x48; 0xb8; 0x00; 0x00; 0x14; 0x00; 0x00; 0x00; 0x0c; 0x00;
                           (* MOV (% rax) (Imm64 (word 3377699721838592)) *)
  0x48; 0x89; 0x44; 0x24; 0x78;
                           (* MOV (Memop Quadword (%% (rsp,120))) (% rax) *)
  0x48; 0xb8; 0x00; 0x00; 0x00; 0x00; 0x00; 0x00; 0xe8; 0xff;
                           (* MOV (% rax) (Imm64 (word 18439988674268495872)) *)
  0x48; 0x89; 0x84; 0x24; 0x80; 0x00; 0x00; 0x00;
                           (* MOV (Memop Quadword (%% (rsp,128))) (% rax) *)
  0x48; 0xb8; 0xff; 0xff; 0xef; 0xff; 0xff; 0xff; 0xfb; 0xff;
                           (* MOV (% rax) (Imm64 (word 18445618173801660415)) *)
  0x48; 0x89; 0x84; 0x24; 0x88; 0x00; 0x00; 0x00;
                           (* MOV (Memop Quadword (%% (rsp,136))) (% rax) *)
  0x48; 0xb8; 0xff; 0xff; 0xeb; 0xff; 0xff; 0xff; 0x0b; 0x00;
                           (* MOV (% rax) (Imm64 (word 3377699719217151)) *)
...
let bignum_montinv_p256_tmc = define_trimmed "bignum_montinv_p256_tmc" bignum_montinv_p256_mc;;

let BIGNUM_MONTINV_P256_EXEC = X86_MK_CORE_EXEC_RULE bignum_montinv_p256_tmc;;

(* ------------------------------------------------------------------------- *)
(* Do the main proof for the core that is sometimes inlined elsewhere        *)
(* ------------------------------------------------------------------------- *)

let CORE_MONTINV_P256_EXEC =
    X86_MK_EXEC_RULE
     ((GEN_REWRITE_CONV RAND_CONV [bignum_montinv_p256_tmc] THENC TRIM_LIST_CONV)
      `TRIM_LIST (17,18) bignum_montinv_p256_tmc`);;
...
let PACKED_DIVSTEP_TAC n shf s =
  ...
  X86_STEPS_TAC CORE_MONTINV_P256_EXEC (s--s') THEN
  SUBGOAL_THEN
...
let LOCAL_WORD_DIVSTEP59_CORRECT = prove
 (`!d f g stackpointer pc.
   nonoverlapping (stackpointer,240) (word pc,0x156a)
   ==> ensures x86
        (\s. bytes_loaded s (word pc)
                  (TRIM_LIST (17,18) bignum_montinv_p256_tmc) /\
             read RIP s = word(pc + 0x60b) /\
  ...
  ASM_CASES_TAC `d rem &2 = &1 /\ f rem &2 = &1 /\ abs(d:int) < &2 pow 62` THENL
   [FIRST_X_ASSUM(CONJUNCTS_THEN STRIP_ASSUME_TAC) THEN ASM_REWRITE_TAC[];
    X86_QUICKSIM_TAC CORE_MONTINV_P256_EXEC [`read RSP s = x`] (1--898)] THEN

  (*** The first packing ***)

  ENSURES_INIT_TAC "s0" THEN
  X86_STEPS_TAC CORE_MONTINV_P256_EXEC (1--10) THEN
  ...
  X86_STEPS_TAC CORE_MONTINV_P256_EXEC (11--20) THEN
  MAP_EVERY (fun n -> PACKED_DIVSTEP_TAC n [20] (13*n+8)) (1--20) THEN

  (*** The unpacking of the first block ***)

  X86_STEPS_TAC CORE_MONTINV_P256_EXEC (276--291) THEN
  ...
  X86_STEPS_TAC CORE_MONTINV_P256_EXEC (292--301) THEN
  ...
  X86_STEPS_TAC CORE_MONTINV_P256_EXEC (302--309) THEN
  RULE_ASSUM_TAC(REWRITE_RULE[DIVSTEP_FGVEC_PACK]) THEN
  FIRST_X_ASSUM(MP_TAC o SPEC `iword(divstep_dvec 0 (d,f,g)):int64` o
    MATCH_MP(MESON[] `read RSI s = a ==> !x. a = x ==> read RSI s = x`)) THEN
  ANTS_TAC THENL [REWRITE_TAC[divstep_dvec; DIVSTEP_D]; DISCH_TAC] THEN
  ...
  X86_STEPS_TAC CORE_MONTINV_P256_EXEC (310--318) THEN
  MAP_EVERY (fun n -> PACKED_DIVSTEP_TAC n [20] (13*n+306)) (1--20) THEN

  (*** The unpacking of the second block ***)

  X86_STEPS_TAC CORE_MONTINV_P256_EXEC (574--585) THEN
  RULE_ASSUM_TAC(REWRITE_RULE[DIVSTEP_MAT_UNPACK_20]) THEN
  RULE_ASSUM_TAC(SIMP_RULE
   [DIVSTEP_MAT_MOD; divstep_dvec; DIVSTEP_D_MOD; ARITH_LE; ARITH_LT]) THEN
  ...
  X86_STEPS_TAC CORE_MONTINV_P256_EXEC (586--595) THEN
  RULE_ASSUM_TAC(REWRITE_RULE[word_ishr]) THEN
  ...
  X86_STEPS_TAC CORE_MONTINV_P256_EXEC (596--603) THEN
  RULE_ASSUM_TAC(REWRITE_RULE[DIVSTEP_FGVEC_PACK]) THEN
  FIRST_X_ASSUM(MP_TAC o SPEC `iword(divstep_dvec 0 (d,f,g)):int64` o
    MATCH_MP(MESON[] `read RSI s = a ==> !x. a = x ==> read RSI s = x`)) THEN
  ANTS_TAC THENL [REWRITE_TAC[divstep_dvec; DIVSTEP_D]; DISCH_TAC] THEN
  ...
  X86_STEPS_TAC CORE_MONTINV_P256_EXEC (604--619) THEN

  (*** The last 19 divsteps ***)

  X86_STEPS_TAC CORE_MONTINV_P256_EXEC (620--628) THEN
  MAP_EVERY (fun n -> PACKED_DIVSTEP_TAC n [19] (13*n+616)) (1--19) THEN

  (*** The final unpacking is a little different as it's 19 not 20 ***)

  X86_STEPS_TAC CORE_MONTINV_P256_EXEC (871--882) THEN
  ...
  X86_STEPS_TAC CORE_MONTINV_P256_EXEC (883--898) THEN
...
let CORE_MONTINV_P256_CORRECT = time prove
 (`!z x n pc stackpointer.
        ALL (nonoverlapping (stackpointer,240))
            [(word pc,0x156a); (z,8 * 4); (x,8 * 4)] /\
        nonoverlapping (word pc,0x156a) (z,8 * 4)
        ==> ensures x86
             (\s. bytes_loaded s (word pc)
                    (TRIM_LIST (17,18) bignum_montinv_p256_tmc) /\
                  read RIP s = word pc /\
             ...
             (\s. read RIP s = word (pc + 0x156a) /\
                  bignum_from_memory(z,4) s =
                  (if p_256 divides n then 0
                   else (2 EXP 512 * inverse_mod p_256 n) MOD p_256))
          (MAYCHANGE [RIP; RDI; RSI; RAX; RBX; RCX; RDX; RBP;
                      R8; R9; R10; R11; R12; R13; R14; R15] ,,
           MAYCHANGE SOME_FLAGS ,,
  ...
  ENSURES_WHILE_UP_TAC `9` `pc + 0x60b` `pc + 0x60b`
   `\i s.
      read RSP s = stackpointer /\
  ...
  THEN REPEAT CONJ_TAC THENL
   [ARITH_TAC;
    ...
    X86_ACCSTEPS_TAC CORE_MONTINV_P256_EXEC
     [17;19;21;23] (1--49) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
    ...
    REWRITE_TAC[INT_REM_REM; INT_MUL_RID; GSYM p_256; INT_MUL_LID] THEN
    ...
    SUBGOAL_THEN `~carry_s23 <=> n < p_256` (SUBST1_TAC o SYM) THENL
    ...
    X86_BIGSTEP_TAC CORE_MONTINV_P256_EXEC "s1" THEN
    FIRST_X_ASSUM(MP_TAC o check (is_imp o concl)) THEN
    ...
    X86_STEPS_TAC CORE_MONTINV_P256_EXEC (2--4) THEN
    SUBGOAL_THEN
    ...
    X86_STEPS_TAC CORE_MONTINV_P256_EXEC (5--20) THEN
    ...
    X86_ACCSTEPS_TAC CORE_MONTINV_P256_EXEC [25;31] (21--32) THEN
    ...
    X86_ACCSTEPS_TAC CORE_MONTINV_P256_EXEC [266;268;270;272;274] (265--274) THEN
    ABBREV_TAC
     `upos =
    ...
    X86_ACCSTEPS_TAC CORE_MONTINV_P256_EXEC
     [278;279;280;281;282;283;289;291;293;295]
     (275--296) THEN
    SUBGOAL_THEN
     `(&2 pow 64 *
       &(bignum_of_wordlist[sum_s289; sum_s291; sum_s293; sum_s295]):int ==
       m00 * &u + m01 * &v) (mod &p_256)`
    ASSUME_TAC THENL
     [FIRST_X_ASSUM(MATCH_MP_TAC o MATCH_MP (REWRITE_RULE[IMP_CONJ_ALT]
          INT_CONG_TRANS)) THEN
      MATCH_MP_TAC INT_CONG_TRANS THEN EXISTS_TAC
       `if &upos + &(val(sum_s266:int64)) * &p_256:int < &2 pow 320
        then &upos + &(val sum_s266) * &p_256:int
        else (&upos + &(val sum_s266) * &p_256) - &2 pow 64 * &p_256` THEN
      ...
      CONJ_TAC THENL
       [REWRITE_TAC[bignum_of_wordlist; GSYM INT_OF_NUM_CLAUSES] THEN
        ...
        ALL_TAC] THEN
      SUBGOAL_THEN
       `&upos + &(val(sum_s266:int64)) * &p_256:int < &2 pow 320 <=>
        ~carry_s283`
      SUBST1_TAC THENL
    ...
    X86_ACCSTEPS_TAC CORE_MONTINV_P256_EXEC [298;300;302;304;306] (297--306) THEN
    ABBREV_TAC
     `vpos =
      bignum_of_wordlist[sum_s298;sum_s300;sum_s302;sum_s304;sum_s306]` THEN
    SUBGOAL_THEN
    ...
    X86_ACCSTEPS_TAC CORE_MONTINV_P256_EXEC
     [310;311;312;313;314;315;321;323;325;327]
     (307--328) THEN
    SUBGOAL_THEN
     `(&2 pow 64 *
       &(bignum_of_wordlist[sum_s321; sum_s323; sum_s325; sum_s327]):int ==
       m10 * &u + m11 * &v) (mod &p_256)`
    ASSUME_TAC THENL
     [FIRST_X_ASSUM(MATCH_MP_TAC o MATCH_MP (REWRITE_RULE[IMP_CONJ_ALT]
          INT_CONG_TRANS)) THEN
      MATCH_MP_TAC INT_CONG_TRANS THEN EXISTS_TAC
       `if &vpos + &(val(sum_s298:int64)) * &p_256:int < &2 pow 320
        then &vpos + &(val sum_s298) * &p_256:int
        else (&vpos + &(val sum_s298) * &p_256) - &2 pow 64 * &p_256` THEN
      CONJ_TAC THENL [ALL_TAC; COND_CASES_TAC THEN CONV_TAC INTEGER_RULE] THEN
      MATCH_MP_TAC INT_EQ_IMP_CONG THEN
      MATCH_MP_TAC INT_CONG_IMP_EQ THEN EXISTS_TAC `(&2:int) pow 320` THEN
      CONJ_TAC THENL
       [REWRITE_TAC[bignum_of_wordlist; GSYM INT_OF_NUM_CLAUSES] THEN
        ...
        ALL_TAC] THEN
      SUBGOAL_THEN
       `&vpos + &(val(sum_s298:int64)) * &p_256:int < &2 pow 320 <=>
        ~carry_s315`
      SUBST1_TAC THENL
       [REWRITE_TAC[INT_OF_NUM_CLAUSES] THEN CONV_TAC SYM_CONV THEN
        MATCH_MP_TAC FLAG_FROM_CARRY_LT THEN EXISTS_TAC `320` THEN
    ...
    ASM_REWRITE_TAC[] THEN DISCARD_STATE_TAC "s328" THEN
    ...
    X_GEN_TAC `i:num` THEN STRIP_TAC THEN
    X86_SIM_TAC CORE_MONTINV_P256_EXEC [] THEN ASM_REWRITE_TAC[];
  ...
  X86_ACCSTEPS_TAC CORE_MONTINV_P256_EXEC
   [35;39;40;41;44;45;46;50;51;52;55;56;57;
    ...
    90;92;94;96;98; 102;103;104;105;106;107;
    113;115;117;119; 130;131;132;133]
   (2--142) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  CONV_TAC(LAND_CONV(ONCE_DEPTH_CONV BIGNUM_LEXPAND_CONV)) THEN
  ASM_REWRITE_TAC[] THEN DISCARD_STATE_TAC "s142" THEN
  ...
  SUBGOAL_THEN
   `(&2 pow 64 *
     &(bignum_of_wordlist[sum_s113; sum_s115; sum_s117; sum_s119]):int ==
  ...
  ABBREV_TAC
   `pra = bignum_of_wordlist[sum_s113;sum_s115;sum_s117;sum_s119]` THEN
  TRANS_TAC EQ_TRANS `&(pra MOD p_256):int` THEN CONJ_TAC THENL
   [REWRITE_TAC[INT_OF_NUM_CLAUSES] THEN CONV_TAC SYM_CONV THEN
    ...
    SUBGOAL_THEN `~carry_s133 <=> pra < p_256` (SUBST1_TAC o SYM) THENL
     [MATCH_MP_TAC FLAG_FROM_CARRY_LT THEN EXISTS_TAC `256` THEN
      EXPAND_TAC "pra" THEN
      REWRITE_TAC[bignum_of_wordlist; MULT_CLAUSES; ADD_CLAUSES] THEN
      REWRITE_TAC[GSYM REAL_OF_NUM_EQ; GSYM REAL_OF_NUM_ADD; p_256;
...
let BIGNUM_MONTINV_P256_CORRECT = time prove
 (`!z x n pc stackpointer.
        ALL (nonoverlapping (stackpointer,240))
            [(word pc,0x158d); (z,8 * 4); (x,8 * 4)] /\
        nonoverlapping (word pc,0x158d) (z,8 * 4)
        ==> ensures x86
             (\s. bytes_loaded s (word pc) (BUTLAST bignum_montinv_p256_tmc) /\
                  read RIP s = word(pc + 0x11) /\
                  read RSP s = stackpointer /\
                  C_ARGUMENTS [z; x] s /\
                  bignum_from_memory (x,4) s = n)
             (\s. read RIP s = word (pc + 0x157b) /\
                  bignum_from_memory(z,4) s =
                  (if p_256 divides n then 0
                   else (2 EXP 512 * inverse_mod p_256 n) MOD p_256))
          (MAYCHANGE [RIP; RDI; RSI; RAX; RBX; RCX; RDX; RBP;
  ...
  X86_SUBROUTINE_SIM_TAC
      ((GEN_REWRITE_CONV RAND_CONV [bignum_montinv_p256_tmc] THENC
        REWRITE_CONV[BUTLAST_CLAUSES])
       `BUTLAST bignum_montinv_p256_tmc`,
       BIGNUM_MONTINV_P256_EXEC,
       0x11,
       (GEN_REWRITE_CONV RAND_CONV [bignum_montinv_p256_tmc] THENC TRIM_LIST_CONV)
       `TRIM_LIST (17,18) bignum_montinv_p256_tmc`,
       CORE_MONTINV_P256_CORRECT)
      [`read RDI s`; `read RSI s`;
       `read (memory :> bytes(read RSI s,8 * 4)) s`;
       `pc + 0x11`; `stackpointer:int64`] 1 THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[]);;

let BIGNUM_MONTINV_P256_NOIBT_SUBROUTINE_CORRECT = time prove
 (`!z x n pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 288),288))
            [(word pc,LENGTH bignum_montinv_p256_tmc); (x,8 * 4)] /\
        ALL (nonoverlapping (z,8 * 4))
            [(word_sub stackpointer (word 288),296); (word pc,LENGTH bignum_montinv_p256_tmc)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_montinv_p256_tmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
  ...
  X86_PROMOTE_RETURN_STACK_TAC bignum_montinv_p256_tmc
     BIGNUM_MONTINV_P256_CORRECT
 ...
 (`!z x n pc stackpointer returnaddress.
        ...
        ==> ensures x86
             ...
             (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * 4);
                    memory :> bytes(word_sub stackpointer (word 288),288)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MONTINV_P256_NOIBT_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let bignum_montinv_p256_windows_mc = define_from_elf "bignum_montinv_p256_windows_mc"
      "x86/p256/bignum_montinv_p256.obj";;

let bignum_montinv_p256_windows_tmc = define_trimmed "bignum_montinv_p256_windows_tmc" bignum_montinv_p256_windows_mc;;

let BIGNUM_MONTINV_P256_NOIBT_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!z x n pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 304),304))
            [(word pc,LENGTH bignum_montinv_p256_windows_tmc); (x,8 * 4)] /\
        ALL (nonoverlapping (z,8 * 4))
            [(word_sub stackpointer (word 304),312); (word pc,LENGTH bignum_montinv_p256_windows_tmc)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_montinv_p256_windows_tmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
  ...
  WINDOWS_X86_WRAP_STACK_TAC bignum_montinv_p256_windows_tmc bignum_montinv_p256_tmc
     BIGNUM_MONTINV_P256_CORRECT
      `[RBX; RBP; R12; R13; R14; R15]` 288);;

let BIGNUM_MONTINV_P256_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!z x n pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 304),304))
            [(word pc,LENGTH bignum_montinv_p256_windows_mc); (x,8 * 4)] /\
        ALL (nonoverlapping (z,8 * 4))
            [(word_sub stackpointer (word 304),312); (word pc,LENGTH bignum_montinv_p256_windows_mc)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_montinv_p256_windows_mc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  WINDOWS_C_ARGUMENTS [z; x] s /\
             ...
             (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * 4);
                    memory :> bytes(word_sub stackpointer (word 304),304)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MONTINV_P256_NOIBT_WINDOWS_SUBROUTINE_CORRECT));;
