...
let bignum_sqr_p521_alt_mc = define_assert_from_elf "bignum_sqr_p521_alt_mc" "x86/p521/bignum_sqr_p521_alt.o"
[
...
let bignum_sqr_p521_alt_tmc = define_trimmed "bignum_sqr_p521_alt_tmc" bignum_sqr_p521_alt_mc;;

let BIGNUM_SQR_P521_ALT_EXEC = X86_MK_CORE_EXEC_RULE bignum_sqr_p521_alt_tmc;;
...
let BIGNUM_SQR_P521_ALT_CORRECT = time prove
 (`!z x n pc stackpointer.
        ALL (nonoverlapping (stackpointer,72))
            [(word pc,0x500); (z,8 * 9); (x,8 * 9)] /\
        nonoverlapping (z,8 * 9) (word pc,0x500)
        ==> ensures x86
             (\s. bytes_loaded s (word pc) (BUTLAST bignum_sqr_p521_alt_tmc) /\
                  read RIP s = word(pc + 0xd) /\
                  read RSP s = stackpointer /\
                  C_ARGUMENTS [z; x] s /\
                  bignum_from_memory (x,9) s = n)
             (\s. read RIP s = word (pc + 0x4f2) /\
                  (n < p_521
  ...
  ASM_CASES_TAC `n < p_521` THENL
   [ASM_REWRITE_TAC[]; X86_SIM_TAC BIGNUM_SQR_P521_ALT_EXEC (1--353)] THEN
  ...
  X86_ACCSTEPS_TAC BIGNUM_SQR_P521_ALT_EXEC (1--308) (1--308) THEN
  X86_STEPS_TAC BIGNUM_SQR_P521_ALT_EXEC [309] THEN
  ...
  X86_ACCSTEPS_TAC BIGNUM_SQR_P521_ALT_EXEC [310] [310] THEN
  ...
  X86_STEPS_TAC BIGNUM_SQR_P521_ALT_EXEC (311--323) THEN
  ...
  X86_ACCSTEPS_TAC BIGNUM_SQR_P521_ALT_EXEC (324--333) (324--333) THEN
  SUBGOAL_THEN
  ...
  X86_ACCSTEPS_TAC BIGNUM_SQR_P521_ALT_EXEC (334--353) (334--353) THEN
...
let BIGNUM_SQR_P521_ALT_NOIBT_SUBROUTINE_CORRECT = prove
 (`!z x n pc stackpointer returnaddress.
        ALL (nonoverlapping (z,8 * 9))
            [(word pc,LENGTH bignum_sqr_p521_alt_tmc); (word_sub stackpointer (word 112),120)] /\
        ALL (nonoverlapping (word_sub stackpointer (word 112),112))
            [(word pc,LENGTH bignum_sqr_p521_alt_tmc); (x,8 * 9)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_sqr_p521_alt_tmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
  ...
  X86_PROMOTE_RETURN_STACK_TAC
   bignum_sqr_p521_alt_tmc BIGNUM_SQR_P521_ALT_CORRECT
   `[RBX; R12; R13; R14; R15]` 112);;

let BIGNUM_SQR_P521_ALT_SUBROUTINE_CORRECT = prove
 (`!z x n pc stackpointer returnaddress.
        ALL (nonoverlapping (z,8 * 9))
            [(word pc,LENGTH bignum_sqr_p521_alt_mc); (word_sub stackpointer (word 112),120)] /\
        ALL (nonoverlapping (word_sub stackpointer (word 112),112))
            [(word pc,LENGTH bignum_sqr_p521_alt_mc); (x,8 * 9)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_sqr_p521_alt_mc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  C_ARGUMENTS [z; x] s /\
             ...
             (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * 9);
                     memory :> bytes(word_sub stackpointer (word 112),112)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_SQR_P521_ALT_NOIBT_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let bignum_sqr_p521_alt_windows_mc = define_from_elf
   "bignum_sqr_p521_alt_windows_mc" "x86/p521/bignum_sqr_p521_alt.obj";;

let bignum_sqr_p521_alt_windows_tmc = define_trimmed "bignum_sqr_p521_alt_windows_tmc" bignum_sqr_p521_alt_windows_mc;;

let BIGNUM_SQR_P521_ALT_NOIBT_WINDOWS_SUBROUTINE_CORRECT = prove
 (`!z x n pc stackpointer returnaddress.
        ALL (nonoverlapping (z,8 * 9))
            [(word pc,LENGTH bignum_sqr_p521_alt_windows_tmc); (word_sub stackpointer (word 128),136)] /\
        ALL (nonoverlapping (word_sub stackpointer (word 128),128))
            [(word pc,LENGTH bignum_sqr_p521_alt_windows_tmc); (x,8 * 9)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_sqr_p521_alt_windows_tmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
  ...
  WINDOWS_X86_WRAP_STACK_TAC
   bignum_sqr_p521_alt_windows_tmc bignum_sqr_p521_alt_tmc
   BIGNUM_SQR_P521_ALT_CORRECT `[RBX; R12; R13; R14; R15]` 112);;

let BIGNUM_SQR_P521_ALT_WINDOWS_SUBROUTINE_CORRECT = prove
 (`!z x n pc stackpointer returnaddress.
        ALL (nonoverlapping (z,8 * 9))
            [(word pc,LENGTH bignum_sqr_p521_alt_windows_mc); (word_sub stackpointer (word 128),136)] /\
        ALL (nonoverlapping (word_sub stackpointer (word 128),128))
            [(word pc,LENGTH bignum_sqr_p521_alt_windows_mc); (x,8 * 9)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_sqr_p521_alt_windows_mc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  WINDOWS_C_ARGUMENTS [z; x] s /\
             ...
             (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * 9);
                     memory :> bytes(word_sub stackpointer (word 128),128)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_SQR_P521_ALT_NOIBT_WINDOWS_SUBROUTINE_CORRECT));;
