...
[
  ...
  0x4c; 0x8d; 0x15; 0x23; 0x2e; 0x00; 0x00;
                           (* LEA (% r10) (Riprel (word 11811)) *)
  0x4c; 0x8d; 0x1d; 0x7c; 0x2e; 0x00; 0x00;
                           (* LEA (% r11) (Riprel (word 11900)) *)
  ...
  0x48; 0x8d; 0x05; 0xb1; 0x2d; 0x00; 0x00;
                           (* LEA (% rax) (Riprel (word 11697)) *)
...
let curve25519_x25519base_tmc = define_trimmed "curve25519_x25519base_tmc" curve25519_x25519base_mc;;
...
let BYTES_LOADED_DATA = prove
 (`bytes_loaded s (word (pc + 0x2e77)) curve25519_x25519base_data <=>
   read (memory :> bytes(word (pc + 0x2e77),48576)) s =
...
let X25519BASE_TABLE_LEMMA = prove
 (`read (memory :> bytes(word (pc + 0x2e77),48576)) s =
   ...
   ==> edwards25519_exprojective
        ...
        (bignum_from_memory(word(pc + 0x2e77),4) s,
         bignum_from_memory(word(pc + 0x2e97),4) s,
         1,
         bignum_from_memory(word(pc + 0x2eb7),4) s) /\
       edwards25519_exprojective
        (group_pow edwards25519_group E_25519 (2 EXP 254 + 8))
        (bignum_from_memory(word(pc + 0x2ed7),4) s,
         bignum_from_memory(word(pc + 0x2ef7),4) s,
         1,
         bignum_from_memory(word(pc + 0x2f17),4) s) /\
       !i. i < 63
           ==> !j. j < 8
                   ==> edwards25519_epprojective
                        (group_pow edwards25519_group E_25519
                           (2 EXP (4 * (i + 1)) * (j + 1)))
         (bignum_from_memory(word(pc + 0x2f37 + 768 * i + 96 * j),4) s,
          bignum_from_memory(word(pc + 0x2f37 + 768 * i + 96 * j + 32),4) s,
          bignum_from_memory(word(pc + 0x2f37 + 768 * i + 96 * j + 64),4) s) /\
         ~(bignum_from_memory(word(pc + 0x2f37 + 768 * i + 96 * j + 64),4) s =
           0)`,
  let GE25519_POWERS =
    end_itlist CONJ
...
let LOCAL_MUL_P25519_TAC =
  X86_MACRO_SIM_ABBREV_TAC curve25519_x25519base_tmc 91 lvs
   `!(t:x86state) pcin pcout q3 n3 q1 n1 q2 n2.
      ...
      ==>
      nonoverlapping (word pc,0xec37) (word_add (read q3 t) (word n3),8 * 4)
      ==> ensures x86
           (\s. bytes_loaded s (word pc) curve25519_x25519base_tmc /\
                read RIP s = pcin /\
...
let CURVE25519_X25519BASE_CORRECT = time prove
 (`!res scalar n pc stackpointer.
  ...
  ENSURES_WHILE_AUP_TAC `1` `64` `pc + 0x19a` `pc + 0x17a4`
   `\i s.
      read (memory :> bytes(word(pc + 0x2e77),48576)) s =
      num_of_bytelist curve25519_x25519base_data /\
      read RSP s = stackpointer /\
   ...
   [ARITH_TAC;
    ...
    SUBGOAL_THEN
     `nonoverlapping_modulo (2 EXP 64) (val(stackpointer:int64),488)
                                       (val(wpc:int64),0xec37)`
    ASSUME_TAC THENL
     [EXPAND_TAC "wpc" THEN NONOVERLAPPING_TAC; ALL_TAC] THEN
    REPEAT(DISCH_THEN(CONJUNCTS_THEN2 ASSUME_TAC MP_TAC)) THEN
    DISCH_THEN(K ALL_TAC) THEN
    BIGNUM_LDIGITIZE_TAC "x0_"
      `bignum_from_memory(word_add wpc (word 0x2e77),4) s0` THEN
    BIGNUM_LDIGITIZE_TAC "y0_"
      `bignum_from_memory(word_add wpc (word 0x2e97),4) s0` THEN
    BIGNUM_LDIGITIZE_TAC "t0_"
      `bignum_from_memory(word_add wpc (word 0x2eb7),4) s0` THEN
    BIGNUM_LDIGITIZE_TAC "x1_"
      `bignum_from_memory(word_add wpc (word 0x2ed7),4) s0` THEN
    BIGNUM_LDIGITIZE_TAC "y1_"
      `bignum_from_memory(word_add wpc (word 0x2ef7),4) s0` THEN
    BIGNUM_LDIGITIZE_TAC "t1_"
      `bignum_from_memory(word_add wpc (word 0x2f17),4) s0` THEN
    ...
    ABBREV_TAC `tab:int64 = word(pc + 0x2f37 + 768 * (i - 1))` THEN
...
let CURVE25519_X25519BASE_SUBROUTINE_CORRECT = time prove
 (`!res scalar n pc stackpointer returnaddress.
    ALL (nonoverlapping (word_sub stackpointer (word 536),536))
        [(word pc,0xec3b); (scalar,32)] /\
    nonoverlapping (res,32) (word pc,0xec3b) /\
    nonoverlapping (res,32) (word_sub stackpointer (word 536),544)
    ==> ensures x86
         (\s. bytes_loaded s (word pc)
         ...
         (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
          MAYCHANGE [memory :> bytes(res,32);
                     memory :> bytes(word_sub stackpointer (word 536),536)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE CURVE25519_X25519BASE_NOIBT_SUBROUTINE_CORRECT));;
 ...
 (`!res scalar n pc stackpointer returnaddress.
    ...
    ==> ensures x86
         ...
         (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
          MAYCHANGE [memory :> bytes(res,32);
                     memory :> bytes(word_sub stackpointer (word 536),536)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE CURVE25519_X25519BASE_BYTE_NOIBT_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let curve25519_x25519base_windows_mc,
    curve25519_x25519base_windows_data =
  define_coda_from_elf 0x2e8b
  "curve25519_x25519base_windows_mc"
  "curve25519_x25519base_windows_data"
  "x86/curve25519/curve25519_x25519base.obj";;

let curve25519_x25519base_windows_tmc = define_trimmed "curve25519_x25519base_windows_tmc" curve25519_x25519base_windows_mc;;
...
let CURVE25519_X25519BASE_NOIBT_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!res scalar n pc stackpointer returnaddress.
    ALL (nonoverlapping (word_sub stackpointer (word 560),560))
        [(word pc,0xec47); (scalar,32)] /\
    nonoverlapping (res,32) (word pc,0xec47) /\
    nonoverlapping (res,32) (word_sub stackpointer (word 560),568)
    ==> ensures x86
         (\s. bytes_loaded s (word pc)
               (APPEND curve25519_x25519base_windows_tmc
                       curve25519_x25519base_windows_data) /\
              read RIP s = word pc /\
              read RSP s = stackpointer /\
              read (memory :> bytes64 stackpointer) s = returnaddress /\
  ...
  let subth =
   REWRITE_RULE[BYTES_LOADED_APPEND_CLAUSE; BYTES_LOADED_DATA;
                fst CURVE25519_X25519BASE_EXEC] baseth in
  ...
  REWRITE_TAC[BYTES_LOADED_APPEND_CLAUSE] THEN
  REWRITE_TAC[fst WINDOWS_CURVE25519_X25519BASE_EXEC] THEN
  GEN_REWRITE_TAC
  ...
  REWRITE_TAC[READ_BYTELIST_EQ_BYTES; CONV_RULE (RAND_CONV LENGTH_CONV)
     (AP_TERM `LENGTH:byte list->num`
      curve25519_x25519base_windows_data)] THEN
  REWRITE_TAC[curve25519_x25519base_windows_data] THEN
  REWRITE_TAC[GSYM curve25519_x25519base_data] THEN
  ...
  RULE_ASSUM_TAC(REWRITE_RULE
   [ARITH_RULE `pc + 0x2e87 = (pc + 16) + 0x2e77`]) THEN
  X86_SUBROUTINE_SIM_TAC
...
let CURVE25519_X25519BASE_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!res scalar n pc stackpointer returnaddress.
    ALL (nonoverlapping (word_sub stackpointer (word 560),560))
        [(word pc,0xec4b); (scalar,32)] /\
    nonoverlapping (res,32) (word pc,0xec4b) /\
    nonoverlapping (res,32) (word_sub stackpointer (word 560),568)
    ==> ensures x86
         (\s. bytes_loaded s (word pc)
               (APPEND curve25519_x25519base_windows_mc
                       curve25519_x25519base_windows_data) /\
              read RIP s = word pc /\
              read RSP s = stackpointer /\
              read (memory :> bytes64 stackpointer) s = returnaddress /\
         ...
         (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
          MAYCHANGE [memory :> bytes(res,32);
                     memory :> bytes(word_sub stackpointer (word 560),560)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE CURVE25519_X25519BASE_NOIBT_WINDOWS_SUBROUTINE_CORRECT));;
 ...
 (`!res scalar n pc stackpointer returnaddress.
    ...
    ==> ensures x86
         ...
         (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
          MAYCHANGE [memory :> bytes(res,32);
                     memory :> bytes(word_sub stackpointer (word 560),560)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE CURVE25519_X25519BASE_BYTE_NOIBT_WINDOWS_SUBROUTINE_CORRECT));;
