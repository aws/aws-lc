...
let bignum_amontredc_tmc = define_trimmed "bignum_amontredc_tmc" bignum_amontredc_mc;;

let BIGNUM_AMONTREDC_EXEC = X86_MK_CORE_EXEC_RULE bignum_amontredc_tmc;;
 ...
 (`!k z r x m p a n pc stackpointer.
  ...
  ASM_CASES_TAC `k = 0` THENL
   [UNDISCH_THEN `k = 0` SUBST_ALL_TAC THEN
    ...
    ENSURES_INIT_TAC "s0" THEN X86_STEPS_TAC BIGNUM_AMONTREDC_EXEC (1--2) THEN
  ...
  CONJ_TAC THENL
   [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN ENSURES_INIT_TAC "s0" THEN
    ...
    X86_STEPS_TAC BIGNUM_AMONTREDC_EXEC (1--9) THEN
    ...
    X86_STEPS_TAC BIGNUM_AMONTREDC_EXEC (10--27) THEN
   ...
   [ASM_CASES_TAC `nx = 0` THENL
    ...
    ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
     [ASM_REWRITE_TAC[ARITH_RULE `MIN a b = 0 <=> a = 0 \/ b = 0`];
      ...
      X_GEN_TAC `i:num` THEN STRIP_TAC THEN VAL_INT64_TAC `i:num` THEN
      X86_SIM_TAC BIGNUM_AMONTREDC_EXEC (1--2) THEN
    ...
    ASM_CASES_TAC `k:num <= nx` THENL
     [ASM_SIMP_TAC[ARITH_RULE `k <= nx ==> MIN k nx = k`] THEN
      REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
      ENSURES_INIT_TAC "s0" THEN
      X86_STEPS_TAC BIGNUM_AMONTREDC_EXEC (1--5) THEN
    ...
    ENSURES_WHILE_AUP_TAC `nx:num` `k:num` `pc + 0xa3` `pc + 0xaa`
     `\i s. read RSP s = stackpointer /\
            ...
            bignum_from_memory (z,i) s = lowdigits a i` THEN
    ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
     [X86_SIM_TAC BIGNUM_AMONTREDC_EXEC (1--5);
      X_GEN_TAC `i:num` THEN STRIP_TAC THEN VAL_INT64_TAC `i:num` THEN
      REWRITE_TAC[BIGNUM_FROM_MEMORY_STEP] THEN
      REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN ENSURES_INIT_TAC "s0" THEN
      X86_STEPS_TAC BIGNUM_AMONTREDC_EXEC (1--2) THEN
      ...
      X_GEN_TAC `i:num` THEN STRIP_TAC THEN VAL_INT64_TAC `i:num` THEN
      X86_SIM_TAC BIGNUM_AMONTREDC_EXEC (1--2);
      X86_SIM_TAC BIGNUM_AMONTREDC_EXEC (1--3) THEN
  ...
  ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
   [X86_SIM_TAC BIGNUM_AMONTREDC_EXEC (1--3) THEN
    ...
    ENSURES_WHILE_UP_TAC `k:num` `pc + 0x13f` `pc + 0x153`
    ...
    ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
     [X86_SIM_TAC BIGNUM_AMONTREDC_EXEC (1--6) THEN
      ...
      REWRITE_TAC[REAL_MUL_RZERO; REAL_ADD_LID; REAL_SUB_REFL];
      X_GEN_TAC `i:num` THEN STRIP_TAC THEN VAL_INT64_TAC `i:num` THEN
      ...
      REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
      ENSURES_INIT_TAC "s0" THEN
      X86_ACCSTEPS_TAC BIGNUM_AMONTREDC_EXEC [4] (1--6) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[GSYM WORD_ADD] THEN
   ...
   [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN ENSURES_INIT_TAC "s0" THEN
    ...
    MP_TAC THENL
     [ASM_REWRITE_TAC[HIGHDIGITS_0; BIGNUM_FROM_MEMORY_BYTES];
      ...
      ASM_REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES; ADD_CLAUSES] THEN
      STRIP_TAC] THEN
    X86_ACCSTEPS_TAC BIGNUM_AMONTREDC_EXEC [5;6] (1--9) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
   ...
   [ASM_CASES_TAC `k = 1` THENL
    ...
    REPEAT CONJ_TAC THENL
     [ASM_REWRITE_TAC[ARITH_RULE `1 < k <=> ~(k = 0 \/ k = 1)`];
      ...
      X86_ACCSTEPS_TAC BIGNUM_AMONTREDC_EXEC [1;4] (1--5) THEN
      RULE_ASSUM_TAC(REWRITE_RULE[WORD_RULE
       `word_sub x (word_neg y):int64 = word_add x y`]) THEN
      ACCUMULATE_ARITH_TAC "s5" THEN
      X86_ACCSTEPS_TAC BIGNUM_AMONTREDC_EXEC [6] (6--10) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
      ...
      X_GEN_TAC `j:num` THEN STRIP_TAC THEN
      MAP_EVERY VAL_INT64_TAC [`j:num`; `j - 1`] THEN
      X86_SIM_TAC BIGNUM_AMONTREDC_EXEC [1];
  ...
  ASM_CASES_TAC `nx:num <= k + i` THENL
   [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN ENSURES_INIT_TAC "s0" THEN
    X86_ACCSTEPS_TAC BIGNUM_AMONTREDC_EXEC [1] (1--8) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
    ...
    REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN ENSURES_INIT_TAC "s0" THEN
    X86_ACCSTEPS_TAC BIGNUM_AMONTREDC_EXEC [1;8;9] (1--11) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
    CONJ_TAC THENL [CONV_TAC WORD_RULE; ALL_TAC]] THEN
...
let BIGNUM_AMONTREDC_NOIBT_SUBROUTINE_CORRECT = time prove
 (`!k z r x m p a n pc stackpointer returnaddress.
        ...
        ALL (nonoverlapping (word_sub stackpointer (word 56),56))
            [(word pc,LENGTH bignum_amontredc_tmc); (x,8 * val r); (m,8 * val k)] /\
        ALL (nonoverlapping (z,8 * val k)) [(word pc,LENGTH bignum_amontredc_tmc); (m,8 * val k)] /\
        (x = z \/ nonoverlapping (x,8 * val r) (z,8 * val k)) /\
        val p < 2 EXP 61 /\ val r < 2 EXP 61
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_amontredc_tmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
  ...
  X86_PROMOTE_RETURN_STACK_TAC bignum_amontredc_tmc BIGNUM_AMONTREDC_CORRECT
...
let BIGNUM_AMONTREDC_SUBROUTINE_CORRECT = time prove
 (`!k z r x m p a n pc stackpointer returnaddress.
        ...
        val p < 2 EXP 61 /\ val r < 2 EXP 61
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_amontredc_mc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  C_ARGUMENTS [k; z; r; x; m; p] s /\
             ...
             (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * val k);
                      memory :> bytes(word_sub stackpointer (word 56),56)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_AMONTREDC_NOIBT_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let bignum_amontredc_windows_mc = define_from_elf
   "bignum_amontredc_windows_mc" "x86/generic/bignum_amontredc.obj";;

let bignum_amontredc_windows_tmc = define_trimmed "bignum_amontredc_windows_tmc" bignum_amontredc_windows_mc;;

let BIGNUM_AMONTREDC_NOIBT_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!k z r x m p a n pc stackpointer returnaddress.
        nonoverlapping (word_sub stackpointer (word 72),80) (z,8 * val k) /\
        ALL (nonoverlapping (word_sub stackpointer (word 72),72))
            [(word pc,LENGTH bignum_amontredc_windows_tmc); (x,8 * val r); (m,8 * val k)] /\
        ALL (nonoverlapping (z,8 * val k)) [(word pc,LENGTH bignum_amontredc_windows_tmc); (m,8 * val k)] /\
        (x = z \/ nonoverlapping (x,8 * val r) (z,8 * val k)) /\
        val p < 2 EXP 61 /\ val r < 2 EXP 61
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_amontredc_windows_tmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
  ...
  WINDOWS_X86_WRAP_STACK_TAC bignum_amontredc_windows_tmc bignum_amontredc_tmc
   BIGNUM_AMONTREDC_CORRECT `[RBX; RBP; R12; R13; R14; R15]` 56);;

let BIGNUM_AMONTREDC_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!k z r x m p a n pc stackpointer returnaddress.
        nonoverlapping (word_sub stackpointer (word 72),80) (z,8 * val k) /\
        ALL (nonoverlapping (word_sub stackpointer (word 72),72))
            [(word pc,LENGTH bignum_amontredc_windows_mc); (x,8 * val r); (m,8 * val k)] /\
        ALL (nonoverlapping (z,8 * val k)) [(word pc,LENGTH bignum_amontredc_windows_mc); (m,8 * val k)] /\
        (x = z \/ nonoverlapping (x,8 * val r) (z,8 * val k)) /\
        val p < 2 EXP 61 /\ val r < 2 EXP 61
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_amontredc_windows_mc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  WINDOWS_C_ARGUMENTS [k; z; r; x; m; p] s /\
             ...
             (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * val k);
                      memory :> bytes(word_sub stackpointer (word 72),72)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_AMONTREDC_NOIBT_WINDOWS_SUBROUTINE_CORRECT));;
