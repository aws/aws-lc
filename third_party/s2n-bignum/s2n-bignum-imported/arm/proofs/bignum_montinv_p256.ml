...
(**** print_literal_from_elf "arm/p256/bignum_montinv_p256.o";;
 ****)

let bignum_montinv_p256_mc = define_assert_from_elf "bignum_montinv_p256_mc" "arm/p256/bignum_montinv_p256.o"
[
  ...
  0xd2a0028a;       (* arm_MOVZ X10 (word 20) 16 *)
  0xb24e054a;       (* arm_ORR X10 X10 (rvalue (word 3377699720527872)) *)
  0xd2fffd0b;       (* arm_MOVZ X11 (word 65512) 48 *)
  0xd29fffed;       (* arm_MOV X13 (rvalue (word 65535)) *)
  0xf2bffded;       (* arm_MOVK X13 (word 65519) 16 *)
  0xf2dfffed;       (* arm_MOVK X13 (word 65535) 32 *)
  0xf2e0016d;       (* arm_MOVK X13 (word 11) 48 *)
  0xb24c2dac;       (* arm_ORR X12 X13 (rvalue (word 18442240474082181120)) *)
  0x926df9ad;       (* arm_AND X13 X13 (rvalue (word 18446744073709289471)) *)
...
let BIGNUM_MONTINV_P256_EXEC = ARM_MK_EXEC_RULE bignum_montinv_p256_mc;;
...
let CORE_MONTINV_P256_EXEC =
    ARM_MK_EXEC_RULE
     ((GEN_REWRITE_CONV RAND_CONV [bignum_montinv_p256_mc] THENC TRIM_LIST_CONV)
      `TRIM_LIST (16,20) bignum_montinv_p256_mc`);;
...
let PACKED_DIVSTEP_TAC n shf s =
  ...
  ARM_STEPS_TAC CORE_MONTINV_P256_EXEC (s--s') THEN
  SUBGOAL_THEN
 ...
 (`!d f g pc.
       ensures arm
        (\s. aligned_bytes_loaded s (word pc)
                 (TRIM_LIST (16,20) bignum_montinv_p256_mc) /\
             read PC s = word(pc + 0x55c) /\
             read X1 s = d /\
             read X2 s = f /\
             read X3 s = g)
        (\s. read PC s = word(pc + 0xed0) /\
             (ival d rem &2 = &1 /\
              ival f rem &2 = &1 /\
              abs(ival d) < &2 pow 62 ==>
  ...
  ASM_CASES_TAC `d rem &2 = &1 /\ f rem &2 = &1 /\ abs(d:int) < &2 pow 62` THENL
   [FIRST_X_ASSUM(CONJUNCTS_THEN STRIP_ASSUME_TAC) THEN ASM_REWRITE_TAC[];
    ARM_QUICKSIM_TAC CORE_MONTINV_P256_EXEC [`read X0 s = x`] (1--605)] THEN

  (*** The first packing ***)

  ENSURES_INIT_TAC "s0" THEN
  ARM_STEPS_TAC CORE_MONTINV_P256_EXEC (1--5) THEN
  ...
  ARM_STEPS_TAC CORE_MONTINV_P256_EXEC (185--194) THEN
  ...
  ARM_STEPS_TAC CORE_MONTINV_P256_EXEC (195--202) THEN
  ...
  ARM_STEPS_TAC CORE_MONTINV_P256_EXEC (203--207) THEN
  RULE_ASSUM_TAC(REWRITE_RULE[DIVSTEP_FGVEC_PACK]) THEN
  FIRST_X_ASSUM(MP_TAC o SPEC `iword(divstep_dvec 0 (d,f,g)):int64` o
    MATCH_MP(MESON[] `read X1 s = a ==> !x. a = x ==> read X1 s = x`)) THEN
  ANTS_TAC THENL [REWRITE_TAC[divstep_dvec; DIVSTEP_D]; DISCH_TAC] THEN
  ...
  ARM_STEPS_TAC CORE_MONTINV_P256_EXEC (387--396) THEN
  RULE_ASSUM_TAC(REWRITE_RULE[DIVSTEP_MAT_UNPACK_20]) THEN
  RULE_ASSUM_TAC(SIMP_RULE
   [DIVSTEP_MAT_MOD; divstep_dvec; DIVSTEP_D_MOD; ARITH_LE; ARITH_LT]) THEN
  ...
  ARM_STEPS_TAC CORE_MONTINV_P256_EXEC (397--404) THEN
  RULE_ASSUM_TAC(REWRITE_RULE[WORD_RULE
   `word(0 + val(iword(--x):int64) * val(iword g:int64)):int64 =
    iword(--(x * g))`]) THEN
  RULE_ASSUM_TAC(REWRITE_RULE[word_ishr; GSYM IWORD_INT_ADD]) THEN
  ...
  ARM_STEPS_TAC CORE_MONTINV_P256_EXEC (405--409) THEN
  RULE_ASSUM_TAC(REWRITE_RULE[DIVSTEP_FGVEC_PACK]) THEN
  FIRST_X_ASSUM(MP_TAC o SPEC `iword(divstep_dvec 0 (d,f,g)):int64` o
    MATCH_MP(MESON[] `read X1 s = a ==> !x. a = x ==> read X1 s = x`)) THEN
  ANTS_TAC THENL [REWRITE_TAC[divstep_dvec; DIVSTEP_D]; DISCH_TAC] THEN
  ...
  ARM_STEPS_TAC CORE_MONTINV_P256_EXEC (500--507) THEN
  ...
  ARM_STEPS_TAC CORE_MONTINV_P256_EXEC (588--597) THEN
  ...
  ARM_STEPS_TAC CORE_MONTINV_P256_EXEC (598--605) THEN
...
let CORE_MONTINV_P256_CORRECT = time prove
 (`!z x n pc stackpointer.
        ...
        ALL (nonoverlapping (stackpointer,160))
            [(word pc,0x10c0); (z,8 * 4); (x,8 * 4)] /\
        nonoverlapping (word pc,0x10c0) (z,8 * 4)
        ==> ensures arm
             (\s. aligned_bytes_loaded s (word pc)
                   (TRIM_LIST (16,20) bignum_montinv_p256_mc) /\
                  read PC s = word pc /\
  ...
  ENSURES_WHILE_UP_TAC `9` `pc + 0x550` `pc + 0x550`
   `\i s.
      read SP s = stackpointer /\
      read X20 s = z /\
  ...
  THEN REPEAT CONJ_TAC THENL
   [ARITH_TAC;
    ...
    ARM_ACCSTEPS_TAC CORE_MONTINV_P256_EXEC
     [9;10;12;13] (1--36) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
    ...
    DISCARD_STATE_TAC "s36" THEN CONV_TAC NUM_REDUCE_CONV THEN
    ...
    ARM_STEPS_TAC CORE_MONTINV_P256_EXEC (5--7) THEN
    SUBGOAL_THEN
    ...
    ARM_STEPS_TAC CORE_MONTINV_P256_EXEC (8--19) THEN
    ...
    ARM_ACCSTEPS_TAC CORE_MONTINV_P256_EXEC [22;25] (20--25) THEN
    ...
    X_GEN_TAC `i:num` THEN STRIP_TAC THEN
    ARM_SIM_TAC CORE_MONTINV_P256_EXEC [] THEN ASM_REWRITE_TAC[];
...
let BIGNUM_MONTINV_P256_CORRECT = time prove
 (`!z x n pc stackpointer.
        aligned 16 stackpointer /\
        ALL (nonoverlapping (stackpointer,160))
            [(word pc,0x10e4); (z,8 * 4); (x,8 * 4)] /\
        nonoverlapping (word pc,0x10e4) (z,8 * 4)
        ==> ensures arm
             (\s. aligned_bytes_loaded s (word pc) bignum_montinv_p256_mc /\
                  read PC s = word(pc + 0x10) /\
                  read SP s = stackpointer /\
                  C_ARGUMENTS [z; x] s /\
                  bignum_from_memory (x,4) s = n)
             (\s. read PC s = word (pc + 0x10d0) /\
                  bignum_from_memory(z,4) s =
                  (if p_256 divides n then 0
                   else (2 EXP 512 * inverse_mod p_256 n) MOD p_256))
          (MAYCHANGE [PC; X0; X1; X2; X3; X4; X5; X6; X7; X8; X9;
  ...
  ARM_SUBROUTINE_SIM_TAC
      (bignum_montinv_p256_mc,BIGNUM_MONTINV_P256_EXEC,0x10,
...
let BIGNUM_MONTINV_P256_SUBROUTINE_CORRECT = time prove
 (`!z x n pc stackpointer returnaddress.
  ...
  ARM_ADD_RETURN_STACK_TAC
   BIGNUM_MONTINV_P256_EXEC BIGNUM_MONTINV_P256_CORRECT
