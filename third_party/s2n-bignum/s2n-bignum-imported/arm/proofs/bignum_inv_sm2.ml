...
(* ========================================================================= *)
(* Inversion modulo p_sm2, the field characteristic for CC SM2.              *)
(* ========================================================================= *)
...
(**** print_literal_from_elf "arm/sm2/bignum_inv_sm2.o";;
 ****)

let bignum_inv_sm2_mc = define_assert_from_elf "bignum_inv_sm2_mc" "arm/sm2/bignum_inv_sm2.o"
[
...
let BIGNUM_INV_SM2_EXEC = ARM_MK_EXEC_RULE bignum_inv_sm2_mc;;

(* ------------------------------------------------------------------------- *)
(* Do the main proof for the core that is sometimes inlined elsewhere        *)
(* ------------------------------------------------------------------------- *)

let CORE_INV_SM2_EXEC =
    ARM_MK_EXEC_RULE
     ((GEN_REWRITE_CONV RAND_CONV [bignum_inv_sm2_mc] THENC TRIM_LIST_CONV)
      `TRIM_LIST (16,20) bignum_inv_sm2_mc`);;
...
let PACKED_DIVSTEP_TAC n shf s =
  ...
  ARM_STEPS_TAC CORE_INV_SM2_EXEC (s--s') THEN
  SUBGOAL_THEN
 ...
 (`!d f g pc.
       ensures arm
        (\s. aligned_bytes_loaded s (word pc)
                 (TRIM_LIST (16,20) bignum_inv_sm2_mc) /\
             read PC s = word(pc + 0x52c) /\
             read X1 s = d /\
             read X2 s = f /\
             read X3 s = g)
        (\s. read PC s = word(pc + 0xea0) /\
             (ival d rem &2 = &1 /\
              ival f rem &2 = &1 /\
              abs(ival d) < &2 pow 62 ==>
  ...
  ASM_CASES_TAC `d rem &2 = &1 /\ f rem &2 = &1 /\ abs(d:int) < &2 pow 62` THENL
   [FIRST_X_ASSUM(CONJUNCTS_THEN STRIP_ASSUME_TAC) THEN ASM_REWRITE_TAC[];
    ARM_QUICKSIM_TAC CORE_INV_SM2_EXEC [`read X0 s = x`] (1--605)] THEN

  (*** The first packing ***)

  ENSURES_INIT_TAC "s0" THEN
  ARM_STEPS_TAC CORE_INV_SM2_EXEC (1--5) THEN
  ...
  ARM_STEPS_TAC CORE_INV_SM2_EXEC (185--194) THEN
  ...
  ARM_STEPS_TAC CORE_INV_SM2_EXEC (195--202) THEN
  ...
  ARM_STEPS_TAC CORE_INV_SM2_EXEC (203--207) THEN
  RULE_ASSUM_TAC(REWRITE_RULE[DIVSTEP_FGVEC_PACK]) THEN
  FIRST_X_ASSUM(MP_TAC o SPEC `iword(divstep_dvec 0 (d,f,g)):int64` o
    MATCH_MP(MESON[] `read X1 s = a ==> !x. a = x ==> read X1 s = x`)) THEN
  ANTS_TAC THENL [REWRITE_TAC[divstep_dvec; DIVSTEP_D]; DISCH_TAC] THEN
  ...
  ARM_STEPS_TAC CORE_INV_SM2_EXEC (387--396) THEN
  RULE_ASSUM_TAC(REWRITE_RULE[DIVSTEP_MAT_UNPACK_20]) THEN
  RULE_ASSUM_TAC(SIMP_RULE
   [DIVSTEP_MAT_MOD; divstep_dvec; DIVSTEP_D_MOD; ARITH_LE; ARITH_LT]) THEN
  ...
  ARM_STEPS_TAC CORE_INV_SM2_EXEC (397--404) THEN
  RULE_ASSUM_TAC(REWRITE_RULE[WORD_RULE
   `word(0 + val(iword(--x):int64) * val(iword g:int64)):int64 =
    iword(--(x * g))`]) THEN
  RULE_ASSUM_TAC(REWRITE_RULE[word_ishr; GSYM IWORD_INT_ADD]) THEN
  ...
  ARM_STEPS_TAC CORE_INV_SM2_EXEC (405--409) THEN
  RULE_ASSUM_TAC(REWRITE_RULE[DIVSTEP_FGVEC_PACK]) THEN
  FIRST_X_ASSUM(MP_TAC o SPEC `iword(divstep_dvec 0 (d,f,g)):int64` o
    MATCH_MP(MESON[] `read X1 s = a ==> !x. a = x ==> read X1 s = x`)) THEN
  ANTS_TAC THENL [REWRITE_TAC[divstep_dvec; DIVSTEP_D]; DISCH_TAC] THEN
  ...
  ARM_STEPS_TAC CORE_INV_SM2_EXEC (500--507) THEN
  ...
  ARM_STEPS_TAC CORE_INV_SM2_EXEC (588--597) THEN
  ...
  ARM_STEPS_TAC CORE_INV_SM2_EXEC (598--605) THEN
...
let CORE_INV_SM2_CORRECT = time prove
 (`!z x n pc stackpointer.
        aligned 16 stackpointer /\
        ALL (nonoverlapping (stackpointer,160))
            [(word pc,0x1088); (z,8 * 4); (x,8 * 4)] /\
        nonoverlapping (word pc,0x1088) (z,8 * 4)
        ==> ensures arm
             (\s. aligned_bytes_loaded s (word pc)
                   (TRIM_LIST (16,20) bignum_inv_sm2_mc) /\
                  read PC s = word pc /\
             ...
             (\s. read PC s = word (pc + 0x1088) /\
                  ...
                  (if p_sm2 divides n then 0 else inverse_mod p_sm2 n))
  ...
  ENSURES_WHILE_UP_TAC `9` `pc + 0x520` `pc + 0x520`
   `\i s.
      read SP s = stackpointer /\
      read X20 s = z /\
  ...
  THEN REPEAT CONJ_TAC THENL
   [ARITH_TAC;
    ...
    ARM_ACCSTEPS_TAC CORE_INV_SM2_EXEC
    ...
    ARM_STEPS_TAC CORE_INV_SM2_EXEC (5--7) THEN
    SUBGOAL_THEN
    ...
    ARM_STEPS_TAC CORE_INV_SM2_EXEC (8--19) THEN
    ...
    ARM_ACCSTEPS_TAC CORE_INV_SM2_EXEC [22;25] (20--25) THEN
    ...
    X_GEN_TAC `i:num` THEN STRIP_TAC THEN
    ARM_SIM_TAC CORE_INV_SM2_EXEC [] THEN ASM_REWRITE_TAC[];
  ...
  REWRITE_TAC[p_sm2] THEN CONJ_TAC THENL [ALL_TAC; ARITH_TAC] THEN
  REWRITE_TAC[GSYM p_sm2; CONG] THEN CONV_TAC MOD_DOWN_CONV THEN
...
let BIGNUM_INV_SM2_CORRECT = time prove
 (`!z x n pc stackpointer.
        aligned 16 stackpointer /\
        ALL (nonoverlapping (stackpointer,160))
            [(word pc,0x10ac); (z,8 * 4); (x,8 * 4)] /\
        nonoverlapping (word pc,0x10ac) (z,8 * 4)
        ==> ensures arm
             (\s. aligned_bytes_loaded s (word pc) bignum_inv_sm2_mc /\
                  read PC s = word(pc + 0x10) /\
                  read SP s = stackpointer /\
                  C_ARGUMENTS [z; x] s /\
                  bignum_from_memory (x,4) s = n)
             (\s. read PC s = word (pc + 0x1098) /\
                  bignum_from_memory(z,4) s =
                  (if p_sm2 divides n then 0 else inverse_mod p_sm2 n))
          (MAYCHANGE [PC; X0; X1; X2; X3; X4; X5; X6; X7; X8; X9;
                      X10; X11; X12; X13; X14; X15; X16; X17;
  ...
  ARM_SUBROUTINE_SIM_TAC
      (bignum_inv_sm2_mc,BIGNUM_INV_SM2_EXEC,0x10,
       (GEN_REWRITE_CONV RAND_CONV [bignum_inv_sm2_mc] THENC TRIM_LIST_CONV)
       `TRIM_LIST (16,20) bignum_inv_sm2_mc`,
       CORE_INV_SM2_CORRECT)
      [`read X0 s`; `read X1 s`;
       `read (memory :> bytes(read X1 s,8 * 4)) s`;
       `pc + 0x10`; `stackpointer:int64`] 1 THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[]);;

let BIGNUM_INV_SM2_SUBROUTINE_CORRECT = time prove
 (`!z x n pc stackpointer returnaddress.
  ...
  ARM_ADD_RETURN_STACK_TAC
   BIGNUM_INV_SM2_EXEC BIGNUM_INV_SM2_CORRECT
