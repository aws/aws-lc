...
let BIGNUM_INV_SM2_EXEC = X86_MK_CORE_EXEC_RULE bignum_inv_sm2_mc;;
...
let CORE_INV_SM2_EXEC =
    X86_MK_EXEC_RULE
     ((GEN_REWRITE_CONV RAND_CONV [bignum_inv_sm2_mc] THENC TRIM_LIST_CONV)
      `TRIM_LIST (17,18) bignum_inv_sm2_mc`);;
...
let BIGNUM_INV_SM2_CORRECT = time prove
 (`!z x n pc stackpointer.
        ...
        ==> ensures x86
             (\s. bytes_loaded s (word pc) (BUTLAST bignum_inv_sm2_mc) /\
  ...
  X86_SUBROUTINE_SIM_TAC
      ((GEN_REWRITE_CONV RAND_CONV [bignum_inv_sm2_mc] THENC
        REWRITE_CONV[BUTLAST_CLAUSES])
       `BUTLAST bignum_inv_sm2_mc`,
       BIGNUM_INV_SM2_EXEC,
       0x11,
       (GEN_REWRITE_CONV RAND_CONV [bignum_inv_sm2_mc] THENC TRIM_LIST_CONV)
       `TRIM_LIST (17,18) bignum_inv_sm2_mc`,
...
let BIGNUM_INV_SM2_SUBROUTINE_CORRECT = time prove
 (`!z x n pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 288),288))
            [(word pc,0x156e); (x,8 * 4)] /\
        ALL (nonoverlapping (z,8 * 4))
            [(word_sub stackpointer (word 288),296); (word pc,0x156e)]
        ==> ensures x86
  ...
  X86_PROMOTE_RETURN_STACK_TAC bignum_inv_sm2_mc
...
let windows_bignum_inv_sm2_mc = define_from_elf "windows_bignum_inv_sm2_mc"
      "x86/sm2/bignum_inv_sm2.obj";;

let WINDOWS_BIGNUM_INV_SM2_SUBROUTINE_CORRECT = time prove
 (`!z x n pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 304),304))
            [(word pc,0x1578); (x,8 * 4)] /\
        ALL (nonoverlapping (z,8 * 4))
            [(word_sub stackpointer (word 304),312); (word pc,0x1578)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) windows_bignum_inv_sm2_mc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
  ...
  WINDOWS_X86_WRAP_STACK_TAC windows_bignum_inv_sm2_mc bignum_inv_sm2_mc
     BIGNUM_INV_SM2_CORRECT
      `[RBX; RBP; R12; R13; R14; R15]` 288);;
