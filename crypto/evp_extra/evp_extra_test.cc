...
TEST(EVPExtraTest, Ed25519i2dPrivateKey) {
  ...
  bssl::UniquePtr<EVP_PKEY> pkey(
      EVP_PKEY_new_raw_private_key(EVP_PKEY_ED25519, nullptr, priv, 32));
  ASSERT_TRUE(pkey);

  // Roundtrip through i2d_PrivateKey / d2i_PrivateKey.
  uint8_t *der_buf = nullptr;
  int der_len = i2d_PrivateKey(pkey.get(), &der_buf);
  ASSERT_GT(der_len, 0);
  bssl::UniquePtr<uint8_t> der_ptr(der_buf);

  const uint8_t *inp = der_buf;
  bssl::UniquePtr<EVP_PKEY> decoded(
      d2i_PrivateKey(EVP_PKEY_ED25519, nullptr, &inp, der_len));
  ASSERT_TRUE(decoded);
  EXPECT_EQ(inp, der_buf + der_len);
  EXPECT_EQ(EVP_PKEY_ED25519, EVP_PKEY_id(decoded.get()));

  // The private keys must match.
  uint8_t raw_orig[32], raw_dec[32];
  size_t len_orig = sizeof(raw_orig), len_dec = sizeof(raw_dec);
  ASSERT_TRUE(EVP_PKEY_get_raw_private_key(pkey.get(), raw_orig, &len_orig));
  ASSERT_TRUE(EVP_PKEY_get_raw_private_key(decoded.get(), raw_dec, &len_dec));
  EXPECT_EQ(Bytes(raw_orig, len_orig), Bytes(raw_dec, len_dec));

  // The public keys must match.
  EXPECT_EQ(1, EVP_PKEY_cmp(pkey.get(), decoded.get()));

  // d2i_PrivateKey with the wrong type should fail.
  inp = der_buf;
  bssl::UniquePtr<EVP_PKEY> wrong_type(
      d2i_PrivateKey(EVP_PKEY_RSA, nullptr, &inp, der_len));
  ...
  bssl::UniquePtr<EVP_PKEY> auto_decoded(
      d2i_AutoPrivateKey(nullptr, &inp, der_len));
  ASSERT_TRUE(auto_decoded);
  EXPECT_EQ(EVP_PKEY_ED25519, EVP_PKEY_id(auto_decoded.get()));
  EXPECT_EQ(1, EVP_PKEY_cmp(pkey.get(), auto_decoded.get()));
}

TEST(EVPExtraTest, Ed25519i2dPublicKey) {
  // Generate an Ed25519 key pair and extract the public key.
  uint8_t pub[32], priv[64];
  ED25519_keypair(pub, priv);
  bssl::UniquePtr<EVP_PKEY> pkey(
  ...
  // Roundtrip through i2d_PublicKey / d2i_PublicKey.
  uint8_t *der_buf = nullptr;
  ...
  const uint8_t *inp = der_buf;
  bssl::UniquePtr<EVP_PKEY> decoded(
      d2i_PublicKey(EVP_PKEY_ED25519, nullptr, &inp, der_len));
  ASSERT_TRUE(decoded);
  EXPECT_EQ(inp, der_buf + der_len);
  EXPECT_EQ(EVP_PKEY_ED25519, EVP_PKEY_id(decoded.get()));
  ...
  inp = der_buf;
  bssl::UniquePtr<EVP_PKEY> wrong_type(
      d2i_PublicKey(EVP_PKEY_EC, nullptr, &inp, der_len));
  EXPECT_FALSE(wrong_type);
  EXPECT_TRUE(ErrorEquals(ERR_peek_last_error(), ERR_LIB_EVP,
                          EVP_R_DIFFERENT_KEY_TYPES));
  ERR_clear_error();
