...
// reachable through |EVP_PKEY_kem_new_raw_secret_key|.
TEST_P(KEMTest, PubCmpExpandedPrivateKey) {
  ...
  // |public_key|. Every PKCS#8 CHOICE now populates it, but
  // |EVP_PKEY_kem_new_raw_secret_key| still takes raw bytes at face value and
  // leaves it unset, so drive the branch through that.
  size_t sk_len = 0;
  ASSERT_TRUE(EVP_PKEY_get_raw_private_key(priv_pkey.get(), nullptr, &sk_len));
  std::vector<uint8_t> sk(sk_len);
  ASSERT_TRUE(EVP_PKEY_get_raw_private_key(priv_pkey.get(), sk.data(), &sk_len));
  bssl::UniquePtr<EVP_PKEY> raw_pkey(
      EVP_PKEY_kem_new_raw_secret_key(test.nid, sk.data(), sk_len));
  ASSERT_TRUE(raw_pkey);
  ASSERT_EQ(raw_pkey->pkey.kem_key->public_key, nullptr);
  EXPECT_EQ(EVP_PKEY_cmp(pub_pkey.get(), raw_pkey.get()), -2);
  EXPECT_EQ(EVP_PKEY_cmp(raw_pkey.get(), pub_pkey.get()), -2);
}

// The expandedKey CHOICE recovers the public key, so unlike before this change a
// key parsed from it works through the public |EVP_PKEY_CTX| encapsulate path.
// That path failing was the old behaviour, and is what ACCP pins.
TEST_P(KEMTest, ExpandedPrivateKeyEncapsDecapsRoundTrip) {
  const KEMTestVector &test = GetParam();

  uint8_t *der = nullptr;
  long der_len = 0;
  ASSERT_TRUE(PEM_to_DER(test.private_pem_expanded_str, &der, &der_len));
  bssl::UniquePtr<uint8_t> free_der(der);
...
// Re-encodes |der| with a third OCTET STRING appended inside the |both|
// SEQUENCE, so the CHOICE element itself is well-formed and complete but its
// contents carry trailing data.
static bool AppendThirdElementToBoth(const uint8_t *der, size_t der_len,
                                     std::vector<uint8_t> *out) {
  CBS pkcs8, algorithm, private_key, sequence;
  uint64_t version = 0;
  CBS_init(&pkcs8, der, der_len);
  if (!CBS_get_asn1(&pkcs8, &pkcs8, CBS_ASN1_SEQUENCE) ||
      !CBS_get_asn1_uint64(&pkcs8, &version) ||
      !CBS_get_asn1_element(&pkcs8, &algorithm, CBS_ASN1_SEQUENCE) ||
      !CBS_get_asn1(&pkcs8, &private_key, CBS_ASN1_OCTETSTRING) ||
      !CBS_get_asn1(&private_key, &sequence, CBS_ASN1_SEQUENCE)) {
    return false;
  }

  bssl::ScopedCBB cbb;
  CBB seq, pk, both, extra;
  if (!CBB_init(cbb.get(), der_len + 16) ||
      !CBB_add_asn1(cbb.get(), &seq, CBS_ASN1_SEQUENCE) ||
      !CBB_add_asn1_uint64(&seq, version) ||
      !CBB_add_bytes(&seq, CBS_data(&algorithm), CBS_len(&algorithm)) ||
      !CBB_add_asn1(&seq, &pk, CBS_ASN1_OCTETSTRING) ||
      !CBB_add_asn1(&pk, &both, CBS_ASN1_SEQUENCE) ||
      // The original seed and expandedKey, verbatim.
      !CBB_add_bytes(&both, CBS_data(&sequence), CBS_len(&sequence)) ||
      // A third element the grammar does not allow.
      !CBB_add_asn1(&both, &extra, CBS_ASN1_OCTETSTRING) ||
      !CBB_add_u8(&extra, 0x00) || !CBB_flush(cbb.get())) {
    return false;
  }
...
// SEQUENCE. Appending after the SEQUENCE only reaches |CBS_len(key) != 0|; this
// covers the separate |CBS_len(&sequence) != 0| check.
TEST(KEMTest, ParsePrivateKeyBothTrailingDataInsideSequence) {
  uint8_t *der = nullptr;
  long der_len = 0;
  ...
  // The unmodified key parses, so the rejection below is attributable to the
  // third element rather than to the re-encoding.
  CBS cbs;
  CBS_init(&cbs, der, der_len);
  ASSERT_TRUE(bssl::UniquePtr<EVP_PKEY>(EVP_parse_private_key(&cbs)));

  std::vector<uint8_t> bad;
  ASSERT_TRUE(AppendThirdElementToBoth(der, der_len, &bad));

  ERR_clear_error();
  CBS bad_cbs;
  CBS_init(&bad_cbs, bad.data(), bad.size());
  EXPECT_FALSE(bssl::UniquePtr<EVP_PKEY>(EVP_parse_private_key(&bad_cbs)));
  uint32_t err = ERR_get_error();
  EXPECT_EQ(ERR_GET_LIB(err), ERR_LIB_EVP);
  EXPECT_EQ(ERR_GET_REASON(err), EVP_R_DECODE_ERROR);
  ERR_clear_error();
...
// Re-encodes the |both| SEQUENCE of |der| with its seed truncated to |seed_len|
// and its expandedKey to |expanded_len|, leaving the encoding well-formed so
// that parsing reaches the component length checks rather than failing earlier.
static bool ReencodeBothWithLengths(const uint8_t *der, size_t der_len,
                                    size_t seed_len, size_t expanded_len,
                                    std::vector<uint8_t> *out) {
  CBS pkcs8, algorithm, private_key, sequence, seed, expanded;
  uint64_t version = 0;
  CBS_init(&pkcs8, der, der_len);
  if (!CBS_get_asn1(&pkcs8, &pkcs8, CBS_ASN1_SEQUENCE) ||
      !CBS_get_asn1_uint64(&pkcs8, &version) ||
      ...
      !CBS_get_asn1(&sequence, &expanded, CBS_ASN1_OCTETSTRING) ||
      seed_len > CBS_len(&seed) || expanded_len > CBS_len(&expanded)) {
    return false;
  }

  bssl::ScopedCBB cbb;
  CBB seq, pk, both, seed_cbb, expanded_cbb;
  if (!CBB_init(cbb.get(), der_len + 16) ||
      !CBB_add_asn1(cbb.get(), &seq, CBS_ASN1_SEQUENCE) ||
      !CBB_add_asn1_uint64(&seq, version) ||
      !CBB_add_bytes(&seq, CBS_data(&algorithm), CBS_len(&algorithm)) ||
      ...
      !CBB_add_bytes(&seed_cbb, CBS_data(&seed), seed_len) ||
      !CBB_add_asn1(&both, &expanded_cbb, CBS_ASN1_OCTETSTRING) ||
      !CBB_add_bytes(&expanded_cbb, CBS_data(&expanded), expanded_len) ||
      !CBB_flush(cbb.get())) {
    return false;
  }
...
// rejected before any key generation is attempted. The encodings below stay
// well-formed DER, so parsing reaches the length checks instead of failing
// while reading the SEQUENCE.
TEST(KEMTest, ParsePrivateKeyBothInvalidLength) {
  ...
  const size_t kSeedLen = 64, kExpandedLen = 1632;
  const struct {
    const char *name;
    size_t seed_len;
    size_t expanded_len;
  } kCases[] = {
      {"seed one byte short", kSeedLen - 1, kExpandedLen},
      {"expandedKey one byte short", kSeedLen, kExpandedLen - 1},
  };

  for (const auto &t : kCases) {
    SCOPED_TRACE(t.name);
    std::vector<uint8_t> bad;
    ASSERT_TRUE(ReencodeBothWithLengths(der, der_len, t.seed_len,
                                        t.expanded_len, &bad));
  ...
  // Re-encoding with the correct lengths must parse, so the rejections above
  // are attributable to the lengths rather than to the re-encoding.
  std::vector<uint8_t> good;
  ASSERT_TRUE(
      ReencodeBothWithLengths(der, der_len, kSeedLen, kExpandedLen, &good));
  CBS good_cbs;
  CBS_init(&good_cbs, good.data(), good.size());
  EXPECT_TRUE(bssl::UniquePtr<EVP_PKEY>(EVP_parse_private_key(&good_cbs)));

  // Truncated DER stays covered as separate malformed-input coverage: the |both|
  // SEQUENCE contents no longer parse as two complete OCTET STRINGs.
  ERR_clear_error();
  CBS cbs;
  CBS_init(&cbs, der, der_len - 1);
  EXPECT_FALSE(bssl::UniquePtr<EVP_PKEY>(EVP_parse_private_key(&cbs)));
  ERR_clear_error();
}
