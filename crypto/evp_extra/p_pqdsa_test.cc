...
static void print_vector_as_hex_array(const uint8_t* data, size_t length) {
...
TEST_P(PQDSAParameterTest, KAT2) {
  /*
  std::string kat_filepath = "crypto/";
  kat_filepath += GetParam().kat_filename;
...
*/
  ...
  const uint8_t kKeyGenSeed[MLDSA44_KEYGEN_SEED_BYTES] = {
  ...
  // Generate key pair from seed xi and assert that public and private keys
  // are equal to expected values from KAT
  ASSERT_TRUE(GetParam().keygen(pub.data(), priv.data(), kKeyGenSeed));
  ...
  ASSERT_TRUE(GetParam().sign(priv.data(),
                             ...
                             rng));
  if (nid == NID_MLDSA44) {
    print_vector_as_hex_array(signature.data(), signature.size());
  }
