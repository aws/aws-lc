...
static bool message_in_errors(string expected_message) {
...
// the test callback immedatly, and then again when BORINGSSL_self_test is called
...
static int expected_runtime_fips_mode() {
  char* broken_kat = getenv("FIPS_CALLBACK_TEST_POWER_ON_TEST_FAILURE");
  return broken_kat == nullptr ? 1 : 0;
}
...
TEST(FIPSCallback, RSARuntimeTest) {
  ...
  if (broken_runtime_test != nullptr && (strcmp(broken_runtime_test, "RSA_PWCT" ) == 0 ||
    ...
    ASSERT_EQ(expected_runtime_fips_mode(), FIPS_mode());
