  ...
  BIO_ADDR ToBIO_ADDR() const {
    ...
    switch (family()) {
      case AF_INET:
        ...
        break;
      case AF_UNIX:
        OPENSSL_memcpy(&bap.s_in, &storage, sizeof(sockaddr_un));
        break;
      default:
        OPENSSL_memcpy(&bap.s_in, &storage, sizeof(sockaddr_in6));
        break;
    }
    return bap;
  }
...
class OwnedSocket {
 public:
  ...
  OwnedSocket(OwnedSocket &&other) noexcept { *this = std::move(other); }
  ~OwnedSocket() { reset(); }
...
static OwnedSocket ListenLoopback(int type, int backlog = 1) {
  // Try binding to IPv6.
  sockaddr_in6 sin6 = {};
  ...
  OwnedSocket sock = Bind(
      AF_INET6, type, reinterpret_cast<const sockaddr *>(&sin6), sizeof(sin6));
  if (!sock.is_valid()) {
    // Try binding to IPv4.
    sockaddr_in sin{};
...
static bssl::UniquePtr<BIO> create_server_bio(int type) {
  // Create a server socket.
  OwnedSocket server_sock(ListenLoopback(type));
  EXPECT_TRUE(server_sock.is_valid()) << LastSocketError();
...
static bssl::UniquePtr<BIO> create_client_bio(int addr_family, int type) {
  ...
  OwnedSocket client_sock(socket(addr_family, type, 0));
  EXPECT_TRUE(client_sock.is_valid()) << LastSocketError();
...
static void test_send_receive(bssl::UniquePtr<BIO> &sender_bio,
  ...
  EXPECT_EQ(Bytes(buff), Bytes(kTestMessage));
}


TEST(BIOTest, SocketDatagramSetPeer) {
  // Wrap the server socket in a BIO.
  bssl::UniquePtr<BIO> server_bio = create_server_bio(SOCK_DGRAM);
  ASSERT_EQ(1, BIO_get_close(server_bio.get()));
...
TEST(BIOTest, SocketDatagramSetConnected) {
  // Wrap the server socket in a BIO.
  bssl::UniquePtr<BIO> server_bio = create_server_bio(SOCK_DGRAM);
  ASSERT_TRUE(server_bio);
  ...
  -1;
#else
    (int)OPENSSL_strnlen(kTestMessage, 32) + 1;
#endif

  ASSERT_EQ(expected_result, BIO_write(client_bio.get(), kTestMessage, sizeof(kTestMessage)))
      << LastSocketError();

}

TEST(BIOTest, SocketDatagramConnect) {
  // Wrap the server socket in a BIO.
  bssl::UniquePtr<BIO> server_bio = create_server_bio(SOCK_DGRAM);
  ASSERT_TRUE(server_bio);
