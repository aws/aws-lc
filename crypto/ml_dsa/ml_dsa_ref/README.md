...
To test ExternalMu modes of ML-DSA, we provide three test functions within `crypto/evp_extra/p_pqdsa_test.cc` that include `PerMLDSATest.ACVPKeyGen`, `PerMLDSATest.ACVPSigGen` and `PerMLDSATest.ACVPSigVer`. The corresponding KAT files `MLDSA_44_ACVP_{keyGen/sigSen/sigVer}.txt` were constructed from the vectors provided by NIST at https://github.com/usnistgov/ACVP-Server. To test ExternalMu in non-deterministic mode, we use `tcId = 19, 21, 23` of sigGen and `tcId = 7, 11, 13` of sigVer.
