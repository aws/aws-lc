...
func makeTestCase(length int, options options) (*testCase, error) {
	...
	input := make([]byte, eiv_payload_len)
	...
	// TODO: check why golang does not allow ad := adFull, which still references adFull
	copy(ad, adFull)
	ad[len(ad)-2] = uint8(length >> 8)
	ad[len(ad)-1] = uint8(length & 0xff)
	...
	// TODO: generate macKey with large size to test |EVP_CTRL_AEAD_SET_MAC_KEY|.
	// More importantly, how does the macKey in TLS gets generated?
	macKey := make([]byte, hash.Size())
	rand.fillBytes(macKey)

	// TODO(DONE): |hmac.New(hash.New, macKey)| handles the case when the key size is larger than block size.
	// https://pkg.go.dev/crypto/hmac#New
	// https://cs.opensource.google/go/go/+/refs/tags/go1.17.8:src/crypto/hmac/hmac.go;l=148-151
