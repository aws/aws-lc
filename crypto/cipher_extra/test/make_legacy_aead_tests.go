...
func main() {
	...
	if *testStitch {
		if *bulkCipher != "aes128" {
			panic("'-stitch' is used to test |EVP_aes_128_cbc_hmac_sha1/256|. The '-cipher' should be 'aes128'.")
		}
		if !((*mac == "sha1") || (*mac == "sha256")) {
			panic("'-stitch' is used to test |EVP_aes_128_cbc_hmac_sha1/256|. The '-mac' should be 'sha1/sha256'.")
		}
	}
