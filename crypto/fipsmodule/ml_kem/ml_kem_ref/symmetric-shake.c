 ...
 *
 * Arguments:   - KECCAK1600_CTX *ctx: pointer to (uninitialized) output Keccak
 *state
...
void kyber_shake256_prf(uint8_t *out, size_t outlen,
  ...
  // Return code checks can be omitted
  // SHAKE256 never returns NULL when the internal SHAKE_Init is called with
  // correct block size value
 ...
 *              - uint8_t nonce: single-byte nonce (public PRF input)
 **************************************************/
void kyber_shake256_rkprf(ml_kem_params *params, uint8_t out[KYBER_SSBYTES],
  ...
  // SHAKE_Absorb always returns 1 processing all data blocks that don't need
  // pad
