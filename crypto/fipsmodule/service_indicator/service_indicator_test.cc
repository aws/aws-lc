...
const uint8_t kDRBGNonce[CTR_DRBG_NONCE_LEN] = {
    'D', 'B', 'R', 'G', ' ', 'N', 'o', 'n',
    'c', 'e', ' ', ' ', ' ', ' ', ' ', ' '};
...
const uint8_t kDRBGOutput[64] = {
    0x9a, 0xad, 0x7c, 0x67, 0x38, 0xd9, 0xb3, 0x25,
    0x67, 0x56, 0x92, 0xf6, 0xa6, 0x5d, 0x75, 0x07,
    0xbd, 0xe7, 0x76, 0x49, 0x6f, 0x78, 0xbc, 0x73,
    0x4d, 0xd0, 0x68, 0x86, 0x97, 0x6d, 0xcc, 0xa9,
    0xbc, 0x7b, 0xac, 0xe0, 0xd7, 0x33, 0x0a, 0xe5,
    0xb9, 0x14, 0x5b, 0x7c, 0x3f, 0xb9, 0x89, 0x4c,
    0xfb, 0xf3, 0x26, 0xa6, 0x1f, 0x6c, 0x30, 0xfb,
    0xe2, 0x28, 0x0d, 0x9c, 0xec, 0xbc, 0x92, 0x15};
...
static const uint8_t kDRBGReseedOutput[64] = {
    0xf2, 0xe0, 0x25, 0xda, 0xad, 0x5b, 0x88, 0x63,
    0x32, 0x7b, 0x36, 0xc5, 0xad, 0x9a, 0x8c, 0x91,
    0xbf, 0xa9, 0x82, 0xc8, 0x83, 0xe2, 0x7b, 0x89,
    0x2f, 0x40, 0xfa, 0xa0, 0xb1, 0xc7, 0x00, 0xa7,
    0x4d, 0xca, 0xa1, 0xff, 0xb2, 0x0a, 0x1c, 0x16,
    0xda, 0xf3, 0x96, 0x34, 0x56, 0x9c, 0x4e, 0xd7,
    0x0f, 0x84, 0x8d, 0x9b, 0x8e, 0xf4, 0xba, 0xf8,
    0xc7, 0xc1, 0x99, 0xc2, 0x15, 0x79, 0x80, 0x0f,
};
...
TEST(ServiceIndicatorTest, DRBG) {
  ...
  // indicate that a service has been fully completed yet.
  CALL_SERVICE_AND_CHECK_APPROVED(approved,
      ASSERT_TRUE(CTR_DRBG_init_df(&drbg, kDRBGEntropy, kDRBGPersonalization,
                                sizeof(kDRBGPersonalization), kDRBGNonce)));
  EXPECT_EQ(approved, AWSLC_NOT_APPROVED);
  CALL_SERVICE_AND_CHECK_APPROVED(approved,
      ASSERT_TRUE(CTR_DRBG_generate_df(&drbg, output, sizeof(kDRBGOutput), kDRBGAD,
                                    sizeof(kDRBGAD))));
  EXPECT_EQ(approved, AWSLC_APPROVED);
  EXPECT_EQ(Bytes(kDRBGOutput), Bytes(output));

  CALL_SERVICE_AND_CHECK_APPROVED(approved,
      ASSERT_TRUE(CTR_DRBG_reseed_df(&drbg, kDRBGEntropy2, kDRBGAD,
                                  sizeof(kDRBGAD))));
  EXPECT_EQ(approved, AWSLC_NOT_APPROVED);
  CALL_SERVICE_AND_CHECK_APPROVED(approved,
      ASSERT_TRUE(CTR_DRBG_generate_df(&drbg, output, sizeof(kDRBGReseedOutput),
...
TEST(ServiceIndicatorTest, AWSLCVersionString) {
  ASSERT_STREQ(awslc_version_string(), "AWS-LC FIPS 3.2.0");
}
...
// Update this when the AWS-LC version number is modified
TEST(ServiceIndicatorTest, AWSLCVersionString) {
  ASSERT_STREQ(awslc_version_string(), "AWS-LC 3.2.0");
}
#endif // AWSLC_FIPS
