...
//  [in]  exp      - array of 2 pointers to {16,24,32} qword values in 2^64
//  radix.
...
int RSAZ_mod_exp_avx512_x2(uint64_t *res1, const uint64_t *base1,
                           const uint64_t *exp1, const uint64_t *m1,
                           const uint64_t *rr1, uint64_t k0_1, uint64_t *res2,
                           const uint64_t *base2, const uint64_t *exp2,
                           const uint64_t *m2, const uint64_t *rr2,
                           uint64_t k0_2, int modlen) {
#ifdef BORINGSSL_DISPATCH_TEST
  BORINGSSL_function_hit[8] = 1;
#endif
  typedef void (*AMM)(uint64_t *res, const uint64_t *a, const uint64_t *b,
                      const uint64_t *m, uint64_t k0);
  int ret = 0;
  ...
  int storage_len_bytes =
      7 * regs_capacity * sizeof(uint64_t) + 64;  // alignment
  ...
  set_bit(coeff_red,
          64 * (int)(bitlen_diff / DIGIT_SIZE) + bitlen_diff % DIGIT_SIZE);
  ...
  ret = rsaz_mod_exp_x2_ifma256(rr1_red, base1_red, exp, m1_red, rr1_red, k0,
                                modlen);
  if (!ret)
    goto err;
...
int rsaz_mod_exp_x2_ifma256(uint64_t *out, const uint64_t *base,
                            const uint64_t *exp[2], const uint64_t *m,
                            const uint64_t *rr, const uint64_t k0[2],
                            int modlen) {
  typedef void (*DAMM)(uint64_t *res, const uint64_t *a, const uint64_t *b,
                       const uint64_t *m, const uint64_t k0[2]);
...
#define DAMS(r, a, m, k0) damm((r), (a), (a), (m), (k0))
  ...
  for (idx = 1; idx < (int)(two_to_exp_win_size / 2); idx++) {
    ...
    damm(&red_table[(2 * idx + 1) * 2 * red_digits],
         &red_table[(2 * idx) * 2 * red_digits], &red_table[1 * 2 * red_digits],
         m, k0);
  }
...
#define EXP_CHUNK1(i) (exp_chunk_no) + 1 + ((i) * (exp_digits + 1))
    ...
    for (exp_bit_no -= exp_win_size; exp_bit_no >= 0;
      ...
      damm((uint64_t *)red_Y, (const uint64_t *)red_Y, (const uint64_t *)red_X,
           m, k0);
    }
  }
