...
static int boringssl_self_test_ecdh_p384(void) {
  int ret = 0;
  static const uint8_t kD[] = {
  ...
  static const uint8_t kPubkey[] = {
      0x04, 0x75, 0x6c, 0x07, 0xdf, 0x0c, 0xe3, 0x2c, 0x83, 0x9d, 0xac,
      0x9f, 0xb4, 0x73, 0x3c, 0x9c, 0x28, 0xb7, 0x01, 0x13, 0xa6, 0x76,
      0xa7, 0x05, 0x7c, 0x38, 0xd2, 0x23, 0xf2, 0x2a, 0x3a, 0x90, 0x95,
      0xa8, 0xd5, 0x64, 0x65, 0x3a, 0xf5, 0x28, 0xe0, 0x4c, 0x7e, 0x18,
      0x24, 0xbe, 0x4a, 0x65, 0x12, 0x17, 0xc2, 0xce, 0x69, 0x62, 0xcb,
      0xd2, 0xa2, 0xe0, 0x66, 0x29, 0x7b, 0x39, 0xd5, 0x7d, 0xd9, 0xbb,
      0x46, 0x80, 0xf0, 0x19, 0x1d, 0x39, 0x0f, 0x70, 0xb4, 0xe4, 0x61,
      0x41, 0x9b, 0x29, 0x72, 0xce, 0x68, 0xad, 0x46, 0x12, 0x7f, 0xdd,
      0xa6, 0xc3, 0x91, 0x95, 0x77, 0x4e, 0xa8, 0x6d, 0xf3,
  };
  static const uint8_t kPeerkey[] = {
      0x04, 0x78, 0x8b, 0xe2, 0x33, 0x6c, 0x52, 0xf4, 0x45, 0x4d, 0x63,
      0xee, 0x94, 0x4b, 0x1e, 0x49, 0xbf, 0xb6, 0x19, 0xa0, 0x83, 0x71,
      0x04, 0x8e, 0x6d, 0xa9, 0x2e, 0x58, 0x4e, 0xae, 0x70, 0xbd, 0xe1,
      0xf1, 0x71, 0xc4, 0xdf, 0x37, 0x8b, 0xd1, 0xf3, 0xc0, 0xab, 0x03,
      0x04, 0x8a, 0x23, 0x78, 0x02, 0x46, 0x73, 0xeb, 0xd8, 0xdb, 0x60,
      0x4e, 0xaf, 0x41, 0x71, 0x17, 0x48, 0xba, 0xb2, 0x96, 0x8a, 0x23,
      0xca, 0x44, 0x76, 0xce, 0x14, 0x4e, 0x72, 0x82, 0x47, 0xf0, 0x8a,
      0xf7, 0x52, 0x92, 0x91, 0x57, 0xb5, 0x83, 0x0f, 0x1e, 0x26, 0x06,
      0x74, 0x66, 0xbd, 0xfa, 0x8b, 0x65, 0x14, 0x5a, 0x33,
  };
  static const uint8_t kECDHSecret[] = {
      0x79, 0x3b, 0xb9, 0xcd, 0x22, 0xa9, 0x3c, 0xf4, 0x68, 0xfa, 0xf8,
      0x04, 0xa3, 0x8d, 0x12, 0xb7, 0x8c, 0xb1, 0x21, 0x89, 0xec, 0x67,
      0x9d, 0xdd, 0x2e, 0x9a, 0xa2, 0x1f, 0xa9, 0xa5, 0xa0, 0xb0,
  };
  ...
  BIGNUM *d = BN_bin2bn(kD, sizeof(kD), NULL);
  if (key == NULL || d == NULL || !EC_KEY_set_group(key, group) ||
      !EC_KEY_set_private_key(key, d) ||
      !EC_POINT_oct2point(group, pub_key, kPubkey, sizeof(kPubkey), NULL) ||
      !EC_POINT_oct2point(group, peer_pub_key, kPeerkey, sizeof(kPeerkey),
                          NULL) ||
      !EC_KEY_set_public_key(key, pub_key) || !EC_KEY_check_key(key)) {
    goto err;
  }

  uint8_t output[EC_MAX_BYTES];
  size_t output_len = sizeof(output);
  if (!ecdh_compute_shared_secret_no_self_test(output, &output_len,
                                               peer_pub_key, key) ||
      !check_test(kECDHSecret, output, sizeof(kECDHSecret), "ECDH KAT")) {
    fprintf(stderr, "ECDH KAT failed.\n");
    goto err;
  }
...
static int boringssl_self_test_ecc(void) {
  ...
  if (!boringssl_self_test_ecdh_p384()) {
    goto err;
  }
...
static void run_self_test_rsa(void) {
  if (!boringssl_self_test_rsa()) {
    AWS_LC_FIPS_error("run_self_test_rsa failed.", ERR_R_FIPS_TEST_FAILURE);
  }
}
...
static void run_self_test_ecc(void) {
  if (!boringssl_self_test_ecc()) {
    AWS_LC_FIPS_error("run_self_test_ecc failed.", ERR_R_FIPS_TEST_FAILURE);
  }
}
...
static void run_self_test_ffdh(void) {
  if (!boringssl_self_test_ffdh()) {
    AWS_LC_FIPS_error("run_self_test_ffdh failed.", ERR_R_FIPS_TEST_FAILURE);
  }
}
...
static int boringssl_self_test_fast(void) {
  ...
  static const uint8_t kAESKey_256[32] = "AWS-LCCrypto 256-bit Long Key!!!";
  ...
  static const uint8_t kAESGCM128EncPlaintext[32] = {
      0x8f, 0xcc, 0x40, 0x99, 0x80, 0x8e, 0x75, 0xca, 0xaf, 0xf5, 0x82, 0x89,
      0x88, 0x48, 0xa8, 0x8d, 0x80, 0x8b, 0x55, 0xab, 0x4e, 0x93, 0x70, 0x79,
      0x7d, 0x94, 0x0b, 0xe8, 0xcc, 0x1d, 0x78, 0x84,
  };
  static const uint8_t kAESGCM128Ciphertext[sizeof(kAESGCM128EncPlaintext) +
                                            16] = {
  ...
  if (!EVP_AEAD_CTX_seal(&aead_ctx, output, &out_len, sizeof(output), nonce,
                         ...
                         kAESGCM128EncPlaintext, sizeof(kAESGCM128EncPlaintext),
                         NULL, 0) ||
      !check_test(kAESGCM128Ciphertext, output, sizeof(kAESGCM128Ciphertext),
                  "AES-128-GCM-encrypt KAT")) {
    fprintf(stderr, "AES-128-GCM-encrypt failed.\n");
    goto err;
  }

  // AES-GCM-128 Decryption KAT
  static const uint8_t kAESGCM128DecCiphertext[48] = {
  ...
  static const uint8_t
      kAESGCM128DecPlaintext[sizeof(kAESGCM128DecCiphertext) - 16] = {
  ...
  if (!EVP_AEAD_CTX_open(&aead_ctx, output, &out_len, sizeof(output), nonce,
                         EVP_AEAD_nonce_length(EVP_aead_aes_128_gcm()),
                         kAESGCM128DecCiphertext,
                         sizeof(kAESGCM128DecCiphertext), NULL, 0) ||
      !check_test(kAESGCM128DecPlaintext, output,
                  sizeof(kAESGCM128DecPlaintext), "AES-128-GCM-decrypt KAT")) {
    fprintf(stderr, "AES-128-GCM-decrypt failed.\n");
    goto err;
  }

  EVP_AEAD_CTX_zero(&aead_ctx);
  if (!EVP_AEAD_CTX_init(&aead_ctx, EVP_aead_aes_256_gcm(), kAESKey_256,
                         sizeof(kAESKey_256), 0, NULL)) {
    fprintf(stderr, "EVP_AEAD_CTX_init for AES-256-GCM failed.\n");
    goto err;
  }

  // AES-GCM-256 Encryption KAT
  static const uint8_t kAESGCM256EncPlaintext[32] = {
      0xb1, 0x2e, 0x6f, 0x1f, 0x81, 0x60, 0xcd, 0x65, 0x27, 0x8c, 0x48, 0xf6,
      0x8a, 0xd5, 0x3c, 0x8c, 0x82, 0xfd, 0x17, 0xc2, 0xc3, 0x9b, 0xbb, 0x10,
      0x9f, 0x07, 0x7c, 0x17, 0xfd, 0xcb, 0x8a, 0x0b
  };
  static const uint8_t kAESGCM256Ciphertext[sizeof(kAESGCM256EncPlaintext) +
                                            16] = {
      0x62, 0x26, 0x92, 0xb6, 0x2a, 0x63, 0x74, 0x09, 0xf1, 0x73, 0xab, 0x97,
      0x49, 0x67, 0x32, 0x6d, 0xcc, 0x76, 0x78, 0xf4, 0xec, 0x74, 0x59, 0xa9,
      0xd1, 0xfb, 0xf8, 0xe0, 0x29, 0xf1, 0x46, 0x98, 0x8b, 0x5b, 0x46, 0xeb,
      0xba, 0x0a, 0x1d, 0xa2, 0xb8, 0x8a, 0x4e, 0x8c, 0x91, 0x44, 0x6a, 0xd6,
  };
  if (!EVP_AEAD_CTX_seal(&aead_ctx, output, &out_len, sizeof(output), nonce,
                         EVP_AEAD_nonce_length(EVP_aead_aes_256_gcm()),
                         kAESGCM256EncPlaintext, sizeof(kAESGCM256EncPlaintext),
                         NULL, 0) ||
      !check_test(kAESGCM256Ciphertext, output, sizeof(kAESGCM256Ciphertext),
                  "AES-GCM-256-encrypt KAT")) {
    fprintf(stderr, "AES-256-GCM-encrypt failed.\n");
    goto err;
  }

  // AES-GCM-256 Decryption KAT
  static const uint8_t kAESGCM256DecCiphertext[48] = {
      0x8a, 0xf6, 0x25, 0x19, 0xb9, 0x34, 0xf0, 0x4c, 0x40, 0xc1, 0x92, 0x43,
      0x89, 0x65, 0x19, 0x1e, 0x9c, 0x3d, 0x32, 0x79, 0xd5, 0x11, 0xe2, 0xff,
      0x6c, 0xcf, 0x19, 0x3a, 0x1e, 0x57, 0xf1, 0x2d, 0x19, 0x1c, 0x71, 0xa9,
      0x92, 0x1d, 0x80, 0x40, 0xb8, 0x5d, 0x84, 0x0f, 0x7a, 0xfe, 0x6f, 0x69
  };
  static const uint8_t
      kAESGCM256DecPlaintext[sizeof(kAESGCM256DecCiphertext) - 16] = {
      0x59, 0xfe, 0xd8, 0xb0, 0x12, 0x37, 0x49, 0x20, 0x96, 0x3e, 0x71, 0x22,
      0x4a, 0xd7, 0x17, 0xff, 0xd2, 0xb6, 0x5d, 0x4f, 0xfa, 0xfe, 0x00, 0x46,
      0x22, 0x33, 0x9d, 0xcd, 0xca, 0x6d, 0x3d, 0xbe,
  };
  if (!EVP_AEAD_CTX_open(&aead_ctx, output, &out_len, sizeof(output), nonce,
                         EVP_AEAD_nonce_length(EVP_aead_aes_256_gcm()),
                         kAESGCM256DecCiphertext,
                         sizeof(kAESGCM256DecCiphertext), NULL, 0) ||
      !check_test(kAESGCM256DecPlaintext, output,
                  sizeof(kAESGCM256DecPlaintext), "AES-256-GCM-decrypt KAT")) {
    fprintf(stderr, "AES-256-GCM-decrypt failed.\n");
    goto err;
  }
