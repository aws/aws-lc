...
static int boringssl_self_test_rsa(void) {
  ...
  unsigned sig_len;
  if (!rsa_digestsign_no_self_test(EVP_sha256(), kRSASignDigest,
                           sizeof(kRSASignDigest),output, &sig_len, rsa_key) ||
  ...
  if (!rsa_digestverify_no_self_test(EVP_sha256(), kRSAVerifyDigest,
