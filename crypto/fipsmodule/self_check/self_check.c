...
#define MAX_NAME 33
#define MAX_ERROR_MSG_SIZE \
  ((2 * MAX_HEXDUMP_SIZE) + sizeof(ERROR_FORMAT) + MAX_NAME)

static void hexdump(char buf[MAX_HEXDUMP_SIZE], const uint8_t *in,
                    size_t in_len) {
...
static ECDSA_SIG *parse_ecdsa_sig(const uint8_t *in, size_t in_len) {
  ...
  if (!ret ||  //
      (in_len & 1) || BN_bin2bn(in, in_len / 2, ret->r) == NULL ||
...
static DH *self_test_ffc_dh_key(const uint8_t *p, size_t p_len,
  ...
  DH *dh = DH_new();
  if (dh == NULL || !set_bignum(&dh->p, p, p_len) ||
      !set_bignum(&dh->q, q, q_len) || !set_bignum(&dh->g, g, g_len) ||
...
static DH *self_test_ffc_dh_fb_key(void) {
  ...
  static const uint8_t kDH_p[256] = {
      ...
      0xac, 0x74, 0x73, 0x09};
  static const uint8_t kDH_q[28] = {0xcc, 0x9c, 0x34, 0x91, 0x8e, 0x8b, 0xa0,
                                    0x86, 0x23, 0xa2, 0x76, 0x82, 0xeb, 0xd1,
                                    0x98, 0x5f, 0xab, 0x27, 0x56, 0x44, 0x66,
                                    0x70, 0x50, 0xc9, 0x35, 0xdc, 0x14, 0xc7};
  ...
  const uint8_t kDH_private_key[28] = {
      0x75, 0x89, 0x8a, 0xbe, 0xc3, 0xc9, 0xc8, 0x7b, 0x04, 0x49,
      0x47, 0xf6, 0xc5, 0x1f, 0x9f, 0x71, 0x7f, 0x4a, 0x1d, 0x7c,
      0xc3, 0x9a, 0xae, 0xcd, 0x83, 0x53, 0xba, 0x25};
  const uint8_t kDH_public_key[256] = {
      ...
      0x11, 0x49, 0x04, 0x12};
  return self_test_ffc_dh_key(kDH_p, sizeof(kDH_p), kDH_q, sizeof(kDH_q), kDH_g,
                              sizeof(kDH_g), kDH_private_key,
                              sizeof(kDH_private_key), kDH_public_key,
                              sizeof(kDH_public_key));
}
...
static int boringssl_self_test_rsa(void) {
  ...
  if (!rsa_digestsign_no_self_test(EVP_sha256(), kRSASignPlaintext,
                                   sizeof(kRSASignPlaintext), output, &sig_len,
                                   rsa_key) ||
  ...
  if (!rsa_digestverify_no_self_test(
          EVP_sha256(), kRSAVerifyPlaintext, sizeof(kRSAVerifyPlaintext),
...
static int boringssl_self_test_ecc(void) {
  ...
  sig = ecdsa_digestsign_no_self_test(EVP_sha256(), kECDSASignPlaintext,
                                      sizeof(kECDSASignPlaintext), ec_key,
                                      ecdsa_k, sizeof(ecdsa_k));
  ...
  if (!sig || !ecdsa_digestverify_no_self_test(
                  EVP_sha256(), kECDSAVerifyPlaintext,
...
static int boringssl_self_test_ml_kem(void) {
  ...
  static const uint8_t kKeyGenEKSeed[MLKEM512_KEYGEN_SEED_LEN] = {
      0xf8, 0x8c, 0xb2, 0x5f, 0x89, 0xa3, 0x55, 0x5f, 0xae, 0xc6, 0x71,
      0xa1, 0xdf, 0xc6, 0xf6, 0x1d, 0x60, 0xd0, 0x62, 0x22, 0x7d, 0x6a,
      0x8f, 0xf6, 0x2b, 0x3c, 0x6d, 0x7b, 0xd6, 0x14, 0x0f, 0x66, 0x24,
      0xc0, 0x84, 0xa6, 0x4d, 0xa7, 0x4c, 0x63, 0x32, 0x7e, 0x11, 0x77,
      0x58, 0xaa, 0x33, 0x8a, 0x02, 0xe4, 0x43, 0x74, 0x10, 0xb8, 0xf9,
      0xf2, 0x00, 0x88, 0xa1, 0x29, 0xc1, 0x68, 0x3d, 0xe7};
...
static int boringssl_self_test_ml_dsa(void) {
  ...
  // Examples kMLDSAKeyGenSeed, kMLDSAKeyGenPublicKey, kMLDSAKeyGenPrivateKey
  // from
  ...
  static const uint8_t kMLDSASignSignature[MLDSA44_SIGNATURE_BYTES] = {
      };

  // Keygen
  uint8_t public_key[MLDSA44_PUBLIC_KEY_BYTES] = {0};
  uint8_t private_key[MLDSA44_PRIVATE_KEY_BYTES] = {0};

  if (!ml_dsa_44_keypair_internal_no_self_test(public_key, private_key,
                                               kMLDSAKeyGenSeed) ||
      !check_test(kMLDSAKeyGenPublicKey, public_key, sizeof(public_key),
                  "ML-DSA keyGen public") ||
      !check_test(kMLDSAKeyGenPrivateKey, private_key, sizeof(private_key),
                  "ML-DSA keyGen private")) {
    goto err;
  }
  ...
  if (!ml_dsa_44_sign_internal_no_self_test(private_key, signature, &sig_len,
                                            kMLDSASignPlaintext, mlen_int, NULL,
                                            0, kMLDSASigGenSeed) ||
      !check_test(kMLDSASignSignature, signature, sizeof(signature),
                  "ML-DSA SigGen signature")) {
    goto err;
  }

  // Verify
  if (!ml_dsa_44_verify_internal_no_self_test(public_key, kMLDSASignSignature,
...
static int boringssl_self_test_eddsa(void) {
  ...
  static const uint8_t kEd25519SignMessage[32] = {
      0x19, 0x61, 0xd1, 0xd5, 0x2d, 0x8c, 0x04, 0x5f, 0xdf, 0xc1, 0xc6,
      0x82, 0xb3, 0x5f, 0x07, 0xaa, 0xe1, 0xd3, 0xb6, 0xe5, 0x48, 0x63,
      0x98, 0x30, 0xee, 0xd9, 0x29, 0xbc, 0x12, 0x2d, 0x79, 0x9f};
  static const uint8_t kEd25519SignSignature[ED25519_SIGNATURE_LEN] = {
      0xa8, 0x81, 0xe8, 0xd9, 0x5d, 0xdb, 0xd5, 0xd1, 0x47, 0x60, 0xaf,
      0x4e, 0xcf, 0xce, 0x45, 0x96, 0xf7, 0x2e, 0x04, 0xd7, 0xee, 0xcc,
      0xb9, 0xc6, 0xa1, 0x93, 0xe2, 0x4d, 0xd7, 0x35, 0xb1, 0x3c, 0x18,
      0xa5, 0x34, 0xc7, 0x79, 0x31, 0x45, 0x46, 0x9d, 0xd1, 0x6f, 0x0c,
      0x5e, 0x03, 0x71, 0xa3, 0xfb, 0x85, 0x06, 0x35, 0x97, 0xc0, 0x92,
      0x45, 0x97, 0xcb, 0x42, 0x75, 0x60, 0xdb, 0x2a, 0x0b};

  if (!ED25519_sign_no_self_test(ed25519_out_sig, kEd25519SignMessage,
                                 sizeof(kEd25519SignMessage),
                                 ed25519_private_key) ||
      !check_test(kEd25519SignSignature, ed25519_out_sig, ED25519_SIGNATURE_LEN,
                  "ED25519-sign")) {
  ...
  static const uint8_t kEd25519VerifyMessage[32] = {
      0x71, 0x3a, 0x7a, 0xde, 0x3d, 0x9e, 0x10, 0x9f, 0x9f, 0xc1, 0x9b,
      0xc3, 0x24, 0xe0, 0x41, 0x72, 0xee, 0x7e, 0x4d, 0x4a, 0xc6, 0x36,
      0x69, 0xb7, 0xbc, 0xdb, 0xd6, 0xd2, 0xde, 0x87, 0xdf, 0x0e};
  static const uint8_t kEd25519VerifySignature[ED25519_SIGNATURE_LEN] = {
      0x44, 0xf2, 0x38, 0xf7, 0xea, 0x71, 0x54, 0xce, 0xdd, 0x95, 0x63,
      0x11, 0x44, 0x07, 0x8f, 0xfe, 0xc6, 0x55, 0x93, 0x8f, 0x73, 0xe2,
      0x96, 0x76, 0x72, 0x8b, 0x40, 0x0f, 0x8f, 0x46, 0xc8, 0x04, 0x8d,
      0x5b, 0xf3, 0xab, 0x12, 0x43, 0x42, 0xeb, 0xae, 0x54, 0xb6, 0xe0,
      0x4f, 0x3f, 0x16, 0x7b, 0x5e, 0xe8, 0xbd, 0xcf, 0xec, 0x9b, 0xe6,
      0xff, 0x65, 0xbc, 0xc6, 0x9a, 0x78, 0x89, 0x67, 0x0a};
  if (!ED25519_verify_no_self_test(
          kEd25519VerifyMessage, sizeof(kEd25519VerifyMessage),
...
static int boringssl_self_test_hasheddsa(void) {
  ...
  static const uint8_t kEd25519Context[32] = {
      0x76, 0x34, 0x2c, 0x15, 0xb7, 0x11, 0x97, 0x5d, 0x86, 0xd0, 0x11,
      0xdd, 0x28, 0xec, 0x76, 0xf9, 0xb9, 0xe7, 0x2a, 0xb1, 0x5a, 0x50,
      0x15, 0xb0, 0xdd, 0xca, 0xfa, 0x8f, 0xed, 0x54, 0x80, 0x66};

  static const uint8_t kEd25519SignMessage[32] = {
      0xf9, 0xbf, 0xec, 0x63, 0xc4, 0xe0, 0x73, 0xfa, 0x97, 0x1a, 0x80,
      0x49, 0x91, 0x47, 0xd0, 0x0a, 0xcd, 0x26, 0xaa, 0xe1, 0xff, 0x03,
      0x64, 0xdb, 0x20, 0xf8, 0xa7, 0xa4, 0x95, 0x4d, 0xb3, 0x87};
  static const uint8_t kEd25519SignSignature[ED25519_SIGNATURE_LEN] = {
      0x0b, 0x93, 0x3d, 0x3f, 0x59, 0x00, 0xe3, 0xa1, 0xe5, 0x39, 0x47,
      0xce, 0x97, 0x32, 0xc7, 0x01, 0x40, 0x37, 0xe9, 0xc9, 0x4b, 0x71,
      0xcd, 0x3a, 0xfb, 0x60, 0x46, 0xaa, 0x29, 0xfe, 0xa9, 0xbb, 0xd8,
      0x1c, 0x50, 0x54, 0x10, 0x64, 0xc6, 0x59, 0xd0, 0x07, 0x5f, 0xb3,
      0x8c, 0x8b, 0x42, 0x0f, 0x81, 0x48, 0x68, 0x2d, 0xc9, 0xf8, 0x38,
      0x43, 0x55, 0x10, 0x5c, 0x39, 0x70, 0xd2, 0x06, 0x09};
  ...
  static const uint8_t kEd25519VerifyMessage[32] = {
      0x36, 0xc7, 0xf4, 0x5a, 0x29, 0xa6, 0x84, 0xa8, 0x01, 0x37, 0x53,
      0xb1, 0xc6, 0x10, 0x09, 0x79, 0x1f, 0xbc, 0x6e, 0xd4, 0xaf, 0x81,
      0x31, 0xaa, 0x4b, 0xc2, 0x76, 0x3d, 0x7f, 0xd5, 0xf7, 0x50};
  static const uint8_t kEd25519VerifySignature[ED25519_SIGNATURE_LEN] = {
      0x10, 0x1b, 0xcc, 0xa2, 0x56, 0xef, 0x62, 0x0b, 0xb0, 0x87, 0x59,
      0x2e, 0x91, 0x73, 0x36, 0xd8, 0x54, 0x2b, 0x71, 0x72, 0x8e, 0x2a,
      0x27, 0x48, 0xc5, 0x5c, 0x71, 0x9b, 0x82, 0x5d, 0xad, 0x45, 0x21,
      0xbf, 0xb3, 0x75, 0x62, 0x4b, 0x27, 0xff, 0xf8, 0x1c, 0xbf, 0x71,
      0x65, 0xe5, 0xba, 0x4a, 0x98, 0xe8, 0xc4, 0x51, 0xb3, 0xc3, 0xc2,
      0xfa, 0x23, 0x27, 0x8f, 0x2b, 0xb7, 0x45, 0x81, 0x07};
  if (!ED25519ph_verify_no_self_test(
          kEd25519VerifyMessage, sizeof(kEd25519VerifyMessage),
          kEd25519VerifySignature, kEd25519PublicKey, kEd25519Context,
          sizeof(kEd25519Context))) {
...
static int boringssl_self_test_hkdf_sha256(void) {
  static const uint8_t kHKDF_ikm_tc1[] = {
      ...
      0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b};
  static const uint8_t kHKDF_salt_tc1[] = {0x00, 0x01, 0x02, 0x03, 0x04,
                                           0x05, 0x06, 0x07, 0x08, 0x09,
                                           0x0a, 0x0b, 0x0c};
  static const uint8_t kHKDF_info_tc1[] = {0xf0, 0xf1, 0xf2, 0xf3, 0xf4,
  ...
  HKDF(output, sizeof(output), EVP_sha256(), kHKDF_ikm_tc1,
       sizeof(kHKDF_ikm_tc1), kHKDF_salt_tc1, sizeof(kHKDF_salt_tc1),
...
static int boringssl_self_test_fast(void) {
  ...
  if (!boringssl_self_test_sha512() || !boringssl_self_test_sha3_256() ||
  ...
  static const uint8_t kKBKDF_ctr_hmac_secret[] = {
      0xdd, 0x1d, 0x91, 0xb7, 0xd9, 0x0b, 0x2b, 0xd3, 0x13, 0x85, 0x33,
  ...
  if (!KBKDF_ctr_hmac(kbkdf_ctr_hmac_output, sizeof(kbkdf_ctr_hmac_output),
                      EVP_sha256(), kKBKDF_ctr_hmac_secret,
                      sizeof(kKBKDF_ctr_hmac_secret), kKBKDF_ctr_hmac_info,
...
int BORINGSSL_self_test(void) {
  if (!boringssl_self_test_fast() ||
      ...
      !boringssl_self_test_ffdh() || !boringssl_self_test_ml_kem() ||
      !boringssl_self_test_ml_dsa() || !boringssl_self_test_eddsa() ||
