...
uint8_t *SHA3_224(const uint8_t *data, size_t len,
  ...
  int ok = (SHA3_Init(&ctx, SHA3_224_DIGEST_BITLENGTH) &&
            SHA3_Update(&ctx, data, len) && SHA3_Final(out, &ctx));
...
uint8_t *SHAKE128(const uint8_t *data, const size_t in_len, uint8_t *out,
                  size_t out_len) {
  FIPS_service_indicator_lock_state();
  KECCAK1600_CTX ctx;
  int ok =
      (SHAKE_Init(&ctx, SHAKE128_BLOCKSIZE) &&
       SHAKE_Absorb(&ctx, data, in_len) && SHAKE_Final(out, &ctx, out_len));
...
uint8_t *SHAKE256(const uint8_t *data, const size_t in_len, uint8_t *out,
                  size_t out_len) {
  FIPS_service_indicator_lock_state();
  KECCAK1600_CTX ctx;
  int ok =
...
// FIPS202 APIs manage internal input/output buffer on top of Keccak1600 API
// layer
...
static int FIPS202_Init(KECCAK1600_CTX *ctx, uint8_t pad, size_t block_size,
                        size_t bit_len) {
  if (pad != SHA3_PAD_CHAR && pad != SHAKE_PAD_CHAR) {
    return 0;
  }
...
// This function should be called once to finalize absorb and initiate squeeze
// phase
...
int SHAKE_Init(KECCAK1600_CTX *ctx, size_t block_size) {
  ...
  if (block_size != SHAKE128_BLOCKSIZE && block_size != SHAKE256_BLOCKSIZE) {
    return 0;
  }
