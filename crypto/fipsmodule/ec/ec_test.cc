...
struct InvalidECPublicKey {
  ...
  int nid;
    ...
    {kP224PublicKey_wrong_compressed_byte,
     sizeof(kP224PublicKey_wrong_compressed_byte), NID_secp224r1},
    {kP224PublicKey_wrong_uncompressed_byte,
     sizeof(kP224PublicKey_wrong_uncompressed_byte), NID_secp224r1},
    ...
    {kP224PublicKey_compressed_too_long,
     sizeof(kP224PublicKey_compressed_too_long), NID_secp224r1},
    {kP224PublicKey_compressed_too_short,
     sizeof(kP224PublicKey_compressed_too_short), NID_secp224r1},
    {kP224PublicKey_compressed_wrong_x,
     sizeof(kP224PublicKey_compressed_wrong_x), NID_secp224r1},
    {kP224PublicKey_uncompressed_too_long,
     sizeof(kP224PublicKey_uncompressed_too_long), NID_secp224r1},
    {kP224PublicKey_uncompressed_too_short,
     sizeof(kP224PublicKey_uncompressed_too_short), NID_secp224r1},
    {kP224PublicKey_uncompressed_wrong_y,
     sizeof(kP224PublicKey_uncompressed_wrong_y), NID_secp224r1}};

class ECPublicKeyInvalidTest
    : public testing::TestWithParam<InvalidECPublicKey> {};

// This is to test |EC_KEY| failing to decode some bytes using
// |o2i_ECPublicKey|.
...
TEST(ECTest, SmallGroupOrder) {
  ...
  bssl::UniquePtr<EC_GROUP> group_org(
  ...
  ASSERT_TRUE(EC_GROUP_get_curve_GFp(group_org.get(), p.get(), a.get(), b.get(),
                                     ctx.get()));
  ...
  bssl::UniquePtr<EC_GROUP> group(
      EC_GROUP_new_curve_GFp(p.get(), a.get(), b.get(), ctx.get()));
  ASSERT_TRUE(group);
  // The generator has to be created using the new group so they match when
  // calling |EC_GROUP_set_generator|
  bssl::UniquePtr<EC_POINT> generator(EC_POINT_new(group.get()));
  ASSERT_TRUE(generator);
  ...
  EXPECT_TRUE(EC_POINT_get_affine_coordinates_GFp(
      group_org.get(), EC_GROUP_get0_generator(group_org.get()), gx.get(),
      gy.get(), ctx.get()));
...
TEST(ECTest, LargeXCoordinateVectors) {
  ...
  FileTestGTest(
      "crypto/fipsmodule/ec/large_x_coordinate_points.txt", [&](FileTest *t) {
        ...
        OPENSSL_memcpy(key.get()->pub_key->raw.X.words, x.get()->d,
                       BN_BYTES * group->field.N.width);
        OPENSSL_memcpy(key.get()->pub_key->raw.Y.words, y.get()->d,
                       BN_BYTES * group->field.N.width);
        OPENSSL_memset(key.get()->pub_key->raw.Z.words, 0,
                       BN_BYTES * group->field.N.width);
        ...
        OPENSSL_memcpy(key.get()->pub_key->raw.X.words, xpp.get()->d,
                       BN_BYTES * group->field.N.width);
        // We expect |EC_KEY_check_fips| to always fail when given key with x >
        // p.
        ASSERT_FALSE(EC_KEY_check_fips(key.get()));
        ...
        if (!is_curve_using_mont_felem_impl(curve_nid)) {
          ...
          EXPECT_PRED2(HasSuffix, file,
                       "ec_key.c");  // within EC_KEY_check_fips
        } else {
          EXPECT_EQ(EC_R_POINT_IS_NOT_ON_CURVE,
                    ERR_GET_REASON(ERR_peek_last_error_line(&file, &line)));
          EXPECT_PRED2(HasSuffix, file, "ec_key.c");  // within EC_KEY_check_key
...
TEST(ECTest, FelemBytes) {
  ...
  for (size_t i = 0; i < sizeof(test_cases) / sizeof(std::tuple<int, int, int>);
       i++) {
...
TEST(ECTest, ECKEYMETHOD) {
  ...
  EC_KEY_METHOD_set_init(ec_method, NULL, openvpn_extkey_ec_finish, NULL, NULL,
                         NULL, NULL);
  ...
  // EVP_PKEY_assign_EC_KEY doesn't up the reference, so do that here for proper
  // test cleanup
  ASSERT_TRUE(EC_KEY_up_ref(ec.get()));
  bssl::UniquePtr<EVP_PKEY_CTX> ec_key_ctx(
      EVP_PKEY_CTX_new(ec_key.get(), NULL));
  ...
  ASSERT_TRUE(
      EVP_PKEY_sign(ec_key_ctx.get(), signature.data(), &sig_len, digest, 20));
