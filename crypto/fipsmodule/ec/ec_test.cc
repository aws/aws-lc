...
TEST(ECTest, ECPKParametersBufferRoundTrip) {
  ...
  for (const EC_GROUP *group : kGroups) {
    ...
    ASSERT_TRUE(decoded);
    EXPECT_EQ(0, EC_GROUP_cmp(group, decoded.get(), nullptr));
...
TEST(ECTest, ECPKParametersReplacesOutputGroup) {
  ...
  EC_GROUP *out_group = EC_GROUP_new_by_curve_name_mutable(NID_secp384r1);
  ASSERT_TRUE(out_group);
  const uint8_t *inp = encoded.data();
  EC_GROUP *decoded =
      d2i_ECPKParameters(&out_group, &inp, static_cast<long>(encoded.size()));

  ASSERT_TRUE(decoded);
  EXPECT_EQ(decoded, out_group);
  // The original |*out_group| was released and replaced by the parsed group,
  // which is the static built-in P-256 group.
  EXPECT_EQ(EC_group_p256(), out_group);
  EXPECT_EQ(encoded.data() + encoded.size(), inp);
  EC_GROUP_free(out_group);
}

TEST(ECTest, ECPKParametersBufferErrors) {
  ERR_clear_error();
  ...
  CBS key, ec_private_key, private_key, explicit_parameters;
  CBS_init(&key, kECKeySpecifiedCurve, sizeof(kECKeySpecifiedCurve));
  ASSERT_TRUE(CBS_get_asn1(&key, &ec_private_key, CBS_ASN1_SEQUENCE));
  uint64_t version = 0;
  ASSERT_TRUE(CBS_get_asn1_uint64(&ec_private_key, &version));
  ASSERT_EQ(1u, version);
  ASSERT_TRUE(
      CBS_get_asn1(&ec_private_key, &private_key, CBS_ASN1_OCTETSTRING));
  ASSERT_TRUE(
      CBS_get_asn1(&ec_private_key, &explicit_parameters,
                   CBS_ASN1_CONSTRUCTED | CBS_ASN1_CONTEXT_SPECIFIC | 0));
  ASSERT_EQ(0u, CBS_len(&ec_private_key));
  ASSERT_EQ(0u, CBS_len(&key));

  const uint8_t *explicit_in = CBS_data(&explicit_parameters);
  ASSERT_EQ(
      EC_group_p256(),
      d2i_ECPKParameters(nullptr, &explicit_in,
                         static_cast<long>(CBS_len(&explicit_parameters))));
  EXPECT_EQ(CBS_data(&explicit_parameters) + CBS_len(&explicit_parameters),
            explicit_in);

  std::vector<uint8_t> unsupported(
      CBS_data(&explicit_parameters),
      CBS_data(&explicit_parameters) + CBS_len(&explicit_parameters));
  ASSERT_GT(unsupported.size(), 4u);
  // The encoding ends with the group order INTEGER followed by the cofactor
  // INTEGER (02 01 01). Flip the low bit of the final byte of the order so the
  // DER stays well-formed but no longer matches a supported named curve.
  ASSERT_EQ(0x02, unsupported[unsupported.size() - 3]);
  ASSERT_EQ(0x01, unsupported[unsupported.size() - 2]);
  ASSERT_EQ(0x01, unsupported[unsupported.size() - 1]);
  unsupported[unsupported.size() - 4] ^= 1;
...
// Validate Brainpool domain parameters against RFC 5639 reference values.
// This is a known-answer test: if the precomputed Montgomery constants in
// builtin_curves.h were subtly wrong, the extracted p/a/b/G/n would not
// match the RFC.
TEST(ECTest, BrainpoolDomainParamsRFC5639) {
  struct BrainpoolTestVector {
    int nid;
    const char *p_hex;
    const char *a_hex;
    const char *b_hex;
  ...
  // RFC 5639, Sections 3.2–3.7
  static const BrainpoolTestVector kVectors[] = {
      {NID_brainpoolP224r1,
  ...
  for (const auto &vec : kVectors) {
    SCOPED_TRACE(vec.nid);
    const EC_GROUP *group = EC_GROUP_new_by_curve_name(vec.nid);
    ASSERT_TRUE(group);

    bssl::UniquePtr<BIGNUM> p(BN_new()), a(BN_new()), b(BN_new());
    ASSERT_TRUE(EC_GROUP_get_curve_GFp(group, p.get(), a.get(), b.get(),
                                       nullptr));

    bssl::UniquePtr<BIGNUM> expected_p(nullptr), expected_a(nullptr),
        expected_b(nullptr);
    {
      BIGNUM *tmp = nullptr;
      ASSERT_TRUE(BN_hex2bn(&tmp, vec.p_hex));
      expected_p.reset(tmp);
      tmp = nullptr;
      ASSERT_TRUE(BN_hex2bn(&tmp, vec.a_hex));
      expected_a.reset(tmp);
      tmp = nullptr;
      ASSERT_TRUE(BN_hex2bn(&tmp, vec.b_hex));
      expected_b.reset(tmp);
    }

    EXPECT_EQ(0, BN_cmp(p.get(), expected_p.get()));
    EXPECT_EQ(0, BN_cmp(a.get(), expected_a.get()));
    EXPECT_EQ(0, BN_cmp(b.get(), expected_b.get()));

    // Validate generator coordinates.
    const EC_POINT *gen = EC_GROUP_get0_generator(group);
    ASSERT_TRUE(gen);
    bssl::UniquePtr<BIGNUM> gx(BN_new()), gy(BN_new());
    ASSERT_TRUE(EC_POINT_get_affine_coordinates_GFp(group, gen, gx.get(),
                                                     gy.get(), nullptr));

    bssl::UniquePtr<BIGNUM> expected_gx(nullptr), expected_gy(nullptr);
    {
      BIGNUM *tmp = nullptr;
      ASSERT_TRUE(BN_hex2bn(&tmp, vec.gx_hex));
      expected_gx.reset(tmp);
      tmp = nullptr;
      ASSERT_TRUE(BN_hex2bn(&tmp, vec.gy_hex));
      expected_gy.reset(tmp);
    }

    EXPECT_EQ(0, BN_cmp(gx.get(), expected_gx.get()));
    EXPECT_EQ(0, BN_cmp(gy.get(), expected_gy.get()));

    // Validate group order.
    const BIGNUM *order = EC_GROUP_get0_order(group);
    ASSERT_TRUE(order);
    bssl::UniquePtr<BIGNUM> expected_n(nullptr);
    {
      BIGNUM *tmp = nullptr;
      ASSERT_TRUE(BN_hex2bn(&tmp, vec.n_hex));
      expected_n.reset(tmp);
    }
    EXPECT_EQ(0, BN_cmp(order, expected_n.get()));
  }
}
