...
// --------------------------------
	...
	table_def_str := fmt.Sprintf("static const ec_nistp_felem_limb p384_g_pre_comp_new[%d] = ", num_subtables * pts_per_subtable * 2 * 6)
	table_def_str2 := fmt.Sprintf("static const ec_nistp_felem_limb p384_g_pre_comp_new[%d] = ", num_subtables * pts_per_subtable * 2 * 6 * 2)
...
func writeU64MontNEW(w *columnWriter, curve elliptic.Curve, n *big.Int, bitSizes []uint) error {
	n = toMontgomery(n, curve.Params().P, 64)
	return writeWords(w, bigIntToU64s(curve, n), formatU64)
}

func writeU32MontNEW(w *columnWriter, curve elliptic.Curve, n *big.Int, bitSizes []uint) error {
	n = toMontgomery(n, curve.Params().P, 32)
...
func writeTableNEW(w *columnWriter, curve elliptic.Curve, table [][2]*big.Int, writeBigInt writeBigIntFunc, writeBigIntBitSizes []uint) error {
	if _, err := io.WriteString(w, "{"); err != nil {
		return err
	}
	indent := w.column
