...
func main() {
	...
	if err := writeECNISTPTables("ec_nistp_tables.h"); err != nil {
	...
	// if err := writeP384TableNEW("p384_table_new.h"); err != nil {
	// 	fmt.Fprintf(os.Stderr, "Error writing p384_table.h: %s\n", err)
	// 	os.Exit(1)
	// }
	...
	// if err := writeP521TableNEW("p521_table_new.h"); err != nil {
	// 	fmt.Fprintf(os.Stderr, "Error writing p521_table.h: %s\n", err)
	// 	os.Exit(1)
	// }
}
...
func writeECNISTP256(w *columnWriter) error {
	...
	table_def_str := fmt.Sprintf("static const ec_nistp_felem_limb ec_nistp_p256_base_point_table[] = ")
	...
	if err := writeTableClean(w, curve, table, writeU64MontClean, nil); err != nil {
		return err
	}
	if _, err := io.WriteString(w, ";\n#else\n" + table_def_str); err != nil {
		return err
	}
	if err := writeTableClean(w, curve, table, writeU32MontClean, nil); err != nil {
		return err
	}
	if _, err := io.WriteString(w, ";\n#endif\n"); err != nil {
		return err
...
func writeECNISTP384(w *columnWriter) error {
	...
	table_def_str := fmt.Sprintf("static const ec_nistp_felem_limb ec_nistp_p384_base_point_table[] = ")
...
func writeECNISTP521(w *columnWriter) error {
	...
	if err := writeTableClean(w, curve, table, writeU64Clean, nil); err != nil {
		return err
	}
	...
	if err := writeTableClean(w, curve, table, writeU58Clean, nil); err != nil {
		return err
	}
	if _, err := io.WriteString(w, ";\n#else\n" + table_def_str); err != nil {
		return err
	...
	if err := writeTableClean(w, curve, table, writeU32CustomClean, bitSizes[:]); err != nil {
		return err
	}
	if _, err := io.WriteString(w, ";\n#endif\n#endif\n"); err != nil {
		return err
...
// This file is generated by make_tables.go.
#ifndef EC_NISTP_TABLES_H
#define EC_NISTP_TABLES_H`
	...
	const fileFooter = `#endif // EC_NISTP_TABLES_H`
	if _, err := io.WriteString(w, fileFooter + "\n"); err != nil {
		return err
	}
    return nil
}

func writeP521TableNEW(path string) error {
    curve := elliptic.P521()
    table := generateECNISTPTable(curve)
...
// This file is generated by make_tables.go.
	...
	table_def_str := fmt.Sprintf("static const ec_nistp_felem_limb p521_g_pre_comp_new[] = ")
...
func writeU64MontClean(w *columnWriter, curve elliptic.Curve, n *big.Int, bitSizes []uint) error {
	n = toMontgomery(n, curve.Params().P, 64)
	return writeWords(w, bigIntToU64s(curve, n), formatU64)
}

func writeU32MontClean(w *columnWriter, curve elliptic.Curve, n *big.Int, bitSizes []uint) error {
	n = toMontgomery(n, curve.Params().P, 32)
	return writeWords(w, bigIntToU32s(curve, n), formatU32)
}
...
func writeU64Clean(w *columnWriter, curve elliptic.Curve, n *big.Int, bitSizes []uint) error {
	return writeWords(w, bigIntToU64s(curve, n), formatU64)
}

// This is needed for P-521 Fiat-crypto implementation.
func writeU58Clean(w *columnWriter, curve elliptic.Curve, n *big.Int, bitSizes []uint) error {
	return writeWords(w, bigIntToU58s(curve, n), formatU64)
}
...
func writeU32CustomClean(w *columnWriter, curve elliptic.Curve, n *big.Int, bitSizes []uint) error {
	return writeWords(w, bigIntToUCustom(curve, n, bitSizes), formatU32)
}

func writeU32Clean(w *columnWriter, curve elliptic.Curve, n *big.Int, bitSizes []uint) error {
	return writeWords(w, bigIntToU32s(curve, n), formatU32)
}
...
func writeTableClean(w *columnWriter, curve elliptic.Curve, table [][2]*big.Int, writeBigInt writeBigIntFunc, writeBigIntBitSizes []uint) error {
	if _, err := io.WriteString(w, "{"); err != nil {
		return err
	}
	indent := w.column
