...
func writeP384Table(path string) error {
	curve := elliptic.P384()
	tables := make([][][2]*big.Int, 0, 21)
	for i := 0; i < 20; i += 1 {
		row := makeOddMultiples(curve, 16, i*20)
		tables = append(tables, row)
	}
...
// --------------------------
//
// The pre-computed table for the base point G of P-384, fiat_p384_g_pre_comp,
...
static const fiat_p384_felem fiat_p384_g_pre_comp[20][16][2] = `
	if _, err := f.WriteString(fileHeader); err != nil {
		return err
	}
	if err := writeTables(f, curve, tables, true, 4, writeU64Mont); err != nil {
	...
	if _, err := f.WriteString(";\n#else\nstatic const fiat_p384_felem fiat_p384_g_pre_comp[20][16][2] = "); err != nil {
		return err
	}
	if err := writeTables(f, curve, tables, true, 4, writeU32Mont); err != nil {
		return err
...
func makeOddMultiples(curve elliptic.Curve, n, shift int) [][2]*big.Int {
  ...
  ret[0] = tmp[0]
	...
	for i := 1; i < 2*n; i++ {
	  x, y := curve.Add(tmp[i-1][0], tmp[i-1][1], tmp[0][0], tmp[0][1])
	  tmp[i] =[2]*big.Int{x, y}
	  if i&1 == 0 {
	    ret[i/2] = tmp[i]
	  }
