...
static int rand_ensure_ctr_drbg_reseed_interval(struct rand_thread_local_state *state,
  ...
  // In our case, I is |len32| and N is |CTR_DRBG_MAX_GENERATE_LENGTH_LOG2|.
  uint32_t generate_calls_needed = ((len32-1) >> CTR_DRBG_MAX_GENERATE_LENGTH_LOG2) + 1;
...
// |out|. |out_len| must be less or equal to
// |CTR_DRBG_MAX_GENERATE_LENGTH| * |kCtrDrbgReseedInterval|. The CTR-DRBG state
...
static void RAND_bytes_core(
  ...
  // of bytes that can be produced in a full reseed interval.
  OPENSSL_STATIC_ASSERT((CTR_DRBG_MAX_GENERATE_LENGTH*kCtrDrbgReseedInterval) >= 0xFFFFFFF, upper_bound_too_small_rand_bytes_pre_condition_assumes_0xfffffff);
