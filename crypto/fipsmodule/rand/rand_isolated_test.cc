...
public:
  ...
  struct UserPredictionResistance {
    std::array<uint8_t, RAND_PRED_RESISTANCE_LEN> predictionResistance1;
    std::array<uint8_t, RAND_PRED_RESISTANCE_LEN> predictionResistance2;
  };
  ...
  static const UserPredictionResistance& getUserPredictionResistance() {
    // Is xor'ed with normal prediction resistance or zero-buffer it none.
    static const UserPredictionResistance resistance = {
      // predictionResistance1
  ...
  static void printVector(const std::vector<uint8_t>& v) {
    for (uint8_t byte : v) {
      printf("%02x ", byte);
    }
    printf("\n");
...
private:
  ...
  static int overrideGetUserPredictionResistanceKat(
    const struct entropy_source_t *entropy_source,
    uint8_t seed[RAND_PRED_RESISTANCE_LEN]) {
    const auto& vectors = RngKatTestData::getTestVectors();
    const auto& user_pr = RngKatTestData::getUserPredictionResistance();

    switch (current_generation_state) {
      case GenerationState::First:
        for (size_t i = 0; i < RAND_PRED_RESISTANCE_LEN; i++) {
          seed[i] = vectors.predictionResistance1[i] ^
                    user_pr.predictionResistance1[i];
        }
        return 1;
      case GenerationState::Second:
        for (size_t i = 0; i < RAND_PRED_RESISTANCE_LEN; i++) {
          seed[i] = vectors.predictionResistance2[i] ^
                    user_pr.predictionResistance2[i];
        }
        return 1;
      default:
        return 0;
  ...
  static PredictionResistanceCallback getPredictionResistanceCallback(TestType type) {
    if (type == TestType::NoReseedAndPrAndUserPr) {
      return &overrideGetUserPredictionResistanceKat;
    }

    if (type == TestType::NoReseedAndPr ||
  ...
  bool generateRandomness(const std::vector<uint8_t>& expected_output,
    ...
    if (kat_test_type_ == TestType::NoReseedAndPrAndUserPr ||
      ...
      const auto& user_pr_input = RngKatTestData::getUserPredictionResistance();
      const std::array<uint8_t, RAND_PRED_RESISTANCE_LEN>* pr = nullptr;
      switch (current_generation_state) {
        case GenerationState::First:
          pr = &user_pr_input.predictionResistance1;
          break;
        case GenerationState::Second:
          pr = &user_pr_input.predictionResistance2;
          break;
        default:
          return false;
      }
    ...
    if (expected_output != output) {
      std::cerr << "Generated randomness not equal to expected value "
                << error_text << '\n';
      RngKatTestData::printVector(output);
      RngKatTestData::printVector(expected_output);
      return false;
    }
