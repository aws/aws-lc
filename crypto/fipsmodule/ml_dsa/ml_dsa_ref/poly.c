 ...
 **************************************************/
void ml_dsa_poly_add(ml_dsa_poly *c, const ml_dsa_poly *a,
                     const ml_dsa_poly *b) {
  unsigned int i;
  for (i = 0; i < ML_DSA_N; ++i) {
 ...
 **************************************************/
void ml_dsa_poly_sub(ml_dsa_poly *c, const ml_dsa_poly *a,
                     const ml_dsa_poly *b) {
  unsigned int i;
  for (i = 0; i < ML_DSA_N; ++i) {
    c->coeffs[i] = a->coeffs[i] - b->coeffs[i];
 ...
 * Arguments:   - poly *a: pointer to input/output polynomial
 **************************************************/
void ml_dsa_poly_ntt(ml_dsa_poly *a) { ml_dsa_ntt(a->coeffs); }
 ...
 **************************************************/
void ml_dsa_poly_pointwise_montgomery(ml_dsa_poly *c, const ml_dsa_poly *a,
                                      const ml_dsa_poly *b) {
  unsigned int i;
  for (i = 0; i < ML_DSA_N; ++i) {
 ...
 **************************************************/
void ml_dsa_poly_power2round(ml_dsa_poly *a1, ml_dsa_poly *a0,
                             const ml_dsa_poly *a) {
  unsigned int i;
  for (i = 0; i < ML_DSA_N; ++i) {
 ...
 *              - const poly *a: pointer to input polynomial
 **************************************************/
void ml_dsa_poly_decompose(ml_dsa_params *params, ml_dsa_poly *a1,
                           ml_dsa_poly *a0, const ml_dsa_poly *a) {
  unsigned int i;
  for (i = 0; i < ML_DSA_N; ++i) {
 ...
 **************************************************/
unsigned int ml_dsa_poly_make_hint(ml_dsa_params *params, ml_dsa_poly *h,
 ...
 **************************************************/
void ml_dsa_poly_use_hint(ml_dsa_params *params, ml_dsa_poly *b,
                          const ml_dsa_poly *a, const ml_dsa_poly *h) {
  unsigned int i;
  for (i = 0; i < ML_DSA_N; ++i) {
 ...
 **************************************************/
static unsigned int ml_dsa_rej_uniform(int32_t *a, unsigned int len,
                                       const uint8_t *buf,
                                       unsigned int buflen) {
...
#define POLY_UNIFORM_NBLOCKS \
  ((768 + SHAKE128_BLOCKSIZE - 1) / SHAKE128_BLOCKSIZE)
void ml_dsa_poly_uniform(ml_dsa_poly *a, const uint8_t seed[ML_DSA_SEEDBYTES],
                         uint16_t nonce) {
 ...
 * random bytes were given.
 **************************************************/
static unsigned int rej_eta(ml_dsa_params *params, int32_t *a, unsigned int len,
                            const uint8_t *buf, unsigned int buflen) {
  assert((params->eta == 2) || (params->eta == 4));
 ...
 *              - uint16_t nonce: 2-byte nonce
 **************************************************/
void ml_dsa_poly_uniform_eta(ml_dsa_params *params, ml_dsa_poly *a,
                             const uint8_t seed[ML_DSA_CRHBYTES],
                             uint16_t nonce) {
  unsigned int ctr;
  unsigned int buflen =
      ML_DSA_POLY_UNIFORM_ETA_NBLOCKS_MAX * SHAKE256_BLOCKSIZE;
  ...
  while (ctr < ML_DSA_N) {
    ...
    ctr += rej_eta(params, a->coeffs + ctr, ML_DSA_N - ctr, buf,
                   SHAKE256_BLOCKSIZE);
  }
...
#define POLY_UNIFORM_GAMMA1_NBLOCKS \
  ((ML_DSA_POLYZ_PACKEDBYTES_MAX + SHAKE256_BLOCKSIZE - 1) / SHAKE256_BLOCKSIZE)
void ml_dsa_poly_uniform_gamma1(ml_dsa_params *params, ml_dsa_poly *a,
                                const uint8_t seed[ML_DSA_CRHBYTES],
                                uint16_t nonce) {
 ...
 *CTILDEBYTES
 **************************************************/
void ml_dsa_poly_challenge(ml_dsa_params *params, ml_dsa_poly *c,
                           const uint8_t *seed) {
 ...
 *              - const poly *a: pointer to input polynomial
 **************************************************/
void ml_dsa_polyeta_pack(ml_dsa_params *params, uint8_t *r,
                         const ml_dsa_poly *a) {
  unsigned int i;
  uint8_t t[8];
 ...
 **************************************************/
void ml_dsa_polyeta_unpack(ml_dsa_params *params, ml_dsa_poly *r,
                           const uint8_t *a) {
  unsigned int i;
  assert((params->eta == 2) || (params->eta == 4));
 ...
 *              - const poly *a: pointer to input polynomial
 **************************************************/
void ml_dsa_polyz_pack(ml_dsa_params *params, uint8_t *r,
                       const ml_dsa_poly *a) {
  unsigned int i;
  uint32_t t[4];

  assert((params->gamma1 == (1 << 17)) || (params->gamma1 == (1 << 19)));
 ...
 *              - const uint8_t *a: byte array with bit-packed polynomial
 **************************************************/
void ml_dsa_polyz_unpack(ml_dsa_params *params, ml_dsa_poly *r,
                         const uint8_t *a) {
  unsigned int i;
 ...
 *              - const poly *a: pointer to input polynomial
 **************************************************/
void ml_dsa_polyw1_pack(ml_dsa_params *params, uint8_t *r,
                        const ml_dsa_poly *a) {
  unsigned int i;
