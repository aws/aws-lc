...
.type   gcm_init_vpclmulqdq_avx2,\@abi-omnipotent
...
# VEX 3-byte prefix: C4 [byte1] [byte2], compared to EVEX: 62 [byte1] [byte2] [byte3]

# Bit 7 contains the src1 register's MSB in inverted form
# Bit 6 is fixed to 1 (no SIB index register used)
# Bit 5 contains the dst register's MSB in inverted form
# Bits 4->0 is fixed to 00010 for vaesenc* instructions and 00011
...
# Bits 6->3 contains the lower 4 bits of src2 register in inverted form
# Bit 2 is the vector length (L): 0 for xmm, 1 for ymm
# Bits 1->0 is fixed to 01 (implied 66 prefix)
sub vex_byte2 {
  my ($src2, $type) = @_;
  $src2 = ($src2 & 0x0f) ^ 0x0f;
  my $L = 0;
...
sub vpclmulqdq {
  ...
  if ($line=~/(vpclmul[a-z]+)\s+\$0x([0-9]+),\s*%([xyz])mm([0-9]+),\s*%[xyz]mm([0-9]+),\s*%[xyz]mm([0-9]+)/) {
        ...
        my $byte1 = vex_byte1($inst_type, $6, $4);
        my $byte2 = vex_byte2($5, $3);
        my $modrm = 0xc0 | (($4 & 7) | (($6 & 7) << 3));
	push @opcode,$byte1,$byte2;
