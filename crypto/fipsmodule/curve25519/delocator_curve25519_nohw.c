...
int x25519_ge_frombytes_vartime(ge_p3 *h, const uint8_t s[32]) {
  ...
  fe_1(&h->Z);
  fe_sq_tt(&w, &h->Y);
  fe_mul_ttt(&vxx, &w, &curve25519_nohw_data()->k25519d);
  ...
  if (fe_isnonzero(&check)) {
    ...
    fe_mul_ttt(&h->X, &h->X, &curve25519_nohw_data()->k25519sqrtm1);
  }
...
void x25519_ge_p3_to_cached(ge_cached *r, const ge_p3 *p) {
  ...
  fe_copy_lt(&r->Z, &p->Z);
  fe_mul_ltt(&r->T2d, &p->T, &curve25519_nohw_data()->k25519d2);
}
...
void x25519_ge_scalarmult_small_precomp(
  ...
  for (i = 0; i < 15; i++) {
    ...
    fe_mul_llt(&out->xy2d, &out->xy2d, &curve25519_nohw_data()->k25519d2);
  }
...
void x25519_ge_scalarmult_base(ge_p3 *h, const uint8_t a[32]) {
  x25519_ge_scalarmult_small_precomp(h, a, curve25519_nohw_data()->k25519SmallPrecomp);
}
...
static void table_select(ge_precomp *t, int pos, signed char b) {
  ...
  ge_precomp_0(t);
  cmov(t, &curve25519_nohw_data()->k25519Precomp[pos][0], equal(babs, 1));
  cmov(t, &curve25519_nohw_data()->k25519Precomp[pos][1], equal(babs, 2));
  cmov(t, &curve25519_nohw_data()->k25519Precomp[pos][2], equal(babs, 3));
  cmov(t, &curve25519_nohw_data()->k25519Precomp[pos][3], equal(babs, 4));
  cmov(t, &curve25519_nohw_data()->k25519Precomp[pos][4], equal(babs, 5));
  cmov(t, &curve25519_nohw_data()->k25519Precomp[pos][5], equal(babs, 6));
  cmov(t, &curve25519_nohw_data()->k25519Precomp[pos][6], equal(babs, 7));
  cmov(t, &curve25519_nohw_data()->k25519Precomp[pos][7], equal(babs, 8));
...
static void ge_double_scalarmult_vartime(ge_p2 *r, const uint8_t *a,
  ...
  for (; i >= 0; --i) {
    ...
    if (bslide[i] > 0) {
      x25519_ge_p1p1_to_p3(&u, &t);
      ge_madd(&t, &u, &curve25519_nohw_data()->k25519Bi[bslide[i] / 2]);
    } else if (bslide[i] < 0) {
      x25519_ge_p1p1_to_p3(&u, &t);
      ge_msub(&t, &u, &curve25519_nohw_data()->k25519Bi[(-bslide[i]) / 2]);
    }
