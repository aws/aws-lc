...
typedef struct {
  int (*pqdsa_keygen)(uint8_t *public_key, uint8_t *private_key);

  int (*pqdsa_keygen_internal)(uint8_t *public_key, uint8_t *private_key,
                               const uint8_t *seed);

  int (*pqdsa_sign_message)(const uint8_t *private_key, uint8_t *sig,
                            size_t *sig_len, const uint8_t *message,
                            size_t message_len, const uint8_t *ctx_string,
                            size_t ctx_string_len);

  int (*pqdsa_sign)(const uint8_t *private_key, uint8_t *sig, size_t *sig_len,
                    const uint8_t *digest, size_t digest_len);

  int (*pqdsa_verify_message)(const uint8_t *public_key, const uint8_t *sig,
                              size_t sig_len, const uint8_t *message,
                              size_t message_len, const uint8_t *ctx_string,
                              size_t ctx_string_len);

  int (*pqdsa_verify)(const uint8_t *public_key, const uint8_t *sig,
                      size_t sig_len, const uint8_t *digest, size_t digest_len);

  int (*pqdsa_pack_pk_from_sk)(uint8_t *public_key, const uint8_t *private_key);
} PQDSA_METHOD;
