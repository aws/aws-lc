...
static int kyber512r3_keygen(uint8_t *public_key, uint8_t *secret_key) {
...
static int kyber512r3_encaps_deterministic(uint8_t *ciphertext,
                                           ...
                                           const uint8_t *seed) {
  return pqcrystals_kyber512_ref_enc_derand(ciphertext, shared_secret,
                                            public_key, seed) == 0;
}

static int kyber512r3_encaps(uint8_t *ciphertext, uint8_t *shared_secret,
                             const uint8_t *public_key) {
  return pqcrystals_kyber512_ref_enc(ciphertext, shared_secret, public_key) ==
         0;
}

static int kyber512r3_decaps(uint8_t *shared_secret, const uint8_t *ciphertext,
...
static int kyber768r3_keygen(uint8_t *public_key, uint8_t *secret_key) {
...
static int kyber768r3_encaps_deterministic(uint8_t *ciphertext,
                                           ...
                                           const uint8_t *public_key,
                                           const uint8_t *seed) {
  return pqcrystals_kyber768_ref_enc_derand(ciphertext, shared_secret,
                                            public_key, seed) == 0;
}

static int kyber768r3_encaps(uint8_t *ciphertext, uint8_t *shared_secret,
                             const uint8_t *public_key) {
  return pqcrystals_kyber768_ref_enc(ciphertext, shared_secret, public_key) ==
         0;
}

static int kyber768r3_decaps(uint8_t *shared_secret, const uint8_t *ciphertext,
                             const uint8_t *secret_key) {
...
static int kyber1024r3_keygen_deterministic(uint8_t *public_key,
                                            uint8_t *secret_key,
                                            const uint8_t *seed) {
  return pqcrystals_kyber1024_ref_keypair_derand(public_key, secret_key,
                                                 seed) == 0;
}

static int kyber1024r3_keygen(uint8_t *public_key, uint8_t *secret_key) {
...
static int kyber1024r3_encaps_deterministic(uint8_t *ciphertext,
                                            ...
                                            const uint8_t *public_key,
                                            const uint8_t *seed) {
  return pqcrystals_kyber1024_ref_enc_derand(ciphertext, shared_secret,
                                             public_key, seed) == 0;
}

static int kyber1024r3_encaps(uint8_t *ciphertext, uint8_t *shared_secret,
                              const uint8_t *public_key) {
  return pqcrystals_kyber1024_ref_enc(ciphertext, shared_secret, public_key) ==
         0;
}

static int kyber1024r3_decaps(uint8_t *shared_secret, const uint8_t *ciphertext,
                              const uint8_t *secret_key) {
...
const KEM *get_legacy_kem_kyber512_r3(void) { return &legacy_kem_kyber512_r3; }
...
const KEM *get_legacy_kem_kyber768_r3(void) { return &legacy_kem_kyber768_r3; }
