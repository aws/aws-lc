...
static int pkey_kyber768_keygen(EVP_PKEY_CTX *ctx, EVP_PKEY *pkey) {
  ...
  if (!EVP_PKEY_set_type(pkey, EVP_PKEY_KYBER768)) {
    OPENSSL_free(key);
    return 0;
  }

  kyber768_keypair(key->pub, key->priv);
  key->has_private = 1;
...
static int pkey_kyber768_encapsulate(EVP_PKEY_CTX *ctx,
                                     uint8_t *ciphertext,
                                     size_t  *ciphertext_len,
                                     uint8_t *shared_secret,
                                     size_t  *shared_secret_len) {
  // Caller is getting parameter values.
  if (ciphertext == NULL) {
    *ciphertext_len = KYBER768_CIPHERTEXT_BYTES;
    *shared_secret_len = KYBER768_SHARED_SECRET_BYTES;
    return 1;
  }

  // The output buffers need to be large enough.
  if (*ciphertext_len < KYBER768_CIPHERTEXT_BYTES ||
      *shared_secret_len < KYBER768_SHARED_SECRET_BYTES) {
      OPENSSL_PUT_ERROR(EVP, EVP_R_BUFFER_TOO_SMALL);
      return 0;
  }
  ...
  if (ctx == NULL ||
      ctx->pkey == NULL ||
      ctx->pkey->pkey.ptr == NULL ||
      ctx->pkey->type != EVP_PKEY_KYBER768) {
      OPENSSL_PUT_ERROR(EVP, EVP_R_OPERATON_NOT_INITIALIZED);
      return 0;
  }

  KYBER768_KEY *key = (KYBER768_KEY*)ctx->pkey->pkey.ptr;
  kyber768_encapsulate(ciphertext, shared_secret, key->pub);
...
static int pkey_kyber768_decapsulate(EVP_PKEY_CTX *ctx,
                                     uint8_t *shared_secret,
                                     size_t  *shared_secret_len,
                                     const uint8_t *ciphertext,
                                     size_t   ciphertext_len) {
  ...
  // The input and output buffers need to be large enough.
  if (*shared_secret_len < KYBER768_SHARED_SECRET_BYTES ||
      ciphertext_len < KYBER768_CIPHERTEXT_BYTES) {
      OPENSSL_PUT_ERROR(EVP, EVP_R_BUFFER_TOO_SMALL);
      return 0;
  }
  ...
  kyber768_decapsulate(shared_secret, ciphertext, key->priv);
...
const EVP_PKEY_METHOD kyber768_pkey_meth = {
    EVP_PKEY_KYBER768,
    NULL /* init */,
    NULL /* copy */,
    NULL /* cleanup */,
    pkey_kyber768_keygen,
    NULL /* sign_init */,
    NULL /* sign */,
    NULL /* sign_message */,
    NULL /* verify_init */,
    ...
    NULL /* paramgen */,
    NULL /* ctrl */,
    pkey_kyber768_encapsulate,
    pkey_kyber768_decapsulate,
};
