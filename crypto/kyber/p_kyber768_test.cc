...
TEST(Kyber768Test, KeyGeneration) {
  EVP_PKEY_CTX *kyber_pkey_ctx = EVP_PKEY_CTX_new_id(EVP_PKEY_KYBER768, nullptr);
  ...
  const KYBER768_KEY *kyber768Key = (KYBER768_KEY *)(kyber_pkey->pkey.ptr);
  EXPECT_TRUE(kyber768Key->has_private);

  uint8_t *buf = nullptr;
  size_t buf_size;
  EXPECT_TRUE(EVP_PKEY_get_raw_public_key(kyber_pkey, buf, &buf_size));
  EXPECT_EQ((size_t)KYBER768_PUBLIC_KEY_BYTES, buf_size);
  ...
  EXPECT_EQ((size_t)KYBER768_SECRET_KEY_BYTES, buf_size);
...
TEST(Kyber768Test, KeyComparison) {
  EVP_PKEY_CTX *kyber_pkey_ctx1 = EVP_PKEY_CTX_new_id(EVP_PKEY_KYBER768, nullptr);
  ...
  EVP_PKEY_CTX *kyber_pkey_ctx2 = EVP_PKEY_CTX_new_id(EVP_PKEY_KYBER768, nullptr);
...
TEST(Kyber768Test, NewKeyFromBytes) {
  // Source key
  EVP_PKEY_CTX *kyber_pkey_ctx = EVP_PKEY_CTX_new_id(EVP_PKEY_KYBER768, nullptr);
  ASSERT_NE(kyber_pkey_ctx, nullptr);
  ...
  // New raw public key
  EVP_PKEY *new_public = EVP_PKEY_new_raw_public_key(EVP_PKEY_KYBER768,
                                                     NULL,
                                                     kyber768Key->pub,
  ...
  // New raw private key
  EVP_PKEY *new_private = EVP_PKEY_new_raw_private_key(EVP_PKEY_KYBER768,
                                                       NULL,
                                                       kyber768Key->priv,
                                                       KYBER768_SECRET_KEY_BYTES);
  ASSERT_NE(new_private, nullptr);
  const KYBER768_KEY *newKyber768Key = (KYBER768_KEY *)(new_private->pkey.ptr);
  EXPECT_EQ(0, OPENSSL_memcmp(kyber768Key->priv, newKyber768Key->priv, KYBER768_SECRET_KEY_BYTES));
...
TEST(Kyber768Test, KeySize) {
  ...
  EXPECT_EQ(KYBER768_PUBLIC_KEY_BYTES + KYBER768_SECRET_KEY_BYTES, EVP_PKEY_size(kyber_pkey));
  EXPECT_EQ(8*(KYBER768_PUBLIC_KEY_BYTES + KYBER768_SECRET_KEY_BYTES), EVP_PKEY_bits(kyber_pkey));

  EVP_PKEY_CTX_free(kyber_pkey_ctx);
}

TEST(Kyber768Test, KEMOperations) {
  // Basic functional test for KYBER768.
  // Simulate two sides of the key exchange mechanism.
  size_t shared_secret_len = KYBER768_SHARED_SECRET_BYTES;
  size_t ciphertext_len = KYBER768_CIPHERTEXT_BYTES;
  uint8_t shared_secret_alice[KYBER768_SHARED_SECRET_BYTES];
  uint8_t shared_secret_bob[KYBER768_SHARED_SECRET_BYTES];
  uint8_t ciphertext_alice[KYBER768_CIPHERTEXT_BYTES];
  uint8_t ciphertext_bob[KYBER768_CIPHERTEXT_BYTES];

  // Alice generates the key pair.
  EVP_PKEY_CTX *kyber_pkey_ctx_alice = EVP_PKEY_CTX_new_id(EVP_PKEY_KYBER768, nullptr);
  ...
  const KYBER768_KEY *kyber_key_alice = (KYBER768_KEY *)(kyber_pkey_alice->pkey.ptr);

  EVP_PKEY_CTX *kyber_pkey_ctx_bob = EVP_PKEY_CTX_new_id(EVP_PKEY_KYBER768, nullptr);
  EVP_PKEY *kyber_pkey_bob = EVP_PKEY_new_raw_public_key(EVP_PKEY_KYBER768,
                                                         NULL,
...
TEST(Kyber768Test, KEMSizeChecks) {
  ...
  EXPECT_EQ(shared_secret_len, (size_t)KYBER768_SHARED_SECRET_BYTES);
  EXPECT_EQ(ciphertext_len, (size_t)KYBER768_CIPHERTEXT_BYTES);
  ...
  uint8_t shared_secret[KYBER768_SHARED_SECRET_BYTES];
...
TEST(Kyber768Test, KEMInvalidKeyType) {
  size_t shared_secret_len = KYBER768_SHARED_SECRET_BYTES;
  size_t ciphertext_len = KYBER768_CIPHERTEXT_BYTES;
  uint8_t shared_secret[KYBER768_SHARED_SECRET_BYTES];
  uint8_t ciphertext[KYBER768_CIPHERTEXT_BYTES];
...
TEST(Kyber768Test, KEMFailureModes) {
  size_t shared_secret_len = KYBER768_SHARED_SECRET_BYTES;
  size_t ciphertext_len = KYBER768_CIPHERTEXT_BYTES;
  uint8_t shared_secret[KYBER768_SHARED_SECRET_BYTES];
  uint8_t ciphertext[KYBER768_CIPHERTEXT_BYTES];
