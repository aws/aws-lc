...
###
###
docker run -v `pwd`:`pwd` -w `pwd` ${DOCKER_IMAGE_HOST}/${DOCKER_IMAGE_REPO_NAME}${DI_ARCH}:${DI_TAG} /bin/bash ${AWS_LC_DIR}/rust/generate/_collect_symbols_build.sh
...
cat ${TMP_DIR}/*/symbols.txt | sort | uniq > ${TMP_DIR}/symbols.txt
