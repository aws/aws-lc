...
bool tls13_process_certificate(SSL_HANDSHAKE *hs, const SSLMessage &msg,
  ...
  if (!CBS_get_u8_length_prefixed(&body, &context) || CBS_len(&context) != 0 ||
  ...
  while (CBS_len(&certificate_list) > 0) {
    ...
    if (!buf || !PushToStack(certs.get(), std::move(buf))) {
      ssl_send_alert(ssl, SSL3_AL_FATAL, SSL_AD_INTERNAL_ERROR);
      return false;
    }
...
bool tls13_process_certificate_verify(SSL_HANDSHAKE *hs,
  ...
  if (!CBS_get_u16(&body, &signature_algorithm) ||
      !CBS_get_u16_length_prefixed(&body, &signature) || CBS_len(&body) != 0) {
    OPENSSL_PUT_ERROR(SSL, SSL_R_DECODE_ERROR);
    ssl_send_alert(ssl, SSL3_AL_FATAL, SSL_AD_DECODE_ERROR);
    return false;
  }
...
static bool tls13_receive_key_update(SSL *ssl, const SSLMessage &msg) {
  ...
  if (!CBS_get_u8(&body, &key_update_request) || CBS_len(&body) != 0 ||
