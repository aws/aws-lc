...
TEST(SSLTest, QuietShutdown) {
  ...
  // unexpected transport EOF, not a close_notify, so |SSL_read| fails with
  // |SSL_ERROR_SSL| and |SSL_R_UNEXPECTED_EOF_WHILE_READING|.
  char buf[1];
...
// |SSL_R_UNEXPECTED_EOF_WHILE_READING| on the error stack, for both the client
// and the server.
TEST(SSLTest, IgnoreUnexpectedEOF) {
  ...
  // shutdown, |SSL_ERROR_ZERO_RETURN|, rather than |SSL_ERROR_SSL| with
...
// Test that, without |SSL_OP_IGNORE_UNEXPECTED_EOF|, an unexpected transport EOF
// (the peer closing without a close_notify) is reported as |SSL_ERROR_SSL| with
// |SSL_R_UNEXPECTED_EOF_WHILE_READING| on the error stack, for both the client
// and the server.
TEST(SSLTest, UnexpectedEOF) {
  bssl::UniquePtr<SSL_CTX> client_ctx(SSL_CTX_new(TLS_method()));
  bssl::UniquePtr<SSL_CTX> server_ctx =
      CreateContextWithTestCertificate(TLS_method());
  ASSERT_TRUE(client_ctx);
  ...
  // Without the option set, an unexpected transport EOF is reported as
  ...
  ret = SSL_read(server.get(), buf, sizeof(buf));
  EXPECT_EQ(ret, -1);
  EXPECT_EQ(SSL_get_error(server.get(), ret), SSL_ERROR_SSL);
  EXPECT_EQ(ERR_GET_LIB(ERR_peek_error()), ERR_LIB_SSL);
  EXPECT_EQ(ERR_GET_REASON(ERR_get_error()),
            SSL_R_UNEXPECTED_EOF_WHILE_READING);
}
