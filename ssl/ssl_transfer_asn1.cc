...
static int SSL3_STATE_to_bytes(SSL3_STATE *in, uint16_t protocol_version,
  ...
  if (!in->pending_app_data.empty()) {
    ...
    // |read_buffer.buf_ptr()|, as the alignment can change on restore.
    // It should be relative to |read_buffer.data()|, which can mean it may
    // currently point towards data before the current offset and thus be negative.
    //
    // Ideally for simplicity we would make this relative to read buffer's alignment
    // offset, but we might not know this in the V1 case if the buffer had been restored
    // from a V1 format :\. So this is the better option at this time.
    ptrdiff_t offset = in->pending_app_data.data() - in->read_buffer.data();
...
static int SSL3_STATE_from_bytes(SSL *ssl, CBS *cbs, const SSL_CTX *ctx) {
  ...
  if (is_tls13) {
    ...
    out->ech_status = static_cast<ssl_ech_status_t>(ech_status);
             ...
             ech_status || pending_hs_data_present) {
    OPENSSL_PUT_ERROR(SSL, SSL_R_SERIALIZATION_INVALID_SSL3_STATE);
    return 0;
  }
  ...
  if (pending_app_data_present) {
    ...
    CBS app_seq;
    int64_t pending_app_data_offset;
    uint64_t pending_app_data_size;
    if (!CBS_get_asn1(&pending_app_data, &app_seq, CBS_ASN1_SEQUENCE) ||
        !CBS_get_asn1_int64(&app_seq, &pending_app_data_offset) ||
        !CBS_get_asn1_uint64(&app_seq, &pending_app_data_size)) {
      OPENSSL_PUT_ERROR(SSL, SSL_R_SERIALIZATION_INVALID_SSL3_STATE);
      return 0;
    }
    if (!fits_in_ptrdiff_int64(pending_app_data_offset)) {
      OPENSSL_PUT_ERROR(SSL, SSL_R_SERIALIZATION_INVALID_SSL3_STATE);
      return 0;
    }
    uint8_t *pending_app_data_ptr =
        out->read_buffer.data() + (ptrdiff_t)pending_app_data_offset;
    if (out->read_buffer.buf_ptr() > pending_app_data_ptr ||
        pending_app_data_ptr + pending_app_data_size >
            out->read_buffer.buf_ptr() + out->read_buffer.buf_size()) {
      OPENSSL_PUT_ERROR(SSL, SSL_R_SERIALIZATION_INVALID_SSL3_STATE);
      return 0;
    }
    out->pending_app_data =
        MakeSpan(pending_app_data_ptr, pending_app_data_size);
  }
