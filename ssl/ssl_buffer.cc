...
static OPENSSL_INLINE size_t compute_buffer_offset(size_t header_len, uint8_t *buffer) {
...
// SSLBuffer ::= SEQUENCE {
//    version                           INTEGER (2),  -- SSLBuffer structure version
...
bool SSLBuffer::DoDeserializationV1(CBS *cbs) {
  ...
  if (!CBS_get_asn1_bool(cbs, &buf_allocated_int) ||
      ...
      !CBS_get_asn1(cbs, &buf, CBS_ASN1_OCTETSTRING) ||
      CBS_len(cbs) != 0) {
    OPENSSL_PUT_ERROR(SSL, SSL_R_SERIALIZATION_INVALID_SSL_BUFFER);
    return false;
  }
  ...
  size_ = (uint16_t)size;
  cap_ = (uint16_t)cap;
  // As we restored from a V1 format we can only serialize as V1 until the next |EnsureCap| call.
  max_serialization_version_ = 1;
  return true;
}
...
bool SSLBuffer::DoDeserializationV2(CBS *cbs) {
  ...
  CBS buf;
  int buf_allocated_int;
  uint64_t header_len, buf_cap, rel_offset, cap;
  size_t align_offset = 0;
  if (!CBS_get_asn1_bool(cbs, &buf_allocated_int) ||
  ...
  if (buf_allocated) {
    ...
    // We could relax this and just allocate a in-memory buffer with correct alignment.
    // But this value is not configurable (unlike SSL3_ALIGN_PAYLOAD), so we can adjust
    // this in the future if we modify sizeof(inline_buf_);
    if (CBS_len(&buf) > sizeof(inline_buf_)) {
      OPENSSL_PUT_ERROR(SSL, SSL_R_SERIALIZATION_INVALID_SSL_BUFFER);
      return false;
    }
