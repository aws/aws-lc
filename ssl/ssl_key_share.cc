 ...
 public:
  ...
  bool Offer(CBB *out) override {
    ...
    if (!EVP_PKEY_keygen_init(ctx_.get()) ||
        !EVP_PKEY_keygen(ctx_.get(), &raw_key) || !raw_key ||
  ...
  bool Accept(CBB *out_public_key, Array<uint8_t> *out_secret,
    ...
    *out_alert = SSL_AD_INTERNAL_ERROR;
    ...
    UniquePtr<EVP_PKEY> pkey(EVP_PKEY_kem_new_raw_public_key(
        nid_, peer_key.begin(), peer_key.size()));
    ...
    if (!CBB_add_space(out_public_key, &ciphertext, ciphertext_len) ||
        !EVP_PKEY_encapsulate(ctx_.get(), ciphertext, &ciphertext_bytes_written,
                              shared_secret.data(), &secret_bytes_written) ||
  ...
  // resulting from the peer encapsulating the shared secret under our public
  // key. In Finish(), we use our previously generated secret key to decrypt
  ...
  bool Finish(Array<uint8_t> *out_secret, uint8_t *out_alert,
    ...
    *out_alert = SSL_AD_INTERNAL_ERROR;
    ...
    size_t secret_len = 0;
    if (!EVP_PKEY_decapsulate(ctx_.get(), nullptr, &secret_len, nullptr,
                              peer_key.size())) {
      OPENSSL_PUT_ERROR(SSL, ERR_R_INTERNAL_ERROR);
      return false;
    }
...
class HybridKeyShare : public SSLKeyShare {
 public:
  HybridKeyShare(uint16_t group_id)
      : group_id_(group_id), exchange_performed(false), hybrid_group_(nullptr) {
    for (const HybridGroup &hybrid_group : HybridGroups()) {
      if (group_id_ == hybrid_group.group_id) {
        ...
        for (size_t i = 0; i < NUM_HYBRID_COMPONENTS; i++) {
          key_shares_[i] =
              SSLKeyShare::Create(hybrid_group.component_group_ids[i]);
        }
        return;
      }
    }
  ...
  bool Accept(CBB *out_public_key, Array<uint8_t> *out_secret,
    ...
    *out_alert = SSL_AD_INTERNAL_ERROR;
    ...
    for (size_t i = 0; i < NUM_HYBRID_COMPONENTS; i++) {
      ...
      if (!get_component_offer_key_share_size(
              &component_key_size, hybrid_group_->component_group_ids[i])) {
        OPENSSL_PUT_ERROR(SSL, ERR_R_INTERNAL_ERROR);
        return false;
      }
      ...
      if (!key_shares_[i] ||
          !key_shares_[i]->Accept(out_public_key, &component_secret, out_alert,
                                  component_key) ||
          !CBB_add_bytes(&hybrid_shared_secret, component_secret.data(),
                         component_secret.size())) {
        CBB_cleanup(&hybrid_shared_secret);
        OPENSSL_PUT_ERROR(SSL, ERR_R_INTERNAL_ERROR);
        return false;
      }
  ...
  bool Finish(Array<uint8_t> *out_secret, uint8_t *out_alert,
    ...
    *out_alert = SSL_AD_INTERNAL_ERROR;
    ...
    for (size_t i = 0; i < NUM_HYBRID_COMPONENTS; i++) {
      size_t component_key_size = 0;
      if (!get_component_accept_key_share_size(
              &component_key_size, hybrid_group_->component_group_ids[i])) {
        OPENSSL_PUT_ERROR(SSL, ERR_R_INTERNAL_ERROR);
        return false;
      }
      ...
      Array<uint8_t> component_secret;
      if (!key_shares_[i] ||
          !key_shares_[i]->Finish(&component_secret, out_alert,
                                  component_key) ||
          !CBB_add_bytes(&hybrid_shared_secret, component_secret.data(),
                         component_secret.size())) {
        CBB_cleanup(&hybrid_shared_secret);
 ...
 private:
  // Only need to support SSL Groups that are a component of a supported
  // HybridKeyShare
  bool get_component_offer_key_share_size(size_t *out,
                                          uint16_t component_group_id) {
  ...
  // Only need to support SSL Groups that are a component of a supported
  // HybridKeyShare
  bool get_component_accept_key_share_size(size_t *out,
                                           uint16_t component_group_id) {
    switch (component_group_id) {
      case SSL_GROUP_SECP256R1:
        *out = 1 + (2 * EC_P256R1_FIELD_ELEM_BYTES);
...
CONSTEXPR_ARRAY uint16_t kPQGroups[] = {SSL_GROUP_KYBER512_R3,
...
bool SSLKeyShare::Accept(CBB *out_public_key, Array<uint8_t> *out_secret,
  ...
  *out_alert = SSL_AD_INTERNAL_ERROR;
  return Offer(out_public_key) && Finish(out_secret, out_alert, peer_key);
}
...
bool ssl_name_to_group_id(uint16_t *out_group_id, const char *name,
                          size_t len) {
  for (const auto &group : kNamedGroups) {
    if (len == strlen(group.name) && !strncmp(group.name, name, len)) {
      *out_group_id = group.group_id;
      return true;
    }
