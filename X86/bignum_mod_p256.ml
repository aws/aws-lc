...
(**** print_literal_from_elf "X86/bignum_mod_p256.o";;
 ****)

let bignum_mod_p256_mc =
  define_assert_from_elf "bignum_mod_p256_mc" "X86/bignum_mod_p256.o"
[
  0x53;                    (* PUSH (% rbx) *)
  0x41; 0x54;              (* PUSH (% r12) *)
 ...
 (`!z k x n pc.
  ...
  ASM_CASES_TAC `k < 4` THENL
   ...
   ASM_REWRITE_TAC[] THENL
    [X86_STEPS_TAC BIGNUM_MOD_P256_EXEC (1--12);
     X86_STEPS_TAC BIGNUM_MOD_P256_EXEC (1--15);
     X86_STEPS_TAC BIGNUM_MOD_P256_EXEC (1--18);
     X86_STEPS_TAC BIGNUM_MOD_P256_EXEC (1--20)] THEN
  ...
  CONJ_TAC THENL
   [ABBREV_TAC `j = k - 4` THEN VAL_INT64_TAC `j:num` THEN
    ...
    X86_ACCSTEPS_TAC BIGNUM_MOD_P256_EXEC [9;11;13;14] (1--14) THEN
    SUBGOAL_THEN `carry_s14 <=> m < p_256` SUBST_ALL_TAC THENL
     [MATCH_MP_TAC FLAG_FROM_CARRY_LT THEN EXISTS_TAC `256` THEN
      ...
      ALL_TAC] THEN
    X86_ACCSTEPS_TAC BIGNUM_MOD_P256_EXEC (18--21) (15--21) THEN
  ...
  THENL
   [GHOST_INTRO_TAC `d0:int64` `read R8` THEN
    ...
    ENSURES_INIT_TAC "s0" THEN X86_STEPS_TAC BIGNUM_MOD_P256_EXEC (1--6) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  ...
  ENSURES_WHILE_PDOWN_TAC `k - 4` `pc + 0x63` `pc + 0xe9`
  ...
  ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
   [VAL_INT64_TAC `k - 4` THEN REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
    ENSURES_INIT_TAC "s0" THEN X86_STEPS_TAC BIGNUM_MOD_P256_EXEC (1--2) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[];
    ALL_TAC; (*** Main loop invariant ***)
    X_GEN_TAC `i:num` THEN STRIP_TAC THEN VAL_INT64_TAC `i:num` THEN
    ASM_REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
    ENSURES_INIT_TAC "s0" THEN X86_STEPS_TAC BIGNUM_MOD_P256_EXEC [1] THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[];
    GHOST_INTRO_TAC `d0:int64` `read R8` THEN
    GHOST_INTRO_TAC `d1:int64` `read R9` THEN
    GHOST_INTRO_TAC `d2:int64` `read R10` THEN
    GHOST_INTRO_TAC `d3:int64` `read R11` THEN
    REWRITE_TAC[SUB_REFL; HIGHDIGITS_0] THEN
    ENSURES_INIT_TAC "s0" THEN X86_STEPS_TAC BIGNUM_MOD_P256_EXEC (1--5) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
    FIRST_X_ASSUM(fun th -> GEN_REWRITE_TAC RAND_CONV [SYM th]) THEN
    REWRITE_TAC[bignum_of_wordlist] THEN
    CONV_TAC(LAND_CONV BIGNUM_EXPAND_CONV) THEN
  ...
  X86_ACCSTEPS_TAC BIGNUM_MOD_P256_EXEC [7;8] (1--8) THEN
  SUBGOAL_THEN
  ...
  X86_STEPS_TAC BIGNUM_MOD_P256_EXEC (9--10) THEN
  ...
  X86_ACCSTEPS_TAC BIGNUM_MOD_P256_EXEC (11--22) (11--22) THEN
  SUBGOAL_THEN
  ...
  X86_ACCSTEPS_TAC BIGNUM_MOD_P256_EXEC [27;29;31;33] (23--35) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
