...
(**** print_literal_from_elf "X86/bignum_gt.o";;
 ****)

let bignum_gt_mc =
  define_assert_from_elf "bignum_gt_mc" "X86/bignum_gt.o"
[
...
let BIGNUM_GT_EXEC = X86_MK_EXEC_RULE bignum_gt_mc;;
    ...
    CONJ_TAC THENL
     [ASM_CASES_TAC `m = 0` THENL
       [UNDISCH_THEN `m = 0` SUBST_ALL_TAC THEN
        ...
        ENSURES_INIT_TAC "s0" THEN X86_STEPS_TAC BIGNUM_GT_EXEC (1--6) THEN
        ENSURES_FINAL_STATE_TAC THEN
        ASM_REWRITE_TAC[LOWDIGITS_0; LT_REFL] THEN CONV_TAC WORD_RULE;
        ALL_TAC] THEN
      ENSURES_WHILE_PUP_TAC `m:num` `pc + 0x10` `pc + 0x1e`
      ...
      ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
       [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES; SUB_0] THEN
        ...
        ENSURES_INIT_TAC "s0" THEN X86_STEPS_TAC BIGNUM_GT_EXEC [1] THEN
        ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[];
        REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN ENSURES_INIT_TAC "s0" THEN
        X86_STEPS_TAC BIGNUM_GT_EXEC (1--2) THEN
      ...
      ASSUME_TAC THENL
       [MAP_EVERY EXPAND_TAC ["y"; "x"] THEN
        ...
        ALL_TAC] THEN
      X86_STEPS_TAC BIGNUM_GT_EXEC (1--4) THEN
      ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
      ...
      ASM_CASES_TAC `n:num = m` THENL
       [UNDISCH_THEN `n:num = m` SUBST_ALL_TAC THEN
        ...
        ENSURES_INIT_TAC "s0" THEN X86_STEPS_TAC BIGNUM_GT_EXEC (1--4) THEN
      ...
      ENSURES_WHILE_PUP_TAC `n - m:num` `pc + 0x22` `pc + 0x30`
      ...
      ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
       [SIMPLE_ARITH_TAC;
        ASM_REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
        ENSURES_INIT_TAC "s0" THEN X86_STEPS_TAC BIGNUM_GT_EXEC (1--2) THEN
        ...
        ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[];
        REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
        ENSURES_INIT_TAC "s0" THEN X86_STEPS_TAC BIGNUM_GT_EXEC (1--3) THEN
    ...
    CONJ_TAC THENL
     [ASM_CASES_TAC `n = 0` THENL
       [UNDISCH_THEN `n = 0` SUBST_ALL_TAC THEN
        ASM_REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES; SUB_0] THEN
        ENSURES_INIT_TAC "s0" THEN X86_STEPS_TAC BIGNUM_GT_EXEC (1--7) THEN
        ENSURES_FINAL_STATE_TAC THEN
        ASM_REWRITE_TAC[LOWDIGITS_0; LT_REFL; WORD_RULE
         `word_add (word_sub (word 0) y) y = word 0`] THEN
        CONV_TAC WORD_RULE;
        ALL_TAC] THEN
      ENSURES_WHILE_PUP_TAC `n:num` `pc + 0x44` `pc + 0x52`
      ...
      ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
       [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES; SUB_0] THEN
        ...
        ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[];
        REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN ENSURES_INIT_TAC "s0" THEN
        X86_STEPS_TAC BIGNUM_GT_EXEC [1] THEN
        ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[]] THEN
      X_GEN_TAC `i:num` THEN STRIP_TAC THEN
      ...
      STRIP_ASSUME_TAC THENL [SIMPLE_ARITH_TAC; ALL_TAC] THEN
      ENSURES_WHILE_PUP_TAC `m - n:num` `pc + 0x54` `pc + 0x63`
