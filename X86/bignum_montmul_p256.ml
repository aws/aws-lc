...
let BIGNUM_MONTMUL_P256_CORRECT = time prove
 (`!z x y a b pc.
        nonoverlapping (word pc,0x28d) (z,8 * 4)
        ==> ensures x86
  ...
  ASM_CASES_TAC `a * b <= 2 EXP 256 * p_256` THENL
   [ASM_REWRITE_TAC[]; X86_SIM_TAC BIGNUM_MONTMUL_P256_EXEC (1--118)] THEN
  ...
  X86_ACCSTEPS_TAC BIGNUM_MONTMUL_P256_EXEC (97--108) (97--108) THEN
  SUBGOAL_THEN
  `sum_s108:int64 = word_neg(word(bitval(p_256 <= t))) /\
   (carry_s108 <=> p_256 <= t)`
  ...
  X86_ACCSTEPS_TAC BIGNUM_MONTMUL_P256_EXEC (109--118) (109--118) THEN
