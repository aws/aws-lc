...
(**** print_literal_from_elf "X86/bignum_triple_p256.o";;
 ****)

let bignum_triple_p256_mc = define_assert_from_elf "bignum_triple_p256_mc" "X86/bignum_triple_p256.o"
[
...
let BIGNUM_TRIPLE_P256_CORRECT = time prove
 (`!z x n pc.
        nonoverlapping (word pc,0x8f) (z,8 * 4)
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_triple_p256_mc /\
                  read RIP s = word pc /\
                  C_ARGUMENTS [z; x] s /\
                  bignum_from_memory (x,4) s = n)
             (\s. read RIP s = word (pc + 0x8e) /\
  ...
  X86_ACCSTEPS_TAC BIGNUM_TRIPLE_P256_EXEC (1--9) (1--9) THEN
  SUBGOAL_THEN
   `bignum_of_wordlist [mullo_s2; sum_s4; sum_s6; sum_s8; sum_s9] =
    3 * n`
  ASSUME_TAC THENL
   [EXPAND_TAC "n" THEN
  ...
  SUBGOAL_THEN `h < 3` ASSUME_TAC THENL
   [UNDISCH_TAC `n < 2 EXP (64 * 4)` THEN EXPAND_TAC "h" THEN ARITH_TAC;
    ALL_TAC] THEN
  SUBGOAL_THEN `sum_s9:int64 = word h` SUBST_ALL_TAC THENL
  ...
  X86_ACCSTEPS_TAC BIGNUM_TRIPLE_P256_EXEC
    [11;14;15;16;17;20;21;22] (10--22) THEN
  MP_TAC(SPECL
   [`sum_s22:int64`;
    `&(bignum_of_wordlist[sum_s11; sum_s16; sum_s17; sum_s20]):real`;
    `256`; `3 * n`; `(h + 1) * p_256`]
  ...
  ASM_REWRITE_TAC[] THEN ANTS_TAC THENL
   [CONJ_TAC THENL
    ...
    SUBST1_TAC(SYM(ASSUME
     `bignum_of_wordlist [mullo_s2; sum_s4; sum_s6; sum_s8; word h] =
      3 * n`)) THEN
  ...
  X86_ACCSTEPS_TAC BIGNUM_TRIPLE_P256_EXEC [27;29;31;33] (23--34) THEN
