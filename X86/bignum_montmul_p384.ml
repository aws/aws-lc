...
(**** print_literal_from_elf "X86/bignum_montmul_p384.o";;
 ****)

let bignum_montmul_p384_mc =
  define_assert_from_elf "bignum_montmul_p384_mc" "X86/bignum_montmul_p384.o"
[
  0x53;                    (* PUSH (% rbx) *)
  0x55;                    (* PUSH (% rbp) *)
  0x41; 0x54;              (* PUSH (% r12) *)
...
let BIGNUM_MONTMUL_P384_EXEC = X86_MK_EXEC_RULE bignum_montmul_p384_mc;;
...
let BIGNUM_MONTMUL_P384_CORRECT = time prove
 (`!z x y a b pc.
  ...
  MP_TAC(fst(EQ_IMP_RULE(SPEC `val(sum_s249:int64)` NUM_AS_BITVAL_ALT))) THEN
