...
void PrintTo(const BitSizeTestParams& params, std::ostream* os) {
  *os << "BitSizeTestParams{numbits=" << params.numbits
...
TEST_F(DhparamTest, BasicGeneration) {
  ...
  bssl::UniquePtr<DH> dh(PEM_read_bio_DHparams(bio.get(), nullptr, nullptr, nullptr));
  ASSERT_TRUE(dh) << "Output is not valid DH parameters";

  // Verify bit size
  const BIGNUM *p = DH_get0_p(dh.get());
  ASSERT_TRUE(p);
  ASSERT_EQ(512u, static_cast<unsigned>(BN_num_bits(p))) << "Wrong bit size in output";
...
TEST_F(DhparamTest, GenerateVariousSizes) {
  ...
  for (unsigned bits : bit_sizes) {
    ...
    EXPECT_TRUE(dhparamTool(args)) << "Failed to generate " << bits << "-bit parameters";
...
TEST_F(DhparamTest, TextOutput) {
  ...
  EXPECT_NE(output.find("DH Parameters:"), std::string::npos) << "Missing 'DH Parameters:' in output";
  EXPECT_NE(output.find("P:"), std::string::npos) << "Missing 'P:' in output";
  EXPECT_NE(output.find("G:"), std::string::npos) << "Missing 'G:' in output";
  EXPECT_NE(output.find("512 bit"), std::string::npos) << "Missing bit size in output";
}
...
TEST_F(DhparamTest, TextWithEncodedOutput) {
  ...
  EXPECT_NE(output.find("DH Parameters:"), std::string::npos) << "Missing text output";
  EXPECT_NE(output.find("-----BEGIN DH PARAMETERS-----"), std::string::npos) << "Missing PEM output";
  EXPECT_NE(output.find("-----END DH PARAMETERS-----"), std::string::npos) << "Missing PEM end marker";
}
...
TEST_F(DhparamTest, DEROutputFormat) {
  ...
  const BIGNUM *p = DH_get0_p(dh.get());
  ASSERT_TRUE(p);
  ASSERT_EQ(512u, static_cast<unsigned>(BN_num_bits(p))) << "Wrong bit size in DER output";
}
...
TEST_F(DhparamTest, ReadDERParameters) {
  ...
  // Now read them back
  args_list_t read_args = {"-inform", "DER", "-in", in_path, "-text", "-noout", "-out", out_path};
...
TEST_F(DhparamTest, ConvertPEMtoDER) {
  ...
  // Convert to DER
  args_list_t convert_args = {"-in", in_path, "-inform", "PEM", "-outform", "DER", "-out", out_path};
...
TEST_F(DhparamTest, ConvertDERtoPEM) {
  ...
  // Convert to PEM
  args_list_t convert_args = {"-in", in_path, "-inform", "DER", "-outform", "PEM", "-out", out_path};
...
TEST_F(DhparamTest, RoundTrip) {
  ...
  ASSERT_TRUE(bio1);
  bssl::UniquePtr<DH> dh1(PEM_read_bio_DHparams(bio1.get(), nullptr, nullptr, nullptr));
  ASSERT_TRUE(dh1);
  ...
  ASSERT_TRUE(bio2);
  bssl::UniquePtr<DH> dh2(PEM_read_bio_DHparams(bio2.get(), nullptr, nullptr, nullptr));
  ASSERT_TRUE(dh2);
  ...
  ASSERT_EQ(0, BN_cmp(g1, g2)) << "Generator values don't match after round-trip";
}
...
TEST_F(DhparamErrorTest, ConflictingOptions) {
  ...
  EXPECT_FALSE(dhparamTool(args)) << "Should fail when both numbits and -in are specified";
}

// Test too many arguments
TEST_F(DhparamErrorTest, TooManyArguments) {
  args_list_t args = {"512", "1024"};
  EXPECT_FALSE(dhparamTool(args)) << "Should fail with multiple numbits arguments";
}

// -------------------- OpenSSL Comparison Tests ------------------------------

// Parameterized tests for bit size generation comparison
class DhparamBitSizeComparisonTest : public ::testing::Test,
...
TEST_P(DhparamBitSizeComparisonTest, CrossCompatibility) {
  ...
  std::string tool_command = std::string(tool_executable_path) + " dhparam -out " +
                            out_path_tool + " " + std::to_string(params.numbits);
  ASSERT_EQ(system(tool_command.c_str()), 0) << "AWS-LC generation failed";

  // Verify OpenSSL can read AWS-LC generated parameters
  std::string openssl_read = std::string(openssl_executable_path) + " dhparam -in " +
                            out_path_tool + " -noout";
  ASSERT_EQ(system(openssl_read.c_str()), 0) << "OpenSSL cannot read AWS-LC generated parameters";

  // Generate with OpenSSL (can be slow, especially for larger sizes)
  std::string openssl_command = std::string(openssl_executable_path) + " dhparam -out " +
                               out_path_openssl + " " + std::to_string(params.numbits);
  ...
  ASSERT_EQ(system(tool_read.c_str()), 0) << "AWS-LC cannot read OpenSSL generated parameters";
}

INSTANTIATE_TEST_SUITE_P(BitSizeTests, DhparamBitSizeComparisonTest,
  ::testing::Values(
    BitSizeTestParams{512, "Bits512"},
    BitSizeTestParams{1024, "Bits1024"}
...
TEST_F(DhparamComparisonTest, NooutComparison) {
  std::string tool_command = std::string(tool_executable_path) + " dhparam -noout 512 > " + out_path_tool;
  std::string openssl_command = std::string(openssl_executable_path) + " dhparam -noout 512 > " + out_path_openssl;
...
TEST_F(DhparamComparisonTest, DERFormatComparison) {
  // Generate parameters
  std::string gen_command = std::string(tool_executable_path) + " dhparam -out " + params_path + " 512";
  ASSERT_EQ(system(gen_command.c_str()), 0);

  // Convert to DER with both tools
  std::string tool_command = std::string(tool_executable_path) + " dhparam -in " + params_path +
                            " -outform DER -out " + out_path_tool;
  std::string openssl_command = std::string(openssl_executable_path) + " dhparam -in " + params_path +
...
TEST_F(DhparamComparisonTest, PEMRoundTripCompatibility) {
  ...
  std::string tool_command = std::string(tool_executable_path) + " dhparam -in " + out_path_openssl +
                            " -out " + out_path_tool;
  ASSERT_EQ(system(tool_command.c_str()), 0);
...
TEST_F(DhparamComparisonTest, FormatConversionCompatibility) {
  ...
  std::string awslc_convert = std::string(tool_executable_path) + " dhparam -in " + params_path +
                             " -outform DER -out " + out_path_tool;
  ASSERT_EQ(system(awslc_convert.c_str()), 0);

  // Convert same PEM to DER with OpenSSL
  std::string openssl_convert = std::string(openssl_executable_path) + " dhparam -in " + params_path +
                               " -outform DER -out " + out_path_openssl;
