...
class PKeyTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    ASSERT_TRUE(in_file);
    ASSERT_TRUE(PEM_write_PrivateKey(in_file.get(), pkey.get(), nullptr,
...
TEST_F(PKeyTest, OutformDER) {
  ...
  ASSERT_TRUE(result);
  {
    ...
    ASSERT_TRUE(out_file);
    bssl::UniquePtr<EVP_PKEY> parsed_pkey(
        d2i_PrivateKey_fp(out_file.get(), nullptr));
    ASSERT_TRUE(parsed_pkey);
  }
}
...
TEST_F(PKeyComparisonTest, Text) {
  ...
  // OpenSSL versions may have slight formatting differences, so we remove
