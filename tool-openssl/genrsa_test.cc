...
class GenRSATestBase : public ::testing::Test {
 protected:
  ...
  bool ValidateEncryptedKeyFile(const char *path, const char *password,
    ...
    bssl::UniquePtr<RSA> rsa(PEM_read_bio_RSAPrivateKey(
...
class GenRSACipherParamTest
    : public GenRSATestBase,
...
TEST_P(GenRSAParamTest, GeneratesKeyFile) {
  ...
  if (is_fips && key_size < 2048) {
    GTEST_SKIP()
        << "Skipping " << key_size
        << "-bit key test in FIPS build/mode (minimum 2048 bits required)";
  }
