...
TEST_F(X509ComparisonTest, X509ToolCompareFingerprintOpenSSL) {
  std::string tool_command =
      ...
      " -fingerprint -subject_hash -subject_hash_old > " + out_path_tool;
  std::string openssl_command =
      std::string(openssl_executable_path) + " x509 -in " + in_path +
      " -fingerprint -subject_hash -subject_hash_old > " + out_path_openssl;
  ...
  tool_command = std::string(tool_executable_path) + " x509 -in " + in_path +
                 " -fingerprint -subject_hash -subject_hash_old -out " +
                 out_path_tool;
  openssl_command =
      std::string(openssl_executable_path) + " x509 -in " + in_path +
      " -fingerprint -subject_hash -subject_hash_old -out " + out_path_openssl;
...
TEST_F(X509ComparisonTest, X509ToolCompareReqSignkeyDaysOpenSSL) {
  ...
  RunCommandsAndCompareOutput(tool_command, openssl_command, out_path_tool,
                              out_path_openssl, tool_output_str,
                              openssl_output_str);
  // Certificates will not be identical, therefore testing that cert header and
  // footer are present
  trim(tool_output_str);
  ASSERT_EQ(tool_output_str.compare(0, CERT_BEGIN.size(), CERT_BEGIN), 0);
  ASSERT_EQ(tool_output_str.compare(tool_output_str.size() - CERT_END.size(),
                                    CERT_END.size(), CERT_END),
            0);

  trim(openssl_output_str);
  ASSERT_EQ(openssl_output_str.compare(0, CERT_BEGIN.size(), CERT_BEGIN), 0);
  ASSERT_EQ(
      openssl_output_str.compare(openssl_output_str.size() - CERT_END.size(),
                                 CERT_END.size(), CERT_END),
      0);
}
