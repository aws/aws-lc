...
let READ_BYTES_EQ = prove
 (`!s s' a n.
  ...
  REWRITE_TAC[MOD_MULT_ADD; READ_BYTES_MOD]);;
...
let bytelist_clauses = prove
 (`(!(a:A word) s. read (bytelist (a,0)) s = []) /\
  ...
  REPEAT STRIP_TAC THENL [
    ...
    CONV_TAC (ONCE_DEPTH_CONV NUM_REDUCE_CONV) THEN DISCH_TAC THEN
    ...
    CONV_TAC (ONCE_DEPTH_CONV NUM_REDUCE_CONV) THEN IMP_REWRITE_TAC [
