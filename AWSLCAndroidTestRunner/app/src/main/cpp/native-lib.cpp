...
Java_software_amazon_aws_crypto_awslcandroidtestrunner_NativeTestFixture_runTest(
        JNIEnv *env,
        jobject /* this */,
        jstring jni_name) {
    const char *test_name = env->GetStringUTFChars(jni_name, nullptr);
    __android_log_print(ANDROID_LOG_INFO, "awslc-test", "RUNNING %s", test_name);

    test_fn_t *test_fn = (test_fn_t*)dlsym(RTLD_DEFAULT, test_name);
    if (!test_fn) {
        __android_log_print(ANDROID_LOG_WARN, "awslc-test", "%s NOT FOUND", test_name);
        return -1;
    }
