...
#if defined(OPENSSL_IS_AWSLC)
static bool SpeedSHAKE256_x4_Absorb_Chunk(std::string name, size_t input_len) {
  ...
  memset(input0.get(), 0, input_len);
  memset(input1.get(), 0, input_len);
  memset(input2.get(), 0, input_len);
  memset(input3.get(), 0, input_len);
  ...
  results.PrintWithBytes(name, input_len);
  return true;
}

static bool SpeedSHAKE256_x4_Squeeze_Chunk(std::string name, size_t output_len) {
  ...
  results.PrintWithBytes(name, output_len);
  return true;
}
...
static bool SpeedSHAKE256_x4_Absorb(const std::string &selected) {
  ...
  for (size_t input_len : g_chunk_lengths) {
    if (!SpeedSHAKE256_x4_Absorb_Chunk("SHAKE256-x4 (Absorb)", input_len)) {
      return false;
    }
  }
...
static bool SpeedSHAKE256_x4_Squeeze(const std::string &selected) {
  ...
  for (size_t output_len : g_chunk_lengths) {
    if (!SpeedSHAKE256_x4_Squeeze_Chunk("SHAKE256-x4 (Squeeze)", output_len)) {
      return false;
    }
  }
