...
struct TimeResults {
 ...
 private:
  ...
  void PrintJSON(const std::string &description, const std::string &size_label,
                 size_t size = 0) const {
    if (first_json_printed) {
      puts(",");
    }
...
static bool SpeedRSA(const std::string &selected) {
  ...
  for (size_t i = 0; i < BM_ARRAY_SIZE(kRSAKeys); i++) {
    ...
    // d2i_RSAPrivateKey expects to be able to modify the input pointer as it
    // parses the input data and we don't want it to modify the original |*key|
    // data. Therefore create a new temp variable that points to the same data
    // and pass in the reference to it. As a sanity check make sure |input_key|
    // points to the end of the |*key|.
    const uint8_t *input_key = kRSAKeys[i].key;
    BM_NAMESPACE::UniquePtr<RSA> key(
        d2i_RSAPrivateKey(NULL, &input_key, (long)kRSAKeys[i].key_len));
    if (key == nullptr) {
    ...
    if (!TimeFunction(&results,
                      [&key, &sig, &fake_sha256_hash, &sig_len]() -> bool {
                        // Usually during RSA signing we're using a long-lived
                        // |RSA| that has already had all of its |BN_MONT_CTX|s
                        // constructed, so it makes sense to use |key| directly
                        // here.
                        return RSA_sign(NID_sha256, fake_sha256_hash,
                                        sizeof(fake_sha256_hash), sig.get(),
                                        &sig_len, key.get());
                      })) {
    ...
    if (!TimeFunction(&results,
                      [&key, &fake_sha256_hash, &sig, sig_len]() -> bool {
                        // Usually during RSA verification we have to parse an
                        // RSA key from a certificate or similar, in which case
                        // we'd need to construct a new RSA key, with a new
                        // |BN_MONT_CTX| for the public modulus. If we were to
                        // use |key| directly instead, then these costs wouldn't
                        // be accounted for.
...
static bool SpeedEvpGenericChunk(const EVP_CIPHER *cipher, std::string name,
  ...
  std::unique_ptr<uint8_t[]> plaintext_storage(
      new uint8_t[chunk_byte_len + kAlignment]);
  std::unique_ptr<uint8_t[]> ciphertext_storage(
      new uint8_t[chunk_byte_len + overhead_len + kAlignment]);
  ...
  uint8_t *const plaintext = static_cast<uint8_t *>(
      align_pointer(plaintext_storage.get(), kAlignment));
  BM_memset(plaintext, 0, chunk_byte_len);
  uint8_t *const ciphertext = static_cast<uint8_t *>(
      align_pointer(ciphertext_storage.get(), kAlignment));
  ...
  if (encrypt) {
    ...
    // Call EVP_EncryptInit_ex once with the cipher and key, the benchmark loop
    // will reuse both
    ...
    if (!TimeFunction(&encryptResults,
                      [&ctx, chunk_byte_len, plaintext, ciphertext, len_ptr,
                       tag, &nonce, &ad, ad_len, &isAead, &result]() -> bool {
                        result = EVP_EncryptInit_ex(ctx.get(), NULL, NULL, NULL,
                                                    nonce.get());
                        if (isAead) {
                          result &= EVP_EncryptUpdate(ctx.get(), NULL, len_ptr,
                                                      ad.get(), ad_len);
                        }
                        result &=
                            EVP_EncryptUpdate(ctx.get(), ciphertext, len_ptr,
                                              plaintext, chunk_byte_len);
                        result &= EVP_EncryptFinal_ex(
                            ctx.get(), ciphertext + *len_ptr, len_ptr);
                        if (isAead) {
                          result &= EVP_CIPHER_CTX_ctrl(
                              ctx.get(), EVP_CTRL_GCM_GET_TAG, 16, tag);
                        }
                        return result;
                      })) {
    ...
    result =
        EVP_EncryptInit_ex(ctx.get(), cipher, NULL, key.get(), nonce.get());
    if (isAead) {
      result &= EVP_EncryptUpdate(ctx.get(), NULL, len_ptr, ad.get(), ad_len);
    }
    result &= EVP_EncryptUpdate(ctx.get(), ciphertext, len_ptr, plaintext,
                                chunk_byte_len);
    ...
    // Call EVP_DecryptInit_ex once with the cipher and key, the benchmark loop
    // will reuse both
    ...
    if (!TimeFunction(&decryptResults,
                      [&ctx, chunk_byte_len, plaintext, ciphertext, len_ptr,
                       tag, &nonce, &ad, ad_len, &isAead, &result]() -> bool {
                        result = EVP_DecryptInit_ex(ctx.get(), NULL, NULL, NULL,
                                                    nonce.get());
                        if (isAead) {
                          result &= EVP_DecryptUpdate(ctx.get(), NULL, len_ptr,
                                                      ad.get(), ad_len);
                        }
                        result &=
                            EVP_DecryptUpdate(ctx.get(), plaintext, len_ptr,
                                              ciphertext, chunk_byte_len);
                        if (isAead) {
                          result &= EVP_CIPHER_CTX_ctrl(
                              ctx.get(), EVP_CTRL_GCM_SET_TAG, 16, tag);
                        }
...
static bool SpeedEvpCipherGeneric(const EVP_CIPHER *cipher,
  ...
  std::unique_ptr<uint8_t[]> nonce(new uint8_t[iv_len]);
  if (!TimeFunction(&results, [&]() -> bool {
        return EVP_EncryptInit_ex(ctx.get(), cipher, NULL, key.get(),
                                  nonce.get());
      })) {
  ...
  if (!TimeFunction(&results, [&]() -> bool {
        return EVP_DecryptInit_ex(ctx.get(), cipher, NULL, key.get(),
                                  nonce.get());
      })) {
...
static bool SpeedSingleKEM(const std::string &name, int nid,
                           const std::string &selected) {
  if (!selected.empty() && name.find(selected) == std::string::npos) {
    return true;
  }
  // Key generation (Alice).
  BM_NAMESPACE::UniquePtr<EVP_PKEY_CTX> a_ctx(
      EVP_PKEY_CTX_new_id(EVP_PKEY_KEM, nullptr));
  if (!a_ctx || !EVP_PKEY_CTX_kem_set_params(a_ctx.get(), nid) ||
  ...
  if (!EVP_PKEY_encapsulate(b_ctx.get(), b_ct.get(), &b_ct_len, b_ss.get(),
                            &b_ss_len) ||
      !EVP_PKEY_decapsulate(a_ctx.get(), a_ss.get(), &a_ss_len, b_ct.get(),
                            b_ct_len) ||
      (a_ss_len != b_ss_len)) {
    return false;
  }
  ...
  if (!TimeFunction(
          &results, [&b_ct, &b_ct_len, &b_ss, &b_ss_len, &b_ctx]() -> bool {
            return EVP_PKEY_encapsulate(b_ctx.get(), b_ct.get(), &b_ct_len,
                                        b_ss.get(), &b_ss_len);
          })) {
    return false;
  }
  ...
  if (!TimeFunction(
          &results, [&b_ct, &b_ct_len, &a_ss, &a_ss_len, &a_ctx]() -> bool {
            return EVP_PKEY_decapsulate(a_ctx.get(), a_ss.get(), &a_ss_len,
                                        b_ct.get(), b_ct_len);
          })) {
    return false;
  }
...
static bool SpeedDigestSignNID(const std::string &name, int nid,
  ...
  BM_NAMESPACE::UniquePtr<EVP_PKEY_CTX> pkey_ctx(
  ...
  if (!TimeFunction(&results,
                    [&md_ctx, &signature, &sig_len, msg_len]() -> bool {
                      return EVP_DigestSign(md_ctx.get(), signature.get(),
                                            &sig_len, msg, msg_len);
                    })) {
    return false;
  }
  ...
  if (!TimeFunction(&results,
                    [&md_ctx, &signature, &sig_len, msg_len]() -> bool {
                      return EVP_DigestVerify(md_ctx.get(), signature.get(),
                                              sig_len, msg, msg_len);
                    })) {
    return false;
  }
...
static bool SpeedAES256XTS(const std::string &name,  // const size_t in_len,
  ...
  // Benchmark just EVP_EncryptInit_ex with the cipher and key, the encrypt
  // benchmark loop will reuse both
  if (!TimeFunction(&results, [&]() -> bool {
  ...
  // Benchmark just EVP_DecryptInit_ex with the cipher and key, the decrypt
  // benchmark loop will reuse both
  if (!TimeFunction(&results, [&]() -> bool {
...
#endif
  if (!TimeFunction(&results, [&]() -> bool {
        return HMAC_Init_ex(ctx.get(), key.get(), key_len, md,
                            NULL /* ENGINE */);
      })) {
...
static bool SpeedHmacChunkOneShot(const EVP_MD *md, std::string name,
  ...
  if (!TimeFunction(&results, [&key, key_len, md, chunk_len, &input]() -> bool {
        ...
        return HMAC(md, key.get(), key_len, input.get(), chunk_len, digest,
                    &md_len) != nullptr;
      })) {
    fprintf(stderr, "HMAC_Final failed.\n");
    ERR_print_errors_fp(stderr);
...
static bool SpeedRandomChunk(RandomFunction function, std::string name,
                             size_t chunk_len) {
...
static bool SpeedRandom(RandomFunction function, const std::string &name,
                        const std::string &selected) {
  if (!selected.empty() && name.find(selected) == std::string::npos) {
    return true;
  }
...
curve_config supported_curves[] = {
    {"P-224", NID_secp224r1},     {"P-256", NID_X9_62_prime256v1},
    {"P-384", NID_secp384r1},     {"P-521", NID_secp521r1},
#if (!defined(OPENSSL_IS_BORINGSSL) && !defined(OPENSSL_IS_AWSLC)) || \
    AWSLC_API_VERSION > 16
...
static EVP_PKEY *evp_generate_key(const int curve_nid) {
  ...
  BM_NAMESPACE::UniquePtr<EVP_PKEY_CTX> evp_pkey_ctx(
      EVP_PKEY_CTX_new_id(local_nid, nullptr));
  ...
  if (local_nid == EVP_PKEY_EC) {
    ...
    if (!EVP_PKEY_paramgen_init(evp_pkey_ctx.get()) ||
        !EVP_PKEY_CTX_set_ec_paramgen_curve_nid(evp_pkey_ctx.get(),
                                                curve_nid) ||
        !EVP_PKEY_paramgen(evp_pkey_ctx.get(), &curve) || curve == nullptr) {
      return nullptr;
    }
...
static bool SpeedEvpEcdhCurve(const std::string &name, int nid,
  ...
  if (nid != NID_X25519) {
    ...
    BM_NAMESPACE::UniquePtr<EC_KEY> only_public_key_ec_key(
    ...
    if (peer_key_ec_key == nullptr ||
        ...
        !EVP_PKEY_assign_EC_KEY(only_public_key_evp_pkey.get(),
                                only_public_key_ec_key.release())) {
      return false;
    }
  ...
  if (!TimeFunction(&results, [nid, &peer_key]() -> bool {
        ...
        if (nid != NID_X25519) {
          // For the supported P NIST curves, the peer public key must be
          // validated to ensure proper computation.
        ...
        BM_NAMESPACE::UniquePtr<EVP_PKEY_CTX> derive_ctx(
            EVP_PKEY_CTX_new(my_key.get(), NULL));
        ...
        std::unique_ptr<uint8_t[]> shared_secret(
            new uint8_t[shared_secret_size]);
        if (!EVP_PKEY_derive(derive_ctx.get(), shared_secret.get(),
                             &shared_secret_size)) {
          return false;
        }
...
static bool SpeedECPOINTCurve(const std::string &name, int nid,
  ...
  EC_POINT_mul(group.get(), pin0.get(), scalar0.get(), nullptr, nullptr,
               ctx.get());
  EC_POINT_mul(group.get(), pin1.get(), scalar1.get(), nullptr, nullptr,
               ctx.get());
  ...
  if (!TimeFunction(&results, [&group, &pout, &ctx, &pin0, &pin1]() -> bool {
        if (!EC_POINT_add(group.get(), pout.get(), pin0.get(), pin1.get(),
                          ctx.get())) {
          return false;
        }
  ...
  if (!TimeFunction(&results, [&group, &pout, &ctx, &pin0, &scalar0]() -> bool {
        if (!EC_POINT_mul(group.get(), pout.get(), nullptr, pin0.get(),
                          scalar0.get(), ctx.get())) {
          return false;
        }
  ...
  if (!TimeFunction(&results, [&group, &pout, &ctx, &scalar0]() -> bool {
        if (!EC_POINT_mul(group.get(), pout.get(), scalar0.get(), nullptr,
                          nullptr, ctx.get())) {
          return false;
        }
  ...
  if (!TimeFunction(&results,
                    [&group, &pout, &pin0, &ctx, &scalar0, &scalar1]() -> bool {
                      if (!EC_POINT_mul(group.get(), pout.get(), scalar1.get(),
                                        pin0.get(), scalar0.get(), ctx.get())) {
                        return false;
                      }
...
#if (!defined(OPENSSL_1_0_BENCHMARK) && !defined(BORINGSSL_BENCHMARK) && \
     !defined(OPENSSL_IS_AWSLC)) ||                                      \
    AWSLC_API_VERSION >= 22
static bool SpeedFFDHGroup(const std::string &name, int nid,
  ...
  TimeResults results;
  if (!TimeFunction(
          &results, [&shared_secret, &server_pub, &dh_size, &nid]() -> bool {
            BM_NAMESPACE::UniquePtr<DH> client_dh(DH_new_by_nid(nid));
            return DH_generate_key(client_dh.get()) &&
                   dh_size == DH_compute_key_padded(shared_secret.get(),
                                                    server_pub,
                                                    client_dh.get());
          })) {
    return false;
...
#endif  //(!defined(OPENSSL_1_0_BENCHMARK) && !defined(BORINGSSL_BENCHMARK) &&
        //! defined(OPENSSL_IS_AWSLC)) || AWSLC_API_VERSION >= 22
...
static bool SpeedSPAKE2(const std::string &selected) {
  ...
  BM_NAMESPACE::UniquePtr<SPAKE2_CTX> alice(
  ...
  if (!TimeFunction(&results, [&alice_msg, alice_msg_len]() -> bool {
        BM_NAMESPACE::UniquePtr<SPAKE2_CTX> bob(
...
static bool SpeedJitter(size_t chunk_size) {
  ...
  if (!TimeFunction(&results, [&jitter_ec, &input, chunk_size]() -> bool {
        size_t bytes = jent_read_entropy(jitter_ec, input.get(), chunk_size);
        if (bytes != chunk_size) {
          return false;
        }
        return true;
...
static bool SpeedPKCS8(const std::string &selected) {
  ...
  BM_NAMESPACE::UniquePtr<EVP_PKEY> key(EVP_PKEY_new_raw_private_key(
      EVP_PKEY_ED25519, nullptr, &privkey[0], ED25519_PRIVATE_KEY_SEED_LEN));
...
static bool SpeedRefcountThreads(std::string name, size_t num_threads) {
  ...
  std::stringstream ss;
  ss << name << " " << iterations_per_thread << " iterations with "
     << num_threads << " threads";
...
static bool SpeedRefcount(const std::string &selected) {
  if (!selected.empty() &&
      selected.find("CRYPTO_refcount_inc") == std::string::npos) {
    return true;
  }
...
bool Speed(const std::vector<std::string> &args) {
  ...
  if (args_map.count("-chunks") != 0) {
    ...
    if (!parseCommaArgument(chunkVector, args_map, "-chunks")) {
      return false;
    }
  ...
  if (args_map.count("-threads") != 0) {
    ...
    if (!parseCommaArgument(threadVector, args_map, "-threads")) {
      return false;
    }
  ...
  if (args_map.count("-primes") != 0) {
    ...
    if (!parseCommaArgument(primeVector, args_map, "-primes")) {
      return false;
    }
  ...
  for (std::string selected : g_filters) {
    if (!SpeedAESBlock("AES-128", 128, selected) ||
        ...
        !SpeedEvpCipherGeneric(EVP_aes_128_gcm(), "EVP-AES-128-GCM", kTLSADLen,
                               selected) ||
        !SpeedEvpCipherGeneric(EVP_aes_192_gcm(), "EVP-AES-192-GCM", kTLSADLen,
                               selected) ||
        !SpeedEvpCipherGeneric(EVP_aes_256_gcm(), "EVP-AES-256-GCM", kTLSADLen,
                               selected) ||
        !SpeedEvpCipherGeneric(EVP_aes_128_ctr(), "EVP-AES-128-CTR", kTLSADLen,
                               selected) ||
        !SpeedEvpCipherGeneric(EVP_aes_192_ctr(), "EVP-AES-192-CTR", kTLSADLen,
                               selected) ||
        !SpeedEvpCipherGeneric(EVP_aes_256_ctr(), "EVP-AES-256-CTR", kTLSADLen,
                               selected) ||
    ...
    AWSLC_API_VERSION > 16
        ...
        !SpeedRandom(RAND_bytes, "RNG", selected) || !SpeedECDH(selected) ||
        ...
        !SpeedEvpEcdh(selected) || !SpeedECPOINT(selected) ||
        // OpenSSL 1.0 doesn't support Scrypt
        !SpeedScrypt(selected) ||
#endif
#if (!defined(OPENSSL_1_0_BENCHMARK) && !defined(BORINGSSL_BENCHMARK) && \
     !defined(OPENSSL_IS_AWSLC)) ||                                      \
    AWSLC_API_VERSION >= 24
        ...
        !SpeedEvpCipherGeneric(EVP_chacha20_poly1305(), "EVP-ChaCha20-Poly1305",
                               kTLSADLen, selected) ||
#endif
#if (!defined(OPENSSL_1_0_BENCHMARK) && !defined(BORINGSSL_BENCHMARK) && \
     !defined(OPENSSL_IS_AWSLC)) ||                                      \
    AWSLC_API_VERSION >= 22
        // OpenSSL 1.0 and BoringSSL don't support DH_new_by_nid, NID_ffdhe2048,
        // or NID_ffdhe4096
        !SpeedFFDH(selected) ||
#endif
        ...
        !SpeedRandom(CRYPTO_sysrand_for_seed, "CRYPTO_sysrand_for_seed",
                     selected) ||
