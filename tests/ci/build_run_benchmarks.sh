...
if [ "${ossl_vs_pr_code}" != 0 ]; then
  aws s3 cp ossl_vs_pr.csv s3://"${AWS_ACCOUNT_ID}-aws-lc-ci-bm-framework-pr-bucket/${COMMIT_ID}/regressions/${CPU_TYPE}${NOHW_TYPE}-ossl_vs_pr.csv"
  aws s3 mv ossl_vs_pr.csv s3://"${AWS_ACCOUNT_ID}-aws-lc-ci-bm-framework-pr-bucket/latest-${PR_NUM}/regressions/${CPU_TYPE}${NOHW_TYPE}-ossl_vs_pr.csv"
fi
if [ "${bssl_vs_pr_code}" != 0 ]; then
  aws s3 cp bssl_vs_pr.csv s3://"${AWS_ACCOUNT_ID}-aws-lc-ci-bm-framework-pr-bucket/${COMMIT_ID}/regressions/${CPU_TYPE}${NOHW_TYPE}-bssl_vs_pr.csv"
  aws s3 mv bssl_vs_pr.csv s3://"${AWS_ACCOUNT_ID}-aws-lc-ci-bm-framework-pr-bucket/latest-${PR_NUM}/regressions/${CPU_TYPE}${NOHW_TYPE}-bssl_vs_pr.csv"
fi
