...
class AwsLcGitHubFuzzCIStack(Stack):
    ...
    def __init__(self,
        ...
        fuzz_policy = iam.PolicyDocument.from_json(code_build_publish_metrics_in_json(env))
        ...
        cfn_codebuild.add_override("Properties.FileSystemLocations", [{
          ...
          "Location": "%s.efs.%s.amazonaws.com:/" % (fuzz_filesystem.file_system_id, env.region),
          "MountPoint": "/efs_fuzzing_root",
          "Type": "EFS"
        }])

        PruneStaleGitHubBuilds(scope=self, id="PruneStaleGitHubBuilds", project=fuzz_codebuild, ec2_permissions=False, env=env)
