...
#include <awslc/hmac.h>
#include <awslc/obj.h>
#include <awslc/sha.h>
#include <awslc/span.h>

#include "internal/fips/rand_internal.h"
...
static bool GetConfig(AWSLC_UNUSED const Span<const uint8_t> args[]) {
...
static awslc::UniquePtr<EC_KEY> ECKeyFromName(Span<const uint8_t> name) {
  int nid;
  ...
  return awslc::UniquePtr<EC_KEY>(EC_KEY_new_by_curve_name(nid));
}
...
static std::pair<std::vector<uint8_t>, std::vector<uint8_t>> GetPublicKeyBytes(
  ...
  if (!EC_POINT_get_affine_coordinates(EC_KEY_get0_group(key),
...
static bool ECDSAKeyGen(const Span<const uint8_t> args[]) {
  awslc::UniquePtr<EC_KEY> key = ECKeyFromName(args[0]);
...
static awslc::UniquePtr<BIGNUM> BytesToBIGNUM(Span<const uint8_t> bytes) {
...
static bool ECDSAKeyVer(const Span<const uint8_t> args[]) {
  ...
  awslc::UniquePtr<BIGNUM> x(BytesToBIGNUM(args[1]));
  awslc::UniquePtr<BIGNUM> y(BytesToBIGNUM(args[2]));

  awslc::UniquePtr<EC_POINT> point(EC_POINT_new(EC_KEY_get0_group(key.get())));
  uint8_t reply[1];
  if (!EC_POINT_set_affine_coordinates(EC_KEY_get0_group(key.get()),
...
static bool ECDSASigGen(const Span<const uint8_t> args[]) {
  awslc::UniquePtr<EC_KEY> key = ECKeyFromName(args[0]);
  awslc::UniquePtr<BIGNUM> d = BytesToBIGNUM(args[1]);
  ...
  awslc::UniquePtr<ECDSA_SIG> sig(ECDSA_do_sign(digest, digest_len, key.get()));
  if (!sig) {
    return false;
  }
...
static bool ECDSASigVer(const Span<const uint8_t> args[]) {
  ...
  auto msg = args[2];
  awslc::UniquePtr<BIGNUM> x(BytesToBIGNUM(args[3]));
  awslc::UniquePtr<BIGNUM> y(BytesToBIGNUM(args[4]));
  awslc::UniquePtr<BIGNUM> r(BytesToBIGNUM(args[5]));
  awslc::UniquePtr<BIGNUM> s(BytesToBIGNUM(args[6]));
  ECDSA_SIG sig;
  sig.r = r.get();
  sig.s = s.get();
