...
#include <awslc/asn1.h>
#include <awslc/asn1t.h>
...
TEST(ASN1Test, LargeTags) {
  ...
  awslc::UniquePtr<ASN1_TYPE> obj(d2i_ASN1_TYPE(NULL, &p, sizeof(kTag258)));
...
TEST(ASN1Test, IntegerSetting) {
  awslc::UniquePtr<ASN1_INTEGER> by_bn(M_ASN1_INTEGER_new());
  awslc::UniquePtr<ASN1_INTEGER> by_long(M_ASN1_INTEGER_new());
  awslc::UniquePtr<ASN1_INTEGER> by_uint64(M_ASN1_INTEGER_new());
  ...
  for (const auto &i : kValues) {
    ...
    const uint64_t abs = i < 0 ? negate_uint64(i) : i;
...
static bool MakeLinkedList(awslc::UniquePtr<uint8_t> *out, size_t *out_len,
                           size_t count) {
  awslc::ScopedCBB cbb;
...
void TestSerialize(T obj, int (*i2d_func)(T a, uint8_t **pp),
                   awslc::Span<const uint8_t> expected) {
