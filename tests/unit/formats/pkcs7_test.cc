...
static void TestCertReparse(const uint8_t *der_bytes, size_t der_len) {
  ...
  awslc::UniquePtr<uint8_t> free_result_data(result_data);
  ...
  awslc::UniquePtr<uint8_t> free_result2_data(result2_data);
  ...
  awslc::UniquePtr<PKCS7> pkcs7_obj(d2i_PKCS7(nullptr, &ptr, der_len));
  ...
  awslc::UniquePtr<uint8_t> free_result3_data(result3_data);
  ...
  awslc::UniquePtr<uint8_t> free_result4_data(result4_data);
...
static void TestCRLReparse(const uint8_t *der_bytes, size_t der_len) {
  awslc::UniquePtr<STACK_OF(X509_CRL)> crls(sk_X509_CRL_new_null());
  ASSERT_TRUE(crls);
  awslc::UniquePtr<STACK_OF(X509_CRL)> crls2(sk_X509_CRL_new_null());
  ASSERT_TRUE(crls2);
  uint8_t *result_data, *result2_data;
  size_t result_len, result2_len;
