...
static void TestImpl(const char *name, awslc::Span<const uint8_t> der,
                     const char *password,
                     const char *friendly_name) {
  SCOPED_TRACE(name);
  awslc::UniquePtr<STACK_OF(X509)> certs(sk_X509_new_null());
  ASSERT_TRUE(certs);

  EVP_PKEY *key = nullptr;
  CBS pkcs12 = der;
  ASSERT_TRUE(PKCS12_get_key_and_certs(&key, certs.get(), &pkcs12, password));
  awslc::UniquePtr<EVP_PKEY> delete_key(key);
...
static void TestCompat(awslc::Span<const uint8_t> der) {
  awslc::UniquePtr<BIO> bio(BIO_new_mem_buf(der.data(), der.size()));
  ASSERT_TRUE(bio);

  awslc::UniquePtr<PKCS12> p12(d2i_PKCS12_bio(bio.get(), nullptr));
  ASSERT_TRUE(p12);
  ...
  awslc::UniquePtr<EVP_PKEY> delete_key(key);
  awslc::UniquePtr<X509> delete_cert(cert);
  awslc::UniquePtr<STACK_OF(X509)> delete_ca_certs(ca_certs);
...
#if defined(AWSLC_UNSAFE_FUZZER_MODE)
...
static awslc::UniquePtr<X509> LoadX509(awslc::Span<const uint8_t> der) {
  const uint8_t *ptr = der.data();
  return awslc::UniquePtr<X509>(d2i_X509(nullptr, &ptr, der.size()));
}

static awslc::UniquePtr<EVP_PKEY> LoadPrivateKey(awslc::Span<const uint8_t> der) {
  CBS cbs = der;
  return awslc::UniquePtr<EVP_PKEY>(EVP_parse_private_key(&cbs));
}

static void TestRoundTrip(const char *password, const char *name,
                          awslc::Span<const uint8_t> key_der,
                          awslc::Span<const uint8_t> cert_der,
                          std::vector<awslc::Span<const uint8_t>> chain_der,
                          int key_nid, int cert_nid, int iterations,
                          int mac_iterations) {
  awslc::UniquePtr<EVP_PKEY> key;
  if (!key_der.empty()) {
    key = LoadPrivateKey(key_der);
    ASSERT_TRUE(key);
  }
  awslc::UniquePtr<X509> cert;
  if (!cert_der.empty()) {
    cert = LoadX509(cert_der);
    ASSERT_TRUE(cert);
  }
  awslc::UniquePtr<STACK_OF(X509)> chain;
  if (!chain_der.empty()) {
    chain.reset(sk_X509_new_null());
    ASSERT_TRUE(chain);
    for (auto der : chain_der) {
      awslc::UniquePtr<X509> x509 = LoadX509(der);
      ASSERT_TRUE(x509);
      ASSERT_TRUE(awslc::PushToStack(chain.get(), std::move(x509)));
    }
  }

  // Make a PKCS#12 blob.
  awslc::UniquePtr<PKCS12> pkcs12(
  ...
  awslc::UniquePtr<STACK_OF(X509)> certs2(sk_X509_new_null());
  ASSERT_TRUE(certs2);
  ASSERT_TRUE(PKCS12_get_key_and_certs(&key2, certs2.get(), &cbs, password));
  awslc::UniquePtr<EVP_PKEY> free_key2(key2);
...
TEST(PKCS12Test, RoundTrip) {
  TestRoundTrip(kPassword, nullptr /* no name */,
                awslc::Span<const uint8_t>(kTestKey),
                awslc::Span<const uint8_t>(kTestCert),
                {awslc::Span<const uint8_t>(kTestCert2)}, 0, 0, 0, 0);
  ...
  TestRoundTrip(kPassword, nullptr /* no name */, {} /* no key */,
                ...
                {awslc::Span<const uint8_t>(kTestCert2)}, 0, 0, 0, 0);
  TestRoundTrip(
      kPassword, nullptr /* no name */, awslc::Span<const uint8_t>(kTestKey),
      awslc::Span<const uint8_t>(kTestCert), {} /* no chain */, 0, 0, 0, 0);
  TestRoundTrip(kPassword, nullptr /* no name */,
                awslc::Span<const uint8_t>(kTestKey), {} /* no leaf */,
                {} /* no chain */, 0, 0, 0, 0);
  ...
  TestRoundTrip(
      kPassword, nullptr /* no name */, awslc::Span<const uint8_t>(kTestKey),
      awslc::Span<const uint8_t>(kTestCert),
      {awslc::Span<const uint8_t>(kTestCert2)}, NID_pbe_WithSHA1And40BitRC2_CBC,
      NID_pbe_WithSHA1And40BitRC2_CBC, 100, 100);
  TestRoundTrip(
      kPassword, nullptr /* no name */, awslc::Span<const uint8_t>(kTestKey),
      awslc::Span<const uint8_t>(kTestCert),
      {awslc::Span<const uint8_t>(kTestCert2)}, NID_pbe_WithSHA1And128BitRC4,
      NID_pbe_WithSHA1And128BitRC4, 100, 100);
  TestRoundTrip(kPassword, nullptr /* no name */,
                awslc::Span<const uint8_t>(kTestKey),
                awslc::Span<const uint8_t>(kTestCert),
                {awslc::Span<const uint8_t>(kTestCert2)},
                NID_pbe_WithSHA1And3_Key_TripleDES_CBC,
                NID_pbe_WithSHA1And3_Key_TripleDES_CBC, 100, 100);
}

static awslc::UniquePtr<EVP_PKEY> MakeTestKey() {
  awslc::UniquePtr<EC_KEY> ec_key(
      EC_KEY_new_by_curve_name(NID_X9_62_prime256v1));
  if (!ec_key ||
      !EC_KEY_generate_key(ec_key.get())) {
    return nullptr;
  }
  awslc::UniquePtr<EVP_PKEY> evp_pkey(EVP_PKEY_new());
  if (!evp_pkey ||
...
static awslc::UniquePtr<X509> MakeTestCert(EVP_PKEY *key) {
  awslc::UniquePtr<X509> x509(X509_new());
  if (!x509) {
    return nullptr;
  }
...
static bool PKCS12CreateVector(std::vector<uint8_t> *out, EVP_PKEY *pkey,
  ...
  awslc::UniquePtr<STACK_OF(X509)> chain(sk_X509_new_null());
  if (!chain) {
    return false;
  }

  for (X509 *cert : certs) {
    if (!awslc::PushToStack(chain.get(), awslc::UpRef(cert))) {
      return false;
    }
  }

  awslc::UniquePtr<PKCS12> p12(PKCS12_create(kPassword, nullptr /* name */, pkey,
...
static void ExpectPKCS12Parse(awslc::Span<const uint8_t> in,
                              EVP_PKEY *expect_key, X509 *expect_cert,
                              const std::vector<X509 *> &expect_ca_certs) {
  awslc::UniquePtr<BIO> bio(BIO_new_mem_buf(in.data(), in.size()));
  ASSERT_TRUE(bio);
...
TEST(PKCS12Test, Order) {
  awslc::UniquePtr<EVP_PKEY> key1 = MakeTestKey();
  ASSERT_TRUE(key1);
  awslc::UniquePtr<X509> cert1 = MakeTestCert(key1.get());
  ASSERT_TRUE(cert1);
  awslc::UniquePtr<X509> cert1b = MakeTestCert(key1.get());
  ASSERT_TRUE(cert1b);
  awslc::UniquePtr<EVP_PKEY> key2 = MakeTestKey();
  ASSERT_TRUE(key2);
  awslc::UniquePtr<X509> cert2 = MakeTestCert(key2.get());
  ASSERT_TRUE(cert2);
  awslc::UniquePtr<EVP_PKEY> key3 = MakeTestKey();
  ASSERT_TRUE(key3);
  awslc::UniquePtr<X509> cert3 = MakeTestCert(key3.get());
  ASSERT_TRUE(cert3);
