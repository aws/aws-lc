...
import (
	...
	"github.com/awslabs/aws-lc/util/testresult"
)
...
var (
	...
	useGDB          = flag.Bool("gdb", false, "If true, run AWS-LC code under gdb")
	useSDE          = flag.Bool("sde", false, "If true, run AWS-LC code under Intel's SDE for each supported chip")
...
// directory is the top of a AWS-LC checkout.
func setWorkingDirectory() {
	...
	panic("Couldn't find .travis.yml in a parent directory!")
}
...
func parseTestConfig(filename string) ([]test, error) {
	...
	for _, args := range testArgs {
		...
		if inDocker && strings.Contains(args[0], "urandom_test") {
			fmt.Printf("Running in docker, skipping: %v %v. See https://github.com/awslabs/aws-lc/issues/38.\n", env, args)
			continue
		}
...
func (t test) getGTestShards() ([]test, error) {
	...
	if t.args[0] != "crypto/crypto_test" && t.args[0] != "decrepit/decrepit_test" {
		return []test{t}, nil
	}
...
func main() {
	...
	testCases, err := parseTestConfig("tests/unit/all_tests.json")
	if err != nil {
