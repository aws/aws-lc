...
static void test(const char *name, const uint8_t *key, size_t key_len,
  ...
  awslc::UniquePtr<CMAC_CTX> ctx(CMAC_CTX_new());
  ASSERT_TRUE(ctx);
  ...
  awslc::UniquePtr<CMAC_CTX> ctx2(CMAC_CTX_new());
  ASSERT_TRUE(ctx2);
...
TEST(CMACTest, CAVPAES128) {
  RunCAVPTest("auth/cavp_aes128_cmac_tests.txt", EVP_aes_128_cbc(),
              false);
}

TEST(CMACTest, CAVPAES192) {
  RunCAVPTest("auth/cavp_aes192_cmac_tests.txt", EVP_aes_192_cbc(),
              false);
}

TEST(CMACTest, CAVPAES256) {
  RunCAVPTest("auth/cavp_aes256_cmac_tests.txt", EVP_aes_256_cbc(),
              false);
}

TEST(CMACTest, CAVP3DES) {
  RunCAVPTest("auth/cavp_3des_cmac_tests.txt", EVP_des_ede3_cbc(), true);
}
