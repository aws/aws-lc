...
#include "internal/fips/aes_internal.h"
...
static void TestRaw(FileTest *t) {
  ...
  AWSLC_memcpy(block, plaintext.data(), AES_BLOCK_SIZE);
  ...
  AWSLC_memcpy(block, ciphertext.data(), AES_BLOCK_SIZE);
...
static void TestKeyWrap(FileTest *t) {
  ...
  AWSLC_memset(buf.get(), 0, ciphertext.size());
  ...
  // Test with explicit IV.
  AWSLC_memset(buf.get(), 0, plaintext.size());
...
TEST(AESTest, TestVectors) {
  FileTestGTest("cipher/aes_tests.txt", [](FileTest *t) {
...
#if defined(BSAES) && !defined(AWSLC_SHARED_LIBRARY)
  ...
  for (int i = 0; i < kNumIterations; i++) {
    ...
    for (unsigned bits : {128u, 192u, 256u}) {
      ...
      for (bool enc : {false, true}) {
        ...
        AWSLC_memset(&nohw, 0xaa, sizeof(nohw));
        AWSLC_memset(&vpaes, 0xaa, sizeof(vpaes));
        AWSLC_memset(&bsaes, 0xaa, sizeof(bsaes));
        ...
        AWSLC_memcpy(&bsaes, &vpaes, sizeof(AES_KEY));
        if (enc) {
          vpaes_encrypt_key_to_bsaes(&bsaes, &vpaes);
        } else {
          vpaes_decrypt_key_to_bsaes(&bsaes, &vpaes);
