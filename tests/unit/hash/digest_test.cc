...
static void TestDigest(const DigestTestVector *test) {
  ...
  AWSLC_memcpy(ptr, test->input, strlen(test->input));
  for (size_t i = 0; i < test->repeat; i++) {
...
TEST(DigestTest, Getters) {
  ...
  awslc::UniquePtr<ASN1_OBJECT> obj(OBJ_txt2obj("1.3.14.3.2.26", 0));
  ASSERT_TRUE(obj);
...
TEST(DigestTest, TransformBlocks) {
  ...
  EXPECT_TRUE(0 == AWSLC_memcmp(ctx1.h, ctx2.h, sizeof(ctx1.h)));
}
