...
#if !defined(AWSLC_WINDOWS) && !defined(AWSLC_IOS) && \
    !defined(AWSLC_FUCHSIA) && !defined(AWSLC_UNSAFE_DETERMINISTIC_MODE)
static bool ForkAndRand(awslc::Span<uint8_t> out) {
  int pipefds[2];
  if (pipe(pipefds) < 0) {
    perror("pipe");
    return false;
...
#endif  // !AWSLC_WINDOWS && !AWSLC_IOS &&
        // !AWSLC_FUCHSIA && !AWSLC_UNSAFE_DETERMINISTIC_MODE
...
#endif  // AWSLC_X86_64 && SUPPORTS_ABI_TEST
