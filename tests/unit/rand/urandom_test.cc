...
#if defined(AWSLC_X86_64) && defined(AWSLC_LINUX) && \
    !defined(AWSLC_SHARED_LIBRARY) &&                \
    !defined(AWSLC_UNSAFE_DETERMINISTIC_MODE)
...
#if defined(AWSLC_NO_ASM)
static int have_rdrand() { return 0; }
#endif

// This test can be run with $AWSLC_ia32cap=~0x4000000000000000 in order to
