...
#include "internal/fips/p256-x86_64.h"
#include "abi_test.h"
#include "file_test.h"
#include "test_util.h"

// Disable tests if AWSLC_SHARED_LIBRARY is defined. These tests need access
// to internal functions.
#if !defined(AWSLC_NO_ASM) && defined(AWSLC_X86_64) && \
    !defined(AWSLC_SMALL) && !defined(AWSLC_SHARED_LIBRARY)

TEST(P256_X86_64Test, SelectW5) {
  // Fill a table with some garbage input.
  alignas(64) P256_POINT table[16];
  for (size_t i = 0; i < 16; i++) {
    AWSLC_memset(table[i].X, 3 * i, sizeof(table[i].X));
    AWSLC_memset(table[i].Y, 3 * i + 1, sizeof(table[i].Y));
    AWSLC_memset(table[i].Z, 3 * i + 2, sizeof(table[i].Z));
  }
  ...
  for (int i = 0; i <= 16; i++) {
    ...
    P256_POINT expected;
    if (i == 0) {
      AWSLC_memset(&expected, 0, sizeof(expected));
    } else {
      expected = table[i-1];
    }
...
TEST(P256_X86_64Test, SelectW7) {
  ...
  for (size_t i = 0; i < 64; i++) {
    AWSLC_memset(table[i].X, 2 * i, sizeof(table[i].X));
    AWSLC_memset(table[i].Y, 2 * i + 1, sizeof(table[i].Y));
  }
...
TEST(P256_X86_64Test, BEEU) {
  ...
  for (BN_ULONG i = 1; i < 2000; i++) {
    ...
    AWSLC_memcpy(in_scalar.bytes, in, sizeof(in));
    AWSLC_memcpy(out_scalar.bytes, out, sizeof(out));
...
static bool GetFieldElement(FileTest *t, BN_ULONG out[P256_LIMBS],
  ...
  // in little-endian.
  AWSLC_memset(out, 0, P256_LIMBS * sizeof(BN_ULONG));
...
static testing::AssertionResult ExpectFieldElementsEqual(
  ...
  if (AWSLC_memcmp(expected, actual, sizeof(BN_ULONG) * P256_LIMBS) == 0) {
...
static bool PointToAffine(P256_POINT_AFFINE *out, const P256_POINT *in) {
  ...
  awslc::UniquePtr<BIGNUM> x(BN_new()), y(BN_new()), z(BN_new());
  ...
  if (BN_is_zero(z.get())) {
    ...
    AWSLC_memset(out, 0, sizeof(P256_POINT_AFFINE));
    return true;
  }
...
static testing::AssertionResult ExpectPointsEqual(
  ...
  if (AWSLC_memcmp(expected, &affine, sizeof(P256_POINT_AFFINE)) != 0) {
    return testing::AssertionFailure()
...
static void TestMulMont(FileTest *t) {
  ...
  if (AWSLC_memcmp(a, b, sizeof(a)) == 0) {
...
static void TestPointAdd(FileTest *t) {
  ...
  AWSLC_memset(&infinity, 0, sizeof(infinity));
  ...
  // point at infinity.
  if (AWSLC_memcmp(&a_affine, &b_affine, sizeof(a_affine)) != 0 ||
      AWSLC_memcmp(&a_affine, &infinity, sizeof(a_affine)) == 0) {
...
TEST(P256_X86_64Test, TestVectors) {
  return FileTestGTest("math/p256-x86_64_tests.txt",
                       [](FileTest *t) {
...
TEST(P256_X86_64Test, ABI) {
  ...
  AWSLC_memcpy(&p, &kA, sizeof(P256_POINT));
