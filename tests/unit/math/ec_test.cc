...
#include "internal/fips/ec_internal.h"
...
static awslc::UniquePtr<EC_KEY> DecodeECPrivateKey(const uint8_t *in,
                                                  size_t in_len) {
  CBS cbs;
  CBS_init(&cbs, in, in_len);
  awslc::UniquePtr<EC_KEY> ret(EC_KEY_parse_private_key(&cbs, NULL));
...
TEST(ECTest, Encoding) {
  ...
  ASSERT_TRUE(x);
  ASSERT_TRUE(y);
  ASSERT_TRUE(EC_POINT_get_affine_coordinates(
      EC_KEY_get0_group(key.get()), pub_key, x.get(), y.get(), NULL));
  awslc::UniquePtr<char> x_hex(BN_bn2hex(x.get()));
  awslc::UniquePtr<char> y_hex(BN_bn2hex(y.get()));
  ASSERT_TRUE(x_hex);
  ASSERT_TRUE(y_hex);
...
TEST(ECTest, ArbitraryCurve) {
  ...
  awslc::UniquePtr<EC_KEY> key(EC_KEY_new_by_curve_name(NID_X9_62_prime256v1));
  ASSERT_TRUE(key);
  ...
  awslc::UniquePtr<BN_CTX> ctx(BN_CTX_new());
  ASSERT_TRUE(ctx);
  awslc::UniquePtr<BIGNUM> p(BN_bin2bn(kP, sizeof(kP), nullptr));
  ASSERT_TRUE(p);
  awslc::UniquePtr<BIGNUM> a(BN_bin2bn(kA, sizeof(kA), nullptr));
  ASSERT_TRUE(a);
  awslc::UniquePtr<BIGNUM> b(BN_bin2bn(kB, sizeof(kB), nullptr));
  ASSERT_TRUE(b);
  awslc::UniquePtr<BIGNUM> gx(BN_bin2bn(kX, sizeof(kX), nullptr));
  ASSERT_TRUE(gx);
  awslc::UniquePtr<BIGNUM> gy(BN_bin2bn(kY, sizeof(kY), nullptr));
  ASSERT_TRUE(gy);
  awslc::UniquePtr<BIGNUM> order(BN_bin2bn(kOrder, sizeof(kOrder), nullptr));
  ASSERT_TRUE(order);

  awslc::UniquePtr<EC_GROUP> group(
      EC_GROUP_new_curve_GFp(p.get(), a.get(), b.get(), ctx.get()));
  ASSERT_TRUE(group);
  awslc::UniquePtr<EC_POINT> generator(EC_POINT_new(group.get()));
  ASSERT_TRUE(generator);
  ASSERT_TRUE(EC_POINT_set_affine_coordinates(
  ...
  awslc::UniquePtr<EC_KEY> key2(EC_KEY_new());
  ASSERT_TRUE(key2);
  awslc::UniquePtr<EC_POINT> point(EC_POINT_new(group.get()));
  ASSERT_TRUE(point);
  awslc::UniquePtr<BIGNUM> x(BN_new()), y(BN_new());
  ASSERT_TRUE(x);
  ...
  ASSERT_TRUE(EC_POINT_get_affine_coordinates(
      ...
      y.get(), nullptr));
  ASSERT_TRUE(EC_POINT_set_affine_coordinates(group.get(), point.get(),
                                                  x.get(), y.get(), nullptr));
  ...
  awslc::UniquePtr<EC_GROUP> group2(
      EC_GROUP_new_curve_GFp(p.get(), a.get(), b.get(), ctx.get()));
  ASSERT_TRUE(group2);
  awslc::UniquePtr<EC_POINT> generator2(EC_POINT_new(group2.get()));
  ...
  awslc::UniquePtr<EC_GROUP> group3(
      EC_GROUP_new_curve_GFp(p.get(), a.get(), b.get(), ctx.get()));
  ASSERT_TRUE(group3);
  awslc::UniquePtr<EC_POINT> generator3(EC_POINT_new(group3.get()));
  ...
  awslc::UniquePtr<EC_GROUP> group4(
      EC_GROUP_new_curve_GFp(p.get(), a.get(), b.get(), ctx.get()));
  ASSERT_TRUE(group4);
  awslc::UniquePtr<EC_POINT> generator4(EC_POINT_new(group4.get()));
...
TEST(ECTest, SetKeyWithoutGroup) {
  awslc::UniquePtr<EC_KEY> key(EC_KEY_new());
  ASSERT_TRUE(key);
...
TEST(ECTest, GroupMismatch) {
  awslc::UniquePtr<EC_KEY> key(EC_KEY_new_by_curve_name(NID_secp384r1));
  ASSERT_TRUE(key);
  awslc::UniquePtr<EC_GROUP> p256(
      EC_GROUP_new_by_curve_name(NID_X9_62_prime256v1));
  ASSERT_TRUE(p256);
...
static awslc::UniquePtr<BIGNUM> HexToBIGNUM(const char *hex) {
  BIGNUM *bn = nullptr;
  BN_hex2bn(&bn, hex);
  return awslc::UniquePtr<BIGNUM>(bn);
}
...
TEST(ECTest, BrainpoolP256r1) {
  ...
  awslc::UniquePtr<BIGNUM> p = HexToBIGNUM(kP), a = HexToBIGNUM(kA),
  ...
  ASSERT_TRUE(group);
  awslc::UniquePtr<EC_POINT> g(EC_POINT_new(group.get()));
  ASSERT_TRUE(g);
  ASSERT_TRUE(EC_POINT_set_affine_coordinates(group.get(), g.get(), x.get(),
                                                  y.get(), nullptr));
  ASSERT_TRUE(
      EC_GROUP_set_generator(group.get(), g.get(), n.get(), BN_value_one()));

  awslc::UniquePtr<EC_POINT> q(EC_POINT_new(group.get()));
  ASSERT_TRUE(q);
  ASSERT_TRUE(
      EC_POINT_mul(group.get(), q.get(), d.get(), nullptr, nullptr, nullptr));
  ASSERT_TRUE(EC_POINT_get_affine_coordinates(group.get(), q.get(), x.get(),
                                                  y.get(), nullptr));
...
class ECCurveTest : public testing::TestWithParam<EC_builtin_curve> {
 ...
 private:
  awslc::UniquePtr<EC_GROUP> group_;
};

TEST_P(ECCurveTest, SetAffine) {
  // Generate an EC_KEY.
  awslc::UniquePtr<EC_KEY> key(EC_KEY_new_by_curve_name(GetParam().nid));
  ASSERT_TRUE(key);
  ASSERT_TRUE(EC_KEY_generate_key(key.get()));
  ...
  ASSERT_TRUE(p);
  EXPECT_TRUE(EC_POINT_get_affine_coordinates(
      group(), EC_KEY_get0_public_key(key.get()), x.get(), y.get(), nullptr));
  EXPECT_TRUE(
      EC_GROUP_get_curve(group(), p.get(), nullptr, nullptr, nullptr));

  // Points on the curve should be accepted.
  auto point = awslc::UniquePtr<EC_POINT>(EC_POINT_new(group()));
  ASSERT_TRUE(point);
  EXPECT_TRUE(EC_POINT_set_affine_coordinates(group(), point.get(), x.get(),
                                                  y.get(), nullptr));
  ...
  awslc::UniquePtr<EC_POINT> invalid_point(EC_POINT_new(group()));
  ASSERT_TRUE(invalid_point);
  EXPECT_FALSE(EC_POINT_set_affine_coordinates(group(), invalid_point.get(),
                                                   x.get(), y.get(), nullptr));
...
TEST_P(ECCurveTest, MulZero) {
  awslc::UniquePtr<EC_POINT> point(EC_POINT_new(group()));
  ASSERT_TRUE(point);
  ...
  awslc::UniquePtr<EC_POINT> generator(EC_POINT_new(group()));
  ASSERT_TRUE(generator);
...
TEST_P(ECCurveTest, MulOutOfRange) {
  awslc::UniquePtr<BIGNUM> n_minus_one(BN_dup(EC_GROUP_get0_order(group())));
  ASSERT_TRUE(n_minus_one);
  ASSERT_TRUE(BN_sub_word(n_minus_one.get(), 1));

  awslc::UniquePtr<BIGNUM> minus_one(BN_new());
  ...
  awslc::UniquePtr<BIGNUM> ten_n_plus_seven(
  ...
  awslc::UniquePtr<EC_POINT> point1(EC_POINT_new(group())),
...
TEST_P(ECCurveTest, Mul) {
  awslc::UniquePtr<EC_POINT> p(EC_POINT_new(group()));
  ASSERT_TRUE(p);
  awslc::UniquePtr<EC_POINT> result(EC_POINT_new(group()));
  ASSERT_TRUE(result);
...
TEST_P(ECCurveTest, MulNonMinimal) {
  ...
  awslc::UniquePtr<EC_POINT> point2(EC_POINT_new(group()));
  ASSERT_TRUE(point2);
...
TEST_P(ECCurveTest, DoubleSpecialCase) {
  ...
  awslc::UniquePtr<EC_POINT> two_g(EC_POINT_new(group()));
  ASSERT_TRUE(two_g);
...
TEST_P(ECCurveTest, P224Bug) {
  // P = -G
  const EC_POINT *g = EC_GROUP_get0_generator(group());
  awslc::UniquePtr<EC_POINT> p(EC_POINT_dup(g, group()));
  ASSERT_TRUE(p);
  ASSERT_TRUE(EC_POINT_invert(group(), p.get(), nullptr));

  // Compute 31 * P + 32 * G = G
  awslc::UniquePtr<EC_POINT> ret(EC_POINT_new(group()));
  ASSERT_TRUE(ret);
  awslc::UniquePtr<BIGNUM> bn31(BN_new()), bn32(BN_new());
  ASSERT_TRUE(bn31);
  ASSERT_TRUE(bn32);
...
TEST_P(ECCurveTest, GPlusMinusG) {
  ...
  ASSERT_TRUE(p);
  ASSERT_TRUE(EC_POINT_invert(group(), p.get(), nullptr));
  awslc::UniquePtr<EC_POINT> sum(EC_POINT_new(group()));
...
static awslc::UniquePtr<EC_GROUP> GetCurve(FileTest *t, const char *key) {
  ...
  if (curve_name == "P-224") {
    return awslc::UniquePtr<EC_GROUP>(EC_GROUP_new_by_curve_name(NID_secp224r1));
  }
  if (curve_name == "P-256") {
    return awslc::UniquePtr<EC_GROUP>(EC_GROUP_new_by_curve_name(
        NID_X9_62_prime256v1));
  }
  if (curve_name == "P-384") {
    return awslc::UniquePtr<EC_GROUP>(EC_GROUP_new_by_curve_name(NID_secp384r1));
  }
  if (curve_name == "P-521") {
    return awslc::UniquePtr<EC_GROUP>(EC_GROUP_new_by_curve_name(NID_secp521r1));
  }
...
static awslc::UniquePtr<BIGNUM> GetBIGNUM(FileTest *t, const char *key) {
  std::vector<uint8_t> bytes;
  if (!t->GetBytes(&bytes, key)) {
    return nullptr;
  }

  return awslc::UniquePtr<BIGNUM>(
...
TEST(ECTest, ScalarBaseMultVectors) {
  ...
  FileTestGTest("math/ec_scalar_base_mult_tests.txt",
                [&](FileTest *t) {
    awslc::UniquePtr<EC_GROUP> group = GetCurve(t, "Curve");
    ASSERT_TRUE(group);
    awslc::UniquePtr<BIGNUM> n = GetBIGNUM(t, "N");
    ASSERT_TRUE(n);
    awslc::UniquePtr<BIGNUM> x = GetBIGNUM(t, "X");
    ASSERT_TRUE(x);
    awslc::UniquePtr<BIGNUM> y = GetBIGNUM(t, "Y");
    ASSERT_TRUE(y);
    bool is_infinity = BN_is_zero(x.get()) && BN_is_zero(y.get());

    awslc::UniquePtr<BIGNUM> px(BN_new());
    ASSERT_TRUE(px);
    awslc::UniquePtr<BIGNUM> py(BN_new());
    ASSERT_TRUE(py);
    ...
    awslc::UniquePtr<EC_POINT> p(EC_POINT_new(group.get()));
    ASSERT_TRUE(p);
    ...
    awslc::UniquePtr<BIGNUM> a(BN_new()), b(BN_new());
    for (int i = -64; i < 64; i++) {
      SCOPED_TRACE(i);
...
TEST(ECTest, DeriveFromSecret) {
  ...
  for (const auto &test : kDeriveTests) {
    ...
    awslc::UniquePtr<EC_GROUP> group(EC_GROUP_new_by_curve_name(test.curve));
    ASSERT_TRUE(group);
    awslc::UniquePtr<EC_KEY> key(EC_KEY_derive_from_secret(
    ...
    awslc::UniquePtr<uint8_t> free_pub(pub);
