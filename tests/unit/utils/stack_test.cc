...
static void TEST_INT_free(TEST_INT *x) { AWSLC_free(x); }

AWSLC_NAMESPACE_BEGIN
AWSLC_MAKE_DELETER(TEST_INT, TEST_INT_free)
AWSLC_NAMESPACE_END

static awslc::UniquePtr<TEST_INT> TEST_INT_new(int x) {
  awslc::UniquePtr<TEST_INT> ret(
      static_cast<TEST_INT *>(AWSLC_malloc(sizeof(TEST_INT))));
  if (!ret) {
    return nullptr;
  }
  *ret = x;
...
TEST(StackTest, Basic) {
  awslc::UniquePtr<STACK_OF(TEST_INT)> sk(sk_TEST_INT_new_null());
  ASSERT_TRUE(sk);
  ...
  for (int i = 0; i < 6; i++) {
    ...
    ASSERT_TRUE(value);
    ASSERT_TRUE(awslc::PushToStack(sk.get(), std::move(value)));
  }
  ...
  awslc::UniquePtr<TEST_INT> removed(sk_TEST_INT_pop(sk.get()));
  EXPECT_EQ(8, *removed);
  ExpectStackEquals(sk.get(), {0, 1, 2, 3, 6, 4, 5, 7});
  ...
  awslc::UniquePtr<STACK_OF(TEST_INT)> copy(sk_TEST_INT_deep_copy(
      sk.get(),
      [](TEST_INT *x) -> TEST_INT * {
...
TEST(StackTest, Sorted) {
  ...
  do {
    awslc::UniquePtr<STACK_OF(TEST_INT)> sk(sk_TEST_INT_new(compare));
    ASSERT_TRUE(sk);
    for (int v : vec) {
    ...
    ASSERT_TRUE(tmp);
    ASSERT_TRUE(awslc::PushToStack(sk.get(), std::move(tmp)));
    EXPECT_FALSE(sk_TEST_INT_is_sorted(sk.get()));
