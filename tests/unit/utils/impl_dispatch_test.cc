...
#if defined(AWSLC_DISPATCH_TEST) && !defined(AWSLC_SHARED_LIBRARY)
...
class ImplDispatchTest : public ::testing::Test {
 public:
  void SetUp() override {
    ...
    aesni_ = AWSLC_ia32cap_P[1] & (1 << (57 - 32));
    avx_movbe_ = ((AWSLC_ia32cap_P[1] >> 22) & 0x41) == 0x41;
    ssse3_ = AWSLC_ia32cap_P[1] & (1 << (41 - 32));
    is_x86_64_ = true;
#endif  // X86_64
  }
 ...
 protected:
  ...
  void AssertFunctionsHit(std::vector<std::pair<size_t, bool>> flags,
                          std::function<void()> f) {
    AWSLC_memset(AWSLC_function_hit, 0, sizeof(AWSLC_function_hit));
    ...
    for (const auto flag : flags) {
      ...
      ASSERT_LT(flag.first, sizeof(AWSLC_function_hit));
      EXPECT_EQ(flag.second, AWSLC_function_hit[flag.first] == 1);
      AWSLC_function_hit[flag.first] = 0;
    }

    for (size_t i = 0; i < sizeof(AWSLC_function_hit); i++) {
      EXPECT_EQ(0u, AWSLC_function_hit[i])
