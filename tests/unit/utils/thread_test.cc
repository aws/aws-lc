...
TEST(ThreadTest, ThreadLocal) {
  ASSERT_EQ(nullptr, CRYPTO_get_thread_local(AWSLC_THREAD_LOCAL_TEST))
      << "Thread-local data was non-NULL at start.";

  std::thread thread([] {
    if (CRYPTO_get_thread_local(AWSLC_THREAD_LOCAL_TEST) != NULL ||
        !CRYPTO_set_thread_local(AWSLC_THREAD_LOCAL_TEST,
                                 &g_destructor_called_count,
                                 thread_local_destructor) ||
        CRYPTO_get_thread_local(AWSLC_THREAD_LOCAL_TEST) !=
            &g_destructor_called_count) {
      return;
    }
