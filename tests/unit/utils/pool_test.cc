...
TEST(PoolTest, Unpooled) {
  ...
  awslc::UniquePtr<CRYPTO_BUFFER> buf2 = awslc::UpRef(buf);
}

TEST(PoolTest, Empty) {
  awslc::UniquePtr<CRYPTO_BUFFER> buf(CRYPTO_BUFFER_new(nullptr, 0, nullptr));
  ASSERT_TRUE(buf);
...
TEST(PoolTest, Pooled) {
  awslc::UniquePtr<CRYPTO_BUFFER_POOL> pool(CRYPTO_BUFFER_POOL_new());
  ASSERT_TRUE(pool);
  ...
  awslc::UniquePtr<CRYPTO_BUFFER> buf2(
      CRYPTO_BUFFER_new(kData, sizeof(kData), pool.get()));
  ASSERT_TRUE(buf2);
...
TEST(PoolTest, Threads) {
  ...
  awslc::UniquePtr<CRYPTO_BUFFER> buf, buf2, buf3;
  {
    std::thread thread([&] {
  ...
  // increment and free.
  {
    awslc::UniquePtr<CRYPTO_BUFFER> buf_ref;
    std::thread thread([&] { buf_ref = awslc::UpRef(buf); });
    buf2.reset();
    thread.join();
  }
