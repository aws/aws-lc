...
TEST(X25519Test, SmallOrder) {
  ...
  AWSLC_memset(private_key, 0x11, sizeof(private_key));

  AWSLC_memset(out, 0xff, sizeof(out));
...
TEST(X25519Test, Iterated) {
  ...
  for (unsigned i = 0; i < 1000; i++) {
    ...
    AWSLC_memcpy(point, scalar, sizeof(point));
    AWSLC_memcpy(scalar, out, sizeof(scalar));
  }
