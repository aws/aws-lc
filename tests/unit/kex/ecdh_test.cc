...
TEST(ECDHTest, TestVectors) {
  FileTestGTest("kex/ecdh_tests.txt", [](FileTest *t) {
    ...
    ASSERT_TRUE(y);
    awslc::UniquePtr<BIGNUM> peer_x = GetBIGNUM(t, "PeerX");
    ASSERT_TRUE(peer_x);
    awslc::UniquePtr<BIGNUM> peer_y = GetBIGNUM(t, "PeerY");
    ASSERT_TRUE(peer_y);
    ...
    ASSERT_TRUE(pub_key);
    awslc::UniquePtr<EC_POINT> peer_pub_key(EC_POINT_new(group.get()));
    ASSERT_TRUE(peer_pub_key);
    ASSERT_TRUE(EC_KEY_set_group(key.get(), group.get()));
    ASSERT_TRUE(EC_KEY_set_private_key(key.get(), priv_key.get()));
    ASSERT_TRUE(EC_POINT_set_affine_coordinates(group.get(), pub_key.get(),
                                                    x.get(), y.get(), nullptr));
...
static void RunWycheproofTest(FileTest *t) {
  ...
  awslc::UniquePtr<EC_GROUP> group = GetWycheproofCurve(t, "curve", true);
  ASSERT_TRUE(group);
  awslc::UniquePtr<BIGNUM> priv_key = GetWycheproofBIGNUM(t, "private", false);
  ASSERT_TRUE(priv_key);
  ...
  awslc::UniquePtr<EVP_PKEY> peer_evp(EVP_parse_public_key(&cbs));
...
static awslc::UniquePtr<EC_GROUP> MakeCustomGroup() {
  ...
  awslc::UniquePtr<BIGNUM> x(BN_bin2bn(kX, sizeof(kX), nullptr));
  awslc::UniquePtr<BIGNUM> y(BN_bin2bn(kY, sizeof(kY), nullptr));
  ...
  if (!generator ||
      !EC_POINT_set_affine_coordinates(group.get(), generator.get(),
                                           x.get(), y.get(), ctx.get()) ||
...
TEST(ECDHTest, GroupMismatch) {
  ...
  std::vector<awslc::UniquePtr<EC_GROUP>> groups;
