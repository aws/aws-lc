...
TEST(BIOTest, SocketConnect) {
  ...
  awslc::UniquePtr<BIO> bio(BIO_new_connect(hostname));
  ASSERT_TRUE(bio);
...
TEST_P(BIOASN1Test, ReadASN1) {
  ...
  awslc::UniquePtr<BIO> bio(BIO_new_mem_buf(input.data(), input.size()));
  ASSERT_TRUE(bio);
  ...
  awslc::UniquePtr<uint8_t> out_storage(out);
...
TEST_P(BIOPairTest, TestPair) {
  ...
  awslc::UniquePtr<BIO> free_bio1(bio1), free_bio2(bio2);
