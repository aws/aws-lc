...
#include "internal/err_internal.h"
...
TEST(ErrTest, PutMacro) {
  ...
  AWSLC_PUT_ERROR(USER, ERR_R_INTERNAL_ERROR);
...
TEST(ErrTest, SaveAndRestore) {
  ...
  awslc::UniquePtr<ERR_SAVE_STATE> saved(ERR_save_state());
  ASSERT_TRUE(saved);
...
TEST(ErrTest, String) {
  ...
  EXPECT_STREQ(
      "error:0e000044:common libcrypto routines:AWSLC_internal:internal "
      "error",
  ...
  EXPECT_STREQ(
      "error:0e000044:common libcrypto routines:AWSLC_internal:internal "
      "error",
      ERR_error_string_n(err, buf, 71));

  // If the buffer is too short, the string is truncated.
  EXPECT_STREQ(
      "error:0e000044:common libcrypto routines:AWSLC_internal:internal "
      "erro",
      ERR_error_string_n(err, buf, 70));
  EXPECT_STREQ("error:0e000044:common libcrypto routines:AWSLC_internal:",
               ERR_error_string_n(err, buf, 57));

  // Truncated log lines always have the right number of colons.
  EXPECT_STREQ("error:0e000044:common libcrypto routines:AWSLC_interna:",
               ERR_error_string_n(err, buf, 56));
  EXPECT_STREQ("error:0e000044:common libcrypto routines:AWSLC_intern:",
               ERR_error_string_n(err, buf, 55));
