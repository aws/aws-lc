...
static awslc::UniquePtr<EVP_PKEY> LoadExampleRSAKey() {
  awslc::UniquePtr<RSA> rsa(RSA_private_key_from_bytes(kExampleRSAKeyDER,
                                           sizeof(kExampleRSAKeyDER)));
  if (!rsa) {
    return nullptr;
  }
  awslc::UniquePtr<EVP_PKEY> pkey(EVP_PKEY_new());
...
TEST(EVPExtraTest, DigestSignInit) {
  awslc::UniquePtr<EVP_PKEY> pkey = LoadExampleRSAKey();
  ASSERT_TRUE(pkey);
...
TEST(EVPExtraTest, VerifyRecover) {
  awslc::UniquePtr<EVP_PKEY> pkey = LoadExampleRSAKey();
  ASSERT_TRUE(pkey);
  awslc::UniquePtr<RSA> rsa(EVP_PKEY_get1_RSA(pkey.get()));
  ASSERT_TRUE(rsa);
  ...
  size_t out_len;
  awslc::UniquePtr<EVP_PKEY_CTX> ctx(EVP_PKEY_CTX_new(pkey.get(), nullptr));
...
static void TestValidPrivateKey(const uint8_t *input, size_t input_len,
  ...
  awslc::UniquePtr<EVP_PKEY> pkey(d2i_AutoPrivateKey(NULL, &p, input_len));
  ASSERT_TRUE(pkey);
...
TEST(EVPExtraTest, BadECKey) {
  ...
  awslc::UniquePtr<PKCS8_PRIV_KEY_INFO> p8inf(
  ...
  awslc::UniquePtr<EVP_PKEY> pkey(EVP_PKCS82PKEY(p8inf.get()));
...
TEST(EVPExtraTest, MarshalEmptyPublicKey) {
  awslc::UniquePtr<EVP_PKEY> empty(EVP_PKEY_new());
  ASSERT_TRUE(empty);
...
static awslc::UniquePtr<EVP_PKEY> ParsePrivateKey(int type, const uint8_t *in,
                                                 size_t len) {
  const uint8_t *ptr = in;
  awslc::UniquePtr<EVP_PKEY> pkey(d2i_PrivateKey(type, nullptr, &ptr, len));
  if (!pkey) {
    return nullptr;
  }
...
TEST(EVPExtraTest, Ed25519) {
  ...
  // Create a public key.
  awslc::UniquePtr<EVP_PKEY> pubkey(EVP_PKEY_new_raw_public_key(
  ...
  awslc::UniquePtr<EVP_PKEY> privkey(EVP_PKEY_new_raw_private_key(
  ...
  awslc::UniquePtr<EVP_PKEY> pubkey2(EVP_PKEY_new_raw_public_key(
...
TEST(EVPExtraTest, ECKeygen) {
  ...
  // |EC_GROUP|.
  awslc::UniquePtr<EVP_PKEY_CTX> ctx(EVP_PKEY_CTX_new_id(EVP_PKEY_EC, nullptr));
  ASSERT_TRUE(ctx);
  ...
  awslc::UniquePtr<EVP_PKEY> pkey(raw);
  raw = nullptr;
...
TEST(EVPExtraTest, Ed25519Keygen) {
  awslc::UniquePtr<EVP_PKEY_CTX> pctx(
