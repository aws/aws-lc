...
#include <awslc/bio.h>
#include <awslc/bytestring.h>
#include <awslc/crypto.h>
#include <awslc/curve25519.h>
#include <awslc/digest.h>
#include <awslc/err.h>
#include <awslc/pem.h>
#include <awslc/pool.h>
#include <awslc/x509.h>
#include <awslc/x509v3.h>

#include "internal/crypto_internal.h"
#include "internal/x509v3_internal.h"
#include "test_util.h"
...
// |X509*|.
static awslc::UniquePtr<X509> CertFromPEM(const char *pem) {
  awslc::UniquePtr<BIO> bio(BIO_new_mem_buf(pem, strlen(pem)));
  return awslc::UniquePtr<X509>(
      PEM_read_bio_X509(bio.get(), nullptr, nullptr, nullptr));
}

// CRLFromPEM parses the given, NUL-terminated pem block and returns an
// |X509_CRL*|.
static awslc::UniquePtr<X509_CRL> CRLFromPEM(const char *pem) {
  awslc::UniquePtr<BIO> bio(BIO_new_mem_buf(pem, strlen(pem)));
  return awslc::UniquePtr<X509_CRL>(
...
// |EVP_PKEY*|.
static awslc::UniquePtr<EVP_PKEY> PrivateKeyFromPEM(const char *pem) {
  awslc::UniquePtr<BIO> bio(
      BIO_new_mem_buf(const_cast<char *>(pem), strlen(pem)));
  return awslc::UniquePtr<EVP_PKEY>(
...
static awslc::UniquePtr<STACK_OF(X509)> CertsToStack(
    const std::vector<X509 *> &certs) {
  awslc::UniquePtr<STACK_OF(X509)> stack(sk_X509_new_null());
  if (!stack) {
    return nullptr;
  }
  for (auto cert : certs) {
    if (!awslc::PushToStack(stack.get(), awslc::UpRef(cert))) {
      return nullptr;
    }
  }
...
static awslc::UniquePtr<STACK_OF(X509_CRL)> CRLsToStack(
    const std::vector<X509_CRL *> &crls) {
  awslc::UniquePtr<STACK_OF(X509_CRL)> stack(sk_X509_CRL_new_null());
  if (!stack) {
    return nullptr;
  }
  for (auto crl : crls) {
    if (!awslc::PushToStack(stack.get(), awslc::UpRef(crl))) {
      return nullptr;
    }
  }
...
static int Verify(X509 *leaf, const std::vector<X509 *> &roots,
  ...
  awslc::UniquePtr<STACK_OF(X509)> roots_stack(CertsToStack(roots));
  awslc::UniquePtr<STACK_OF(X509)> intermediates_stack(
      CertsToStack(intermediates));
  awslc::UniquePtr<STACK_OF(X509_CRL)> crls_stack(CRLsToStack(crls));
  ...
  awslc::UniquePtr<X509_STORE_CTX> ctx(X509_STORE_CTX_new());
  awslc::UniquePtr<X509_STORE> store(X509_STORE_new());
  if (!ctx ||
      !store) {
    return X509_V_ERR_UNSPECIFIED;
  }
...
TEST(X509Test, TestVerify) {
  awslc::UniquePtr<X509> cross_signing_root(CertFromPEM(kCrossSigningRootPEM));
  awslc::UniquePtr<X509> root(CertFromPEM(kRootCAPEM));
  awslc::UniquePtr<X509> root_cross_signed(CertFromPEM(kRootCrossSignedPEM));
  awslc::UniquePtr<X509> intermediate(CertFromPEM(kIntermediatePEM));
  awslc::UniquePtr<X509> intermediate_self_signed(
      CertFromPEM(kIntermediateSelfSignedPEM));
  awslc::UniquePtr<X509> leaf(CertFromPEM(kLeafPEM));
  awslc::UniquePtr<X509> leaf_no_key_usage(CertFromPEM(kLeafNoKeyUsagePEM));
  awslc::UniquePtr<X509> forgery(CertFromPEM(kForgeryPEM));
...
TEST(X509Test, ZeroLengthsWithX509PARAM) {
  awslc::UniquePtr<X509> leaf(CertFromPEM(kSANTypesLeaf));
  awslc::UniquePtr<X509> root(CertFromPEM(kSANTypesRoot));
  ASSERT_TRUE(leaf);
  ASSERT_TRUE(root);
...
TEST(X509Test, TestCRL) {
  awslc::UniquePtr<X509> root(CertFromPEM(kCRLTestRoot));
  awslc::UniquePtr<X509> leaf(CertFromPEM(kCRLTestLeaf));
  awslc::UniquePtr<X509_CRL> basic_crl(CRLFromPEM(kBasicCRL));
  awslc::UniquePtr<X509_CRL> revoked_crl(CRLFromPEM(kRevokedCRL));
  awslc::UniquePtr<X509_CRL> bad_issuer_crl(CRLFromPEM(kBadIssuerCRL));
  awslc::UniquePtr<X509_CRL> known_critical_crl(CRLFromPEM(kKnownCriticalCRL));
  awslc::UniquePtr<X509_CRL> unknown_critical_crl(
      CRLFromPEM(kUnknownCriticalCRL));
  awslc::UniquePtr<X509_CRL> unknown_critical_crl2(
...
TEST(X509Test, ManyNamesAndConstraints) {
  awslc::UniquePtr<X509> many_constraints(
      CertFromPEM(GetTestData("certs/many_constraints.pem").c_str()));
  ASSERT_TRUE(many_constraints);
  awslc::UniquePtr<X509> many_names1(
      CertFromPEM(GetTestData("certs/many_names1.pem").c_str()));
  ASSERT_TRUE(many_names1);
  awslc::UniquePtr<X509> many_names2(
      CertFromPEM(GetTestData("certs/many_names2.pem").c_str()));
  ASSERT_TRUE(many_names2);
  awslc::UniquePtr<X509> many_names3(
      CertFromPEM(GetTestData("certs/many_names3.pem").c_str()));
  ASSERT_TRUE(many_names3);
  awslc::UniquePtr<X509> some_names1(
      CertFromPEM(GetTestData("certs/some_names1.pem").c_str()));
  ASSERT_TRUE(some_names1);
  awslc::UniquePtr<X509> some_names2(
      CertFromPEM(GetTestData("certs/some_names2.pem").c_str()));
  ASSERT_TRUE(some_names2);
  awslc::UniquePtr<X509> some_names3(
      CertFromPEM(GetTestData("certs/some_names3.pem").c_str()));
...
TEST(X509Test, TestPSS) {
  awslc::UniquePtr<X509> cert(CertFromPEM(kExamplePSSCert));
  ASSERT_TRUE(cert);

  awslc::UniquePtr<EVP_PKEY> pkey(X509_get_pubkey(cert.get()));
  ASSERT_TRUE(pkey);
...
TEST(X509Test, TestPSSBadParameters) {
  awslc::UniquePtr<X509> cert(CertFromPEM(kBadPSSCertPEM));
  ASSERT_TRUE(cert);
...
TEST(X509Test, TestEd25519) {
  awslc::UniquePtr<X509> cert(CertFromPEM(kEd25519Cert));
  ASSERT_TRUE(cert);
...
TEST(X509Test, TestEd25519BadParameters) {
  awslc::UniquePtr<X509> cert(CertFromPEM(kEd25519CertNull));
  ASSERT_TRUE(cert);
...
static bool SignatureRoundTrips(EVP_MD_CTX *md_ctx, EVP_PKEY *pkey) {
  ...
  awslc::UniquePtr<X509> cert(CertFromPEM(kLeafPEM));
...
TEST(X509Test, RSASign) {
  awslc::UniquePtr<EVP_PKEY> pkey(PrivateKeyFromPEM(kRSAKey));
  ASSERT_TRUE(pkey);
  // Test PKCS#1 v1.5.
  awslc::ScopedEVP_MD_CTX md_ctx;
  ASSERT_TRUE(
...
static bool PEMToDER(awslc::UniquePtr<uint8_t> *out, size_t *out_len,
                     const char *pem) {
  awslc::UniquePtr<BIO> bio(BIO_new_mem_buf(pem, strlen(pem)));
  if (!bio) {
    return false;
...
TEST(X509Test, TestFromBuffer) {
  size_t data_len;
  awslc::UniquePtr<uint8_t> data;
  ASSERT_TRUE(PEMToDER(&data, &data_len, kRootCAPEM));

  awslc::UniquePtr<CRYPTO_BUFFER> buf(
      CRYPTO_BUFFER_new(data.get(), data_len, nullptr));
  ASSERT_TRUE(buf);
  awslc::UniquePtr<X509> root(X509_parse_from_buffer(buf.get()));
  ASSERT_TRUE(root);
...
TEST(X509Test, TestFromBufferWithTrailingData) {
  ...
  AWSLC_memcpy(trailing_data.get(), data.get(), data_len);

  awslc::UniquePtr<CRYPTO_BUFFER> buf_trailing_data(
      CRYPTO_BUFFER_new(trailing_data.get(), data_len + 1, nullptr));
  ASSERT_TRUE(buf_trailing_data);

  awslc::UniquePtr<X509> root_trailing_data(
...
TEST(X509Test, TestFromBufferModified) {
  ...
  awslc::UniquePtr<ASN1_INTEGER> fourty_two(ASN1_INTEGER_new());
...
TEST(X509Test, TestFromBufferReused) {
  ...
  ASSERT_GE(i2d_len, 0);
  awslc::UniquePtr<uint8_t> i2d_storage(i2d);

  ASSERT_TRUE(PEMToDER(&data2, &data2_len, kLeafPEM));

  ASSERT_EQ(static_cast<long>(data2_len), i2d_len);
  ASSERT_EQ(0, AWSLC_memcmp(data2.get(), i2d, i2d_len));
  ASSERT_EQ(nullptr, root->buf);
}
...
TEST(X509Test, TestFailedParseFromBuffer) {
  ...
  awslc::UniquePtr<X509> cert(X509_parse_from_buffer(buf.get()));
  ASSERT_FALSE(cert);
  ERR_clear_error();
  ...
  AWSLC_memcpy(data_with_trailing_byte.get(), data.get(), data_len);
  data_with_trailing_byte[data_len] = 0;

  awslc::UniquePtr<CRYPTO_BUFFER> buf_with_trailing_byte(
      CRYPTO_BUFFER_new(data_with_trailing_byte.get(), data_len + 1, nullptr));
  ASSERT_TRUE(buf_with_trailing_byte);

  awslc::UniquePtr<X509> root(
...
TEST(X509Test, TestPrintUTCTIME) {
  ...
  for (auto t : asn1_utctime_tests) {
    SCOPED_TRACE(t.val);
    awslc::UniquePtr<ASN1_UTCTIME> tm(ASN1_UTCTIME_new());
    awslc::UniquePtr<BIO> bio(BIO_new(BIO_s_mem()));
...
TEST(X509Test, PrettyPrintIntegers) {
  ...
  for (const char *in : kTests) {
    ...
    awslc::UniquePtr<BIGNUM> free_bn(bn);

    {
      awslc::UniquePtr<ASN1_INTEGER> asn1(BN_to_ASN1_INTEGER(bn, nullptr));
      ASSERT_TRUE(asn1);
      awslc::UniquePtr<char> out(i2s_ASN1_INTEGER(nullptr, asn1.get()));
      ASSERT_TRUE(out.get());
      EXPECT_STREQ(in, out.get());
    }

    {
      awslc::UniquePtr<ASN1_ENUMERATED> asn1(BN_to_ASN1_ENUMERATED(bn, nullptr));
      ASSERT_TRUE(asn1);
      awslc::UniquePtr<char> out(i2s_ASN1_ENUMERATED(nullptr, asn1.get()));
      ASSERT_TRUE(out.get());
      EXPECT_STREQ(in, out.get());
    }
  }
...
TEST(X509Test, X509NameSet) {
  awslc::UniquePtr<X509_NAME> name(X509_NAME_new());
...
TEST(X509Test, StringDecoding) {
  ...
  for (size_t i = 0; i < AWSLC_ARRAY_SIZE(kTests); i++) {
    SCOPED_TRACE(i);
...
TEST(X509Test, NoBasicConstraintsCertSign) {
  awslc::UniquePtr<X509> root(CertFromPEM(kSANTypesRoot));
  awslc::UniquePtr<X509> intermediate(
      CertFromPEM(kNoBasicConstraintsCertSignIntermediate));
  awslc::UniquePtr<X509> leaf(CertFromPEM(kNoBasicConstraintsCertSignLeaf));
...
TEST(X509Test, NoBasicConstraintsNetscapeCA) {
  ...
  awslc::UniquePtr<X509> leaf(CertFromPEM(kNoBasicConstraintsNetscapeCALeaf));
...
TEST(X509Test, MismatchAlgorithms) {
  awslc::UniquePtr<X509> cert(CertFromPEM(kSelfSignedMismatchAlgorithms));
  ASSERT_TRUE(cert);
...
TEST(X509Test, PEMX509Info) {
  ...
  awslc::UniquePtr<BIO> bio(BIO_new_mem_buf(pem.data(), pem.size()));
  ASSERT_TRUE(bio);
  awslc::UniquePtr<STACK_OF(X509_INFO)> infos(
      PEM_X509_INFO_read_bio(bio.get(), nullptr, nullptr, nullptr));
  ASSERT_TRUE(infos);
  ASSERT_EQ(AWSLC_ARRAY_SIZE(kExpected), sk_X509_INFO_num(infos.get()));
  for (size_t i = 0; i < AWSLC_ARRAY_SIZE(kExpected); i++) {
    SCOPED_TRACE(i);
  ...
  ASSERT_EQ(2 * AWSLC_ARRAY_SIZE(kExpected), sk_X509_INFO_num(infos.get()));
  for (size_t i = 0; i < AWSLC_ARRAY_SIZE(kExpected); i++) {
    SCOPED_TRACE(i);
    check_info(&kExpected[i], sk_X509_INFO_value(infos.get(), i));
    check_info(
        &kExpected[i],
        sk_X509_INFO_value(infos.get(), i + AWSLC_ARRAY_SIZE(kExpected)));
  }
  ...
  ASSERT_TRUE(bio);
  awslc::UniquePtr<STACK_OF(X509_INFO)> infos2(
      PEM_X509_INFO_read_bio(bio.get(), nullptr, nullptr, nullptr));
  EXPECT_FALSE(infos2);
  ...
  EXPECT_FALSE(
      PEM_X509_INFO_read_bio(bio.get(), infos.get(), nullptr, nullptr));
  EXPECT_EQ(2 * AWSLC_ARRAY_SIZE(kExpected), sk_X509_INFO_num(infos.get()));
}

TEST(X509Test, ReadBIOEmpty) {
  awslc::UniquePtr<BIO> bio(BIO_new_mem_buf(nullptr, 0));
  ASSERT_TRUE(bio);

  // CPython expects |ASN1_R_HEADER_TOO_LONG| on EOF, to terminate a series of
  // certificates.
  awslc::UniquePtr<X509> x509(d2i_X509_bio(bio.get(), nullptr));
  EXPECT_FALSE(x509);
  uint32_t err = ERR_get_error();
...
TEST(X509Test, ReadBIOOneByte) {
  awslc::UniquePtr<BIO> bio(BIO_new_mem_buf("\x30", 1));
  ASSERT_TRUE(bio);
...
TEST(X509Test, PartialBIOReturn) {
  ...
  awslc::UniquePtr<BIO_METHOD> method(BIO_meth_new(0, nullptr));
  ASSERT_TRUE(method);
  ...
  awslc::UniquePtr<BIO> bio(BIO_new(method.get()));
  ASSERT_TRUE(bio);
  ...
  ASSERT_GT(der_len, 0);
  awslc::UniquePtr<uint8_t> free_der(der);
  ...
  awslc::UniquePtr<X509> cert2(d2i_X509_bio(bio.get(), nullptr));
  ASSERT_TRUE(cert2);
...
TEST(X509Test, CommonNameFallback) {
  awslc::UniquePtr<X509> root = CertFromPEM(kSANTypesRoot);
  ASSERT_TRUE(root);
  awslc::UniquePtr<X509> with_sans = CertFromPEM(kCommonNameWithSANs);
  ASSERT_TRUE(with_sans);
  awslc::UniquePtr<X509> without_sans = CertFromPEM(kCommonNameWithoutSANs);
  ASSERT_TRUE(without_sans);
  awslc::UniquePtr<X509> with_email = CertFromPEM(kCommonNameWithEmailSAN);
  ASSERT_TRUE(with_email);
  awslc::UniquePtr<X509> with_ip = CertFromPEM(kCommonNameWithIPSAN);
  ASSERT_TRUE(with_ip);
...
TEST(X509Test, CommonNameAndNameConstraints) {
  awslc::UniquePtr<X509> root = CertFromPEM(kSANTypesRoot);
  ASSERT_TRUE(root);
  awslc::UniquePtr<X509> intermediate = CertFromPEM(kConstrainedIntermediate);
  ASSERT_TRUE(intermediate);
  awslc::UniquePtr<X509> permitted = CertFromPEM(kCommonNamePermittedLeaf);
  ASSERT_TRUE(permitted);
  awslc::UniquePtr<X509> not_permitted =
      CertFromPEM(kCommonNameNotPermittedLeaf);
  ASSERT_TRUE(not_permitted);
  awslc::UniquePtr<X509> not_permitted_with_sans =
      CertFromPEM(kCommonNameNotPermittedWithSANsLeaf);
  ASSERT_TRUE(not_permitted_with_sans);
  awslc::UniquePtr<X509> not_dns = CertFromPEM(kCommonNameNotDNSLeaf);
  ASSERT_TRUE(not_dns);
...
TEST(X509Test, ServerGatedCryptoEKUs) {
  awslc::UniquePtr<X509> root = CertFromPEM(kSANTypesRoot);
  ASSERT_TRUE(root);
  awslc::UniquePtr<X509> ms_sgc = CertFromPEM(kMicrosoftSGCCert);
  ASSERT_TRUE(ms_sgc);
  awslc::UniquePtr<X509> ns_sgc = CertFromPEM(kNetscapeSGCCert);
  ASSERT_TRUE(ns_sgc);
  awslc::UniquePtr<X509> server_eku = CertFromPEM(kServerEKUCert);
  ASSERT_TRUE(server_eku);
  awslc::UniquePtr<X509> server_eku_plus_ms_sgc =
      CertFromPEM(kServerEKUPlusMicrosoftSGCCert);
  ASSERT_TRUE(server_eku_plus_ms_sgc);
  awslc::UniquePtr<X509> any_eku = CertFromPEM(kAnyEKU);
  ASSERT_TRUE(any_eku);
  awslc::UniquePtr<X509> no_eku = CertFromPEM(kNoEKU);
  ASSERT_TRUE(no_eku);
