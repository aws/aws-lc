...
#if (defined(AWSLC_ARM) || defined(AWSLC_AARCH64)) &&       \
    !defined(AWSLC_STATIC_ARMCAP)
#include <awslc/arm_arch.h>
...
int main(int argc, char **argv) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (strncmp(argv[i], "--cpu=", 6) == 0) {
      ...
      uint32_t *armcap_ptr = AWSLC_get_armcap_pointer_for_test();
