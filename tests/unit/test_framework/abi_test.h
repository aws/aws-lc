...
#if defined(LOOP_CALLER_STATE_REGISTERS) && !defined(AWSLC_NO_ASM) && \
    !defined(AWSLC_SHARED_LIBRARY)
...
inline crypto_word_t ToWord(T t) {
...
#elif defined(AWSLC_X86_64) || defined(AWSLC_AARCH64)
  ...
  AWSLC_memset(&ret, 0xaa, sizeof(ret));
  AWSLC_memcpy(&ret, &t, sizeof(t));
  return ret;
#else
...
inline crypto_word_t CheckImpl(Result *out, bool unwind, R (*func)(Args...),
  ...
  // We only support up to 8 arguments, so all arguments on aarch64
