...
#if defined(AWSLC_X86_64) && defined(SUPPORTS_ABI_TEST)
#if defined(AWSLC_LINUX) && defined(AWSLC_HAVE_LIBUNWIND)
...
#include <sys/types.h>
#include <unistd.h>
#elif defined(AWSLC_WINDOWS)
#define SUPPORTS_UNWIND_TEST
AWSLC_MSVC_PRAGMA(warning(push, 3))
#include <windows.h>
#include <dbghelp.h>
...
static std::array<char, sizeof(crypto_word_t) * 2 + 1> WordToHex(
  ...
  for (int i = sizeof(crypto_word_t) - 1; i >= 0; i--) {
    uint8_t b = v & 0xff;
    v >>= 8;
    ret[i * 2] = kHex[b >> 4];
...
static void StrCatSignalSafeImpl(AWSLC_UNUSED awslc::Span<char> out) {}

template <typename... Args>
static void StrCatSignalSafeImpl(awslc::Span<char> out, const char *str,
                                 Args... args) {
  AWSLC_strlcat(out.data(), str, out.size());
  StrCatSignalSafeImpl(out, args...);
}

template <typename... Args>
static void StrCatSignalSafe(awslc::Span<char> out, Args... args) {
  if (out.empty()) {
    return;
  }
  out[0] = '\0';
...
#else
  AWSLC_UNUSED ssize_t unused_ret = 
...
#error "unknown architecture"
  ...
  const char *ToString() {
    ...
    // debugging.
    StrCatSignalSafe(awslc::Span<char>(starting_ip_buf_).subspan(len), "+",
 ...
 private:
  ...
  int InitAtSignalFrame(unw_cursor_t *cursor) {
    ...
    AWSLC_memset(cursor, 0, sizeof(*cursor));
...
static void TrapHandler(AWSLC_UNUSED int sig) {
...
static void EnableUnwindTestsImpl() {
  ...
  AWSLC_memset(&trap_action, 0, sizeof(trap_action));
