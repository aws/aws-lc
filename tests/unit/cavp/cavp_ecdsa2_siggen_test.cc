...
static bool TestECDSA2SigGenImpl(FileTest *t, bool is_component) {
  ...
  awslc::UniquePtr<BIGNUM> qx(BN_new()), qy(BN_new());
  awslc::UniquePtr<EC_KEY> key(EC_KEY_new_by_curve_name(nid));
  std::vector<uint8_t> msg;
  if (!qx || !qy || !key ||
      !EC_KEY_generate_key_fips(key.get()) ||
      !EC_POINT_get_affine_coordinates(EC_KEY_get0_group(key.get()),
  ...
  if (is_component) {
    ...
    AWSLC_memcpy(digest, msg.data(), msg.size());
...
static bool TestECDSA2SigGen(FileTest *t, AWSLC_UNUSED void *arg) {
  return TestECDSA2SigGenImpl(t, false);
}

static bool TestECDSA2SigGenComponent(FileTest *t, AWSLC_UNUSED void *arg) {
