...
static bool TestKAS(FileTest *t, void *arg) {
  ...
  awslc::UniquePtr<BIGNUM> their_x(GetBIGNUM(t, "QeCAVSx"));
  awslc::UniquePtr<BIGNUM> their_y(GetBIGNUM(t, "QeCAVSy"));
  awslc::UniquePtr<EC_KEY> ec_key(EC_KEY_new_by_curve_name(nid));
  ...
  awslc::UniquePtr<EC_POINT> their_point(EC_POINT_new(group));
  if (!their_point ||
  ...
  if (validate) {
    awslc::UniquePtr<BIGNUM> our_k(GetBIGNUM(t, "deIUT"));
    if (!our_k ||
  ...
  if (validate) {
    ...
    const bool ok =
        ...
        AWSLC_memcmp(digest, expected_shared_bytes.data(), digest_len) == 0;
    ...
    if (!x || !y ||
        !EC_POINT_get_affine_coordinates(group, pub, x.get(), y.get(),
                                             ctx.get())) {
      return false;
    }
