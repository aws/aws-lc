...
#include "../src/third_party/fiat/internal.h"

// TODO(agl): add tests with fixed vectors once SPAKE2 is nailed down.

struct SPAKE2Run {
  bool Run() {
    awslc::UniquePtr<SPAKE2_CTX> alice(SPAKE2_CTX_new(
        spake2_role_alice,
    ...
    awslc::UniquePtr<SPAKE2_CTX> bob(SPAKE2_CTX_new(
        spake2_role_bob,
    ...
    key_matches_ = (alice_key_len == bob_key_len &&
                    AWSLC_memcmp(alice_key, bob_key, alice_key_len) == 0);
